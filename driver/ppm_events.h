...
struct event_filler_arguments
{
...
#endif
    struct task_struct *sched_prev; /* for context switch events, the task that
				       is being scheduled out */
    struct task_struct *sched_next; /* for context switch events, the task that
				       is being scheduled in */

#ifdef CONFIG_ARM64
    struct task_struct
	    *child; /* for sched_process_fork events, this is the child task */
#endif
    ...
    bool compat;
#endif
    int fd; /* Passed by some of the fillers to val_to_ring to compute the
	       snaplen dynamically */
...
int32_t val_to_ring(struct event_filler_arguments *args, u64 val, u32 val_len,
