...
/// TODO: @Andreagit97 @FedeDP we need to remove the ifdef, this patch should
/// also work on x86.
#ifdef CONFIG_ARM64
#include <trace/syscall.h>
#endif /* CONFIG_ARM64 */
...
static int record_event_consumer(struct ppm_consumer_t *consumer,
				 ...
				 enum syscall_flags drop_flags, nanoseconds ns,
...
void ppm_task_cputime_adjusted(struct task_struct *p, cputime_t *ut,
			       cputime_t *st);
#endif
...
#if(LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 35))
TRACEPOINT_PROBE(sched_switch_probe, struct rq *rq, struct task_struct *prev,
		 struct task_struct *next);
#elif(LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 0))
TRACEPOINT_PROBE(sched_switch_probe, struct task_struct *prev,
		 struct task_struct *next);
#else
TRACEPOINT_PROBE(sched_switch_probe, bool preempt, struct task_struct *prev,
		 struct task_struct *next);
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,35)) */
#endif /* CAPTURE_CONTEXT_SWITCHES */

#ifdef CAPTURE_SIGNAL_DELIVERIES
TRACEPOINT_PROBE(signal_deliver_probe, int sig, struct siginfo *info,
		 struct k_sigaction *ka);
#endif

/* tracepoints `page_fault_user/kernel` don't exist on ARM64 architecture .*/
#ifdef CAPTURE_PAGE_FAULTS
TRACEPOINT_PROBE(page_fault_probe, unsigned long address, struct pt_regs *regs,
		 unsigned long error_code);
#endif

#ifdef CONFIG_ARM64
TRACEPOINT_PROBE(sched_proc_exec_probe, struct task_struct *p, pid_t old_pid,
		 struct linux_binprm *bprm);
TRACEPOINT_PROBE(sched_proc_fork_probe, struct task_struct *parent,
		 struct task_struct *child);
#endif
...
static void check_remove_consumer(struct ppm_consumer_t *consumer,
				  int remove_from_list)
{
	...
	struct ppm_ring_buffer_context *ring =
		per_cpu_ptr(consumer->ring_buffers, cpu);
...
static int ppm_open(struct inode *inode, struct file *filp)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(el, &g_consumer_list, node) { ++num_consumers; }
	rcu_read_unlock();
	...
	/*
	 * Note, we have two loops here because the first one makes sure that
	 * ALL of the rings are properly initialized to null, since the second
	 * one could be interrupted and cause issues in the cleanup phase. This
	 * might not be necessary, because alloc_percpu memsets the allocated
	 * entries to 0, but better be extra safe.
	 */
	for_each_possible_cpu(cpu)
	{
    ...
    /*
     * Check if the CPU pointed by this device is online. If it isn't stop here
     * and return ENODEV. The cpu could be online while buffer is NULL if
     * there's a cpu online hotplug callback between the first open on this
     * consumer and the open for this particular device.
     */
    ...
    /*
     * ring->preempt_count is not reset to 0 on purpose, to prevent a race
     * condition: if the same device is quickly closed and then reopened,
     * record_event() might still be executing (with ring->preempt_count to 1)
     * while ppm_open() resets ring->preempt_count to 0. When record_event()
     * will exit, it will decrease ring->preempt_count which will become < 0,
     * leading to the complete loss of all the events for that CPU.
     */
    ...
    bitmap_fill(
	    consumer->events_mask,
	    PPM_EVENT_MAX); /* Enable all syscall to be passed to userspace */
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 20)
	ret = compat_register_trace(syscall_exit_probe, "sys_exit",
				    tp_sys_exit);
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 20)
	ret = compat_register_trace(syscall_enter_probe, "sys_enter",
				    tp_sys_enter);
#else
	...
	ret = compat_register_trace(syscall_procexit_probe,
				    "sched_process_exit",
...
#ifdef CAPTURE_CONTEXT_SWITCHES
	ret = compat_register_trace(sched_switch_probe, "sched_switch",
				    tp_sched_switch);
	if(ret)
	{
...
#ifdef CAPTURE_SIGNAL_DELIVERIES
	ret = compat_register_trace(signal_deliver_probe, "signal_deliver",
				    tp_signal_deliver);
	if(ret)
	{
...
#ifdef CONFIG_ARM64
	ret = compat_register_trace(sched_proc_exec_probe, "sched_process_exec",
				    tp_sched_proc_exec);
	if(ret)
	{
	...
	ret = compat_register_trace(sched_proc_fork_probe, "sched_process_fork",
				    tp_sched_proc_fork);
	if(ret)
	{
...
#ifdef CONFIG_ARM64
err_sched_proc_fork:
    compat_unregister_trace(sched_proc_exec_probe, "sched_process_exec",
			    tp_sched_proc_exec);
err_sched_proc_exec:
    compat_unregister_trace(signal_deliver_probe, "signal_deliver",
			    tp_signal_deliver);
#endif
#ifdef CAPTURE_SIGNAL_DELIVERIES
err_signal_deliver:
    compat_unregister_trace(sched_switch_probe, "sched_switch",
			    tp_sched_switch);
#endif
err_sched_switch:
...
static int ppm_release(struct inode *inode, struct file *filp)
{
    ...
    vpr_info("closing ring %d, consumer:%p evt:%llu, dr_buf:%llu, dr_pf:%llu, "
	     "pr:%llu, cs:%llu\n",
	     ring_no, consumer_id, ring->info->n_evts,
	     ring->info->n_drops_buffer, ring->info->n_drops_pf,
	     ring->info->n_preemptions, ring->info->n_context_switches);
	...
	if(g_tracepoint_registered)
	{
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 20)
	    compat_unregister_trace(syscall_exit_probe, "sys_exit",
				    tp_sys_exit);
	    compat_unregister_trace(syscall_enter_probe, "sys_enter",
				    tp_sys_enter);
#else
	    unregister_trace_syscall_exit(syscall_exit_probe);
	    unregister_trace_syscall_enter(syscall_enter_probe);
#endif
	    compat_unregister_trace(syscall_procexit_probe,
				    "sched_process_exit",
				    tp_sched_process_exit);
	    ...
	    {
		compat_unregister_trace(page_fault_probe, "page_fault_user",
					tp_page_fault_user);
		compat_unregister_trace(page_fault_probe, "page_fault_kernel",
	    ...
	    compat_unregister_trace(sched_proc_exec_probe, "sched_process_exec",
				    tp_sched_proc_exec);
	    compat_unregister_trace(sched_proc_fork_probe, "sched_process_fork",
				    tp_sched_proc_fork);
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 20)
	     ...
	     */
	    for_each_possible_cpu(cpu) { per_cpu(g_n_tracepoint_hit, cpu) = 0; }
	}
	else
	{
	    ASSERT(false);
...
static long ppm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	memsize = sizeof(struct ppm_proclist_info) +
		  sizeof(struct ppm_proc_info) * pli.max_entries;
...
#else
		    proclist_info->entries[nentries].utime =
			    nsec_to_clock_t(utime);
		    proclist_info->entries[nentries].stime =
...
if(nentries >= pli.max_entries)
{
    vpr_info("PPM_IOCTL_GET_PROCLIST: not enough space (%d avail, %d "
	     "required)\n",
	     (int)pli.max_entries, (int)nentries);

    if(copy_to_user((void *)arg, proclist_info,
		    sizeof(struct ppm_proclist_info)))
    {
	ret = -EINVAL;
	goto cleanup_ioctl_procinfo;
    }
...
{
    memsize = sizeof(struct ppm_proclist_info) +
	      sizeof(struct ppm_proc_info) * nentries;
...
ret = 0;
cleanup_ioctl_procinfo : vfree((void *)proclist_info);
goto cleanup_ioctl_nolock;
}
...
#endif
    struct ppm_ring_buffer_context *ring =
	    per_cpu_ptr(consumer->ring_buffers, ring_no);
...
case PPM_IOCTL_DISABLE_DROPPING_MODE:
{
    ...
    /*
     * Push an event into the ring buffer so that the user can know that
     * dropping mode has been disabled
     */
    event_data.category = PPMC_CONTEXT_SWITCH;
    event_data.event_info.context_data.sched_prev =
	    (void *)DEI_DISABLE_DROPPING;
    event_data.event_info.context_data.sched_next = (void *)0;

    record_event_consumer(consumer, PPME_SCAPEVENT_E, UF_NEVER_DROP,
			  ppm_nsecs(), &event_data);
...
case PPM_IOCTL_ENABLE_DROPPING_MODE:
{
    ...
    if(new_sampling_ratio != 1 && new_sampling_ratio != 2 &&
       new_sampling_ratio != 4 && new_sampling_ratio != 8 &&
       new_sampling_ratio != 16 && new_sampling_ratio != 32 &&
...
case PPM_IOCTL_ENABLE_SIGNAL_DELIVER:
{
    ...
    if(g_tracepoint_registered)
	compat_register_trace(signal_deliver_probe, "signal_deliver",
			      tp_signal_deliver);
    ret = 0;
    goto cleanup_ioctl;
}
...
case PPM_IOCTL_ENABLE_PAGE_FAULTS:
{
    ...
    if(!g_fault_tracepoint_registered)
    {
	ret = compat_register_trace(page_fault_probe, "page_fault_user",
				    tp_page_fault_user);
	if(ret)
	{
	...
	ret = compat_register_trace(page_fault_probe, "page_fault_kernel",
				    tp_page_fault_kernel);
	if(ret)
	{
...
err_page_fault_kernel
	: compat_unregister_trace(page_fault_probe, "page_fault_user",
				  tp_page_fault_user);
#endif
cleanup_ioctl : mutex_unlock(&g_consumer_mutex);
cleanup_ioctl_nolock : return ret;
}
...
static int ppm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	vpr_info("mmap for consumer %p, CPU %d, start=%lu len=%ld "
		 "page_size=%lu\n",
		 consumer_id, ring_no, useraddr, length, PAGE_SIZE);
	...
	{
	    pr_err("Ring buffer size too small (%ld bytes, must be at least "
		   "%ld bytes\n",
		   (long)RING_BUF_SIZE, (long)PAGE_SIZE);
	    ret = -EIO;
	    goto cleanup_mmap;
	}
	...
	{
	    /*
	     * When the size requested by the user is smaller than a page, we
	     * assume she's mapping the ring info structure
	     */
	    ...
	    ret = remap_pfn_range(vma, useraddr, pfn, PAGE_SIZE,
				  vma->vm_page_prot);
	    if(ret < 0)
	    {
	...
	else if(length == RING_BUF_SIZE * 2)
	{
	    ...
	    /*
	     * When the size requested by the user equals the ring buffer size,
	     * we map the full buffer
	     */
	     ...
	     * This effectively mirrors the buffer at its end and helps simplify
	     * buffer management in userland.
	     */
...
static const unsigned char nas[21] = {AL(0), AL(3), AL(3), AL(3), AL(2), AL(3),
...
static const unsigned char compat_nas[21] = {
	AL(0), AL(3), AL(3), AL(3), AL(2), AL(3), AL(3),
...
static enum ppm_event_type
parse_socketcall(struct event_filler_arguments *filler_args,
		 struct pt_regs *regs)
{
	...
	if(unlikely(ppm_copy_from_user(socketcall_args32, compat_ptr(args[1]),
				       compat_nas[socketcall_id])))
	    return PPME_GENERIC_E;
	for(j = 0; j < 6; ++j)
	    filler_args->socketcall_args[j] =
		    (unsigned long)socketcall_args32[j];
    }
    else
    {
#endif
	if(unlikely(ppm_copy_from_user(filler_args->socketcall_args, scargs,
				       nas[socketcall_id])))
	    return PPME_GENERIC_E;
#ifdef CONFIG_COMPAT
    }
...
static inline void record_drop_e(struct ppm_consumer_t *consumer,
				 nanoseconds ns, enum syscall_flags drop_flags)
{
    struct event_data_t event_data = {0};

    if(record_event_consumer(consumer, PPME_DROP_E, UF_NEVER_DROP, ns,
			     &event_data) == 0)
    {
				 ...
				 nanoseconds ns, enum syscall_flags drop_flags)
{
    ...
    if(record_event_consumer(consumer, PPME_DROP_X, UF_NEVER_DROP, ns,
			     &event_data) == 0)
    {
				       ...
				       struct event_data_t *event_datap)
{
    ...
    {
	record_event_consumer(consumer, event_type, drop_flags, ns,
			      event_datap);
    }
    rcu_read_unlock();
}
				 ...
				 struct event_data_t *event_datap)
{
    ...
    if(event_datap->category == PPMC_SYSCALL &&
       event_datap->event_info.syscall_data.regs &&
       event_datap->event_info.syscall_data.id ==
	       event_datap->socketcall_syscall)
    {
	enum ppm_event_type tet;

	args.is_socketcall = true;
	args.compat = event_datap->compat;
	tet = parse_socketcall(&args,
			       event_datap->event_info.syscall_data.regs);
     ...
     * We need at least space for the header plus 16 bit per parameter for the
     * lengths.
     */
...
#endif
	args.buffer_size =
		min(freespace, delta_from_end) -
		sizeof(struct ppm_evt_hdr); /* freespace is guaranteed to be
					       bigger than sizeof(struct
					       ppm_evt_hdr) */
	...
	if(event_datap->category == PPMC_SYSCALL)
	{
	    ...
	    args.cur_g_syscall_code_routing_table =
		    event_datap->event_info.syscall_data
			    .cur_g_syscall_code_routing_table;
	    args.compat = event_datap->compat;
	}
	else
	{
	...
	if(event_datap->category == PPMC_SIGNAL)
	{
	    ...
	    {
		args.spid = event_datap->event_info.signal_data.info->_sifields
				    ._kill._pid;
	    }
	    else if(args.signo == SIGTERM || args.signo == SIGHUP ||
		    args.signo == SIGINT || args.signo == SIGTSTP ||
		    args.signo == SIGQUIT)
	    {
		if(event_datap->event_info.signal_data.info->si_code ==
			   SI_USER ||
		   event_datap->event_info.signal_data.info->si_code ==
			   SI_QUEUE ||
		   event_datap->event_info.signal_data.info->si_code <= 0)
		{
		    args.spid =
			    event_datap->event_info.signal_data.info->si_pid;
		}
	    }
	...
	/* For events with category `PPMC_SCHED_PROC_EXEC` or
	 * `PPMC_SCHED_PROC_FORK` we need to call dedicated fillers that are not
	 * in our `g_ppm_events` table.
	 */
	...
	case PPMC_SCHED_PROC_FORK:
	    /* First of all we need to update the event header with the child
	     * pid. */
	...
	if(likely(cbres == PPM_SUCCESS))
	{
	    ...
	    {
		pr_err("corrupted filler for event type %d (added %u args, "
		       "should have added %u)\n",
		       event_type, args.curarg, args.nargs);
		ASSERT(0);
	    }
	}
    }
	...
	if(unlikely(next >= RING_BUF_SIZE))
	{
	    ...
	    {
		memcpy(ring->buffer, ring->buffer + RING_BUF_SIZE,
		       next - RING_BUF_SIZE);
	    }
    ...
    if(MORE_THAN_ONE_SECOND_AHEAD(ns, ring->last_print_time + 1) &&
       !(drop_flags & UF_ATOMIC))
    {
	vpr_info("consumer:%p CPU:%d, use:%d%%, ev:%llu, dr_buf:%llu, "
		 "dr_pf:%llu, pr:%llu, cs:%llu\n",
		 consumer->consumer_id, smp_processor_id(),
		 (usedspace * 100) / RING_BUF_SIZE, ring_info->n_evts,
		 ring_info->n_drops_buffer, ring_info->n_drops_pf,
		 ring_info->n_preemptions, ring->info->n_context_switches);
...
static inline void g_n_tracepoint_hit_inc(void)
{
    ...
    /* this_cpu_inc has been added with 2.6.33 but backported by RHEL/CentOS
     * to 2.6.32 so just checking the existence of the symbol rather than
...
TRACEPOINT_PROBE(syscall_enter_probe, struct pt_regs *regs, long id)
{
    ...
    /*
     * If this is a 32bit process running on a 64bit kernel (see the
     * CONFIG_IA32_EMULATION kernel flag), we switch to the ia32 syscall table.
     */
	...
	if(used)
	    ...
	    record_event_all_consumers(PPME_GENERIC_E, UF_ALWAYS_DROP,
				       &event_data);
    }
}
...
TRACEPOINT_PROBE(syscall_exit_probe, struct pt_regs *regs, long ret)
{
     ...
     * but the id of the syscall is __NR_execve, so to correctly parse it we
     * need to use 64bit syscall table. On 32bit __NR_execve is equal to
     * __NR_ia32_oldolduname which is a very old syscall, not used anymore by
     * most applications
     */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 0)
	...
	if(used)
	    ...
	    record_event_all_consumers(PPME_GENERIC_X, UF_ALWAYS_DROP,
				       &event_data);
    }
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 1)
int __access_remote_vm(struct task_struct *t, struct mm_struct *mm,
...
#endif
{
     ...
     */
    record_event_all_consumers(PPME_SCHEDSWITCH_6_E, UF_USED | UF_ATOMIC,
			       &event_data);
}
#endif
...
#else
    return info == (struct siginfo *)SEND_SIG_NOINFO ||
	   info == (struct siginfo *)SEND_SIG_PRIV;
#endif
}
		 ...
		 struct k_sigaction *ka)
{
    ...
    record_event_all_consumers(PPME_SIGNALDELIVER_E, UF_USED | UF_ALWAYS_DROP,
			       &event_data);
}
#endif
		 ...
		 struct linux_binprm *bprm)
{
    ...
    record_event_all_consumers(PPME_SYSCALL_EXECVE_19_X, UF_NEVER_DROP,
			       &event_data);
}
		 ...
		 struct task_struct *child)
{
    ...
    record_event_all_consumers(PPME_SYSCALL_CLONE_20_X, UF_NEVER_DROP,
			       &event_data);
}
#endif
...
static int init_ring_buffer(struct ppm_ring_buffer_context *ring)
{
    ...
    /*
     * Allocate the buffer.
     * Note how we allocate 2 additional pages: they are used as additional
     * overflow space for the event data generation functions, so that they
     * always operate on a contiguous buffer.
     */
...
static int get_tracepoint_handles(void)
{
    ...
    {
	pr_notice("failed to find page_fault_user tracepoint, disabling "
		  "page-faults\n");
	g_fault_tracepoint_disabled = true;
    }
    if(!tp_page_fault_kernel)
    {
	pr_notice("failed to find page_fault_kernel tracepoint, disabling "
		  "page-faults\n");
	g_fault_tracepoint_disabled = true;
    }
#endif
...
static int do_cpu_callback(unsigned long cpu, long sd_action)
{
	...
	record_event_all_consumers(PPME_CPU_HOTPLUG_E, UF_NEVER_DROP,
				   &event_data);
    }
    return 0;
}
...
int scap_init(void)
{
    ...
    num_cpus = 0;
    for_each_possible_cpu(cpu) { ++num_cpus; }
...
#else
    g_ppm_devs =
	    kmalloc_array(g_ppm_numdevs, sizeof(struct ppm_device), GFP_KERNEL);
#endif
    if(!g_ppm_devs)
    {
	...
	device = class_device_create(
		...
		g_ppm_devs[j].dev, NULL, /* no additional data */
		DRIVER_DEVICE_NAME "%d", j);
    ...
    hp_ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN,
				       ...
				       scap_cpu_online, scap_cpu_offline);
    if(hp_ret <= 0)
    {
