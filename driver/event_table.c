...
const struct ppm_event_info g_event_info[PPM_EVENT_MAX] = {
	/* PPME_GENERIC_E */ {"syscall",
			      ...
			      EF_NONE,
			      2,
			      {{"ID", PT_SYSCALLID, PF_DEC},
			       {"nativeID", PT_UINT16, PF_DEC}}},
	/* PPME_GENERIC_X */
	...
	{"close",
	 EC_IO_OTHER,
	 EF_DESTROYS_FD | EF_USES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS,
	 1,
	 {{"fd", PT_FD, PF_DEC}}},
	...
	{"read",
	 EC_IO_READ,
	 EF_USES_FD | EF_READS_FROM_FD | EF_DROP_SIMPLE_CONS,
	 2,
	...
	{"write",
	 EC_IO_WRITE,
	 EF_USES_FD | EF_WRITES_TO_FD | EF_DROP_SIMPLE_CONS,
	 2,
	 {{"fd", PT_FD, PF_DEC}, {"size", PT_UINT32, PF_DEC}}},
	...
	{"connect",
	 ...
	 EF_USES_FD | EF_MODIFIES_STATE,
	 2,
	 {{"res", PT_ERRNO, PF_DEC}, {"tuple", PT_SOCKTUPLE, PF_NA}}},
	/* PPME_SOCKET_LISTEN_E */
	{"listen",
	 EC_NET,
	 EF_USES_FD,
	 2,
	 {{"fd", PT_FD, PF_DEC}, {"backlog", PT_UINT32, PF_DEC}}},
	/* PPME_SOCKET_LISTEN_X */
	{"listen", EC_NET, EF_USES_FD, 1, {{"res", PT_ERRNO, PF_DEC}}},
	/* PPME_SOCKET_ACCEPT_E */
	{"accept", EC_NET, EF_CREATES_FD | EF_MODIFIES_STATE | EF_OLD_VERSION,
	 0},
	...
	{"shutdown",
	 EC_NET,
	 EF_USES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS,
	 2,
	 {{"fd", PT_FD, PF_DEC}, {"how", PT_ENUMFLAGS8, PF_HEX, shutdown_how}}},
	...
	{"getsockopt", EC_NET, EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS, 0},
	...
	{"eventfd",
	 EC_IPC,
	 EF_CREATES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS,
	 2,
	 {{"initval", PT_UINT64, PF_DEC}, {"flags", PT_FLAGS32, PF_HEX}}},
	...
	{"futex",
	 ...
	 {{"addr", PT_UINT64, PF_HEX},
	  {"op", PT_ENUMFLAGS16, PF_HEX, futex_operations},
	  {"val", PT_UINT64, PF_DEC}}},
	/* PPME_SYSCALL_FUTEX_X */
	{"futex", EC_IPC, EF_DROP_SIMPLE_CONS, 1, {{"res", PT_ERRNO, PF_DEC}}},
	...
	{"fstat",
	 EC_FILE,
	 EF_USES_FD | EF_DROP_SIMPLE_CONS,
	 1,
	 {{"fd", PT_FD, PF_NA}}},
	...
	{"lstat64", EC_FILE, EF_DROP_SIMPLE_CONS, 0},
	...
	{"select", EC_WAIT, EF_WAITS | EF_DROP_SIMPLE_CONS, 0},
	...
	/* Note: path is PT_CHARBUF and not PT_FSPATH because we assume it's
	   absolute and will never need resolution */
	...
	/* Note: path is PT_CHARBUF and not PT_FSPATH because we don't want it
	   to be resolved, since the event handler already changes it */
	...
	{"timerfd_create",
	 ...
	 EF_CREATES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS,
	 2,
	 {{"clockid", PT_UINT8, PF_DEC}, {"flags", PT_FLAGS8, PF_HEX}}},
	...
	{"switch",
	 EC_SCHEDULER,
	 EF_SKIPPARSERESET | EF_OLD_VERSION | EF_DROP_SIMPLE_CONS,
	 1,
	 {{"next", PT_PID, PF_DEC}}},
	/* PPME_SCHEDSWITCH_1_X */
	{"NA2", EC_SCHEDULER, EF_SKIPPARSERESET | EF_UNUSED | EF_OLD_VERSION,
	 0},
	/* PPME_DROP_E */
	{"drop",
	 EC_INTERNAL,
	...
	{"getuid", EC_USER, EF_DROP_SIMPLE_CONS, 1, {{"uid", PT_UID, PF_DEC}}},
	/* PPME_SYSCALL_GETEUID_E */
	{"geteuid", EC_USER, EF_DROP_SIMPLE_CONS, 0},
	...
	{"getgid", EC_USER, EF_DROP_SIMPLE_CONS, 1, {{"gid", PT_GID, PF_DEC}}},
	/* PPME_SYSCALL_GETEGID_E */
	{"getegid", EC_USER, EF_DROP_SIMPLE_CONS, 0},
	...
	{"getresuid", EC_USER, EF_DROP_SIMPLE_CONS, 0},
	...
	{"getresgid", EC_USER, EF_DROP_SIMPLE_CONS, 0},
	...
	/* PPME_CONTAINER_X */
	{"container", EC_INTERNAL, EF_UNUSED | EF_OLD_VERSION, 0},
	...
	{"procinfo",
	 EC_INTERNAL,
	 EF_SKIPPARSERESET | EF_DROP_SIMPLE_CONS,
	 2,
	...
	{"setns",
	 ...
	 EF_USES_FD,
	 2,
	 {{"fd", PT_FD, PF_NA}, {"nstype", PT_FLAGS32, PF_HEX, clone_flags}}},
	...
	{"userfaultfd", EC_FILE, EF_CREATES_FD | EF_MODIFIES_STATE, 0},
	...
	{"pluginevent",
	 ...
	 EF_LARGE_PAYLOAD,
	 2,
	 {{"plugin ID", PT_UINT32, PF_DEC}, {"event_data", PT_BYTEBUF, PF_NA}}},
	...
	{"clone3", EC_PROCESS, EF_MODIFIES_STATE, 0},
	...
	{"open_by_handle_at", EC_FILE, EF_CREATES_FD | EF_MODIFIES_STATE, 0},
	...
	{"io_uring_setup", EC_IO_OTHER,
	 EF_CREATES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS, 0},
	...
	{"io_uring_enter", EC_IO_OTHER, EF_DROP_SIMPLE_CONS, 0},
	...
	{"io_uring_register", EC_IO_OTHER, EF_DROP_SIMPLE_CONS, 0},
	/* PPME_SYSCALL_TO_URING_REGISTER_X */
	{"io_uring_register",
	 EC_IO_OTHER,
	 EF_USES_FD | EF_MODIFIES_STATE | EF_DROP_SIMPLE_CONS,
	 5,
	...
	{"munlock", EC_MEMORY, EF_DROP_SIMPLE_CONS, 0},
	...
	{"mlockall", EC_MEMORY, EF_DROP_SIMPLE_CONS, 0},
	...
	{"munlockall", EC_MEMORY, EF_DROP_SIMPLE_CONS, 0},
	...
	{"capset", EC_PROCESS, EF_MODIFIES_STATE, 0},
	...
	/* NB: Starting from scap version 1.2, event types will no longer be
	 * changed when an event is modified, and the only kind of change
	 * permitted for pre-existent events is adding parameters. New event
	 * types are allowed only for new syscalls or new internal events. The
	 * number of parameters can be used to differentiate between event
	 * versions.
	 */
	/* NB: all events that have the "EF_USES_FD" flag should return as first
	 *parameter a file descriptor. "libsinsp" will try to access the first
	 *parameter and use it as a file descriptor. If the event has 0
	 *parameters but has the "EF_USES_FD" flag then a runtime error will
	 *occurr shutting down the process. Furthermore if an exit event has the
	 *"EF_USES_FD" then also the related enter event must have it (following
	 *the logic described above).Otherwise the exit event will not trigger
	 *"libsinsp" code in order to properly manage the file descriptor
	 *returned by the exit event.
	 */
};
