...
#define merge_64(hi, lo)                                                       \
    ((((unsigned long long)(hi)) << 32) + ((lo)&0xffffffffUL))

#ifndef UDIG
static inline struct pid_namespace *
pid_ns_for_children(struct task_struct *task)
{
...
int f_sys_generic(struct event_filler_arguments *args)
{
    ...
    {
	enum ppm_syscall_code sc_code =
		cur_g_syscall_code_routing_table[table_index];
...
int f_sys_open_e(struct event_filler_arguments *args)
{
     ...
     */
    syscall_get_arguments_deprecated(current, args->regs, 0, 1, &val);
    if(likely(ppm_strncpy_from_user(args->str_storage, (const void __user *)val,
				    PPM_MAX_PATH_SIZE) >= 0))
    {
    ...
    /*
     * Flags
     * Note that we convert them into the ppm portable representation before
     * pushing them to the ring
     */
...
static int append_cgroup(const char *subsys_name, int subsys_id, char *buf,
			 int *available)
{
    int pathlen;
    int subsys_len;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)
#define SUBSYS(_x)                                                             \
    if(append_cgroup(#_x, _x##_cgrp_id,                                        \
...
#define SUBSYS(_x)                                                             \
    if(append_cgroup(#_x, _x##_subsys_id,                                      \
		     args->str_storage + STR_STORAGE_SIZE - available,         \
		     &available))                                              \
	goto cgroups_error;
			   ...
			   int available)
{
	...
	n_bytes_copied = ppm_strncpy_from_user(&str_storage[len], p, available);
...
static int ppm_get_tty(void)
{
    ...
    if(unlikely(
	       copy_from_kernel_nofault(&driver, &tty->driver, sizeof(driver))))
	return 0;
    ...
    if(unlikely(
	       copy_from_kernel_nofault(&major, &driver->major, sizeof(major))))
	return 0;

    if(unlikely(copy_from_kernel_nofault(&minor_start, &driver->minor_start,
					 sizeof(minor_start))))
	return 0;
...
int f_proc_startupdate(struct event_filler_arguments *args)
{
    ...
    int args_len = 0; /*the combined length of the arguments string + executable
			 string */
	...
	*args->str_storage = 0;
	...
	if(likely(retval >= 0))
	{
		...
		if(unlikely(ppm_copy_from_user(
			   args->str_storage,
			   (const void __user *)mm->arg_start, args_len)))
		    args_len = 0;
		else
	...
	else
	{
		...
		syscall_get_arguments_deprecated(current, args->regs, 1, 1,
						 &val);
		break;

	    case PPME_SYSCALL_EXECVEAT_X:
		syscall_get_arguments_deprecated(current, args->regs, 2, 1,
						 &val);
		break;
		...
		args_len = compat_accumulate_argv_or_env(
			(compat_uptr_t)val, args->str_storage, available);
	    else
#endif
		args_len =
			accumulate_argv_or_env((const char __user *__user *)val,
	 ...
	 * Args
	 */
	res = val_to_ring(args, (int64_t)(long)args->str_storage + exe_len,
			  args_len - exe_len, false, 0);
	if(unlikely(res != PPM_SUCCESS))
	    return res;
    }
...
cgroups_error:
    ...
    res = val_to_ring(args, (int64_t)(long)args->str_storage,
		      STR_STORAGE_SIZE - available, false, 0);
    if(unlikely(res != PPM_SUCCESS))
	return res;
...
#ifdef __NR_clone3
	    syscall_get_arguments_deprecated(current, args->regs, 0, 1, &val);
	    res = ppm_copy_from_user(&cl_args, (void *)val,
...
#endif
	res = val_to_ring(args, (uint64_t)clone_flags_to_scap(val) | in_pidns,
			  0, false, 0);
	if(unlikely(res != PPM_SUCCESS))
	    return res;
	...
	if(likely(retval >= 0))
	{
		...
		if(unlikely(ppm_copy_from_user(
			   args->str_storage,
			   (const void __user *)mm->env_start, env_len)))
		    env_len = 0;
		else
	...
	else
	{
	    ...
	    case PPME_SYSCALL_EXECVEAT_X:
		syscall_get_arguments_deprecated(current, args->regs, 3, 1,
						 &val);
		break;
...
#ifdef CONFIG_COMPAT
	    if(unlikely(args->compat))
		env_len = compat_accumulate_argv_or_env(
			(compat_uptr_t)val, args->str_storage, available);
	    else
#endif
		env_len =
	 ...
	 * environ
	 */
	res = val_to_ring(args, (int64_t)(long)args->str_storage, env_len,
			  false, 0);
	if(unlikely(res != PPM_SUCCESS))
	    return res;
	...
	if(exe_file != NULL)
	{
		...
		exe_writable |= (inode_permission(current_user_ns(),
						  file_inode(exe_file),
						  MAY_WRITE) == 0);
		exe_writable |= inode_owner_or_capable(current_user_ns(),
						       file_inode(exe_file));
#else
		exe_writable |= (inode_permission(file_inode(exe_file),
						  MAY_WRITE) == 0);
	   ...
	   args->event_type == PPME_SYSCALL_EXECVEAT_X)
	{
	    ...
	    val = ((uint64_t)cred->cap_inheritable.cap[1] << 32) |
		  cred->cap_inheritable.cap[0];
	    res = val_to_ring(args, capabilities_to_scap(val), 0, false, 0);
	    if(unlikely(res != PPM_SUCCESS))
		goto out;

	    val = ((uint64_t)cred->cap_permitted.cap[1] << 32) |
		  cred->cap_permitted.cap[0];
	    res = val_to_ring(args, capabilities_to_scap(val), 0, false, 0);
	    if(unlikely(res != PPM_SUCCESS))
		goto out;

	    val = ((uint64_t)cred->cap_effective.cap[1] << 32) |
...
int f_sys_socket_bind_x(struct event_filler_arguments *args)
{
	...
	if(likely(err >= 0))
	{
	     ...
	     */
	    size = pack_addr((struct sockaddr *)&address, val, targetbuf,
			     STR_STORAGE_SIZE);
	}
    }
...
int f_sys_connect_x(struct event_filler_arguments *args)
{
	...
	if(usrsockaddr != NULL && val != 0)
	{
		 ...
		 * Convert the fd into socket endpoint information
		 */
		size = fd_to_socktuple(fd, (struct sockaddr *)&address, val,
				       true, false, targetbuf,
				       STR_STORAGE_SIZE);
	    }
	}
    }
...
int f_sys_socketpair_x(struct event_filler_arguments *args)
{
	...
	{
#endif
	    if(unlikely(ppm_copy_from_user(fds, (const void __user *)val,
					   sizeof(fds))))
		return PPM_FAILURE_INVALID_USER_MEMORY;
#ifdef CONFIG_COMPAT
	}
	...
	{
	    if(unlikely(ppm_copy_from_user(
		       fds, (const void __user *)compat_ptr(val), sizeof(fds))))
		return PPM_FAILURE_INVALID_USER_MEMORY;
	}
#endif
...
static int parse_sockopt(struct event_filler_arguments *args, int level,
			 int optname, const void __user *optval, int optlen)
{
    union
    {
	uint32_t val32;
	...
	case SO_ERROR:
		...
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    return val_to_ring(args, -(int)u.val32, 0, false,
			       PPM_SOCKOPT_IDX_ERRNO);
#endif
	...
	default:
	    return val_to_ring(args, (unsigned long)optval, optlen, true,
...
int f_sys_setsockopt_x(struct event_filler_arguments *args)
{
    ...
    /* optname */
    res = val_to_ring(args, sockopt_optname_to_scap(val[1], val[2]), 0, true,
		      0);
    if(unlikely(res != PPM_SUCCESS))
	return res;

    /* optval */
    res = parse_sockopt(args, val[1], val[2], (const void __user *)val[3],
			val[4]);
    if(unlikely(res != PPM_SUCCESS))
	return res;
...
int f_sys_getsockopt_x(struct event_filler_arguments *args)
{
    ...
    if(unlikely(ppm_copy_from_user(&optlen, (const void __user *)val[4],
				   sizeof(optlen))))
	return PPM_FAILURE_INVALID_USER_MEMORY;
...
int f_sys_accept_x(struct event_filler_arguments *args)
{
     ...
     * Convert the fd into socket endpoint information
     */
    size = fd_to_socktuple(fd, NULL, 0, false, true, targetbuf,
			   STR_STORAGE_SIZE);
...
int f_sys_sendto_e(struct event_filler_arguments *args)
{
    ...
    *targetbuf = 250;
	...
	if(likely(err >= 0))
	{
	     ...
	     * Convert the fd into socket endpoint information
	     */
	    size = fd_to_socktuple(fd, (struct sockaddr *)&address, val, true,
				   false, targetbuf, STR_STORAGE_SIZE);
	}
    }
...
int f_sys_recvfrom_x(struct event_filler_arguments *args)
{
	...
	if(usrsockaddr != NULL && val != 0)
	{
	    ...
	    {
		if(unlikely(ppm_copy_from_user(
			   &addrlen, (const void __user *)compat_ptr(val),
			   sizeof(addrlen))))
		    return PPM_FAILURE_INVALID_USER_MEMORY;
	    }
#endif
	     ...
	     * Copy the address
	     */
	    err = addr_to_kernel(usrsockaddr, addrlen,
		 ...
		 * Convert the fd into socket endpoint information
		 */
		size = fd_to_socktuple(fd, (struct sockaddr *)&address, addrlen,
				       true, true, targetbuf, STR_STORAGE_SIZE);
	    }
	}
    }
...
int f_sys_sendmsg_e(struct event_filler_arguments *args)
{
    ...
    {
#endif
	if(unlikely(ppm_copy_from_user(&mh, (const void __user *)val,
				       sizeof(mh))))
	    return PPM_FAILURE_INVALID_USER_MEMORY;
	...
	res = parse_readv_writev_bufs(args, iov, iovcnt,
	 ...
	 * size
	 */
	compat_iov = (const struct compat_iovec __user *)compat_ptr(
		compat_mh.msg_iov);
	iovcnt = compat_mh.msg_iovlen;

	res = compat_parse_readv_writev_bufs(
		args, compat_iov, iovcnt, args->consumer->snaplen,
		PRB_FLAG_PUSH_SIZE | PRB_FLAG_IS_WRITE);
	...
	if(likely(err >= 0))
	{
	     ...
	     */
	    size = fd_to_socktuple(fd, (struct sockaddr *)&address, addrlen,
				   true, false, targetbuf, STR_STORAGE_SIZE);
	}
    }
...
int f_sys_recvmsg_x(struct event_filler_arguments *args)
{
	...
	res = parse_readv_writev_bufs(args, iov, iovcnt, retval,
				      PRB_FLAG_PUSH_ALL);
#ifdef CONFIG_COMPAT
    }
    else
	...
	res = compat_parse_readv_writev_bufs(args, compat_iov, iovcnt, retval,
					     PRB_FLAG_PUSH_ALL);
    }
#endif
...
static int poll_parse_fds(struct event_filler_arguments *args, bool enter_event)
{
     ...
     */
    if(unlikely(sizeof(struct pollfd) * nfds + 2 + 10 * nfds >
		STR_STORAGE_SIZE))
	return PPM_FAILURE_BUFFER_FULL;
...
static int timespec_parse(struct event_filler_arguments *args,
			  unsigned long val)
{
    uint64_t longtime;
    char *targetbuf = args->str_storage;
    ...
    /*
     * interval
     * We copy the timespec structure and then convert it to a 64bit relative
     * time
     */
#ifdef CONFIG_COMPAT
    if(!args->compat)
    {
#endif
	cfulen = (int)ppm_copy_from_user(targetbuf, (void __user *)val,
					 sizeof(*tts));
	...
	longtime = ((uint64_t)compat_tts->tv_sec) * 1000000000 +
		   compat_tts->tv_nsec;
    }
#endif
...
int f_sys_pwrite64_e(struct event_filler_arguments *args)
{
	...
	/*
	 * pos
	 * NOTE: this is a 64bit value, which means that on 32bit systems it
	 * uses two separate registers that we need to merge.
	 */
#if defined CONFIG_X86
    syscall_get_arguments_deprecated(current, args->regs, 3, 1, &pos0);
    syscall_get_arguments_deprecated(current, args->regs, 4, 1, &pos1);
...
int f_sys_getrlimit_setrlrimit_x(struct event_filler_arguments *args)
{
	...
	{
#endif
	    if(unlikely(ppm_copy_from_user(&rl, (const void __user *)val,
...
int f_sys_prlimit_x(struct event_filler_arguments *args)
{
	...
	{
	    if(unlikely(ppm_copy_from_user(&compat_rl, (const void __user *)val,
					   sizeof(struct compat_rlimit))))
	    {
		newcur = -1;
		newmax = -1;
...
static inline int parse_ptrace_addr(struct event_filler_arguments *args,
				    u16 request)
{
    unsigned long val;
    uint64_t dst;
...
static inline int parse_ptrace_data(struct event_filler_arguments *args,
				    u16 request)
{
    unsigned long val;
    unsigned long len;
	...
	{
#endif
	    len = ppm_copy_from_user(&dst, (const void __user *)val,
				     sizeof(long));
#ifdef CONFIG_COMPAT
	}
	else
	{
	    len = ppm_copy_from_user(&dst, (const void __user *)compat_ptr(val),
...
#ifdef __NR_openat2
    /*
     * how: we get the data structure, and put its fields in the buffer one by
     * one
     */
    syscall_get_arguments_deprecated(current, args->regs, 2, 1, &val);
...
int f_sys_open_by_handle_at_x(struct event_filler_arguments *args)
{
    ...
    {
	char *pathname;
	// String storage size is exactly one page. PAGE_SIZE = 4096 byte like
	// PATH_MAX in unix conventions.
	...
	// `pathname` will be a pointer inside the the buffer `buf`, where the
	// file path effectively starts.
...
#ifdef __NR_io_uring_setup
    /*
     * io_uring_params: we get the data structure, and put its fields in the
     * buffer one by one
     */
    syscall_get_arguments_deprecated(current, args->regs, 1, 1, &val);
    res = ppm_copy_from_user(&params, (void *)val,
...
int f_sys_procexit_e(struct event_filler_arguments *args)
{
    ...
    /* Ret code */
    res = val_to_ring(args, __WEXITSTATUS(args->sched_prev->exit_code), 0,
		      false, 0);
    if(unlikely(res != PPM_SUCCESS))
	return res;
    ...
    {
	res = val_to_ring(args, __WTERMSIG(args->sched_prev->exit_code), 0,
			  false, 0);
    }
    else
    {
    ...
    res = val_to_ring(args, __WCOREDUMP(args->sched_prev->exit_code) != 0, 0,
		      false, 0);
#else
    res = val_to_ring(args, 0, 0, false, 0);
    if(unlikely(res != PPM_SUCCESS))
...
int f_sys_sendfile_e(struct event_filler_arguments *args)
{
	...
	{
	    res = ppm_copy_from_user(&offset, (void *)compat_ptr(val),
				     sizeof(compat_off_t));
	}
#endif
	if(unlikely(res))
...
int f_sys_quotactl_e(struct event_filler_arguments *args)
{
    ...
    if((cmd == PPM_Q_GETQUOTA) || (cmd == PPM_Q_SETQUOTA) ||
       (cmd == PPM_Q_XGETQUOTA) || (cmd == PPM_Q_XSETQLIM))
    {
	/*
...
int f_sys_quotactl_x(struct event_filler_arguments *args)
{
    ...
    {
	len = ppm_copy_from_user(&dqinfo, (void *)val,
				 sizeof(struct if_dqinfo));
	if(unlikely(len != 0))
	    return PPM_FAILURE_INVALID_USER_MEMORY;
    }
...
int f_sys_getresuid_and_gid_x(struct event_filler_arguments *args)
{
    ...
    {
	len = ppm_copy_from_user(&uid, (void *)compat_ptr(val),
				 sizeof(uint32_t));
    }
#endif
    if(unlikely(len != 0))
...
int f_sys_pagefault_e(struct event_filler_arguments *args)
{
    ...
    res = val_to_ring(args, pf_flags_to_scap(args->fault_data.error_code), 0,
		      false, 0);
    if(unlikely(res != PPM_SUCCESS))
	return res;
...
int f_sys_semop_x(struct event_filler_arguments *args)
{
	...
	for(j = 0; j < max_nsops; j++)
	{
	    ...
	    if(nsops--)
		if(unlikely(ppm_copy_from_user(&sops, (void *)&ptr[j],
	    ...
	    res = val_to_ring(args, semop_flags_to_scap(sops.sem_flg), 0, true,
			      0);
	    if(unlikely(res != PPM_SUCCESS))
		return res;
	}
...
int f_sched_prog_exec(struct event_filler_arguments *args)
{
    ...
    correctly_read = ppm_copy_from_user(
	    args->str_storage, (const void __user *)mm->arg_start, args_len);
...
cgroups_error:
    ...
    correctly_read = ppm_copy_from_user(
	    args->str_storage, (const void __user *)mm->env_start, env_len);
	...
	*args->str_storage = 0;
    ...
    res = val_to_ring(args, (int64_t)(long)args->str_storage, env_len, false,
		      0);
    if(unlikely(res != PPM_SUCCESS))
    {
	return res;
    ...
    res = val_to_ring(
	    args,
	    (int64_t)task_pgrp_nr_ns(current, task_active_pid_ns(current)), 0,
	    false, 0);
    if(unlikely(res != PPM_SUCCESS))
    {
	return res;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)
	    exe_writable |=
		    (inode_permission(current_user_ns(), file_inode(exe_file),
				      MAY_WRITE) == 0);
	    exe_writable |= inode_owner_or_capable(current_user_ns(),
						   file_inode(exe_file));
#else
	    exe_writable |=
		    (inode_permission(file_inode(exe_file), MAY_WRITE) == 0);
	    exe_writable |= inode_owner_or_capable(file_inode(exe_file));
#endif
	}
	fput(exe_file);
