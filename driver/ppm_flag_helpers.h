...
io_uring_register_opcodes_to_scap(unsigned long flags)
{
    /*
     ...
     * we will just print its value to userspace without mapping it to a string
     * flag. We then need to append new flags to both flags_table and
     * ppm_events_public PPM_ flags.
     */
    return flags;
}
...
#endif
    default:
	/* no ASSERT as there are legitimate other levels we don't just support
	 * yet */
...
static __always_inline u8 sockopt_optname_to_scap(int level, int optname)
{
	...
	// Basically, when labels values are similar AND the switch has many
	// labels, compiler tends to build a jump table as optimization. This
	// breaks with eBPF, and in our Makefile we already have the
	// -fno-jump-tables; most probably clang5 had some kind of bug that
	// caused -O2 mode to still use jump tables. Let's add a "very distant"
	// label value to forcefully disable jump table.
	//
...
static __always_inline u32 semctl_cmd_to_scap(unsigned cmd)
{
	...
	return PPM_SETVAL;
#ifdef __BPF_TRACING__
    // forcefully disable switch jump table, see sockopt_optname_to_scap() for
    // more info
    case INT_MAX:
	return 0;
#endif
