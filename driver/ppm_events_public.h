 ...
 */
#if !defined(CYGWING_AGENT) && (defined(_WIN64) || defined(WIN64) ||           \
 ...
 * Limits
 */
#define PPM_MAX_EVENT_PARAMS                                                   \
    (1 << 5) /* Max number of parameters an event can have */
#define PPM_MAX_PATH_SIZE                                                      \
    256 /* Max size that an event parameter can have in the circular buffer,   \
	   in bytes */
...
#define PPM_O_RDWR                                                             \
    (PPM_O_RDONLY | PPM_O_WRONLY) /* Open for reading and writing */
#define PPM_O_CREAT (1 << 2)	  /* Create a new file if it doesn't exist. */
#define PPM_O_APPEND                                                           \
    (1 << 3) /* If set, the file offset shall be set to the end of the file    \
		prior to each write. */
...
#define PPM_EXVAT_AT_EMPTY_PATH                                                \
    (1 << 0) /* If pathname is an empty string, operate on the file referred   \
		to by dirfd */
...
enum ppm_event_category
{
    ...
    EC_IO_OTHER = 34,	/* General I/O that is neither read not write (can be
			   file, socket, IPC...) */
...
enum ppm_event_flags
{
    ...
    EF_MODIFIES_STATE =
	    (1 << 5),	  /* This event causes the machine state to change and
			     should not be dropped by the filtering engine. */
    EF_UNUSED = (1 << 6), /* This event is not used */
    EF_WAITS = (1 << 7),  /* This event reads data from an FD. */
    EF_SKIPPARSERESET = (1 << 8), /* This event shouldn't pollute the parser
				     lastevent state tracker. */
    EF_OLD_VERSION =
	    (1 << 9), /* This event is kept for backward compatibility */
    EF_DROP_SIMPLE_CONS =
	    (1 << 10), /* This event can be skipped by consumers that privilege
			  low overhead to full event capture */
    EF_LARGE_PAYLOAD =
	    (1 << 11), /* This event has a large payload, ie: up to UINT32_MAX
			  bytes. DO NOT USE ON syscalls-driven events!!! */
};
...
enum ppm_param_type
{
    ...
    PT_ERRNO =
	    11, /* this is an INT64, but will be interpreted as an error code */
    PT_SOCKADDR = 12, /* A sockaddr structure, 1byte family + data */
    PT_SOCKTUPLE =
	    13,	 /* A sockaddr tuple,1byte family + 12byte data + 12byte data */
    PT_FD = 14,	 /* An fd, 64bit */
    PT_PID = 15, /* A pid/tid, 64bit */
    PT_FDLIST = 16,    /* A list of fds, 16bit count + count * (64bit fd + 16bit
			  flags) */
    ...
    PT_RELTIME =
	    20, /* A relative time. Seconds * 10^9  + nanoseconds. 64bit. */
    ...
    PT_DYN = 27, /* Type can vary depending on the context. Used for filter
		    fields like evt.rawarg. */
    PT_FLAGS8 =
	    28, /* this is an UINT8, but will be interpreted as 8 bit flags. */
    PT_FLAGS16 = 29, /* this is an UINT16, but will be interpreted as 16 bit
			flags. */
    PT_FLAGS32 = 30, /* this is an UINT32, but will be interpreted as 32 bit
			flags. */
    PT_UID = 31,     /* this is an UINT32, MAX_UINT32 will be interpreted as no
			value. */
    PT_GID = 32,     /* this is an UINT32, MAX_UINT32 will be interpreted as no
			value. */
    PT_DOUBLE = 33,  /* this is a double precision floating point number. */
    PT_SIGSET = 34,  /* sigset_t. I only store the lower UINT32 of it */
    PT_CHARBUFARRAY =
	    35, /* Pointer to an array of strings, exported by the user events
		   decoder. 64bit. For internal use only. */
    PT_CHARBUF_PAIR_ARRAY =
	    36, /* Pointer to an array of string pairs, exported by the user
    ...
    PT_IPADDR = 40,   /* Either an IPv4 or IPv6 address. The length indicates
			 which one it is. */
    ...
    PT_ENUMFLAGS8 = 44,	 /* this is an UINT8, but will be interpreted as an enum
			    flag, ie: contiguous values flag. */
    PT_ENUMFLAGS16 = 45, /* this is an UINT16, but will be interpreted as an
			    enum flag, ie: contiguous values flag. */
    PT_ENUMFLAGS32 = 46, /* this is an UINT32, but will be interpreted as an
			    enum flag, ie: contiguous values flag. */
...
enum ppm_print_format
{
    ...
    PF_10_PADDED_DEC = 3, /* decimal padded to 10 digits, useful to print the
			     fractional part of a ns timestamp */
    PF_ID = 4,
    PF_DIR = 5,
    PF_OCT = 6, /* octal */
};
...
struct ppm_param_info
{
    ...
    enum ppm_print_format
	    fmt; /**< If this is a numeric parameter, this flag specifies if it
		    should be rendered as decimal or hex. */
    const void *info; /**< If this is a flags parameter, it points to an array
			 of ppm_name_value, if this is a FSRELPATH parameter, it
			 references the related dirfd, else if this is a dynamic
			 parameter it points to an array of ppm_param_info */
    uint8_t ninfo;    /**< Number of entry in the info array. */
} _packed;

/*!
  \brief Event information.
  This structure contains the full description of an event type (e.g. 'open')
...
enum syscall_flags
{
    ...
    UF_SIMPLEDRIVER_KEEP =
	    (1 << 3), ///< Mark a syscall to be kept in simpledriver mode, see
		      ///< scap_enable_simpledriver_mode()
    UF_ATOMIC = (1 << 4), ///< The handler should not block (interrupt context)
    UF_UNINTERESTING =
	    (1 << 5), ///< Marks a syscall as not interesting. Currently only
		      ///< used by BPF probe to avoid tracing uninteresting
		      ///< syscalls. Kmod uses a different logic path as we
		      ///< communicate with it through ioctls
};
...
#define PPM_FAILURE_FRAME_SCRATCH_MAP_FULL                                     \
    -5 /* this is used only inside bpf, kernel module does not have a frame    \
	  scratch map*/
