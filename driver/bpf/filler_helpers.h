...
// Old kernels (like 4.14) have too strict limits on the bpf program length to
// support 32 path components. For the moment we decrease the limit to 16.
#define MAX_PATH_COMPONENTS 16
#define MAX_PATH_LENGTH 4096

static __always_inline bool in_port_range(uint16_t port, uint16_t min,
					  uint16_t max)
{
...
// Kernel 5.10 introduced a new bpf_helper called `bpf_d_path` to extract a file
// path starting from a file descriptor. Libscap loads our bpf programs as
// `BPF_PROG_TYPE_RAW_TRACEPOINT` programs. This type of program doesn't seem
// able to call this new helper because it is out of its scope. For more details
// see here
// https://github.com/torvalds/linux/blob/58e1100fdc5990b0cc0d4beaf2562a92e621ac7d/kernel/trace/bpf_trace.c#L1574
static __always_inline char *bpf_get_path(struct filler_data *data, int fd)
{
    struct file *f = bpf_fget(fd);
    const unsigned char **pointers_buf =
	    (const unsigned char **)data->tmp_scratch;
    char *filepath =
	    (char *)&data->tmp_scratch[(MAX_PATH_COMPONENTS *
...
static __always_inline struct socket *
bpf_sockfd_lookup(struct filler_data *data, int fd)
{
    struct file *file;
...
static __always_inline void bpf_get_fd_dev_ino(int fd, unsigned long *dev,
					       unsigned long *ino)
{
...
bpf_getsockname(struct socket *sock, struct sockaddr_storage *addr, int peer)
{
    struct sock *sk;
    sa_family_t family;
...
#define get_buf(x)                                                             \
    data->buf[(data->state->tail_ctx.curoff + (x)) & SCRATCH_SIZE_HALF]
					       ...
					       u32 lookahead_size)
{
	...
	else
	{
	    ...
	    if(addrlen != 0)
	    {
		if(bpf_addr_to_kernel(usrsockaddr, addrlen,
	...
	if(lookahead_size >= 5)
	{
	    if(get_buf(0) == 3 || get_buf(1) == 3 || get_buf(2) == 3 ||
	       get_buf(3) == 3 || get_buf(4) == 3)
	    {
		return 2000;
	    }
	...
	{
	    if((get_buf(0) == 'Q' &&
		get_buf(1) == 0) || /* SimpleQuery command */
	       (get_buf(0) == 'P' &&
		get_buf(1) == 0) || /* Prepare statement command */
	       (get_buf(4) == 0 && get_buf(5) == 3 &&
		get_buf(6) == 0) || /* startup command */
	       (get_buf(0) == 'E' &&
		get_buf(1) == 0) /* error or execute command */
	    )
	    {
		return 2000;
	    }
    ...
    else if((lookahead_size >= 4 && get_buf(1) == 0 && get_buf(2) == 0 &&
	     get_buf(2) == 0) || /* matches command */
	    (lookahead_size >= 16 &&
	     (*(s32 *)&get_buf(12) == 1 || /* matches header */
	      *(s32 *)&get_buf(12) == 2001 || *(s32 *)&get_buf(12) == 2002 ||
	      *(s32 *)&get_buf(12) == 2003 || *(s32 *)&get_buf(12) == 2004 ||
	      *(s32 *)&get_buf(12) == 2005 || *(s32 *)&get_buf(12) == 2006 ||
	      *(s32 *)&get_buf(12) == 2007)))
    {
	return 2000;
    }
	...
	if(lookahead_size >= 5)
	{
	    ...
	    if(buf == 0x20544547 || // "GET "
	       ...
	       buf == 0x4954504F || // "OPTI"
	       (buf == 0x50545448 &&
		data->buf[(data->state->tail_ctx.curoff + 4) &
			  SCRATCH_SIZE_HALF] == '/'))
	    { // "HTTP/"
		return 2000;
	    }
...
static __always_inline int unix_socket_path(char *dest, const char *user_ptr,
					    size_t size)
{
    int res = bpf_probe_read_str(dest, size, user_ptr);
    /*
...
static __always_inline u16 bpf_pack_addr(struct filler_data *data,
					 struct sockaddr *usrsockaddr, int ulen)
{
    u32 ip;
    u16 port;
	...
	data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF] =
		socket_family_to_scap(family);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 1) &
			  SCRATCH_SIZE_HALF],
	       &ip, 4);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 5) &
			  SCRATCH_SIZE_HALF],
	       &port, 2);
	...
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 17) &
			  SCRATCH_SIZE_HALF],
	       &port, 2);
	...
	res = unix_socket_path(&data->buf[(data->state->tail_ctx.curoff + 1) &
					  SCRATCH_SIZE_HALF],
...
static __always_inline long bpf_fd_to_socktuple(struct filler_data *data,
						int fd,
						struct sockaddr *usrsockaddr,
						int ulen, bool use_userdata,
						bool is_inbound, char *tmp_area)
{
    struct sockaddr_storage *sock_address;
    struct sockaddr_storage *peer_address;
	...
	{
	    struct sockaddr_in *usrsockaddr_in =
		    (struct sockaddr_in *)usrsockaddr;
	...
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 5) &
			  SCRATCH_SIZE_HALF],
	       &sport, 2);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 7) &
			  SCRATCH_SIZE_HALF],
	       &dip, 4);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 11) &
			  SCRATCH_SIZE_HALF],
	       &dport, 2);
	...
	else
	{
	     ...
	     */
	    struct sockaddr_in6 *usrsockaddr_in6 =
		    (struct sockaddr_in6 *)usrsockaddr;
	...
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 17) &
			  SCRATCH_SIZE_HALF],
	       &sport, 2);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 19) &
			  SCRATCH_SIZE_HALF],
	       dip6, 16);
	memcpy(&data->buf[(data->state->tail_ctx.curoff + 35) &
			  SCRATCH_SIZE_HALF],
	       &dport, 2);
	...
	if(is_inbound)
	{
	    memcpy(&data->buf[(data->state->tail_ctx.curoff + 1) &
			      SCRATCH_SIZE_HALF],
		   &us, 8);
	    memcpy(&data->buf[(data->state->tail_ctx.curoff + 1 + 8) &
			      SCRATCH_SIZE_HALF],
		   &speer, 8);
	}
	else
	...
	else
	{
	     ...
	     */
	    struct sockaddr_un *usrsockaddr_un =
		    (struct sockaddr_un *)usrsockaddr;
	...
	int res = unix_socket_path(
		&data->buf[(data->state->tail_ctx.curoff + 1 + 8 + 8) &
			   SCRATCH_SIZE_HALF],
...
static __always_inline int __bpf_val_to_ring(struct filler_data *data,
					     ...
					     u8 dyn_idx, bool enforce_snaplen)
{
	...
	*((u8 *)&data->buf[curoff_bounded]) = dyn_idx;
	...
	if(!data->curarg_already_on_frame)
	{
	    ...
	    res = bpf_probe_read_str(&data->buf[curoff_bounded],
				     PPM_MAX_ARG_SIZE, (const void *)val);
...
#else
		if(bpf_probe_read(&data->buf[curoff_bounded],
				  read_size & SCRATCH_SIZE_HALF, (void *)val))
#endif
			return PPM_FAILURE_INVALID_USER_MEMORY;
	    }
	}
	...
	if(!data->curarg_already_on_frame)
	{
	    bpf_printk("expected arg already on frame: evt_type %d, curarg %d, "
		       "type %d\n",
	...
	*((s64 *)&data->buf[curoff_bounded]) = val;
    ...
    default:
    {
	bpf_printk("unhandled type in bpf_val_to_ring: evt_type %d, curarg %d, "
		   "type %d\n",
		   data->state->tail_ctx.evt_type, data->state->tail_ctx.curarg,
		   type);
	return PPM_FAILURE_BUG;
    }
    }
					   ...
					   unsigned long val)
{
    ...
    param_info = &data->evt->params[data->state->tail_ctx.curarg &
				    (PPM_MAX_EVENT_PARAMS - 1)];
