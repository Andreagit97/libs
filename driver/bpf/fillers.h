...
/* Linux kernel 4.15 introduced the new const `UID_GID_MAP_MAX_BASE_EXTENTS` in
 * place of the old `UID_GID_MAP_MAX_EXTENTS`, which instead has changed its
 * meaning. For more info see
...
#define FILLER_RAW(x)                                                          \
    static __always_inline int __bpf_##x(struct filler_data *data);            \
                                                                               \
    __bpf_section(TP_NAME                                                      \
		  "filler/" #x) static __always_inline int bpf_##x(void *ctx)
...
FILLER_RAW(terminate_filler)
{
	...
	bpf_printk("PPM_FAILURE_BUFFER_FULL event=%d curarg=%d\n",
		   state->tail_ctx.evt_type, state->tail_ctx.curarg);
...
FILLER(sys_empty, true) { return PPM_SUCCESS; }
					      ...
					      bool enter_event)
{
    ...
    if(read_size)
	if(bpf_probe_read(fds, ((read_size - 1) & SCRATCH_SIZE_MAX) + 1,
			  (void *)val))
#else
...
#pragma unroll
    ...
    *((u16 *)&data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF]) =
	    fds_count;
    data->curarg_already_on_frame = true;
    return __bpf_val_to_ring(data, 0, off - data->state->tail_ctx.curoff,
			     PT_FDLIST, -1, false);
}
...
bpf_parse_readv_writev_bufs(struct filler_data *data,
			    const struct iovec __user *iovsrc,
			    unsigned long iovcnt, long retval, int flags)
{
...
#ifdef BPF_FORBIDS_ZERO_ACCESS
    if(copylen)
	if(bpf_probe_read((void *)iov, ((copylen - 1) & SCRATCH_SIZE_MAX) + 1,
			  (void *)iovsrc))
#else
    if(bpf_probe_read((void *)iov, copylen & SCRATCH_SIZE_MAX, (void *)iovsrc))
#endif
	    return PPM_FAILURE_INVALID_USER_MEMORY;
...
#else
		if(bpf_probe_read(&data->buf[off_bounded],
				  to_read & SCRATCH_SIZE_HALF, iov[j].iov_base))
#endif
			return PPM_FAILURE_INVALID_USER_MEMORY;
...
FILLER(sys_writev_e, true)
{
    ...
    val = bpf_syscall_get_argument(data, 1);
    iovcnt = bpf_syscall_get_argument(data, 2);
    res = bpf_parse_readv_writev_bufs(data, (const struct iovec __user *)val,
				      iovcnt, 0,
...
FILLER(sys_connect_e, true)
{
	...
	if(usrsockaddr && val != 0)
	{
		 ...
		 */
		size = bpf_pack_addr(data, (struct sockaddr *)data->tmp_scratch,
				     val);
	    }
	}
    }
...
FILLER(sys_connect_x, true)
{
	...
	if(usrsockaddr && val != 0)
	{
		 ...
		 * Convert the fd into socket endpoint information
		 */
		size = bpf_fd_to_socktuple(
			data, fd, (struct sockaddr *)data->tmp_scratch, val,
			true, false,
...
static int __always_inline parse_sockopt(struct filler_data *data, int level,
					 int optname, void *optval, int optlen)
{
	...
	case SO_ERROR:
		...
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    return bpf_val_to_ring_dyn(data, -u.val32, PT_ERRNO,
	...
	case SO_SNDTIMEO:
		...
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    return bpf_val_to_ring_dyn(
		    data, u.tv.tv_sec * 1000000000 + u.tv.tv_usec * 1000,
		    PT_RELTIME, PPM_SOCKOPT_IDX_TIMEVAL);

#ifdef SO_COOKIE
	case SO_COOKIE:
	    if(bpf_probe_read(&u.val64, sizeof(u.val64), optval))
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    return bpf_val_to_ring_dyn(data, u.val64, PT_UINT64,
				       PPM_SOCKOPT_IDX_UINT64);
#endif
	...
	case SO_INCOMING_CPU:
	    ...
	    if(bpf_probe_read(&u.val32, sizeof(u.val32), optval))
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    return bpf_val_to_ring_dyn(data, u.val32, PT_UINT32,
				       PPM_SOCKOPT_IDX_UINT32);

	default:
	    return __bpf_val_to_ring(data, (unsigned long)optval, optlen,
				     PT_BYTEBUF, PPM_SOCKOPT_IDX_UNKNOWN,
				     false);
	}
    }
    else
    {
	return __bpf_val_to_ring(data, (unsigned long)optval, optlen,
				 PT_BYTEBUF, PPM_SOCKOPT_IDX_UNKNOWN, false);
    }
}
...
FILLER(sys_setsockopt_x, true)
{
    ...
    res = bpf_val_to_ring_type(data, sockopt_optname_to_scap(level, optname),
			       PT_FLAGS8);
    if(res != PPM_SUCCESS)
	return res;
...
FILLER(sys_sendto_e, true)
{
	...
	if(err >= 0)
	{
	     ...
	     */
	    size = bpf_fd_to_socktuple(
		    data, fd, (struct sockaddr *)data->tmp_scratch, val, true,
		    false, data->tmp_scratch + sizeof(struct sockaddr_storage));
	}
    }
...
{
#if(PPM_RHEL_RELEASE_CODE > 0 &&                                               \
    PPM_RHEL_RELEASE_CODE >= PPM_RHEL_RELEASE_VERSION(8, 1))
...
static __always_inline struct pid_namespace *
bpf_task_active_pid_ns(struct task_struct *tsk)
{
...
static __always_inline int
__bpf_append_cgroup(struct css_set *cgroups, int subsys_id, char *buf, int *len)
{
    ...
    int res = bpf_probe_read_str(&buf[off_bounded], SCRATCH_SIZE_HALF,
				 subsys_name);
    if(res == -EFAULT)
	return PPM_FAILURE_INVALID_USER_MEMORY;
...
#pragma unroll MAX_CGROUP_PATHS
	...
	if(cgroup_path[k])
	{
	    ...
	    res = bpf_probe_read_str(&buf[off_bounded], SCRATCH_SIZE_HALF,
				     cgroup_path[k]);
	    if(res > 1)
	    {
		off += res - 1;
...
#pragma unroll
	...
	len = bpf_probe_read_str(&data->buf[off & SCRATCH_SIZE_HALF],
				 SCRATCH_SIZE_HALF, arg);
	if(len == -EFAULT)
	    return PPM_FAILURE_INVALID_USER_MEMORY;
	...
	*args_len += len;
	...
	if(*args_len > ARGS_ENV_SIZE_MAX)
	{
	    *args_len = ARGS_ENV_SIZE_MAX;
	    data->buf[(data->state->tail_ctx.curoff + *args_len - 1) &
		      SCRATCH_SIZE_MAX] = 0;
	    break;
	}
    }
...
static __always_inline bool bpf_groups_search(struct group_info *group_info,
					      kgid_t grp)
{
...
static __always_inline u32 bpf_map_id_up(struct uid_gid_map *map, u32 id)
{
    ...
    /* Kernel 4.15 increased the number of extents to `340` while all the
     * previous kernels have the limit set to `5`. So the `if` case should be
     * enough.
     */
...
static __always_inline bool bpf_kuid_has_mapping(struct user_namespace *targ,
						 kuid_t kuid)
{
...
static __always_inline bool bpf_kgid_has_mapping(struct user_namespace *targ,
						 kgid_t kgid)
{
...
static __always_inline bool get_exe_writable(struct task_struct *task)
{
    ...
    // inode_owner_or_capable check. If the owner matches the exe counts as
    // writable
    ...
    // Basic file permission check -- this may not work in all cases as kernel
    // functions are more complex and take into account different types of ACLs
    // which can use custom function pointers, but I don't think we can inspect
    // those in eBPF

    // basic acl_permission_check()

    // XXX this doesn't attempt to locate extra POSIX ACL checks (if supported
    // by the kernel)
    ...
    if(cap_raised(_READ(cred->cap_effective), CAP_DAC_OVERRIDE) &&
       kuid_mapped && kgid_mapped)
    {
	return true;
    }

    // Check if the user is capable. Even if it doesn't own the file or the read
    // bits are not set, root with CAP_FOWNER can do what it wants.
...
FILLER(proc_startupdate, true)
{
	...
	if(args_len)
	{
...
#ifdef BPF_FORBIDS_ZERO_ACCESS
	    if(bpf_probe_read(&data->buf[data->state->tail_ctx.curoff &
					 SCRATCH_SIZE_HALF],
	    ...
	    if(bpf_probe_read(&data->buf[data->state->tail_ctx.curoff &
					 SCRATCH_SIZE_HALF],
			      args_len & SCRATCH_SIZE_HALF, (void *)arg_start))
#endif
		args_len = 0;
	    else
		data->buf[(data->state->tail_ctx.curoff + args_len - 1) &
	 ...
	 */
	data->curarg_already_on_frame = true;
	res = __bpf_val_to_ring(data, 0, args_len - exe_len, PT_BYTEBUF, -1,
				false);
	if(res != PPM_SUCCESS)
	    return res;
    }
...
FILLER(proc_startupdate_2, true)
{
    ...
    res = __bpf_val_to_ring(data, (unsigned long)data->tmp_scratch, cgroups_len,
			    PT_BYTEBUF, -1, false);
    if(res != PPM_SUCCESS)
	return res;
...
FILLER(proc_startupdate_3, true)
{
...
#ifdef __NR_clone3
	    flags = bpf_syscall_get_argument(data, 0);
	    if(bpf_probe_read(&cl_args, sizeof(struct clone_args),
			      (void *)flags))
	    {
		return PPM_FAILURE_INVALID_USER_MEMORY;
	    }
	...
	else
	{
		...
		struct pid_namespace *pid_ns_for_children =
			_READ(nsproxy->pid_ns_for_children);
	...
	if(retval >= 0)
	{
		...
		if(bpf_probe_read(&data->buf[data->state->tail_ctx.curoff &
		    ...
		    data->buf[(data->state->tail_ctx.curoff + env_len - 1) &
			      SCRATCH_SIZE_MAX] = 0;
	    }
	}
	else
...
FILLER(sys_recvfrom_x, true)
{
	...
	if(usrsockaddr && val != 0)
	{
	    if(bpf_probe_read(&addrlen, sizeof(addrlen), (void *)val))
		return PPM_FAILURE_INVALID_USER_MEMORY;
		 ...
		 */
		size = bpf_fd_to_socktuple(
			data, fd, (struct sockaddr *)data->tmp_scratch, addrlen,
			true, true,
			data->tmp_scratch + sizeof(struct sockaddr_storage));
	    }
	}
...
FILLER(sys_recvmsg_x_2, true)
{
	...
	if(usrsockaddr && addrlen != 0)
	{
	     ...
	     * Copy the address
	     */
	    res = bpf_addr_to_kernel(usrsockaddr, addrlen,
				     (struct sockaddr *)data->tmp_scratch);
...
#pragma unroll 2
	for(j = 0; j < 2; j++)
	{
	    ...
	    if(nsops--)
		if(bpf_probe_read(&sops, sizeof(sops), (void *)&ptr[j]))
		    return PPM_FAILURE_INVALID_USER_MEMORY;
	    ...
	    res = bpf_val_to_ring_type(data, semop_flags_to_scap(sops.sem_flg),
				       PT_FLAGS16);
	    if(res != PPM_SUCCESS)
		return res;
	}
...
FILLER(sys_quotactl_x, true)
{
    ...
    if(cmd == PPM_Q_GETQUOTA || cmd == PPM_Q_SETQUOTA)
    {
	if(bpf_probe_read(&dqblk, sizeof(dqblk), (void *)val))
	    return PPM_FAILURE_INVALID_USER_MEMORY;
    }
    ...
    {
	if(bpf_probe_read(&dqinfo, sizeof(dqinfo), (void *)val))
	    return PPM_FAILURE_INVALID_USER_MEMORY;
    }
...
static __always_inline int bpf_parse_ptrace_addr(struct filler_data *data,
						 u16 request)
{
...
static __always_inline int bpf_parse_ptrace_data(struct filler_data *data,
						 u16 request)
{
    enum ppm_param_type type;
    unsigned long val;
...
FILLER(sched_prog_exec, false)
{
    ...
    int correctly_read = bpf_probe_read(
	    &data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF],
	    args_len & SCRATCH_SIZE_HALF, (void *)arg_start);
	...
	int exe_len = bpf_probe_read_str(
		&data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF],
		SCRATCH_SIZE_HALF,
		&data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF]);
...
FILLER(sched_prog_exec_3, false)
{
	...
	if(bpf_probe_read(
		   &data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF],
		   env_len & SCRATCH_SIZE_HALF, (void *)env_start))
	{
	    env_len = 0;
	}
	else
