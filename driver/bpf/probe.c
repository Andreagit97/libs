 ...
 * the raw tracepoint arguments! This is not so relevant for
 * `sched_process_exec` since we can access all needed information from the
 * current task, but it is essential for `sched_process_fork` since the only way
 * we have to access the child task struct is through the raw tracepoint
 * arguments.
 *
 * Since we need to use `BPF_PROG_TYPE_RAW_TRACEPOINT`, the ARM64 support for
 * our BPF probe requires kernel versions greater or equal than `4.17`. If you
 * run old kernels, you can use the kernel module which requires kernel versions
 * greater or equal than `3.4`.
 */
...
uint64_t probe_schema_ver __bpf_section("schema_version") =
	PPM_SCHEMA_CURRENT_VERSION;
