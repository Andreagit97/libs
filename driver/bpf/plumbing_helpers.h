...
bpf_syscall_get_argument_from_args(unsigned long *args, int idx)
{
    unsigned long arg;
...
static __always_inline unsigned long
bpf_syscall_get_argument_from_ctx(void *ctx, int idx)
{
    unsigned long arg;
...
static __always_inline unsigned long
bpf_syscall_get_argument(struct filler_data *data, int idx)
{
#ifdef BPF_SUPPORTS_RAW_TRACEPOINTS
...
static __always_inline const struct ppm_event_info *
get_event_info(enum ppm_event_type event_type)
{
...
static __always_inline const struct ppm_event_entry *
get_event_filler_info(enum ppm_event_type event_type)
{
...
static __always_inline struct scap_bpf_per_cpu_state *
get_local_state(unsigned int cpu)
{
...
acquire_local_state(struct scap_bpf_per_cpu_state *state)
{
    if(state->in_use)
    {
...
static __always_inline bool
release_local_state(struct scap_bpf_per_cpu_state *state)
{
    if(!state->in_use)
    {
...
static __always_inline int init_filler_data(void *ctx, struct filler_data *data,
					    bool is_syscall)
{
    unsigned int cpu;
...
static __always_inline void call_filler(void *ctx, void *stack_ctx,
					enum ppm_event_type evt_type,
					struct scap_bpf_settings *settings,
					enum syscall_flags drop_flags)
{
    ...
    bpf_printk("Can't tail call filler evt=%d, filler=%d\n",
	       state->tail_ctx.evt_type, filler_info->filler_id);
