...
static __always_inline unsigned long bpf_syscall_get_socketcall_arg(void *ctx, int idx)
{
	...
	struct sys_enter_args *args = (struct sys_enter_args *)ctx;
	user_pt_regs *user_regs = (user_pt_regs *)args->regs;
	/* Get the first argument that is a pointer to the real socket syscall */
	bpf_probe_read_kernel(&args_pointer, sizeof(unsigned long), (void*)&user_regs->gprs[3]);
...
static __always_inline long conver_network_syscalls(void *ctx)
{
