...
static __always_inline void write_evt_hdr(struct filler_data *data)
{
    ...
    data->state->tail_ctx.curoff =
	    sizeof(struct ppm_evt_hdr) + sizeof(u16) * data->evt->nparams;
					      ...
					      unsigned int arglen)
{
    ...
    *((u16 *)&p[sizeof(struct ppm_evt_hdr)] +
      (argnumv & (PPM_MAX_EVENT_PARAMS - 1))) = arglen;
}
...
static __always_inline int push_evt_frame(void *ctx, struct filler_data *data)
{
    ...
    {
	bpf_printk("corrupted filler for event type %d (added %u args, should "
		   "have added %u)\n",
    ...
    int res = bpf_perf_event_output(
	    ctx, &perf_map, BPF_F_CURRENT_CPU, data->buf,
	    ((data->state->tail_ctx.len - 1) & SCRATCH_SIZE_MAX) + 1);
#else
    int res =
	    bpf_perf_event_output(ctx, &perf_map, BPF_F_CURRENT_CPU, data->buf,
