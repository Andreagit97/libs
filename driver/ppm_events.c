 ...
 * strncpy_from_user, instead of returning -1 after a page fault, schedules the
 * process, so we drop events because of the preemption. This function reads the
 * user buffer in atomic chunks, and returns when there's an error or the
 * terminator is found
 */
long ppm_strncpy_from_user(char *to, const char __user *from, unsigned long n)
{
    ...
    {
	/*
	 * Read bytes_to_read bytes at a time, and look for the terminator.
	 * Should be fast since the copy_from_user is optimized for the
	 * processor
	 */
	if(n < bytes_to_read)
	    bytes_to_read = n;
...
#ifndef UDIG
inline int sock_getname(struct socket *sock, struct sockaddr *sock_address,
			int peer)
{
			   ...
			   u32 lookahead_size)
{
	...
	else
	{
		 ...
		 * Copy the address
		 */
		err = addr_to_kernel(usrsockaddr, val,
	...
	else
	{
	    ...
	    {
		usrsockaddr = (struct sockaddr __user *)compat_ptr(
			compat_mh.msg_name);
    ...
    if(max_port > 0 && (in_port_range(sport, min_port, max_port) ||
	...
	if(lookahead_size >= 5)
	{
	    if(buf[0] == 3 || buf[1] == 3 || buf[2] == 3 || buf[3] == 3 ||
	       buf[4] == 3)
	    {
		res = dynamic_snaplen;
		goto done;
	...
	if(lookahead_size >= 7 && (buf[4] == 0 && buf[5] == 3 && buf[6] == 0))
	{ /* startup command */
	    res = dynamic_snaplen;
	    goto done;
	}
 ...
 */
int val_to_ring(struct event_filler_arguments *args, uint64_t val, u32 val_len,
		bool fromuser, u8 dyn_idx)
{
    ...
    {
#ifndef UDIG
	pr_err("(%u)val_to_ring: too many arguments for event #%u, type=%u, "
	       "curarg=%u, nargs=%u tid:%u\n",
	       smp_processor_id(), args->nevents, (u32)args->event_type,
	       args->curarg, args->nargs, current->pid);
	...
	dyn_params = (const struct ppm_param_info *)patch_pointer(
		(uint8_t *)param_info->info);
#else
	...
	*psize = 0;
	...
	else
	{
	     ...
	     */
	    len = (int)strlcpy(args->buffer + args->arg_data_offset, "(NULL)",
			       max_arg_size);
	...
	if(likely(val != 0 && val_len))
	{
	    ...
	    {
		/*
		 * Copy the lookahead portion of the buffer that we will use
		 * DPI-based snaplen calculation
		 */
		...
		if(likely((dpi_lookahead_size != val_len)))
		{
...
#ifndef UDIG
			sl = compute_snaplen(
				args, args->buffer + args->arg_data_offset,
				dpi_lookahead_size);
#endif
			if(val_len > sl)
			    val_len = sl;
			...
			len = (int)ppm_copy_from_user(
				args->buffer + args->arg_data_offset +
					dpi_lookahead_size,
				(const uint8_t __user *)(syscall_arg_t)val +
					dpi_lookahead_size,
		...
		if(likely(args->enforce_snaplen))
		{
		    ...
		    u32 sl = args->consumer->snaplen;
#else
		    u32 sl = compute_snaplen(args, (char *)(syscall_arg_t)val,
					     val_len);
#endif
		    if(val_len > sl)
			val_len = sl;
	...
	if(likely(val != 0))
	{
		...
		len = (int)ppm_copy_from_user(
			args->buffer + args->arg_data_offset,
			(const void __user *)(syscall_arg_t)val, val_len);
	...
	ASSERT(0);
#ifndef UDIG
	pr_err("val_to_ring: invalid argument type %d. Event %u (%s) might "
	       "have less parameters than what has been declared in nparams\n",
	       (int)g_event_info[args->event_type].params[args->curarg].type,
	       (u32)args->event_type, g_event_info[args->event_type].name);
#endif
	return PPM_FAILURE_BUG;
    }
...
/*
static struct socket *ppm_sockfd_lookup_light(int fd, int *err, int
*fput_needed)
{
	struct file *file;
	struct socket *sock;
...
static void unix_socket_path(char *dest, const char *path, size_t size)
{
    ...
    {
	snprintf(dest, size, "%s", path); /* we assume this will be smaller than
					     (targetbufsize - (1 + 8 + 8)) */
    }
}
 ...
 * targetbuf
 */
u16 pack_addr(struct sockaddr *usrsockaddr, int ulen, char *targetbuf,
	      u16 targetbufsize)
{
    u32 ip;
    u16 port;
 ...
 * targetbuf
 */
u16 fd_to_socktuple(int fd, struct sockaddr *usrsockaddr, int ulen,
		    bool use_userdata, bool is_inbound, char *targetbuf,
		    u16 targetbufsize)
{
    int err = 0;
#ifdef WDIG
     ...
     * NOTE: sockfd_lookup() locks the socket, so we don't need to worry when we
     * dig in it
     */
    ...
    {
	/*
	 * This usually happens if the call failed without being able to
	 * establish a connection, i.e. if it didn't return something like
	 * SE_EINPROGRESS.
	 */
	if(sock)
	    sockfd_put(sock);
	...
	if(!use_userdata)
	{
	    ...
	    err = udig_getpeername(fd, (struct sockaddr *)&peer_address,
				   &palen);
#else
	    err = sock_getname(sock, (struct sockaddr *)&peer_address, 1);
#endif
		...
		if(is_inbound)
		{
		    sip = ((struct sockaddr_in *)&peer_address)
				  ->sin_addr.s_addr;
		    sport = ntohs(
			    ((struct sockaddr_in *)&peer_address)->sin_port);
		    dip = ((struct sockaddr_in *)&sock_address)
				  ->sin_addr.s_addr;
		    dport = ntohs(
		...
		{
		    sip = ((struct sockaddr_in *)&sock_address)
				  ->sin_addr.s_addr;
		    sport = ntohs(
			    ((struct sockaddr_in *)&sock_address)->sin_port);
		    dip = ((struct sockaddr_in *)&peer_address)
				  ->sin_addr.s_addr;
		    dport = ntohs(
			    ((struct sockaddr_in *)&peer_address)->sin_port);
		}
	...
	*(u16 *)(targetbuf + 11) = dport;
	...
	if(!use_userdata)
	{
	    ...
	    if(is_inbound)
	    {
		sip6 = ((struct sockaddr_in6 *)&peer_address)
			       ->sin6_addr.s6_addr;
		sport = ntohs(
			((struct sockaddr_in6 *)&peer_address)->sin6_port);
		dip6 = ((struct sockaddr_in6 *)&sock_address)
			       ->sin6_addr.s6_addr;
		dport = ntohs(
			((struct sockaddr_in6 *)&sock_address)->sin6_port);
	    }
	    else
	    {
		sip6 = ((struct sockaddr_in6 *)&sock_address)
			       ->sin6_addr.s6_addr;
		sport = ntohs(
			((struct sockaddr_in6 *)&sock_address)->sin6_port);
		dip6 = ((struct sockaddr_in6 *)&peer_address)
			       ->sin6_addr.s6_addr;
		dport = ntohs(
			((struct sockaddr_in6 *)&peer_address)->sin6_port);
	    }
...
int32_t parse_readv_writev_bufs(struct event_filler_arguments *args,
				...
				unsigned long iovcnt, int64_t retval, int flags)
{
    int32_t res;
	...
	/*
	 * Size is the total size of the buffers provided by the user. The
	 * number of received bytes can be smaller
	 */
	...
	if(retval > 0 && iovcnt > 0)
	{
		...
		if((flags & PRB_FLAG_IS_WRITE) == 0)
		{
		    ...
		    tocopy_len =
			    min(tocopy_len, (size_t)targetbuflen - bufsize - 1);
		}
		else
		{
		    tocopy_len =
			    min(iov[j].iov_len, targetbuflen - bufsize - 1);
		}

		notcopied_len = (int)ppm_copy_from_user(
			targetbuf + bufsize, iov[j].iov_base, tocopy_len);
	    ...
	    res = val_to_ring(args, (unsigned long)targetbuf, bufsize, false,
			      0);
	    if(unlikely(res != PPM_SUCCESS))
		return res;
	}
...
int32_t compat_parse_readv_writev_bufs(struct event_filler_arguments *args,
				       ...
				       unsigned long iovcnt, int64_t retval,
				       int flags)
{
    int32_t res;
	...
	if(retval > 0 && iovcnt > 0)
	{
		...
		if((flags & PRB_FLAG_IS_WRITE) == 0)
		{
		    ...
		    tocopy_len = min(iov[j].iov_len,
				     (compat_size_t)((size_t)retval - bufsize));
		    tocopy_len = min(tocopy_len, (compat_size_t)(targetbuflen -
								 bufsize - 1));
		}
		else
		{
		    tocopy_len =
			    min(iov[j].iov_len,
				(compat_size_t)(targetbuflen - bufsize - 1));
		}

		notcopied_len = (int)ppm_copy_from_user(
			targetbuf + bufsize, compat_ptr(iov[j].iov_base),
			tocopy_len);
...
int f_sys_autofill(struct event_filler_arguments *args)
{
	...
	else if(evinfo->autofill_args[j].id == AF_ID_RETVAL)
	{
	     ...
	     * Return value
	     */
	    retval = (int64_t)(long)syscall_get_return_value(current,
							     args->regs);
	...
	else if(evinfo->autofill_args[j].id == AF_ID_USEDEFAULT)
	{
	     ...
	     * Default Value
	     */
	    res = val_to_ring(args, evinfo->autofill_args[j].default_val, 0,
			      false, 0);
	    if(unlikely(res != PPM_SUCCESS))
		return res;
	}
