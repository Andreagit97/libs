...
void event_test::client_to_server(send_data send_d, receive_data receive_d, protocol_L3 proto_L3, protocol_L4 proto_L4)
{
	...
	switch(proto_L3)
	{
		case protocol_L3::IPv4:
			...
			{
				this->connect_ipv4_client_to_server(&client_socket_fd, &client_addr, &server_socket_fd, &server_addr);
			}
			else
			{
				this->connect_ipv4_udp_client_to_server(&client_socket_fd, &client_addr, &server_socket_fd, &server_addr);
			}
			// for the `recv*` syscalls we will use the memory of the server sockaddr but this will be overwritten by the kernel so it shouldn't be an issue.
		...
		case protocol_L3::IPv6:
			if(proto_L4 == protocol_L4::TCP)
			{
				this->connect_ipv6_client_to_server(&client_socket_fd, &client_addr6, &server_socket_fd, &server_addr6);
			}
			else
			{
				this->connect_ipv6_udp_client_to_server(&client_socket_fd, &client_addr6, &server_socket_fd, &server_addr6);
			}
	...
	case __NR_sendto:
		{
			...
			sent_data = (const void*)MESSAGE_SHORTER_THAN_SNAPLEN;
			sent_data_len = MESSAGE_SHORTER_THAN_SNAPLEN_LEN;

			if(send_d.greater_snaplen)
			{
				sent_data = (const void*)MESSAGE_LONGER_THAN_SNAPLEN;
				sent_data_len = MESSAGE_LONGER_THAN_SNAPLEN_LEN;
			}
			...
			int64_t sent_bytes = syscall(__NR_sendto, client_socket_fd, sent_data, sent_data_len, sendto_flags, addr, addrlen);
	...
	case __NR_sendmsg:
		{
			...
			iov[0].iov_base = (void*)MESSAGE_SHORTER_THAN_SNAPLEN;
			iov[0].iov_len = MESSAGE_SHORTER_THAN_SNAPLEN_LEN;
			
			if(send_d.greater_snaplen)
			{
				iov[0].iov_base = (void*)MESSAGE_LONGER_THAN_SNAPLEN;
				iov[0].iov_len = MESSAGE_LONGER_THAN_SNAPLEN_LEN;
			}
	...
	case __NR_write:
		{
			const void* sent_data = (const void*)MESSAGE_SHORTER_THAN_SNAPLEN;
			size_t sent_data_len = MESSAGE_SHORTER_THAN_SNAPLEN_LEN;
	...
	case __NR_recvfrom:
		{
			...
			int64_t received_bytes = syscall(__NR_recvfrom, receive_socket_fd, received_data_ptr, received_data_len, recvfrom_flags, addr, &addrlen);
