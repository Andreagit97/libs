...
/* This test is useful to check the behavior of the 3 drivers with an empty string
 * passed from userspace
 */
TEST(SyscallExit, chdirX_empty_dir)
{
	auto evt_test = get_syscall_event_test(__NR_chdir, EXIT_EVENT);
	...
	const char* new_dir = EMPTY_STRING;
	assert_syscall_state(SYSCALL_FAILURE, "chdir", syscall(__NR_chdir, new_dir));
	int64_t errno_value = -errno;
