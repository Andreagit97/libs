...
TEST(SyscallExit, recvmsgX_ipv4_tcp_message_shorter_than_snaplen)
{
	...
	evt_test->assert_numeric_param(1, (int64_t)MESSAGE_SHORTER_THAN_SNAPLEN_LEN);

	/* Parameter 2: size (type: PT_UINT32) */
	evt_test->assert_numeric_param(2, (uint32_t)MESSAGE_SHORTER_THAN_SNAPLEN_LEN);

	/* Parameter 3: data (type: PT_BYTEBUF) */
	evt_test->assert_bytebuf_param(3, MESSAGE_SHORTER_THAN_SNAPLEN, MESSAGE_SHORTER_THAN_SNAPLEN_LEN);
...
TEST(SyscallExit, recvmsgX_ipv4_tcp_message_longer_than_snaplen_truncated)
{
	...
	/* Parameter 3: data (type: PT_BYTEBUF) */
	evt_test->assert_bytebuf_param(3, MESSAGE_LONGER_THAN_SNAPLEN, DEFAULT_SNAPLEN);
...
TEST(SyscallExit, recvmsgX_ipv4_tcp_message_longer_than_snaplen_not_truncated_fullcapture_port)
{
	...
	/* Parameter 3: data (type: PT_BYTEBUF) */
	evt_test->assert_bytebuf_param(3, MESSAGE_LONGER_THAN_SNAPLEN, MAX_RECV_BUF_SIZE);
...
TEST(SyscallExit, recvmsg_ancillary_data)
{
	...
	struct iovec iov = {
		.iov_base = (void *)MESSAGE_LONGER_THAN_SNAPLEN,
		.iov_len = MESSAGE_LONGER_THAN_SNAPLEN_LEN,
	};
	...
	{
		char buf[MESSAGE_LONGER_THAN_SNAPLEN_LEN];
	...
	/* Parameter 1: res (type: PT_ERRNO) */
	evt_test->assert_numeric_param(1, (int64_t)MESSAGE_LONGER_THAN_SNAPLEN_LEN);

	/* Parameter 2: size (type: PT_UINT32) */
	evt_test->assert_numeric_param(2, (uint32_t)MESSAGE_LONGER_THAN_SNAPLEN_LEN);
