...
const static struct luaL_Reg ll_sysdig[] = {
	...
	{"get_thread_table_barebone_nofds",
	 &lua_cbacks::get_thread_table_barebone_nofds},
...
const static struct luaL_Reg ll_evt[] = {{"field", &lua_cbacks::field},
...
sinsp_chisel::~sinsp_chisel() { free_lua_chisel(); }
...
void parse_lua_chisel_arg(lua_State* ls, OUT chisel_desc* cd)
{
	...
	{
	    throw sinsp_exception(string(lua_tostring(ls, -2)) +
				  " is not a string");
	}
...
void sinsp_chisel::parse_view_column(lua_State* ls, OUT chisel_desc* cd,
				     OUT void* columns)
{
    vector<sinsp_view_column_info>* cols =
	...
	else if(fldname == "tags")
	{
		...
		while(lua_next(ls, -2) != 0)
		{
			...
			throw sinsp_exception(
				"tags column entries must be strings");
		    }
    ...
    if(filterfield != "" && ((flags & TEF_IS_KEY) == 0) &&
       ((flags & TEF_IS_GROUPBY_KEY) == 0))
    {
	throw sinsp_exception("wrong view column syntax: filterfield specified "
			      "for a non key column");
    }

    cols->push_back(sinsp_view_column_info(
	    field, name, description, colsize, (uint32_t)flags, aggregation,
	    groupby_aggregation, tags, filterfield));
}

void sinsp_chisel::parse_view_columns(lua_State* ls, OUT chisel_desc* cd,
				      OUT void* columns)
{
    string name;
    string type;
...
void sinsp_chisel::parse_view_action(lua_State* ls, OUT chisel_desc* cd,
				     OUT void* actions)
{
    vector<sinsp_view_action_info>* keys =
	...
	if(fldname == "hotkey")
	{
	    ...
	    {
		throw sinsp_exception(
			"action 'key' field must be a single character string");
	    }
	}
    ...
    keys->push_back(sinsp_view_action_info(key, command, description,
					   ask_confirmation, waitfinish));
}

void sinsp_chisel::parse_view_actions(lua_State* ls, OUT chisel_desc* cd,
				      OUT void* actions)
{
    string name;
    string type;
...
bool sinsp_chisel::parse_view_info(lua_State* ls, OUT chisel_desc* cd)
{
	...
	else if(fldname == "tags")
	{
		...
		while(lua_next(ls, -2) != 0)
		{
		    ...
		    {
			throw sinsp_exception("error in view " + cd->m_name +
					      ": " +
	...
	else if(fldname == "tips")
	{
		...
		while(lua_next(ls, -2) != 0)
		{
		    ...
		    {
			throw sinsp_exception(
				"error in view " + cd->m_name + ": " +
				"tips column entries must be strings");
		    }
    ...
    cd->m_viewinfo = sinsp_view_info(
	    vt, id, name, description, tags, tips, columns, applies_to, filter,
	    drilldown_target, use_defaults, is_root, actions,
	    drilldown_increase_depth, spectro_type, propagate_filter);
...
void sinsp_chisel::get_chisel_list(vector<chisel_desc>* chisel_descs)
{
	...
	while(dir.has_next)
	{
	    ...
	    for(vector<chisel_desc>::const_iterator it_desc =
			chisel_descs->begin();
...
void sinsp_chisel::load(string cmdstr, bool is_file)
{
    ...
    {
	throw sinsp_exception("Failed to load chisel " + m_filename + ": " +
			      lua_tostring(m_ls, -1));
    }
...
void sinsp_chisel::set_args(string args)
{
	...
	for(j = 0; j < args.size(); j++)
	{
	    if(args[j] == ' ' && !inquotes)
	    {
		m_argvals.push_back(args.substr(
			token_begin, j - quote_correction - token_begin));
		token_begin = j + 1;
		quote_correction = 0;
	    }
	...
	if(inquotes)
	{
	    throw sinsp_exception("corrupted parameters for chisel " +
				  m_filename);
	}

	m_argvals.push_back(
		args.substr(token_begin, j - quote_correction - token_begin));
    }
    ...
    {
	throw sinsp_exception(
		"wrong number of parameters for chisel " + m_filename + ", " +
    ...
    {
	throw sinsp_exception(
		"too many parameters for chisel " + m_filename + ", " +
		to_string((long long int)(n_required_args)) + " required, " +
...
void sinsp_chisel::set_args(vector<pair<string, string>> args)
{
	...
	{
	    lua_pop(m_ls, 1);
	    throw sinsp_exception("chisel " + m_filename +
				  " misses a set_arg() function.");
	}
	...
	{
	    throw sinsp_exception(m_filename +
				  " chisel error: " + lua_tostring(m_ls, -1));
	}
	...
	if(!lua_isboolean(m_ls, -1))
	{
	    throw sinsp_exception(
		    m_filename +
		    " chisel error: wrong set_arg() return value.");
	}
	...
	if(!sares)
	{
	    throw sinsp_exception("set_arg() for chisel " + m_filename +
				  " failed.");
	}
...
void sinsp_chisel::on_init()
{
	...
	{
	    throw sinsp_exception(m_filename +
				  " chisel error: wrong init() return value.");
	}

	if(!lua_toboolean(m_ls, -1))
	{
	    throw sinsp_exception("init() for chisel " + m_filename +
				  " failed.");
	}
    }
...
void sinsp_chisel::first_event_inits(sinsp_evt* evt)
{
    ...
    {
	m_lua_last_interval_sample_time =
		ts - ts % m_lua_cinfo->m_callback_interval;
    }
...
void sinsp_chisel::do_timeout(sinsp_evt* evt)
{
	...
	if(sample_time != m_lua_last_interval_sample_time)
	{
		...
		throw sinsp_exception(
			m_filename +
			" chisel error: calling on_interval() failed:" +
			lua_tostring(m_ls, -1));
	    }
	    ...
	    if(oeres == false)
	    {
		throw sinsp_exception("execution terminated by the " +
				      m_filename + " chisel");
	    }
	...
	if(ts - m_lua_last_interval_sample_time >= interval)
	{
	    ...
	    for(t = m_lua_last_interval_sample_time; t <= ts - interval;
		t += interval)
	    {
		lua_getglobal(m_ls, "on_interval");
...
void sinsp_chisel::do_end_of_sample()
{
	...
	throw sinsp_exception(
		m_filename +
		" chisel error: calling on_end_of_sample() failed:" +
		lua_tostring(m_ls, -1));
    }
    ...
    if(oeres == false)
    {
	throw sinsp_exception("execution terminated by the " + m_filename +
			      " chisel");
    }
#endif // HAS_LUA_CHISELS
}
...
void sinsp_chisel::on_capture_start()
{
	...
	{
	    throw sinsp_exception(m_filename +
				  " chisel error: wrong on_capture_start() "
