...
uint32_t lua_cbacks::rawval_to_lua_stack(lua_State* ls, uint8_t* rawval,
					 ppm_param_type ptype, uint32_t len)
{
    ASSERT(rawval != NULL);
	...
	snprintf(ch->m_lua_fld_storage, sizeof(ch->m_lua_fld_storage),
	...
	strlcpy(ch->m_lua_fld_storage, address, sizeof(ch->m_lua_fld_storage));
	...
	{
	    throw sinsp_exception("rawval_to_lua_stack called with IP address "
				  "of incorrect size " +
				  to_string(len));
	}
    }
...
int lua_cbacks::request_field(lua_State* ls)
{
    ...
    sinsp_filter_check* chk =
	    g_filterlist.new_filter_check_from_fldname(fld, inspector, false);
...
int lua_cbacks::field(lua_State* ls)
{
    ...
    {
	// todo: Do something better here. For now, only support single-value
	// extracted fields
	return rawval_to_lua_stack(ls, chk->m_extracted_values[0].ptr,
				   chk->get_field_info()->m_type,
				   chk->m_extracted_values[0].len);
    }
    else
    {
	lua_pushnil(ls);
...
int lua_cbacks::set_global_filter(lua_State* ls)
{
    ...
    {
	string err =
		"invalid filter in chisel " + ch->m_filename + ": " + e.what();
	fprintf(stderr, "%s\n", err.c_str());
	throw sinsp_exception("chisel error");
    }
...
int lua_cbacks::set_output_format(lua_State* ls)
{
    ...
    {
	string err =
		"invalid set_output_format value in chisel " + ch->m_filename;
	fprintf(stderr, "%s\n", err.c_str());
	throw sinsp_exception("chisel error");
    }
...
int lua_cbacks::get_machine_info(lua_State* ls)
{
    ...
    if(minfo == NULL)
    {
	string err = "get_machine_info can only be called from the "
		     "on_capture_start callback";
	fprintf(stderr, "%s\n", err.c_str());
	throw sinsp_exception("chisel error");
    }
				     ...
				     bool barebone)
{
	...
	if(filterstr != "")
	{
	    ...
	    {
		string err = "invalid filter argument for get_thread_table in "
			     "chisel " +
			     ch->m_filename + ": " + e.what();
		fprintf(stderr, "%s\n", err.c_str());
		throw sinsp_exception("chisel error");
	    }
    ...
    threadinfo_map_t* threadtable =
	    ch->m_inspector->m_thread_manager->get_threads();
    ...
    threadtable->loop(
		...
		if(filter != NULL)
		{
		    ...
		    for(fdit = fdtable->m_table.begin();
			fdit != fdtable->m_table.end(); ++fdit)
		    {
			tevt.m_tinfo = &tinfo;
		...
		if(!barebone)
		{
		    ...
		    lua_pushstring(
			    ls,
			    to_string((long long int)tinfo.m_clone_ts).c_str());
		    lua_settable(ls, -3);
		...
		if(include_fds)
		{
			...
			if(!barebone)
			{
			    ...
			    lua_pushstring(
				    ls, fdit->second.tostring_clean().c_str());
			    lua_settable(ls, -3);
			...
			if(evt_type == SCAP_FD_IPV4_SOCK ||
			   evt_type == SCAP_FD_IPV4_SERVSOCK ||
			   evt_type == SCAP_FD_IPV6_SOCK ||
			   evt_type == SCAP_FD_IPV6_SERVSOCK)
			{
			    bool include_client;
				...
				af = AF_INET;
				cip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv4info
						.m_fields.m_sip);
				sip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv4info
						.m_fields.m_dip);
				cport = fdit->second.m_sockinfo.m_ipv4info
						.m_fields.m_sport;
				sport = fdit->second.m_sockinfo.m_ipv4info
						.m_fields.m_dport;
				...
				cip = NULL;
				sip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv4serverinfo
						.m_ip);
				sport = fdit->second.m_sockinfo.m_ipv4serverinfo
						.m_port;
				is_server = true;
			    }
				...
				af = AF_INET6;
				cip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv6info
						.m_fields.m_sip);
				sip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv6info
						.m_fields.m_dip);
				cport = fdit->second.m_sockinfo.m_ipv6info
						.m_fields.m_sport;
				sport = fdit->second.m_sockinfo.m_ipv6info
						.m_fields.m_dport;
				is_server = fdit->second.is_role_server();
			    }
			    else
				...
				cip = NULL;
				sip = (uint8_t*)&(
					fdit->second.m_sockinfo.m_ipv6serverinfo
						.m_ip);
				sport = fdit->second.m_sockinfo.m_ipv6serverinfo
						.m_port;
				is_server = true;
			    }
			    ...
			    if(NULL ==
			       inet_ntop(af, sip, sipbuf, sizeof(sipbuf)))
			    {
				strcpy(sipbuf, "<NA>");
			    }
			    ...
			    {
				if(NULL ==
				   inet_ntop(af, cip, cipbuf, sizeof(cipbuf)))
				{
				    strcpy(cipbuf, "<NA>");
				}
			    }
...
int lua_cbacks::get_container_table(lua_State* ls)
{
    ...
    //
    const sinsp_container_manager::map_ptr_t ctable =
	    ch->m_inspector->m_container_manager.get_containers();
...
int lua_cbacks::udp_send(lua_State* ls)
{
    ...
    if(sendto(ch->m_udp_socket, message.c_str(), message.size(), 0,
	      (struct sockaddr*)&ch->m_serveraddr,
	      sizeof(ch->m_serveraddr)) < 0)
    {
...
int lua_cbacks::push_metric(lua_State* ls)
{
    ...
    {
	string err = "error in chisel " + ch->m_filename +
    ...
    {
	string err = "errord in chisel " + ch->m_filename +
