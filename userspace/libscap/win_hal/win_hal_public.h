...
//
// This MUST be smaller of equal than SCAP_LASTERR_SIZE, or bad things will
// happen
//
...
#define WH_MAX_CGROUPS_SIZE 4096
    ...
    __declspec(dllexport) wh_proc_perf_info
	    wh_wmi_get_proc_perf_info(wh_t* handle, uint64_t pid);
    ...
    __declspec(dllexport) wh_tidlist
	    wh_get_pid_tids(wh_t* handle, uint64_t pid);
    ...
    __declspec(dllexport) wh_interfacelist
	    wh_wmi_get_net_interfaces(wh_t* handle);
    __declspec(dllexport) wh_machine_bandwidth_info
	    wh_wmi_get_machine_net_totbytes(wh_t* handle);
    __declspec(dllexport) wh_machine_disk_bandwidth_info
	    wh_wmi_get_machine_disk_bandwidth(wh_t* handle);
    ...
    __declspec(dllexport) wh_docker_container_info
	    wh_docker_resolve_pid(wh_t* handle, uint64_t pid);
    __declspec(dllexport) bool wh_query_docker(wh_t* handle, char* query,
					       const char** response);
    __declspec(dllexport) wh_docker_io_bytes
	    wh_docker_get_io_bytes(wh_t* handle, const char* container_id);
