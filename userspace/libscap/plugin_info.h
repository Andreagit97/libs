...
//         	* if the field specified by the user is foo.bar, arg will be
//         NULL
...
//   The array represent the list of extracted values for this field from a
//   given event. Each array element should be filled with a char* string if the
//   corresponding field was type==string, and with a uint64 value if the
...
//   If the field is a list type, then res_len can must be any value from 0 to
//   N, depending on how many values can be extracted from a given event.
//   Setting res_len to 0 means that no value of this field can be extracted
//   from a given event.
...
//
// The structs below define the functions and arguments for plugins
// capabilities:
// * event sourcing
// * extractor
// The structs are used by the plugin framework to load and interface with
// plugins.
//
...
typedef struct
{
    ...
    // NOTE: to ensure correct interoperability between the engine and the
    // plugins,
    //       we use a semver approach. Plugins are required to specify the
    //       version of the API they run against, and the engine will take care
    //       of checking and enforcing compatibility.
    //
    ...
    // - rc: pointer to a ss_plugin_rc that will contain the initialization
    // result Return value: pointer to the plugin state that will be treated as
    // opaque
    ...
    //
    // Return the descriptions of the plugin, which will be printed when
    // displaying information about the plugin or its events. Required: yes
    //
    ...
    // Required: yes
    // Return value: a string with a version identifier, in the following
    // format:
    //        "<major>.<minor>.<patch>", e.g. "1.2.3".
	...
	// Required: yes
	// EVERY PLUGIN WITH EVENT SOURCING CAPABILITIES MUST OBTAIN AN OFFICIAL
	// ID FROM THE FALCOSECURITY ORGANIZATION, OTHERWISE IT WON'T PROPERLY
	// COEXIST WITH OTHER PLUGINS.
	//
	uint32_t (*get_id)();
	//
	// Return a string describing the events generated by this plugin with
	// event sourcing capabilities. Required: yes Example event sources
	// would be strings like "syscall", "k8s_audit", etc. The source can be
	// used by extractor plugins to filter the events they receive.
	//
	...
	// - params: the open parameters, as a string. The format is defined by
	// the plugin
	//   itsef
	// - rc: pointer to a ss_plugin_rc that will contain the open result
	// Return value: a pointer to the open context that will be passed to
	// next_batch(),
	//   close(), event_to_string() and extract_fields.
	//
	ss_instance_t* (*open)(ss_plugin_t* s, const char* params,
			       ss_plugin_rc* rc);
	//
	// Close a capture.
	// Required: yes
	...
	// Any of the values in the returned list are valid parameters for
	// open(). Required: no Return value: a string with the list of open
	// params encoded as
	//   a json array.
	...
	//   [
	//      {"value": "resource1", "desc": "An example of openable
	//      resource"},
	//      {"value": "resource2", "desc": "Another example of openable
	//      resource"}
	//   ]
	...
	// Required: no
	// Arguments:
	// - progress_pct: the read progress, as a number between 0 (no data has
	// been read)
	//   and 10000 (100% of the data has been read). This encoding allows
	//   the engine to print progress decimals without requiring to deal
	//   with floating point numbers (which could cause incompatibility
	...
	//   get_progress().
	// NOTE: reporting progress is optional and in some case could be
	// impossible. However,
	//       when possible, it's recommended as it provides valuable
	//       information to the user.
	//
	const char* (*get_progress)(ss_plugin_t* s, ss_instance_t* h,
				    uint32_t* progress_pct);
	//
	// Return a text representation of an event generated by this plugin
	// with event sourcing capabilities. Required: no Arguments:
	...
	//   next_batch() call.
	// Return value: the text representation of the event. This is used, for
	// example,
	...
	//   event_to_string().
	//
	const char* (*event_to_string)(ss_plugin_t* s,
	...
	// This function is optional--if NULL or an empty array, then if plugin
	// has sourcing capability it will only receive events matching its
	// event source, otherwise it will receive every event for extraction.
	//
	const char* (*get_extract_event_sources)();
	//
	// Return the list of extractor fields exported by this plugin.
	// Extractor fields can be used in Falco rule conditions. Required: yes
	...
	//                   that the field requires an argument e.g.
	//                   field[arg].
	//     "arg": (optional) if present, notes that the field can accept
	//             an argument e.g. field[arg]. More precisely, the
	//             following flags could be specified: "isRequired": if
	//             true, the argument is required. "isIndex": if true, the
	//             field is numeric. "isKey": if true, the field is a
	//             string. If "isRequired" is true, one between "isIndex"
	//             and "isKey" must be true, to specify the argument type.
	...
	// Example return value:
	// [
	//    {"type": "string", "name": "field1", "argRequired": true, "desc":
	//    "Describing field 1"}, [DEPRECATED 'argRequired' property]
	//    {"type": "uint64", "name": "field2", "desc": "Describing field
	//    2"},
	//    {"type": "string", "name": "field3", "arg": {"isRequired": true,
	//    "isIndex": true,}, "desc": "Describing field 3"},
	// ]
	...
	//   contains a single field + optional argument as input, and the
	//   corresponding extracted value as output. Memory pointers set as
	//   output must be allocated by the plugin and must not be deallocated
	//   or modified until the next extract_fields() call.
	//
	// Return value: A ss_plugin_rc with values SS_PLUGIN_SUCCESS or
	// SS_PLUGIN_FAILURE.
	//
