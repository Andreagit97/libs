...
extern const enum ppm_syscall_code
	g_syscall_code_routing_table[SYSCALL_TABLE_SIZE];
...
scap_open_args args = {
	.mode = SCAP_MODE_LIVE}; /* scap args used in `scap_open`. */
...
void print_sorted_syscalls(
	char string_vector[SYSCALL_TABLE_SIZE][SYSCALL_NAME_MAX_LEN], int dim)
{
...
/* This are the real interesting syscalls that we want to support in the new
 * probe.
...
void print_modern_probe_syscalls()
{
	...
	/* TAKE ALWAYS: If the syscall has `UF_NEVER_DROP` flag we cannot use
	 * simple consumer. */
	if(g_syscall_table[syscall_id].flags & UF_NEVER_DROP)
	{
	    ...
	    strcpy(str[interesting_syscall++],
		   g_syscall_info_table[ppm_syscall_code].name);
	    continue;
	}

	/* TAKE NEVER: If we use generic events, we can drop the syscall with
	 * the simple consumer logic. Same thing if the syscall has the
	 * `EF_DROP_SIMPLE_CONS`.
	 */
	if(g_syscall_table[syscall_id].enter_event_type == PPME_GENERIC_E ||
	   g_syscall_info_table[ppm_syscall_code].flags & EF_DROP_SIMPLE_CONS)
	{
	    continue;
	}
...
error:
    printf("unexpected error, please check with `%s` option",
	   VALIDATION_OPTION);
}
...
void print_actual_drivers_syscalls()
{
	...
	for(int syscall_nr = 0; syscall_nr < SYSCALL_TABLE_SIZE; syscall_nr++)
	{
	    ...
	    if(args.ppm_sc_of_interest.ppm_sc[i] ||
	       g_syscall_table[syscall_nr].flags & UF_NEVER_DROP)
	    {
		strcpy(str[interesting_syscall++],
		       g_syscall_info_table[i].name);
	    }
	}
    }
...
void validate_syscalls()
{
	...
	ppm_syscall_code = g_syscall_code_routing_table[syscall_id];
	/* If the syscall has `UF_NEVER_DROP` flag we must have its name inside
	 * the `g_syscall_info_table`.
	 */
	if(g_syscall_table[syscall_id].flags & UF_NEVER_DROP &&
	   !g_syscall_info_table[ppm_syscall_code].name)
	{
	    printf("ERROR: the syscall with real id `%d` has a `UF_NEVER_DROP` "
		   "syscall in `g_syscall_table` but not a name in the "
	...
	/* This is an error since it means that a syscall we want to trace is
	 * not tracked in our `g_syscall_info_table`. We have `EC_UNKNOWN` when
	 * we don't have an entry in the `g_syscall_info_table`.
	 */
	if(g_syscall_info_table[ppm_syscall_code].category == EC_UNKNOWN)
	{
	    printf("ERROR: the syscall with ppm code '%d' has an event "
		   "associated but it is unknown in our "
		   "`g_syscall_info_table`.\n",
		   ppm_syscall_code);
	    success = false;
	    continue;
...
void print_help()
{
    ...
    printf("'%s <num_events>': number of events to catch before terminating. "
	   "(default: UINT64_MAX)\n",
	   NUM_EVENTS_OPTION);
    printf("'%s <event_type>': every event of this type will be printed to "
	   "console. (default: -1, no print)\n",
	   EVENT_TYPE_OPTION);
    printf("\n------> VALIDATION OPTIONS\n");
    printf("'%s': validation checks.\n", VALIDATION_OPTION);
    printf("\n------> PRINT OPTIONS\n");
    printf("'%s': print all supported syscalls with different sources and "
	   "configurations.\n",
...
void parse_CLI_options(int argc, char** argv)
{
	...
	if(!strcmp(argv[i], SIMPLE_CONSUMER_OPTION))
	{
	    ...
	    /* Starting from '1' since we ignore all the unknown syscalls
	     * (PPM_SC_UNKNOWN). */
	    for(int j = 1; j < PPM_SC_MAX; j++)
	    {
		args.ppm_sc_of_interest.ppm_sc[j] =
			!(g_syscall_info_table[j].flags & EF_DROP_SIMPLE_CONS);
	    }
	...
	if(!strcmp(argv[i], NUM_EVENTS_OPTION))
	{
	    if(!(i + 1 < argc))
	    {
		printf("\nYou need to specify also the number of events to "
		       "catch! Bye!\n");
		exit(EXIT_FAILURE);
	    }
	...
	if(!strcmp(argv[i], EVENT_TYPE_OPTION))
	{
	    if(!(i + 1 < argc))
	    {
		printf("\nYou need to specify also the event type number! "
		       "Bye!\n");
		exit(EXIT_FAILURE);
	    }
...
void print_stats()
{
    ...
    printf("Number of dropped events caused by invalid memory access: %" PRIu64
	   "\n",
	   s.n_drops_pf);
    printf("Number of dropped events caused by an invalid condition in the "
	   "kernel instrumentation: %" PRIu64 "\n",
	   s.n_drops_bug);
    printf("Number of preemptions: %" PRIu64 "\n", s.n_preemptions);
    printf("Number of events skipped due to the tid being in a set of "
	   "suppressed tids: %" PRIu64 "\n",
	   s.n_suppressed);
...
int main(int argc, char** argv)
{
    ...
    {
	fprintf(stderr,
		"An error occurred while setting SIGINT signal handler.\n");
	return EXIT_FAILURE;
    }
