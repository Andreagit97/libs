...
// This function behaves exactly like scap_event_encode_params but it will
// allocate the event and return it by setting the event pointer.
static int32_t scap_event_generate(scap_evt **event, char *error,
				   ...
				   ...)
{
    ...
    va_list args;
    va_start(args, n);
    int32_t ret = scap_event_encode_params_v(event_buf, &event_size,
					     count_error, event_type, n, args);
    va_end(args);
	...
	if(ret == SCAP_SUCCESS)
	{
	    snprintf(count_error, SCAP_LASTERR_SIZE,
		     "Could not generate event. Expected SCAP_INPUT_TOO_SMALL, "
		     "got SCAP_SUCCESS for event type %d with %d args",
		     event_type, n);
	}
    ...
    va_start(args, n);
    ret = scap_event_encode_params_v(event_buf, &event_size, count_error,
				     event_type, n, args);
    va_end(args);
...
TEST(scap_event, empty_clone)
{
    ...
    scap_evt *maybe_evt;
    uint32_t status = scap_event_generate(&maybe_evt, scap_error,
					  PPME_SYSCALL_CLONE_20_E, 0);
    ASSERT_EQ(status, SCAP_SUCCESS)
	    << "scap_event_generate failed with error " << scap_error;
...
TEST(scap_event, empty_buffers)
{
    ...
    status = scap_event_generate(&maybe_evt, scap_error, PPME_SYSCALL_READ_X, 2,
				 0, scap_const_sized_buffer{nullptr, 0});
    ASSERT_EQ(status, SCAP_SUCCESS)
	    << "scap_event_generate failed with error " << scap_error;
    ASSERT_NE(maybe_evt, nullptr);
    evt.reset(maybe_evt);
