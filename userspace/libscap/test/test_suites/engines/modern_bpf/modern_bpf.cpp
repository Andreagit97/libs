...
#include <syscall.h>
/* We want to check some internal details of the engine */
// #include "scap-int.h"
// #include "engine/modern_bpf/scap_modern_bpf.h"
...
TEST(modern_bpf, empty_buffer_dim)
{
	...
	ASSERT_EQ(!h || ret != SCAP_SUCCESS, true) << "The buffer dimension is 0, we should fail!" << std::endl;
}
...
TEST(modern_bpf, wrong_buffer_dim)
{
	...
	ASSERT_EQ(!h || ret != SCAP_SUCCESS, true) << "The buffer dimension is not a multiple of the page size, we should fail!" << std::endl;
}
