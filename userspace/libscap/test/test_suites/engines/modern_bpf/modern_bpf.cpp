...
scap_t* open_modern_bpf_engine(char* error_buf, int32_t* rc, unsigned long buffer_dim, uint16_t cpus_for_each_buffer, std::unordered_set<uint32_t> tp_set = {}, std::unordered_set<uint32_t> ppm_sc_set = {})
{
...
TEST(modern_bpf, not_enough_CPUs)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
...
TEST(modern_bpf, one_buffer_per_CPU)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
	scap_t* h = open_modern_bpf_engine(error_buffer, &ret, 4 * 4096, 1);
...
TEST(modern_bpf, one_buffer_every_two_CPUs)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
...
TEST(modern_bpf, one_buffer_shared_between_all_CPUs_with_special_value)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
 ...
 */
TEST(modern_bpf, one_buffer_shared_between_all_CPUs_with_explicit_CPUs_number)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
...
TEST(modern_bpf, read_in_order_one_buffer_per_CPU)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
...
TEST(modern_bpf, read_in_order_one_buffer_every_two_CPUs)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
	/* We use buffers of 1 MB to be sure that we don't have drops */
...
TEST(modern_bpf, read_in_order_one_buffer_shared_between_all_CPUs)
{
	char error_buffer[FILENAME_MAX] = {0};
	int ret = 0;
	/* We use buffers of 1 MB to be sure that we don't have drops */
