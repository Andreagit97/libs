...
template<class T>
uint32_t prepare_message(char *message, uint32_t message_size,
			 uint16_t message_type, T &gvisor_evt)
{
    ...
    memcpy(&message[sizeof(uint16_t) + sizeof(uint16_t)], &dropped_count,
	   sizeof(uint32_t));

    // Serialize proto
    gvisor_evt.SerializeToArray(&message[header_size],
...
TEST(gvisor_parsers, parse_execve_e)
{
    ...
    uint32_t total_size =
	    prepare_message(message, 1024, message_type, gvisor_evt);
    ...
    scap_gvisor::parsers::parse_result res =
	    scap_gvisor::parsers::parse_gvisor_proto(gvisor_msg, scap_buf);
    EXPECT_EQ("", res.error);
    ...
    EXPECT_EQ(n, 1);
    EXPECT_STREQ(static_cast<const char *>(decoded_params[0].buf),
		 "/usr/bin/ls");
}
...
TEST(gvisor_parsers, parse_execve_x)
{
    ...
    uint32_t n = scap_event_decode_params(res.scap_events[0], decoded_params);
    EXPECT_EQ(n, 20);
    EXPECT_STREQ(static_cast<const char *>(decoded_params[1].buf),
		 "/usr/bin/ls"); // exe
    EXPECT_STREQ(static_cast<const char *>(decoded_params[6].buf),
		 "/root"); // cwd
    EXPECT_STREQ(static_cast<const char *>(decoded_params[13].buf),
		 "ls"); // comm
}
...
TEST(gvisor_parsers, parse_container_start)
{
    ...
    uint16_t message_type =
	    gvisor::common::MessageType::MESSAGE_CONTAINER_START;
...
TEST(gvisor_parsers, procfs_entry)
{
    ...
    EXPECT_STREQ(res.error.c_str(),
		 "Malformed json string: cannot parse procfs entry");
    ...
    std::string json = R"(
{
  ...
  "status": {
    ...
    EXPECT_TRUE(env.find("PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/"
			 "bin:/sbin:/bin") != std::string::npos);
    EXPECT_TRUE(env.find("HOME=/root") != std::string::npos);

    std::string json_missing_fields = "{\"exe\":\"/usr/bin/bash\"}\n";
    res = scap_gvisor::parsers::parse_procfs_json(json_missing_fields,
						  sandbox_id);
    EXPECT_EQ(res.status, SCAP_FAILURE);
    EXPECT_STREQ(res.error.c_str(),
		 "Missing json field or wrong type: cannot parse procfs entry");
