...
#if !defined(CYGWING_AGENT) && !defined(_WIN32)
int32_t scap_proc_fill_cwd(scap_t* handle, char* procdirname,
...
int32_t scap_proc_fill_info_from_stats(scap_t* handle, char* procdirname,
				       struct scap_threadinfo* tinfo)
{
    char filename[SCAP_MAX_PATH_SIZE];
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "open status file %s failed (%s)", filename,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "read stat file %s failed (%s)", filename,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	fclose(f);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not read from stat file %s (%s)", filename,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
    line[ssres] = 0;
	...
	fclose(f);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not find closing bracket in stat file %s", filename);
	return SCAP_FAILURE;
    }
    ...
    //
    if(sscanf(s + 2,
	      "%c %" PRId64 " %" PRId64 " %" PRId64 " %" PRId32 " %" PRId64
	      " %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64,
	      &tmpc, &tmp, &pgid, &sid, &tty, &tmp, &tmp, &pfminor, &tmp,
	      &pfmajor) != 10)
    {
	ASSERT(false);
	fclose(f);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not read expected fields from stat file %s", filename);
	return SCAP_FAILURE;
    }
...
//
// use prlimit to extract the RLIMIT_NOFILE for the tid. On systems where
// prlimit is not supported, just return -1
//
static int32_t scap_proc_fill_flimit(scap_t* handle, uint64_t tid,
				     struct scap_threadinfo* tinfo)
#ifdef SYS_prlimit64
{
    struct rlimit rl;
...
int32_t scap_proc_fill_cgroups(scap_t* handle, struct scap_threadinfo* tinfo,
			       const char* procdirname)
{
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "open cgroup file %s failed (%s)", filename,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	if(token == NULL)
	{
	    ...
	    fclose(f);
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Did not find id in cgroup file %s", filename);
	    return SCAP_FAILURE;
	}
	...
	if(subsys_list == NULL)
	{
	    ...
	    fclose(f);
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Did not find subsys in cgroup file %s", filename);
	    return SCAP_FAILURE;
	}
	...
	if(subsys_list - token - strlen(token) > 1)
	{
	    ...
	    //              contains
	    //                 a unique hierarchy ID number that can be matched
	    //                 to a hierarchy ID in /proc/cgroups.  For the
	    //                 cgroups version 2 hierarchy, this field contains
	    //                 the value 0.
	    //
	    //              2. For cgroups version 1 hierarchies, this field
	    //              contains
	    //                 a comma-separated list of the controllers bound
	    //                 to the hierarchy.  For the cgroups version 2
	    //                 hierarchy, this field is empty.
	    //
	    //              3. This field contains the pathname of the control
	    //              group
	    //                 in the hierarchy to which the process belongs.
	    //                 This pathname is relative to the mount point of
	    //                 the hierarchy.
	    //
	    // -> for cgroup2: id is always 0 and subsys list is always empty
	    // (single unified hierarchy)
	    // -> for cgroup1: skip subsys empty because it means controller is
	    // not mounted on any hierarchy
	    if(handle->m_cgroup_version == 2 && strcmp(token, "0") == 0)
	    {
		cgroup = subsys_list;
		subsys_list =
			default_subsys_list; // force-set a default subsys list
	    }
	    else
	    {
	...
	{
	    // cgroup should be the only thing remaining so use newline as the
	    // delimiter.
		...
		fclose(f);
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "Did not find cgroup in cgroup file %s", filename);
		return SCAP_FAILURE;
	    }
	}
	...
	{
	    subsys_list = NULL;
	    if(strlen(cgroup) + 1 + strlen(token) + 1 >
	       SCAP_MAX_CGROUPS_SIZE - tinfo->cgroups_len)
	    {
		ASSERT(false);
		fclose(f);
		return SCAP_SUCCESS;
	    ...
	    snprintf(tinfo->cgroups + tinfo->cgroups_len,
		     SCAP_MAX_CGROUPS_SIZE - tinfo->cgroups_len, "%s=%s", token,
		     cgroup);
...
static int32_t scap_get_vtid(scap_t* handle, uint64_t tid, int64_t* vtid)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Cannot get vtid (not in live mode)");
	return SCAP_FAILURE;
    }
...
static int32_t scap_get_vpid(scap_t* handle, int64_t pid, int64_t* vpid)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Cannot get vpid (not in live mode)");
	return SCAP_FAILURE;
    }
...
int32_t scap_proc_fill_root(scap_t* handle, struct scap_threadinfo* tinfo,
			    const char* procdirname)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "readlink %s failed (%s)", root_path,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
}

int32_t scap_proc_fill_loginuid(scap_t* handle, struct scap_threadinfo* tinfo,
				const char* procdirname)
{
    uint32_t loginuid;
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Open loginuid file %s failed (%s)", loginuid_path,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not read loginuid from %s (%s)", loginuid_path,
		 scap_strerror(handle, errno));
	fclose(f);
	return SCAP_FAILURE;
    }
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not read loginuid from %s", loginuid_path);
	return SCAP_FAILURE;
    }
}

int32_t scap_proc_fill_exe_writable(scap_t* handle,
				    struct scap_threadinfo* tinfo, uint32_t uid,
				    uint32_t gid, const char* procdirname,
				    const char* exetarget)
{
    char proc_exe_path[SCAP_MAX_PATH_SIZE];
    struct stat targetstat;

    snprintf(proc_exe_path, sizeof(proc_exe_path), "%sroot%s", procdirname,
	     exetarget);

    // if the file doesn't exist we can't determine if it was writable, assume
    // false
    ...
    // if you're the user owning the file you can chmod, so you can effectively
    // write to it
    ...
    //
    // In order to check whether the current user can access the file we need to
    // temporarily set the effective uid and gid of our thread to the target
    // ones and then check access, but keep in mind that:
    //  - seteuid()/setegid() libc functions change the euid/egid of the whole
    //  process, not just
    //    the current thread
    //  - setfsuid()/setfsgid() operate on threads but cannot be paired with
    //  access(),
    //    so we would need to open() the file, but opening executable files in
    //    use may result in "text file busy" errors
    //
    // Therefore we need to directly call the appropriate setresuid syscall that
    // operate on threads, implemented in the thread_seteuid() and
    // thread_setegid() functions.
    //
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not restore original euid from %d to %d", uid,
		 orig_uid);
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not restore original egid from %d to %d", gid,
		 orig_gid);
	return SCAP_FAILURE;
    }
...
//
static int32_t
scap_proc_add_from_proc(scap_t* handle, uint32_t tid, char* procdirname,
			struct scap_ns_socket_list** sockets_by_ns,
			scap_threadinfo** procinfo, char* error)
{
    char dir_name[256];
	...
	if(f)
	{
		...
		// Neither docker nor podman support this:
		// https://github.com/docker/for-linux/issues/1256
	...
	else
	{
	    ...
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "failed to fetch cgroup version information");
	    return SCAP_FAILURE;
	}
    }
    ...
    target_res = readlink(filename, target_name,
			  sizeof(target_name) -
				  1); // Getting the target of the exe, i.e. to
	...
	//  - a process that has been containerized or has some weird thing
	...
	snprintf(error, SCAP_LASTERR_SIZE, "can't allocate procinfo struct: %s",
		 handle->m_lasterr);
	return SCAP_FAILURE;
    }
    ...
    if(f == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE, "can't open %s (error %s)", filename,
		 scap_strerror(handle, errno));
	free(tinfo);
	return SCAP_FAILURE;
    }
    ...
    bool suppressed;
    if((res = scap_update_suppressed(handle, tinfo->comm, tid, 0,
				     &suppressed)) != SCAP_SUCCESS)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "can't update set of suppressed tids (%s)", handle->m_lasterr);
	free(tinfo);
	return res;
    }
    ...
    if(tinfo->vtid == 0 &&
       scap_get_vtid(handle, tinfo->tid, &tinfo->vtid) == SCAP_FAILURE)
    {
	tinfo->vtid = tinfo->tid;
    }

    if(tinfo->vpid == 0 &&
       scap_get_vpid(handle, tinfo->tid, &tinfo->vpid) == SCAP_FAILURE)
    {
	tinfo->vpid = tinfo->pid;
    }
    ...
    {
	tinfo->clone_ts =
		dirstat.st_ctim.tv_sec * 1000000000 + dirstat.st_ctim.tv_nsec;
    }

    // If tid is different from pid, assume this is a thread and that the FDs
    // are shared, and set the corresponding process flags.
    // XXX we should see if the process creation flags are stored somewhere in
    // /proc and handle this properly instead of making assumptions.
    //
    ...
    if(SCAP_FAILURE == scap_proc_fill_exe_writable(handle, tinfo, tinfo->uid,
						   tinfo->gid, dir_name,
						   target_name))
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "can't fill exe writable access for %s (%s)", dir_name,
		 handle->m_lasterr);
	free(tinfo);
	return SCAP_FAILURE;
    }
    ...
    {
	//
	// Done. Add the entry to the process table, or fire the notification
	// callback
	//
	...
	*procinfo = tinfo;
    ...
    {
	res = scap_fd_scan_fd_dir(handle, dir_name, tinfo, sockets_by_ns,
				  error);
    }
...
//
int32_t scap_proc_read_thread(scap_t* handle, char* procdirname, uint64_t tid,
			      struct scap_threadinfo** pi, char* error,
			      bool scan_sockets)
{
    ...
    res = scap_proc_add_from_proc(handle, tid, procdirname, &sockets_by_ns, pi,
				  add_error);
    if(res != SCAP_SUCCESS)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "cannot add proc tid = %" PRIu64 ", dirname = %s, error=%s",
		 tid, procdirname, add_error);
    }
...
//
static int32_t _scap_proc_scan_proc_dir_impl(scap_t* handle, char* procdirname,
					     int parenttid, char* error)
{
    DIR* dir_p;
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "error opening the %s directory (%s)", procdirname,
		 scap_strerror(handle, errno));
	...
	{
	    ASSERT(false);
	    snprintf(error, SCAP_LASTERR_SIZE, "duplicate process %" PRIu64,
		     tid);
	    res = SCAP_FAILURE;
	    break;
	}
	...
	//
	res = scap_proc_add_from_proc(handle, tid, procdirname, &sockets_by_ns,
				      NULL, add_error);
	if(res != SCAP_SUCCESS)
	{
	    //
	...
	// Note the use of recursion will re-enter this function for the
	// childdir.
	//
	if(parenttid == -1 && handle->m_mode != SCAP_MODE_NODRIVER)
	{
	    snprintf(childdir, sizeof(childdir), "%s/%u/task", procdirname,
		     (int)tid);
	    if(_scap_proc_scan_proc_dir_impl(handle, childdir, tid, error) ==
	       SCAP_FAILURE)
	    {
		res = SCAP_FAILURE;
		break;
...
int32_t scap_os_getpid_global(struct scap_engine_handle engine, int64_t* pid,
			      char* error)
{
#ifdef _WIN32
    *pid = _getpid();
    ...
    snprintf(filename, sizeof(filename), "%s/proc/self/status",
	     scap_get_host_root());
...
int32_t scap_getpid_global(scap_t* handle, int64_t* pid)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->getpid_global(handle->m_engine, pid,
					       handle->m_lasterr);
    }
    if(handle->m_mode != SCAP_MODE_LIVE)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Cannot get pid (not in live mode)");
	ASSERT(false);
	return SCAP_FAILURE;
    }
    ...
    ASSERT(false);
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "Cannot get pid (capture not enabled)");
    return SCAP_FAILURE;
}
				      ...
				      bool scan_sockets)
{
    ...
    if(scap_proc_read_thread(handle, filename, tid, &tinfo, handle->m_lasterr,
			     scan_sockets) != SCAP_SUCCESS)
    {
	free(tinfo);
	return NULL;
    }
			  ...
			  const char* comm)
{
    ...
    snprintf(charbuf, sizeof(charbuf),
	     "%s/proc/%" PRId64 "/task/%" PRId64 "/comm", scap_get_host_root(),
	     pid, tid);
    ...
    {
	//
	// If /proc/<pid>/task/<tid>/comm does not exist but
	// /proc/<pid>/task/<tid>/exe does exist, we assume we're on an ancient
	// OS like RHEL5 and we return true.
	// This could generate some false positives on such old distros, and
	// we're going to accept it.
	//
	snprintf(charbuf, sizeof(charbuf),
		 "%s/proc/%" PRId64 "/task/%" PRId64 "/exe",
		 scap_get_host_root(), pid, tid);
	f = fopen(charbuf, "r");
	if(f != NULL)
	{
	    fclose(f);
...
{
    struct scap_threadinfo* tinfo =
	    (struct scap_threadinfo*)calloc(1, sizeof(scap_threadinfo));
    if(tinfo == NULL)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "process table allocation error (1)");
	return NULL;
    }
...
int32_t scap_proc_add(scap_t* handle, uint64_t tid, scap_threadinfo* tinfo)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not add tid to hash table");
	return SCAP_FAILURE;
    }
}
		    ...
		    scap_fdinfo* fdinfo)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not add fd to hash table");
	return SCAP_FAILURE;
    }
}
...
{
    fprintf(stderr,
	    "TID:%" PRIu64 " PID:%" PRIu64 " FLAGS:%" PRIu32
	    " COMM:%s EXE:%s ARGS:%s CWD:%s FLIMIT:%" PRId64 "\n",
	    tinfo->tid, tinfo->pid, tinfo->flags, tinfo->comm, tinfo->exe,
	    tinfo->args, tinfo->cwd, tinfo->fdlimit);
...
int32_t scap_update_suppressed(scap_t* handle, const char* comm, uint64_t tid,
			       uint64_t ptid, bool* suppressed)
{
    uint32_t i;
    scap_tid* stid;
	...
	*suppressed = true;
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "can't add tid to suppressed hash table");
	    free(stid);
	    return SCAP_FAILURE;
	}
	*suppressed = true;
			      ...
			      bool* suppressed)
{
    ...
    *suppressed = false;
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Could not find process comm in event argument list");
	    return SCAP_FAILURE;
	}
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Could not find ptid in event argument list");
	    return SCAP_FAILURE;
	}

	comm = valptr;

	if((res = scap_update_suppressed(handle, comm, pevent->tid, *ptid,
					 suppressed)) != SCAP_SUCCESS)
	{
...
int32_t scap_procfs_get_threadlist(struct scap_engine_handle engine,
				   ...
				   char* lasterr)
{
    ...
    if(dir_p == NULL)
    {
	snprintf(lasterr, SCAP_LASTERR_SIZE, "error opening the %s directory",
		 procdirname);
	goto error;
    }
	...
	snprintf(tasksdirname, sizeof(tasksdirname), "%s/%s/task", procdirname,
		 dir_entry_p->d_name);
	...
	if(taskdir_p == NULL)
	{
	    snprintf(lasterr, SCAP_LASTERR_SIZE,
		     "error opening the %s directory", tasksdirname);
	    continue;
	}
	...
	while((taskdir_entry_p = readdir(taskdir_p)) != NULL)
	{
	    ...
	    if(strspn(taskdir_entry_p->d_name, "0123456789") !=
	       strlen(taskdir_entry_p->d_name))
	    {
		continue;
	    }

	    snprintf(filename, sizeof(filename), "%s/%s/stat", tasksdirname,
	    ...
	    {
		if(!scap_alloc_proclist_info(
			   procinfo_p, (*procinfo_p)->n_entries + 256, lasterr))
		{
		    goto error;
		}
	    }
...
int32_t scap_fd_scan_vtable(scap_t* handle, const scap_threadinfo* src_tinfo,
			    scap_threadinfo* dst_tinfo, char* error)
{
    ...
    res = handle->m_vtable->get_fdinfos(handle->m_engine, src_tinfo, &n_fdinfos,
					&fdinfos);
    if(res != SCAP_SUCCESS)
    {
	return res;
    ...
    {
	res = scap_fd_allocate_fdinfo(handle, &fdi, fdinfos[i].fd,
				      fdinfos[i].type);
	if(res != SCAP_SUCCESS)
	{
	    snprintf(error, SCAP_LASTERR_SIZE,
...
int32_t scap_proc_scan_vtable(char* error, scap_t* handle)
{
    ...
    res = handle->m_vtable->get_threadinfos(handle->m_engine, &n_tinfos,
					    &tinfos);
    if(res != SCAP_SUCCESS)
    {
	return res;
	...
	if((tinfo = scap_proc_alloc(handle)) == NULL)
	{
	    // Error message saved in handle->m_lasterr
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "can't allocate procinfo struct: %s", handle->m_lasterr);
	    return SCAP_FAILURE;
	}
