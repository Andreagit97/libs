...
static inline void ringbuffer_advance_tail(struct scap_device* dev)
{
    ...
    // Tail is never updated when we serve the data, because we assume that the
    // caller is using the buffer we give to her until she calls us again.
    //
    ttail = dev->m_bufinfo->tail + dev->m_lastreadsize;

    //
    // Make sure every read of the old buffer is completed before we move the
    // tail and the producer (on another CPU) can start overwriting it. I use
    // this instead of asm(mfence) because it should be portable even on the
    // weirdest CPUs
    //
    mem_barrier();
...
static inline int32_t ringbuffer_readbuf(struct scap_device* dev,
					 OUT char** buf, OUT uint32_t* len)
{
    uint64_t thead;
    uint64_t ttail;
    uint64_t read_size;
...
static inline int32_t refill_read_buffers(struct scap_device_set* devset)
{
	...
	devset->m_buffer_empty_wait_time_us =
		MIN(devset->m_buffer_empty_wait_time_us * 2,
    ...
    //
    // Note: we might return a spurious timeout here in case the previous loop
    // extracted valid data to parse.
    //       It's ok, since this is rare and the caller will just call us again
    //       after receiving a SCAP_TIMEOUT.
    //
    return SCAP_TIMEOUT;
}
				      ...
				      OUT uint16_t* pcpuid)
{
    ...
    *pcpuid = 65535;
	...
	if(pe->ts < max_ts)
	{
	    ...
	    {
		snprintf(devset->m_lasterr, SCAP_LASTERR_SIZE,
			 "scap_next buffer corruption");

		//
		// if you get the following assertion, first recompile the
		// driver and libscap
		//
		ASSERT(false);
		return SCAP_FAILURE;
    ...
    {
	//
	// All the buffers have been consumed. Check if there's enough data to
	// keep going or if we should wait.
	//
...
ringbuffer_get_max_buf_used(struct scap_device_set* devset)
{
    uint64_t i;
    uint64_t max = 0;
