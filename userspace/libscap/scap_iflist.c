...
int32_t scap_create_iflist(scap_t *handle)
{
	...
	if(tempIfAddr->ifa_addr->sa_family == AF_INET)
	{
	    ...
	    {
		handle->m_addrlist->v4list[ifcnt4].netmask = *(uint32_t *)&(
			((struct sockaddr_in *)tempIfAddr->ifa_netmask)
				->sin_addr);
	    }
	    else
	    {
	    ...
	    {
		handle->m_addrlist->v4list[ifcnt4].bcast = *(uint32_t *)&(
			((struct sockaddr_in *)
				 tempIfAddr->ifa_ifu.ifu_broadaddr)
				->sin_addr);
	    }
	    else
	    {
	    ...
	    handle->m_addrlist->v4list[ifcnt4].bcast = 0;
#endif
	    strlcpy(handle->m_addrlist->v4list[ifcnt4].ifname,
		    tempIfAddr->ifa_name,
		    sizeof(handle->m_addrlist->v4list[ifcnt4].ifname));
	    handle->m_addrlist->v4list[ifcnt4].ifnamelen =
		    strlen(tempIfAddr->ifa_name);
	...
	else if(tempIfAddr->ifa_addr->sa_family == AF_INET6)
	{
	    ...
	    tempAddrPtr =
		    &((struct sockaddr_in6 *)tempIfAddr->ifa_addr)->sin6_addr;
		...
		memcpy(handle->m_addrlist->v6list[ifcnt6].netmask,
		       &(((struct sockaddr_in6 *)tempIfAddr->ifa_netmask)
				 ->sin6_addr),
		       16);
	    }
	    else
	    ...
	    strlcpy(handle->m_addrlist->v6list[ifcnt6].ifname,
		    tempIfAddr->ifa_name,
		    sizeof(handle->m_addrlist->v6list[ifcnt6].ifname));
	    handle->m_addrlist->v6list[ifcnt6].ifnamelen =
		    strlen(tempIfAddr->ifa_name);

	    handle->m_addrlist->v6list[ifcnt6].linkspeed = 0;

	    handle->m_addrlist->totlen +=
		    (sizeof(scap_ifinfo_ipv6) +
		     handle->m_addrlist->v6list[ifcnt6].ifnamelen -
		     SCAP_MAX_PATH_SIZE);
	    ifcnt6++;
	}
...
// captures
//
int32_t scap_create_iflist(scap_t *handle) { return SCAP_FAILURE; }
#endif // _WIN32
#endif // HAS_CAPTURE
