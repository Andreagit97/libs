...
static int32_t copy_comms(scap_t* handle, const char** suppressed_comms)
{
	...
	uint32_t i;
	const char* comm;
	for(i = 0, comm = suppressed_comms[i];
	    comm && i < SCAP_MAX_SUPPRESSED_COMMS;
	    i++, comm = suppressed_comms[i])
	{
	    int32_t res;
...
scap_t* scap_open_live_int(char* error, int32_t* rc,
			   ...
			   void* proc_callback_context, bool import_users,
			   const char* bpf_probe, const char** suppressed_comms,
			   interesting_ppm_sc_set* ppm_sc_of_interest)
{
    snprintf(error, SCAP_LASTERR_SIZE, "live capture not supported on %s",
	     PLATFORM_NAME);
...
{
    snprintf(error, SCAP_LASTERR_SIZE, "udig capture not supported on %s",
	     PLATFORM_NAME);
    *rc = SCAP_NOT_SUPPORTED;
    return NULL;
}
			   ...
			   interesting_ppm_sc_set* ppm_sc_of_interest)
{
    ...
    memcpy(&oargs.suppressed_comms, suppressed_comms,
	   sizeof(*suppressed_comms));
    ...
    {
	memcpy(&oargs.ppm_sc_of_interest, ppm_sc_of_interest,
	       sizeof(*ppm_sc_of_interest));
    }
	...
	snprintf(error, SCAP_LASTERR_SIZE,
		 "error allocating the scap_t structure");
	*rc = SCAP_FAILURE;
	return NULL;
    }
    ...
    handle->m_engine.m_handle =
	    handle->m_vtable->alloc_handle(handle, handle->m_lasterr);
    if(!handle->m_engine.m_handle)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "error allocating the engine structure");
	free(handle);
	return NULL;
    }
    ...
    handle->m_machine_info.memory_size_bytes =
	    (uint64_t)sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGESIZE);
    gethostname(handle->m_machine_info.hostname,
		sizeof(handle->m_machine_info.hostname) /
			sizeof(handle->m_machine_info.hostname[0]));
	...
	if((*rc = scap_create_userlist(handle)) != SCAP_SUCCESS)
	{
	    scap_close(handle);
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "error creating the interface list");
	    return NULL;
	}
    }
    else
    ...
    *rc = check_api_compatibility(handle, handle->m_lasterr);
    ...
    if((*rc = scap_proc_scan_proc_dir(handle, filename, proc_scan_err)) !=
       SCAP_SUCCESS)
    {
	scap_close(handle);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "scap_open_live_int() error creating the process list: %s. "
		 "Make sure you have root credentials.",
		 proc_scan_err);
	return NULL;
    }
			   ...
			   const char** suppressed_comms)
{
    ...
    *rc = handle->m_vtable->init(handle, NULL);
    ...
    handle->m_proc_callback_context = proc_callback_context;
#ifdef _WIN32
    scap_get_machine_info_windows(&handle->m_machine_info.num_cpus,
				  &handle->m_machine_info.memory_size_bytes);
#else
    handle->m_machine_info.num_cpus = sysconf(_SC_NPROCESSORS_ONLN);
    handle->m_machine_info.memory_size_bytes =
    ...
    if((*rc = scap_proc_scan_proc_dir(handle, filename, procerr)) !=
       SCAP_SUCCESS)
    {
	scap_close(handle);
...
scap_t* scap_open_gvisor_int(char* error, int32_t* rc, scap_open_args* args)
{
    ...
    *rc = handle->m_vtable->init(handle, args);
    ...
    // XXX - interface list initialization and user list initalization goes here
    // if necessary
...
{
    snprintf(error, SCAP_LASTERR_SIZE,
	     "gvisor not supported on this build (platform: %s)",
	     PLATFORM_NAME);
    *rc = SCAP_NOT_SUPPORTED;
    return NULL;
}
...
scap_t* scap_open_offline_int(scap_reader_t* reader, char* error, int32_t* rc,
			      proc_entry_callback proc_callback,
			      void* proc_callback_context, bool import_users,
			      ...
			      const char** suppressed_comms)
{
	...
	*rc = SCAP_FAILURE;
    ...
    //
    // If this is a merged file, we might have to move the read offset to the
    // next section
    //
    if(start_offset != 0)
    {
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE, "Could not initialize reader: %s",
			       ...
			       void* proc_callback_context, bool import_users)
{
	...
	*rc = SCAP_FAILURE;
    ...
    handle->m_machine_info.reserved4 = 0;
    handle->m_driver_procinfo = NULL;
    handle->m_fd_lookup_limit =
	    SCAP_NODRIVER_MAX_FD_LOOKUP; // fd lookup is limited here because is
					 // very expensive
	...
	*rc = SCAP_FAILURE;
	...
	scap_close(handle);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "error creating the process list: %s. Make sure you have root "
		 "credentials.",
		 proc_scan_err);
	return NULL;
    }
...
scap_t* scap_open_plugin_int(char* error, int32_t* rc,
			     scap_source_plugin* input_plugin,
			     char* input_plugin_params)
{
    scap_t* handle = NULL;
...
scap_t* scap_open(scap_open_args args, char* error, int32_t* rc)
{
	...
	if(gzfile == NULL)
	{
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE, "can't open file %s",
			 args.fname);
	    }
	    *rc = SCAP_FAILURE;
	    return NULL;
	...
	scap_reader_t* reader = scap_reader_open_gzfile(gzfile);
	return scap_open_offline_int(reader, error, rc, args.proc_callback,
	...
	if(args.udig)
	{
	    return scap_open_udig_int(error, rc, args.proc_callback,
				      args.proc_callback_context,
				      args.import_users, args.suppressed_comms);
	}
	else if(args.gvisor)
	{
	...
	{
	    return scap_open_live_int(
		    error, rc, args.proc_callback, args.proc_callback_context,
		    args.import_users, args.bpf_probe, args.suppressed_comms,
		    &args.ppm_sc_of_interest);
	}
#else
	snprintf(error, SCAP_LASTERR_SIZE,
		 "scap_open: live mode currently not supported on Windows.");
	*rc = SCAP_NOT_SUPPORTED;
	return NULL;
#endif
	...
	handle = scap_open_plugin_int(error, rc, args.input_plugin,
				      args.input_plugin_params);
...
uint32_t scap_restart_capture(scap_t* handle)
{
	...
	if(handle->m_unexpected_block_readsize > 0)
	{
	    scap_reader_seek(
		    handle->m_reader,
		    (int64_t)0 - (int64_t)handle->m_unexpected_block_readsize,
		    SEEK_CUR);
	    handle->m_unexpected_block_readsize = 0;
	}
	if((res = scap_read_init(handle, handle->m_reader)) != SCAP_SUCCESS)
	{
	    ...
	    snprintf(error, SCAP_LASTERR_SIZE, "could not restart capture: %s",
		     scap_getlasterr(handle));
...
int32_t scap_readbuf(scap_t* handle, uint32_t cpuid, OUT char** buf,
		     OUT uint32_t* len)
{
...
int32_t scap_next(scap_t* handle, OUT scap_evt** pevent, OUT uint16_t* pcpuid)
{
	...
	if((res = scap_check_suppressed(handle, *pevent, &suppressed)) !=
	   SCAP_SUCCESS)
	{
	    return res;
	}
...
#if !defined(HAS_CAPTURE) || defined(CYGWING_AGENT)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "live capture not supported on %s", PLATFORM_NAME);
    return SCAP_FAILURE;
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "cannot stop offline live captures");
    ASSERT(false);
    return SCAP_FAILURE;
#endif // HAS_CAPTURE
}
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "cannot start offline live captures");
    ASSERT(false);
    return SCAP_FAILURE;
#endif // HAS_CAPTURE
}
...
int32_t scap_enable_tracers_capture(scap_t* handle)
{
	...
	return handle->m_vtable->configure(handle->m_engine,
					   SCAP_TRACERS_CAPTURE, 1, 0);
    }
#if defined(HAS_CAPTURE) && !defined(CYGWING_AGENT) && !defined(_WIN32)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_enable_tracers_capture not supported on this scap mode");
    ASSERT(false);
    return SCAP_FAILURE;
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_enable_tracers_capture not supported on %s", PLATFORM_NAME);
    return SCAP_FAILURE;
#endif
}
...
int32_t scap_enable_page_faults(scap_t* handle)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->configure(handle->m_engine, SCAP_PAGE_FAULTS,
					   1, 0);
    }
#if defined(HAS_CAPTURE) && !defined(CYGWING_AGENT) && !defined(_WIN32)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_enable_page_faults not supported on this scap mode");
    ASSERT(false);
    return SCAP_FAILURE;
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_enable_page_faults not supported on %s", PLATFORM_NAME);
    return SCAP_FAILURE;
#endif
}
...
int32_t scap_stop_dropping_mode(scap_t* handle)
{
    ...
    {
	return handle->m_vtable->configure(handle->m_engine,
					   SCAP_SAMPLING_RATIO, 1, 0);
    }
#if !defined(HAS_CAPTURE) || defined(CYGWING_AGENT) || defined(_WIN32)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_stop_dropping_mode not supported on %s", PLATFORM_NAME);
    return SCAP_FAILURE;
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_stop_dropping_mode not supported on this scap mode");
    ASSERT(false);
    return SCAP_FAILURE;
#endif
}
...
int32_t scap_start_dropping_mode(scap_t* handle, uint32_t sampling_ratio)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->configure(
		handle->m_engine, SCAP_SAMPLING_RATIO, sampling_ratio, 1);
    }
#if !defined(HAS_CAPTURE) || defined(CYGWING_AGENT) || defined(_WIN32)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "live capture not supported on %s", PLATFORM_NAME);
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_start_dropping_mode not supported on this scap mode");
    ASSERT(false);
    return SCAP_FAILURE;
#endif
}
...
//
scap_userlist* scap_get_user_list(scap_t* handle) { return handle->m_userlist; }
...
int32_t scap_set_snaplen(scap_t* handle, uint32_t snaplen)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->configure(handle->m_engine, SCAP_SNAPLEN,
					   snaplen, 0);
    }
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "setting snaplen not supported on this scap mode");
    return SCAP_FAILURE;
#endif
}
...
static int32_t scap_handle_eventmask(scap_t* handle, uint32_t op,
				     uint32_t event_id)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->configure(handle->m_engine, SCAP_EVENTMASK, op,
					   event_id);
    }
#if !defined(HAS_CAPTURE) || defined(_WIN32)
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "eventmask not supported on %s", PLATFORM_NAME);
    return SCAP_FAILURE;
#else
    if(handle == NULL)
    {
    ...
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "manipulating eventmasks not supported on this scap mode");
    return SCAP_FAILURE;
#endif // HAS_CAPTURE
}
			      ...
			      uint32_t n_entries, char* error)
{
    ...
    struct ppm_proclist_info* procinfo =
	    (struct ppm_proclist_info*)realloc(*proclist_p, memsize);
    if(procinfo == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "driver process list allocation error");
	return false;
    }
...
struct ppm_proclist_info* scap_get_threadlist(scap_t* handle)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_get_threadlist not supported on this scap mode");
	return NULL;
    }
    ...
    {
	if(scap_alloc_proclist_info(&handle->m_driver_procinfo,
				    SCAP_DRIVER_PROCINFO_INITIAL_SIZE,
				    handle->m_lasterr) == false)
	{
	    return NULL;
	}
    }

    int res = handle->m_vtable->get_threadlist(
	    handle->m_engine, &handle->m_driver_procinfo, handle->m_lasterr);
    if(res != SCAP_SUCCESS)
    {
	return NULL;
    }
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "setting simpledriver mode not supported on this scap mode");
    return SCAP_FAILURE;
#endif
}
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "getting n_tracepoint_hit not supported on this scap mode");
    return SCAP_FAILURE;
#endif
}

#ifdef CYGWING_AGENT
wh_t* scap_get_wmi_handle(scap_t* handle) { return handle->m_whh; }
#endif
...
int32_t scap_suppress_events_comm(scap_t* handle, const char* comm)
{
    ...
    handle->m_suppressed_comms =
	    (char**)realloc(handle->m_suppressed_comms,
			    handle->m_num_suppressed_comms * sizeof(char*));

    handle->m_suppressed_comms[handle->m_num_suppressed_comms - 1] =
	    strdup(comm);
...
int32_t scap_set_fullcapture_port_range(scap_t* handle, uint16_t range_start,
					uint16_t range_end)
{
    if(handle->m_vtable)
    {
...
#else
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_set_fullcapture_port_range not supported on this scap mode");
    return SCAP_FAILURE;
#endif
}
...
int32_t scap_set_statsd_port(scap_t* const handle, const uint16_t port)
{
    if(handle->m_vtable)
    {
	return handle->m_vtable->configure(handle->m_engine, SCAP_STATSD_PORT,
					   port, 0);
    }
