...
extern "C"
{
    ...
    /*!
	    ...
	    libscap is the low-level component that exports the following
       functionality:
...
//
// The minimum API and schema versions the driver has to support before we can
// use it
//
...
// call `scap_get_driver_api_version()` and/or
// `scap_get_driver_schema_version()` and handle the result
//
...
#define SCAP_LASTERR_SIZE 256
    ...
    */
    ...
    {
	uint64_t n_drops_buffer; ///< Number of dropped events caused by full
				 ///< buffer.
	uint64_t n_drops_scratch_map; ///< Number of dropped events caused by
				      ///< full frame scratch map.
	uint64_t n_drops_pf;  ///< Number of dropped events caused by invalid
			      ///< memory access.
	uint64_t n_drops_bug; ///< Number of dropped events caused by an invalid
			      ///< condition in the kernel instrumentation.
	uint64_t n_preemptions;	    ///< Number of preemptions.
	uint64_t n_suppressed;	    ///< Number of events skipped due to the tid
				    ///< being in a set of suppressed tids.
	uint64_t n_tids_suppressed; ///< Number of threads currently being
				    ///< suppressed.
    } scap_stats;
    ...
    */
	...
	uint32_t type;	  ///< The event type. See the ppm_event_type enum in
    } evt_param_info;
    ...
    */
    ...
    {
	SCAP_L4_UNKNOWN =
		0, ///< unknown protocol, likely caused by some parsing problem
	SCAP_L4_NA =
		1, ///< protocol not available, because the fd is not a socket
	SCAP_L4_TCP = 2,
	SCAP_L4_UDP = 3,
	SCAP_L4_ICMP = 4,
    ...
    */
    ...
    {
	int64_t fd;   ///< The FD number, which uniquely identifies this file
		      ///< descriptor.
	...
	union
	{
		...
		uint8_t l4proto; ///< Transport protocol. See \ref
				 ///< scap_l4_proto.
		...
		uint8_t l4proto; ///< Transport protocol. See \ref
				 ///< scap_l4_proto.
	    } ipv4serverinfo; ///< Information specific to IPv4 server sockets,
			      ///< e.g. sockets used for bind().
	    struct
	    {
		...
		uint8_t l4proto; ///< Transport protocol. See \ref
				 ///< scap_l4_proto.
	    } ipv6serverinfo; ///< Information specific to IPv6 server sockets,
			      ///< e.g. sockets used for bind().
	    struct
	    {
		uint64_t source;		///< Source socket endpoint
		uint64_t destination;		///< Destination socket endpoint
		char fname[SCAP_MAX_PATH_SIZE]; ///< Name associated to this
						///< unix socket
		...
		char fname[SCAP_MAX_PATH_SIZE]; ///< Name associated to this
						///< file
		uint32_t mount_id; ///< The id of the vfs mount the file is in
				   ///< until we find dev major:minor
		uint32_t dev; ///< Major/minor number of the device containing
			      ///< this file
    ...
    */
	...
	uint64_t pid;  ///< The id of the process containing this thread. In
		       ///< single thread processes, this is equal to tid.
	...
	uint64_t vpgid; ///< The process group of this thread, as seen from its
	...
	bool exe_writable; ///< true if the original executable is writable by
			   ///< the same user that spawned it.
	char args[SCAP_MAX_ARGS_SIZE +
		  1];	   ///< Command line arguments (e.g. "-d1")
	...
	int64_t fdlimit; ///< The maximum number of files this thread is allowed
			 ///< to open
...
#define USERBLOCK_TYPE_GROUP 1
    ...
    */
    ...
    {
	SCAP_DF_NONE = 0,
	SCAP_DF_STATE_ONLY = 1, ///< The event should be used for state update
				///< but it should not be shown to the user
	SCAP_DF_TRACER = (1 << 1), ///< This event is a tracer
	SCAP_DF_LARGE = (1 << 2)   ///< This event has large payload (up to
				   ///< UINT_MAX Bytes, ie 4GB)
    } scap_dump_flags;
...
#define UDIG_RING_SIZE (8 * 1024 * 1024)
    ...
    int32_t udig_alloc_ring(void* ring_id, uint8_t** ring, uint32_t* ringsize,
			    char* error);
    int32_t udig_alloc_ring_descriptors(
	    void* ring_descs_id, struct ppm_ring_buffer_info** ring_info,
    ...
    /*!
      ...
      \param error Pointer to a buffer that will contain the error string in
      case the function fails. The buffer must have size SCAP_LASTERR_SIZE.
      \param rc Integer pointer that will contain the scap return code in case
      the function fails.

      \return The capture instance handle in case of success. NULL in case of
      failure.
    */
    ...
    */
    ...
    /*!
      ...
      \param args a \ref scap_open_args structure containing the open
      parameters. \param error Pointer to a buffer that will contain the error
      string in case the function fails. The buffer must have size
      SCAP_LASTERR_SIZE. \param rc Integer pointer that will contain the scap
      return code in case the function fails.
    ...
    */
	...
	Only supported for captures in SCAP_MODE_CAPTURE mode.
	    ...
	    it by trying to read the scap header section. The underlying
      instance of scap_reader_t is preserved, and the header section is read
      starting from its current offset.
    ...
    */
	...
	For offline handles, the return value indicates the OS where the data
      was originally captured.
    */
    scap_os_platform scap_get_os_platform(scap_t* handle);

    /*!
      \brief Return a string with the last error that happened on the given
      capture.
    */
    ...
    /*!
      ...
      \param handle Handle to the capture instance.
      \param pevent User-provided event pointer that will be initialized with
      address of the event. \param pcpuid User-provided event pointer that will
      be initialized with the ID if the CPU where the event was captured.

      \return SCAP_SUCCESS if the call is successful and pevent and pcpuid
      contain valid data. SCAP_TIMEOUT in case the read timeout expired and no
      event is available. SCAP_EOF when the end of an offline capture is
      reached. On Failure, SCAP_FAILURE is returned and scap_getlasterr() can be
      used to obtain the cause of the error.
    */
    int32_t scap_next(scap_t* handle, OUT scap_evt** pevent,
		      OUT uint16_t* pcpuid);
    ...
    */
    ...
    /*!
      \brief Get the number of events that have been captured from the given
      capture instance
    ...
    */
    ...
    /*!
      \brief Return the current offset in the file opened by
      scap_open_offline(), or -1 if this is a live capture.
    ...
    */
    ...
    /*!
      ...
      \return Dump handle that can be used to identify this specific dump
      instance.
    */
    ...
    */
    ...
    /*!
      ...
      \param e pointer to an event returned by \ref scap_next.
      \param cpuid The cpu from which the event was captured. Returned by \ref
      scap_next. \param bytes The number of bytes to write

      \return SCAP_SUCCESS if the call is successful.
       On Failure, SCAP_FAILURE is returned and scap_getlasterr() can be used to
    ...
    */
    ...
    /*!
      ...
      \param e pointer to an event returned by \ref scap_next.
      \param cpuid The cpu from which the event was captured. Returned by \ref
      scap_next. \param flags The event flags. 0 means no flags.

      \return SCAP_SUCCESS if the call is successful.
       On Failure, SCAP_FAILURE is returned and scap_getlasterr() can be used to
      obtain the cause of the error.
    */
    int32_t scap_dump(scap_t* handle, scap_dumper_t* d, scap_evt* e,
		      uint16_t cpuid, uint32_t flags);
    ...
    /*!
      ...
      for live captures, the process list is created when the capture starts by
      scanning the proc file system. For offline captures, it is retrieved from
      the file. The process list contains information about the processes that
      were already open when the capture started. It can be traversed with
      uthash, using the following syntax:
	...
	// do something with pi
      ...
      Refer to the documentation of the \ref scap_threadinfo struct for details
      about its content.
    */
    ...
    /*!
      ...
      \param handle Handle to the capture instance.
      \param stats Pointer to a \ref scap_stats structure that will be filled
      with the statistics.
    ...
    */
    ...
    /*!
      \brief Start capture the events, if it was stopped with \ref
      scap_stop_capture.
    ...
    */
    ...
    /*!
      \brief Return the list of the the user interfaces of the machine from
      which the events are being captured.

      \param handle Handle to the capture instance.

      \return The pointer to a \ref scap_addrlist structure containing the
      interface list, or NULL if the function fails.
    */
    ...
    /*!
      ...
      \return The pointer to a \ref scap_userlist structure containing the user
      and group lists, or NULL if the function fails.
    */
    ...
    /*!
      ...
      \return The pointer to a table of \ref scap_userlist entries, each of
      which describes one of the events that can come from the driver. The table
      contains PPM_EVENT_MAX entries, and the position of each entry in the
      table corresponds to its event ID. The ppm_event_info contains the full
      information necessary to decode an event coming from \ref scap_next.
    */
    ...
    /*!
      ...
      \return The pointer to a table of \ref ppm_syscall_desc entries, each of
      which describes one of the events that can come from the driver. The table
      contains SYSCALL_TABLE_SIZE entries, and the position of each entry in the
      table corresponds to the system call ID.

      This table can be used to interpret the ID parameter of PPME_GENERIC_E and
      PPME_GENERIC_X.
    */
    ...
    /*!
      ...
      \return The pointer to a \ref scap_machine_info structure containing the
      information.

      \note for live captures, the information is collected from the operating
      system. For offline captures, it comes from the capture file.
    */
    const scap_machine_info* scap_get_machine_info(scap_t* handle);

    /*!
      \brief Set the capture snaplen, i.e. the maximum size an event parameter
      can reach before the driver starts truncating it.

      \param handle Handle to the capture instance.
      \param snaplen the snaplen for this capture instance, in bytes.

      \note This function can only be called for live captures.
      \note By default, the driver captures the first 80 bytes of the buffers
      coming from events like read, write, send, recv, etc. If you're not
      interested in payloads, smaller values will save capture buffer space and
      make capture files smaller.
      Conversely, big values should be used with care because they can easily
      generate huge capture files.
    */
    ...
    */
    ...
    /*!
      ...
      \param params An array large enough to contain at least one entry per
      event parameter (which is at most PPM_MAX_EVENT_PARAMS).
     */
    uint32_t scap_event_decode_params(const scap_evt* e,
				      struct scap_sized_buffer* params);

    /*!
      \brief Create an event from the parameters given as arguments.

      Create any event from the event_table passing the type, n and the
      parameters as variadic arguments as follows:
       - Any integer type is passed from the correct type
       - String types (including PT_FSPATH, PT_FSRELPATH) are passed via a
      null-terminated char*
       - Buffer types, variable size types and similar, including PT_BYTEBUF,
      PT_SOCKTUPLE are passed with a struct scap_const_sized_buffer

      If the event was written successfully, SCAP_SUCCESS is returned. If the
      supplied buffer is not large enough to contain the event,
      SCAP_INPUT_TOO_SMALL is returned and event_size is set with the required
      size to contain the entire event.

      \param event_buf The buffer where to store the encoded event.
      \param event_size Output value that will be filled with the size of the
      event. \param error A pointer to a scap error string to be filled in case
      of error. \param event_type The event type (normally PPME_*) \param n The
      number of parameters for this event. This is required as the number of
      parameters used for each event can change between versions. \param ...
      \return int32_t The error value. If the event was written successfully,
      SCAP_SUCCESS is returned. If the supplied buffer is not large enough for
      the event SCAP_INPUT_TOO_SMALL is returned and event_size is set with the
      required size to contain the entire event. In other error cases,
      SCAP_FAILURE is returned.

     */
    int32_t scap_event_encode_params(struct scap_sized_buffer event_buf,
				     size_t* event_size, char* error,
				     enum ppm_event_type event_type, uint32_t n,
				     ...);
    int32_t scap_event_encode_params_v(struct scap_sized_buffer event_buf,
				       size_t* event_size, char* error,
				       enum ppm_event_type event_type,
				       uint32_t n, va_list args);
    ...
    //
    // Return the number of event capture devices that the library is handling.
    // Each processor has its own event capture device.
    //
    uint32_t scap_get_ndevs(scap_t* handle);

    // Retrieve a buffer of events from one of the cpus
    extern int32_t scap_readbuf(scap_t* handle, uint32_t cpuid, OUT char** buf,
				OUT uint32_t* len);
    ...
    bool scap_is_thread_alive(scap_t* handle, int64_t pid, int64_t tid,
			      const char* comm);
    ...
    int32_t scap_fd_add(scap_t* handle, scap_threadinfo* tinfo, uint64_t fd,
			scap_fdinfo* fdinfo);
    ...
    int32_t scap_write_proclist_entry_bufs(
	    ...
	    uint32_t len, const char* comm, const char* exe,
	    const char* exepath, const struct iovec* args, int argscnt,
	    const struct iovec* envs, int envscnt, const char* cwd,
	    const struct iovec* cgroups, int cgroupscnt, const char* root);

    // Turn on processing only a subset syscalls. This is only appliable when
    // scap is in LIVE mode.
    ...
    /**
     * By default we have an expanded snaplen for the default statsd port. If
     * the statsd port is non-standard, communicate that port value to the
     * kernel to get the expanded snaplen for the correct port.
     */
     ...
     */
    bool scap_is_api_compatible(unsigned long driver_api_version,
				unsigned long required_api_version);
