...
int32_t scap_fd_handle_socket(struct scap_proclist *proclist, char *fname, scap_threadinfo *tinfo, scap_fdinfo *fdi, char* procdir, uint64_t net_ns, struct scap_ns_socket_list **sockets_by_ns, char *error)
{
	...
	if(sockets == NULL)
	{
		...
		scap_fd_read_sockets(procdir, sockets, fd_error);
	}
...
int32_t scap_fd_scan_fd_dir(scap_t *handle, char *procdir, scap_threadinfo *tinfo, struct scap_ns_socket_list **sockets_by_ns, char *error)
{
		...
		(handle->m_fd_lookup_limit == 0 || fd_added < handle->m_fd_lookup_limit))
	{
		...
		case S_IFSOCK:
			printf("S_IFSOCK\n");
			/* Create an unknown filedescriptor */
	...
	printf("INTERNAL CYCLES: %d, res: %d\n", cycles, res);
