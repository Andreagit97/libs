...
int32_t scap_fd_print_ipv6_socket_info(scap_t *handle, scap_fdinfo *fdi,
				       OUT char *str, uint32_t stlen)
{
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 ...
		 fdi->info.ipv6info.sip[0], fdi->info.ipv6info.sip[1],
		 fdi->info.ipv6info.sip[2], fdi->info.ipv6info.sip[3],
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
    if(NULL ==
       inet_ntop(AF_INET6, fdi->info.ipv6info.dip, destination_address, 100))
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not convert IPv6 source address 0x%x%x%x%x (%s)",
		 fdi->info.ipv6info.dip[0], fdi->info.ipv6info.dip[1],
		 fdi->info.ipv6info.dip[2], fdi->info.ipv6info.dip[3],
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
    snprintf(str, stlen, "%s:%u->%s:%u", source_address,
	     fdi->info.ipv6info.sport, destination_address,
	     fdi->info.ipv6info.dport);
    return SCAP_SUCCESS;
}

int32_t scap_fd_print_ipv6_server_socket_info(scap_t *handle, scap_fdinfo *fdi,
					      OUT char *str, uint32_t stlen)
{
    char address[100];
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not convert IPv6 source address 0x%x%x%x%x (%s)",
		 fdi->info.ipv6serverinfo.ip[0], fdi->info.ipv6serverinfo.ip[1],
		 fdi->info.ipv6serverinfo.ip[2], fdi->info.ipv6serverinfo.ip[3],
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
...
//
int32_t scap_fd_info_to_string(scap_t *handle, scap_fdinfo *fdi, OUT char *str,
			       uint32_t stlen)
{
    //
    // Input validation
	...
	snprintf(str, stlen,
		 "%" PRIi64 " %" PRIu64 " %" PRIX64 "-> %" PRIX64 " %s",
		 fdi->fd, fdi->ino, fdi->info.unix_socket_info.source,
		 fdi->info.unix_socket_info.destination,
		 fdi->info.unix_socket_info.fname);
	break;
...
uint32_t scap_fd_info_len(scap_fdinfo *fdi)
{
	...
	res += sizeof(uint64_t) + // unix source
	       ...
	       (uint32_t)strnlen(fdi->info.unix_socket_info.fname,
				 SCAP_MAX_PATH_SIZE) +
	       2;
	break;
    case SCAP_FD_FILE_V2:
	res += sizeof(uint32_t) + // open_flags
	       (uint32_t)strnlen(fdi->info.regularinfo.fname,
				 SCAP_MAX_PATH_SIZE) +
	       2 + sizeof(uint32_t); // dev
	break;
    case SCAP_FD_FIFO:
    case SCAP_FD_FILE:
    case SCAP_FD_DIRECTORY:
...
//
int32_t scap_fd_write_to_disk(scap_t *handle, scap_fdinfo *fdi,
			      scap_dumper_t *d, uint32_t len)
{
    ...
    {
    case SCAP_FD_IPV4_SOCK:
	if(scap_dump_write(d, &(fdi->info.ipv4info.sip), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4info.dip), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4info.sport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4info.dport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4info.l4proto), sizeof(uint8_t)) !=
		   sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi2)");
	    return SCAP_FAILURE;
	}
	break;
    case SCAP_FD_IPV4_SERVSOCK:
	if(scap_dump_write(d, &(fdi->info.ipv4serverinfo.ip),
			   sizeof(uint32_t)) != sizeof(uint32_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4serverinfo.port),
			   sizeof(uint16_t)) != sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv4serverinfo.l4proto),
			   sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi3)");
	    return SCAP_FAILURE;
	}
	break;
    case SCAP_FD_IPV6_SOCK:
	if(scap_dump_write(d, (char *)fdi->info.ipv6info.sip,
			   sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_dump_write(d, (char *)fdi->info.ipv6info.dip,
			   sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_dump_write(d, &(fdi->info.ipv6info.sport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv6info.dport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv6info.l4proto), sizeof(uint8_t)) !=
		   sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi7)");
	}
	break;
    case SCAP_FD_IPV6_SERVSOCK:
	if(scap_dump_write(d, &(fdi->info.ipv6serverinfo.ip),
			   sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_dump_write(d, &(fdi->info.ipv6serverinfo.port),
			   sizeof(uint16_t)) != sizeof(uint16_t) ||
	   scap_dump_write(d, &(fdi->info.ipv6serverinfo.l4proto),
			   sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi8)");
	}
	break;
    case SCAP_FD_UNIX_SOCK:
	if(scap_dump_write(d, &(fdi->info.unix_socket_info.source),
			   sizeof(uint64_t)) != sizeof(uint64_t) ||
	   scap_dump_write(d, &(fdi->info.unix_socket_info.destination),
			   sizeof(uint64_t)) != sizeof(uint64_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi4)");
	    return SCAP_FAILURE;
	}
	stlen = (uint16_t)strnlen(fdi->info.unix_socket_info.fname,
				  SCAP_MAX_PATH_SIZE);
	if(scap_dump_write(d, &stlen, sizeof(uint16_t)) != sizeof(uint16_t) ||
	   (stlen > 0 && scap_dump_write(d, fdi->info.unix_socket_info.fname,
					 stlen) != stlen))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi5)");
	    return SCAP_FAILURE;
	}
	break;
    case SCAP_FD_FILE_V2:
	...
	stlen = (uint16_t)strnlen(fdi->info.regularinfo.fname,
				  SCAP_MAX_PATH_SIZE);
	if(scap_dump_write(d, &stlen, sizeof(uint16_t)) != sizeof(uint16_t) ||
	   (stlen > 0 &&
	    scap_dump_write(d, fdi->info.regularinfo.fname, stlen) != stlen))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi1)");
	    return SCAP_FAILURE;
	...
	break;
    default:
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "Unknown fdi type %d",
		 fdi->type);
	ASSERT(false);
	return SCAP_FAILURE;
    }
...
uint32_t scap_fd_read_prop_from_disk(scap_t *handle, OUT void *target,
				     size_t expected_size, OUT size_t *nbytes,
				     scap_reader_t *r)
{
    size_t readsize;
...
uint32_t scap_fd_read_fname_from_disk(scap_t *handle, char *fname,
				      OUT size_t *nbytes, scap_reader_t *r)
{
    size_t readsize;
    uint16_t stlen;
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "invalid filename len %" PRId32, stlen);
	return SCAP_FAILURE;
    }
...
//
uint32_t scap_fd_read_from_disk(scap_t *handle, OUT scap_fdinfo *fdi,
				OUT size_t *nbytes, uint32_t block_type,
				scap_reader_t *r)
{
    uint8_t type;
    uint32_t toread;
    ...
    *nbytes = 0;
	...
	scap_fd_read_prop_from_disk(handle, &sub_len, sizeof(uint32_t), nbytes,
				    r)) ||
       scap_fd_read_prop_from_disk(handle, &(fdi->fd), sizeof(fdi->fd), nbytes,
				   r) ||
       scap_fd_read_prop_from_disk(handle, &(fdi->ino), sizeof(fdi->ino),
				   nbytes, r) ||
       scap_fd_read_prop_from_disk(handle, &type, sizeof(uint8_t), nbytes, r))
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not read prop block for fd");
	return SCAP_FAILURE;
    }
    ...
    {
    case SCAP_FD_IPV4_SOCK:
	if(scap_reader_read(r, &(fdi->info.ipv4info.sip), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4info.dip), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4info.sport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4info.dport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4info.l4proto),
			    sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error reading the fd info from file (1)");
	    return SCAP_FAILURE;
	}

	(*nbytes) += (sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint16_t) +
		      sizeof(uint16_t) + sizeof(uint8_t));

	break;
    case SCAP_FD_IPV4_SERVSOCK:
	if(scap_reader_read(r, &(fdi->info.ipv4serverinfo.ip),
			    sizeof(uint32_t)) != sizeof(uint32_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4serverinfo.port),
			    sizeof(uint16_t)) != sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv4serverinfo.l4proto),
			    sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error reading the fd info from file (2)");
	    return SCAP_FAILURE;
	}
	...
	break;
    case SCAP_FD_IPV6_SOCK:
	if(scap_reader_read(r, (char *)fdi->info.ipv6info.sip,
			    sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_reader_read(r, (char *)fdi->info.ipv6info.dip,
			    sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_reader_read(r, &(fdi->info.ipv6info.sport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv6info.dport), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv6info.l4proto),
			    sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi3)");
	...
	break;
    case SCAP_FD_IPV6_SERVSOCK:
	if(scap_reader_read(r, (char *)fdi->info.ipv6serverinfo.ip,
			    sizeof(uint32_t) * 4) != sizeof(uint32_t) * 4 ||
	   scap_reader_read(r, &(fdi->info.ipv6serverinfo.port),
			    sizeof(uint16_t)) != sizeof(uint16_t) ||
	   scap_reader_read(r, &(fdi->info.ipv6serverinfo.l4proto),
			    sizeof(uint8_t)) != sizeof(uint8_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (fi4)");
	...
	break;
    case SCAP_FD_UNIX_SOCK:
	if(scap_reader_read(r, &(fdi->info.unix_socket_info.source),
			    sizeof(uint64_t)) != sizeof(uint64_t) ||
	   scap_reader_read(r, &(fdi->info.unix_socket_info.destination),
			    sizeof(uint64_t)) != sizeof(uint64_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error reading the fd info from file (fi5)");
	    return SCAP_FAILURE;
	}

	(*nbytes) += (sizeof(uint64_t) + sizeof(uint64_t));
	res = scap_fd_read_fname_from_disk(
		handle, fdi->info.unix_socket_info.fname, nbytes, r);
	break;
    case SCAP_FD_FILE_V2:
	if(scap_reader_read(r, &(fdi->info.regularinfo.open_flags),
			    sizeof(uint32_t)) != sizeof(uint32_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error reading the fd info from file (fi1)");
	    return SCAP_FAILURE;
	}

	(*nbytes) += sizeof(uint32_t);
	res = scap_fd_read_fname_from_disk(handle, fdi->info.regularinfo.fname,
					   nbytes, r);
	if(!sub_len || (sub_len < *nbytes + sizeof(uint32_t)))
	{
	    break;
	}
	if(scap_reader_read(r, &(fdi->info.regularinfo.dev),
			    sizeof(uint32_t)) != sizeof(uint32_t))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error reading the fd info from file (dev)");
	    return SCAP_FAILURE;
	}
	(*nbytes) += sizeof(uint32_t);
	break;
    ...
    default:
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "error reading the fd info from file, wrong fd type %u",
		 (uint32_t)fdi->type);
	return SCAP_FAILURE;
    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "corrupted input file. Had read %zu bytes, but fdlist "
		     "entry have length %u.",
		     *nbytes, sub_len);
	    return SCAP_FAILURE;
	}
...
void scap_fd_free_ns_sockets_list(scap_t *handle,
				  struct scap_ns_socket_list **sockets)
{
...
//
// Add the file descriptor info pointed by fdi to the fd table for process
// tinfo. Note: silently skips if fdi->type is SCAP_FD_UNKNOWN.
//
int32_t scap_add_fd_to_proc_table(scap_t *handle, scap_threadinfo *tinfo,
				  scap_fdinfo *fdi, char *error)
{
	...
	//  - an fd has been closed by clone() or execve() (it happens when the
	//  fd is opened with the FD_CLOEXEC flag,
	//    which we don't currently parse.
	// In either case, removing the old fd, replacing it with the new one
	// and keeping going is a reasonable choice.
	//
	...
	if(uth_status != SCAP_SUCCESS)
	{
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "process table allocation error (2)");
	    return SCAP_FAILURE;
	}
    }
    else
...
int32_t scap_fd_handle_pipe(scap_t *handle, char *fname, scap_threadinfo *tinfo,
			    scap_fdinfo *fdi, char *error)
{
    ...
    if(r <= 0)
    {
	snprintf(error, SCAP_LASTERR_SIZE, "Could not read link %s (%s)", fname,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
    link_name[r] = '\0';
...
uint32_t scap_get_device_by_mount_id(scap_t *handle, const char *procdir,
...
void scap_fd_flags_file(scap_t *handle, scap_fdinfo *fdi, const char *procdir)
{
    ...
    snprintf(fd_dir_name, SCAP_MAX_PATH_SIZE, "%sfdinfo/%" PRId64, procdir,
	     fdi->fd);
    finfo = fopen(fd_dir_name, "r");
    if(finfo == NULL)
    {
	...
	if(!strncmp(line, "flags:\t", sizeof("flags:\t") - 1))
	{
	    ...
	    errno = 0;
	    unsigned long flags =
		    strtoul(line + sizeof("flags:\t") - 1, NULL, 8);
	...
	else if(!strncmp(line, "mnt_id:\t", sizeof("mnt_id:\t") - 1))
	{
	    ...
	    unsigned long mount_id =
		    strtoul(line + sizeof("mnt_id:\t") - 1, NULL, 10);
...
int32_t scap_fd_handle_regular_file(scap_t *handle, char *fname,
				    scap_threadinfo *tinfo, scap_fdinfo *fdi,
				    const char *procdir, char *error)
{
    char link_name[SCAP_MAX_PATH_SIZE];
    ssize_t r;
	...
	strlcpy(fdi->info.regularinfo.fname, link_name,
		sizeof(fdi->info.regularinfo.fname));
    }
    else
    {
	strlcpy(fdi->info.fname, link_name, sizeof(fdi->info.fname));
...
int32_t scap_fd_handle_socket(scap_t *handle, char *fname,
			      scap_threadinfo *tinfo, scap_fdinfo *fdi,
			      char *procdir, uint64_t net_ns,
			      ...
			      char *error)
{
	...
	if(sockets == NULL)
	{
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "socket list allocation error");
		free(sockets);
		return SCAP_FAILURE;
	    }

	    if(scap_fd_read_sockets(handle, procdir, sockets, fd_error) ==
	       SCAP_FAILURE)
	    {
		snprintf(error, SCAP_LASTERR_SIZE, "Cannot read sockets (%s)",
			 fd_error);
		sockets->sockets = NULL;
		return SCAP_FAILURE;
	    }
...
int32_t scap_fd_read_unix_sockets_from_proc_fs(scap_t *handle,
					       const char *filename,
					       scap_fdinfo **sockets)
{
    FILE *f;
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not open sockets file %s (%s)", filename,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	if(NULL != token)
	{
	    strlcpy(fdinfo->info.unix_socket_info.fname, token,
...
int32_t scap_fd_read_netlink_sockets_from_proc_fs(scap_t *handle,
						  const char *filename,
						  scap_fdinfo **sockets)
{
    FILE *f;
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not open netlink sockets file %s (%s)", filename,
		 scap_strerror(handle, errno));
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "netlink socket allocation error");
	    fclose(f);
	    free(fdinfo);
	    return SCAP_FAILURE;
	}
...
int32_t scap_fd_read_ipv4_sockets_from_proc_fs(scap_t *handle, const char *dir,
					       int l4proto,
					       scap_fdinfo **sockets)
{
    FILE *f;
	...
	free(scan_buf);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not open ipv4 sockets dir %s (%s)", dir,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
...
{
    return 0 == ip6_addr[0] && 0 == ip6_addr[1] && 0 == ip6_addr[2] &&
	   0 == ip6_addr[3];
}

int32_t scap_fd_read_ipv6_sockets_from_proc_fs(scap_t *handle, char *dir,
					       int l4proto,
					       scap_fdinfo **sockets)
{
    FILE *f;
	...
	free(scan_buf);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Could not open ipv6 sockets dir %s (%s)", dir,
		 scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
	...
	while(scan_pos <= scan_end)
	{
	    ...
	    *(scan_pos) = tc;
		...
		fdinfo->info.ipv6serverinfo.ip[0] =
			fdinfo->info.ipv6info.sip[0];
		fdinfo->info.ipv6serverinfo.ip[1] =
			fdinfo->info.ipv6info.sip[1];
		fdinfo->info.ipv6serverinfo.ip[2] =
			fdinfo->info.ipv6info.sip[2];
		fdinfo->info.ipv6serverinfo.ip[3] =
...
int32_t scap_fd_read_sockets(scap_t *handle, char *procdir,
			     struct scap_ns_socket_list *sockets, char *error)
{
	...
	//
	snprintf(netroot, sizeof(netroot), "%s/proc/net/",
		 scap_get_host_root());
    }

    snprintf(filename, sizeof(filename), "%stcp", netroot);
    if(scap_fd_read_ipv4_sockets_from_proc_fs(handle, filename, SCAP_L4_TCP,
					      &sockets->sockets) ==
       SCAP_FAILURE)
    {
	scap_fd_free_table(handle, &sockets->sockets);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "Could not read ipv4 tcp sockets (%s)", handle->m_lasterr);
	return SCAP_FAILURE;
    }

    snprintf(filename, sizeof(filename), "%sudp", netroot);
    if(scap_fd_read_ipv4_sockets_from_proc_fs(handle, filename, SCAP_L4_UDP,
					      &sockets->sockets) ==
       SCAP_FAILURE)
    {
	scap_fd_free_table(handle, &sockets->sockets);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "Could not read ipv4 udp sockets (%s)", handle->m_lasterr);
	return SCAP_FAILURE;
    }

    snprintf(filename, sizeof(filename), "%sraw", netroot);
    if(scap_fd_read_ipv4_sockets_from_proc_fs(handle, filename, SCAP_L4_RAW,
					      &sockets->sockets) ==
       SCAP_FAILURE)
    {
	scap_fd_free_table(handle, &sockets->sockets);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "Could not read ipv4 raw sockets (%s)", handle->m_lasterr);
	return SCAP_FAILURE;
    }

    snprintf(filename, sizeof(filename), "%sunix", netroot);
    if(scap_fd_read_unix_sockets_from_proc_fs(
	       handle, filename, &sockets->sockets) == SCAP_FAILURE)
    {
	scap_fd_free_table(handle, &sockets->sockets);
	snprintf(error, SCAP_LASTERR_SIZE, "Could not read unix sockets (%s)",
		 handle->m_lasterr);
	return SCAP_FAILURE;
    }

    snprintf(filename, sizeof(filename), "%snetlink", netroot);
    if(scap_fd_read_netlink_sockets_from_proc_fs(
	       handle, filename, &sockets->sockets) == SCAP_FAILURE)
    {
	scap_fd_free_table(handle, &sockets->sockets);
	snprintf(error, SCAP_LASTERR_SIZE,
		 "Could not read netlink sockets (%s)", handle->m_lasterr);
	return SCAP_FAILURE;
    }
    ...
    {
	if(scap_fd_read_ipv6_sockets_from_proc_fs(handle, filename, SCAP_L4_TCP,
						  &sockets->sockets) ==
	   SCAP_FAILURE)
	{
	    scap_fd_free_table(handle, &sockets->sockets);
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "Could not read ipv6 tcp sockets (%s)", handle->m_lasterr);
	    return SCAP_FAILURE;
	}

	snprintf(filename, sizeof(filename), "%sudp6", netroot);
	if(scap_fd_read_ipv6_sockets_from_proc_fs(handle, filename, SCAP_L4_UDP,
						  &sockets->sockets) ==
	   SCAP_FAILURE)
	{
	    scap_fd_free_table(handle, &sockets->sockets);
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "Could not read ipv6 udp sockets (%s)", handle->m_lasterr);
	    return SCAP_FAILURE;
	}

	snprintf(filename, sizeof(filename), "%sraw6", netroot);
	if(scap_fd_read_ipv6_sockets_from_proc_fs(handle, filename, SCAP_L4_RAW,
						  &sockets->sockets) ==
	   SCAP_FAILURE)
	{
	    scap_fd_free_table(handle, &sockets->sockets);
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "Could not read ipv6 raw sockets (%s)", handle->m_lasterr);
	    return SCAP_FAILURE;
	}
    }
...
int32_t scap_fd_allocate_fdinfo(scap_t *handle, scap_fdinfo **fdi, int64_t fd,
				scap_fd_type type)
{
    ASSERT(NULL == *fdi);
...
//
int32_t scap_fd_scan_fd_dir(scap_t *handle, char *procdir,
			    ...
			    char *error)
{
    ...
    if(dir_p == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE, "error opening the directory %s",
		 fd_dir_name);
    ...
    while((dir_entry_p = readdir(dir_p)) != NULL &&
	  (handle->m_fd_lookup_limit == 0 ||
	   fd_added < handle->m_fd_lookup_limit))
    {
	fdi = NULL;
	snprintf(f_name, SCAP_MAX_PATH_SIZE, "%s/%s", fd_dir_name,
		 dir_entry_p->d_name);

	if(-1 == stat(f_name, &sb) ||
	   1 != sscanf(dir_entry_p->d_name, "%" PRIu64, &fd))
	{
	    continue;
	}
	...
	case S_IFIFO:
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "can't allocate scap fd handle for fifo fd %" PRIu64,
			 fd);
		break;
	    }
	...
	case S_IFLNK:
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "can't allocate scap fd handle for file fd %" PRIu64,
			 fd);
		break;
	    }
	    fdi->ino = sb.st_ino;
	    res = scap_fd_handle_regular_file(handle, f_name, tinfo, fdi,
					      procdir, error);
	    break;
	case S_IFDIR:
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "can't allocate scap fd handle for dir fd %" PRIu64,
			 fd);
		break;
	    }
	    fdi->ino = sb.st_ino;
	...
	case S_IFSOCK:
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "can't allocate scap fd handle for sock fd %" PRIu64,
			 fd);
		break;
	    }
	    res = scap_fd_handle_socket(handle, f_name, tinfo, fdi, procdir,
					net_ns, sockets_by_ns, error);
	    if(handle->m_proc_callback == NULL)
	    {
	    ...
	    break;
	default:
	    res = scap_fd_allocate_fdinfo(handle, &fdi, fd,
					  SCAP_FD_UNSUPPORTED);
	    if(SCAP_FAILURE == res)
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "can't allocate scap fd handle for unsupported fd "
			 "%" PRIu64,
			 fd);
		break;
	    }
...
void scap_fd_print_fd_table(scap_t *handle, scap_fdinfo *fds)
{
    ...
    HASH_ITER(hh, fds, fdi, tfdi)
    {
	if(scap_fd_info_to_string(handle, fdi, str, SCAP_MAX_PATH_SIZE) !=
	   SCAP_SUCCESS)
	{
	    ASSERT(false);
