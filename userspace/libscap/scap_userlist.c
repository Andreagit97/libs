...
int32_t scap_create_userlist(scap_t *handle)
{
    ...
    // because this is not the first user list block), free it
    //
    ...
    handle->m_userlist->users =
	    (scap_userinfo *)malloc(usercnt * sizeof(scap_userinfo));
    ...
    handle->m_userlist->groups =
	    (scap_groupinfo *)malloc(grpcnt * sizeof(scap_groupinfo));
	...
	{
	    usercnt <<= 1;
	    void *tmp = realloc(handle->m_userlist->users,
				usercnt * sizeof(scap_userinfo));
	    if(tmp)
	    {
	...
	handle->m_userlist->totsavelen += sizeof(uint8_t) +  // type
					  sizeof(uint32_t) + // uid
					  sizeof(uint32_t) + // gid
					  strlen(user->name) + 2 +
    ...
    {
	// Reduce array size
	handle->m_userlist->users = realloc(handle->m_userlist->users,
					    useridx * sizeof(scap_userinfo));
    }
	...
	{
	    grpcnt <<= 1;
	    void *tmp = realloc(handle->m_userlist->groups,
				grpcnt * sizeof(scap_groupinfo));
	    if(tmp)
	    {
    ...
    {
	// Reduce array size
	handle->m_userlist->groups = realloc(handle->m_userlist->groups,
					     grpidx * sizeof(scap_groupinfo));
    }
    return SCAP_SUCCESS;
}
#else // HAS_CAPTURE
...
{
    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	     "scap_create_userlist not implement on this platform");
    return SCAP_FAILURE;
}
#endif // WIN32
#endif // HAS_CAPTURE
