...
typedef wh_proc_perf_info (*fwh_wmi_get_proc_perf_info)(wh_t* handle,
							uint64_t pid);
...
int32_t scap_windows_hal_import(char* error)
{
	...
	snprintf(error, SCAP_LASTERR_SIZE,
		 "dragent_win_hal.dll not found. Make sure the the windows HAL "
		 "is installed. Error: %u",
		 GetLastError());
	return SCAP_FAILURE;
    }

    p_wh_getlasterror =
	    (fwh_getlasterror)GetProcAddress(pdll, "wh_getlasterror");
    if(p_wh_getlasterror == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_getlasterror not found");
	return SCAP_FAILURE;
    }
    ...
    p_wh_wmi_get_procs =
	    (fwh_wmi_get_procs)GetProcAddress(pdll, "wh_wmi_get_procs");
    if(p_wh_wmi_get_procs == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_wmi_get_procs not found");
	return SCAP_FAILURE;
    }

    p_wh_is_docker_present =
	    (fwh_is_docker_present)GetProcAddress(pdll, "wh_is_docker_present");
    if(p_wh_is_docker_present == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_is_docker_present not found");
	return SCAP_FAILURE;
    }

    p_wh_docker_refresh =
	    (fwh_docker_refresh)GetProcAddress(pdll, "wh_docker_refresh");
    if(p_wh_docker_refresh == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_docker_refresh not found");
	return SCAP_FAILURE;
    }

    p_wh_wmi_update_procs_perf = (fwh_wmi_update_procs_perf)GetProcAddress(
	    pdll, "wh_wmi_update_procs_perf");
    if(p_wh_wmi_update_procs_perf == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_wmi_update_procs_perf not found");
	return SCAP_FAILURE;
    }

    p_wh_wmi_get_proc_perf_info = (fwh_wmi_get_proc_perf_info)GetProcAddress(
	    pdll, "wh_wmi_get_proc_perf_info");
    if(p_wh_wmi_get_proc_perf_info == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_wmi_get_proc_perf_info not found");
	return SCAP_FAILURE;
    }

    p_wh_create_fd_list =
	    (fwh_create_fd_list)GetProcAddress(pdll, "wh_create_fd_list");
    if(p_wh_create_fd_list == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_create_fd_list not found");
	return SCAP_FAILURE;
    }

    p_wh_free_fd_list =
	    (fwh_free_fd_list)GetProcAddress(pdll, "wh_free_fd_list");
    if(p_wh_free_fd_list == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_free_fd_list not found");
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_get_pid_fds not found");
	return SCAP_FAILURE;
    }

    p_wh_create_tid_list =
	    (fwh_create_tid_list)GetProcAddress(pdll, "wh_create_tid_list");
    if(p_wh_create_tid_list == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_create_tid_list not found");
	return SCAP_FAILURE;
    }

    p_wh_free_tid_list =
	    (fwh_free_tid_list)GetProcAddress(pdll, "wh_free_tid_list");
    if(p_wh_free_tid_list == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_free_tid_list not found");
	return SCAP_FAILURE;
    }

    p_wh_get_pid_tids =
	    (fwh_get_pid_tids)GetProcAddress(pdll, "wh_get_pid_tids");
    if(p_wh_get_pid_tids == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "win hal symbol wh_get_pid_tids not found");
	return SCAP_FAILURE;
    }
...
void scap_windows_hal_close(wh_t* handle) { p_wh_close(handle); }

void scap_get_machine_info_windows(OUT uint32_t* num_cpus,
				   OUT uint64_t* memory_size_bytes)
{
    SYSTEM_INFO si;
    GetSystemInfo(&si);
...
int32_t scap_create_userlist_windows(scap_t* handle)
{
    ...
    nueres = NetUserEnum(NULL, level,
			 FILTER_NORMAL_ACCOUNT, // global users
			 (LPBYTE*)&resbuf, maxlen, &eread, &etot,
			 &resume_handle);
    ...
    handle->m_userlist->users = (scap_userinfo*)malloc(
	...
	{
	    size_t clen = wcstombs(handle->m_userlist->users[j].name,
				   ui->usri3_name, SCAP_MAX_PATH_SIZE);
	...
	//
	// Disabled because NetUserEnum seems to return a corrupted
	// usri3_home_dir. Not a big deal.
	//
	// clen = wcstombs(handle->m_userlist->users[j].homedir,
	// ui->usri3_home_dir, SCAP_MAX_PATH_SIZE); if(clen ==
	// SCAP_MAX_PATH_SIZE)
	// {
	//	 handle->m_userlist->users[j].name[clen - 1] = 0;
	// }
...
int32_t scap_create_iflist_windows(scap_t* handle)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_create_iflist memory allocation error (1)");
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_create_iflist memory allocation error (2)");
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "getifaddrs allocation failed(1)");
	free(adapter_addresses);
	return SCAP_FAILURE;
    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "getifaddrs allocation failed(2)");
	    free(adapter_addresses);
	...
	for(ua = aa->FirstUnicastAddress; ua != NULL; ua = ua->Next)
	{
		...
		void* tempAddrPtr =
			&((struct sockaddr_in*)ua->Address.lpSockaddr->sa_data)
				 ->sin_addr;
		handle->m_addrlist->v4list[ifcnt4].addr =
			*(uint32_t*)tempAddrPtr;

		if(ua->OnLinkPrefixLength != 0)
		{
		    ConvertLengthToIpv4Mask(
			    ua->OnLinkPrefixLength,
			    &handle->m_addrlist->v4list[ifcnt4].netmask);
		}
		else
		{
		...
		handle->m_addrlist->v4list[ifcnt4].ifnamelen =
			(uint16_t)wcstombs(
				handle->m_addrlist->v4list[ifcnt4].ifname,
				aa->FriendlyName, SCAP_MAX_PATH_SIZE);

		handle->m_addrlist->v4list[ifcnt4].linkspeed = 0;

		handle->m_addrlist->totlen +=
			(sizeof(scap_ifinfo_ipv4) +
			 handle->m_addrlist->v4list[ifcnt4].ifnamelen -
			 SCAP_MAX_PATH_SIZE);
		ifcnt4++;
	    }
...
static int32_t addprocess_windows(wh_procinfo* wpi, scap_t* handle, char* error)
{
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "process table construction in scap not supported on windows");
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "addprocess_windows memory allocation error");
	return SCAP_FAILURE;
    }
    ...
    wh_proc_perf_info pinfo =
	    p_wh_wmi_get_proc_perf_info(handle->m_whh, tinfo->pid);
    if(pinfo.m_result != 0)
    {
	...
	for(uint32_t j = 0; j < pfl.m_count; j++)
	{
		...
		strncpy(fdi->info.fname,
			wfd->info.fname + 4, // the +4 removes the "\\?\" from
    ...
    handle->m_proc_callback(handle->m_proc_callback_context, handle, tinfo->tid,
			    tinfo, NULL);
...
int32_t scap_get_procs_windows(scap_t* handle, char* error)
{
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE, "%s",
		 p_wh_getlasterror(handle->m_whh));
	return SCAP_FAILURE;
    }

    //
    // While we're here, refresh the docker state and the process performance
    // table
    //
