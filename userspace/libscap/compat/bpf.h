 ...
 *bpf_probe_read_kernel_str() instead. Return On success, the strictly positive
 *length of the string, including the trailing NUL character. On error, a
 *negative value.
 *
 ...
 * 	Description
 * 		Copy a NUL terminated string from an unsafe kernel address
 **unsafe_ptr* to *dst*. Same semantics as with bpf_probe_read_user_str() apply.
 * 	Return
 * 		On success, the strictly positive length of the string,
 ...
 *retrieve the branch records (struct perf_branch_entry) associated to *ctx* and
 *store it in	the buffer pointed by *buf* up to size *size* bytes. Return On
 *success, number of bytes written to *buf*. On error, a negative value.
 *
 ...
 *
 * int bpf_xdp_output(void *ctx, struct bpf_map *map, u64 flags, void *data, u64
 *size) Description Write raw *data* blob into a special BPF perf event held by
 *		*map* of type **BPF_MAP_TYPE_PERF_EVENT_ARRAY**. This perf
 *		event must have the following attributes: **PERF_SAMPLE_RAW**
 *		as **sample_type**, **PERF_TYPE_SOFTWARE** as **type**, and
...
#define __BPF_FUNC_MAPPER(FN)                                                  \
    FN(unspec), FN(map_lookup_elem), FN(map_update_elem), FN(map_delete_elem), \
	    ...
	    FN(get_prandom_u32), FN(get_smp_processor_id),                     \
	    FN(skb_store_bytes), FN(l3_csum_replace), FN(l4_csum_replace),     \
	    FN(tail_call), FN(clone_redirect), FN(get_current_pid_tgid),       \
	    ...
	    FN(get_numa_node_id), FN(skb_change_head), FN(xdp_adjust_head),    \
	    FN(probe_read_str), FN(get_socket_cookie), FN(get_socket_uid),     \
	    FN(set_hash), FN(setsockopt), FN(skb_adjust_room),                 \
	    FN(redirect_map), FN(sk_redirect_map), FN(sock_map_update),        \
	    FN(xdp_adjust_meta), FN(perf_event_read_value),                    \
	    ...
	    FN(sock_hash_update), FN(msg_redirect_hash), FN(sk_redirect_hash), \
	    FN(lwt_push_encap), FN(lwt_seg6_store_bytes),                      \
	    ...
	    FN(sk_release), FN(map_push_elem), FN(map_pop_elem),               \
	    FN(map_peek_elem), FN(msg_push_data), FN(msg_pop_data),            \
	    FN(rc_pointer_rel), FN(spin_lock), FN(spin_unlock),                \
	    FN(sk_fullsock), FN(tcp_sock), FN(skb_ecn_set_ce),                 \
	    FN(get_listener_sock), FN(skc_lookup_tcp),                         \
	    FN(tcp_check_syncookie), FN(sysctl_get_name),                      \
	    FN(sysctl_get_current_value), FN(sysctl_get_new_value),            \
	    FN(sysctl_set_new_value), FN(strtol), FN(strtoul),                 \
	    FN(sk_storage_get), FN(sk_storage_delete), FN(send_signal),        \
	    FN(tcp_gen_syncookie), FN(skb_output), FN(probe_read_user),        \
	    FN(probe_read_kernel), FN(probe_read_user_str),                    \
	    FN(probe_read_kernel_str), FN(tcp_send_ack),                       \
	    FN(send_signal_thread), FN(jiffies64), FN(read_branch_records),    \
	    FN(get_ns_current_pid_tgid), FN(xdp_output), FN(get_netns_cookie), \
