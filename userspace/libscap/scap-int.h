...
inline const char* gzerror(FILE* F, int* E)
{
    *E = ferror(F);
...
#define READER_BUF_SIZE (1 << 16) // UINT16_MAX + 1, ie: 65536
    ...
    int32_t scap_readbuf(scap_t* handle, uint32_t proc, OUT char** buf,
			 OUT uint32_t* len);
    // Read a single thread info from /proc
    int32_t scap_proc_read_thread(scap_t* handle, char* procdirname,
				  uint64_t tid, struct scap_threadinfo** pi,
				  char* error, bool scan_sockets);
    // Scan a directory containing process information
    int32_t scap_proc_scan_proc_dir(scap_t* handle, char* procdirname,
				    char* error);
    ...
    // int32_t scap_proc_copy_fd_table(scap_t* handle, scap_threadinfo* dst,
    // scap_threadinfo* src); Internal helper function to output the process
    // table to screen
    ...
    // NOTE: this is different from scap_event_getprocinfo() because it returns
    // the full event information struct scap_threadinfo*
    // scap_proc_get_from_event(scap_t* handle, scap_evt* e); Return the process
    // info entry given a tid Free an fd table and set it to NULL when done
    ...
    int32_t scap_fd_info_to_string(scap_t* handle, scap_fdinfo* fdi,
				   OUT char* str, uint32_t strlen);
    ...
    int32_t scap_fd_write_to_disk(scap_t* handle, scap_fdinfo* fdi,
				  scap_dumper_t* dumper, uint32_t len);
    ...
    // pi. Note: silently skips if fdi->type is SCAP_FD_UNKNOWN.
    int32_t scap_add_fd_to_proc_table(scap_t* handle, scap_threadinfo* pi,
    ...
    int32_t scap_fd_scan_fd_dir(scap_t* handle, char* procdir,
				...
				char* error);
    // scan fd information for a specific thread from engine vtable. src_tinfo
    // is a pointer to a threadinfo returned by the engine
    int32_t scap_fd_scan_vtable(scap_t* handle,
				const scap_threadinfo* src_tinfo,
				scap_threadinfo* dst_tinfo, char* error);
    // read tcp or udp sockets from the proc filesystem
    int32_t scap_fd_read_ipv4_sockets_from_proc_fs(scap_t* handle,
						   const char* dir, int l4proto,
						   scap_fdinfo** sockets);
    ...
    int32_t scap_fd_allocate_fdinfo(scap_t* handle, scap_fdinfo** fdi,
				    int64_t fd, scap_fd_type type);
    // Free a file descriptor
    void scap_fd_free_fdinfo(scap_fdinfo** fdi);

    int32_t scap_fd_post_process_unix_sockets(scap_t* handle,
					      scap_fdinfo* sockets);

    int32_t scap_proc_fill_cgroups(scap_t* handle,
    ...
    // Returns SCAP_FAILURE if we tried to add the tid to the suppressed
    // tid set, but it could *not* be added, SCAP_SUCCESS otherwise.
    int32_t scap_update_suppressed(scap_t* handle, const char* comm,
    ...
    int32_t scap_procfs_get_threadlist(struct scap_engine_handle engine,
				       ...
				       char* lasterr);
    int32_t scap_os_getpid_global(struct scap_engine_handle engine,
				  int64_t* pid, char* error);
...
#define CHECK_READ_SIZE(read_size, expected_size)                              \
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,                         \
		 "expecting %d bytes, read %d at %s, line %d. Is the file "    \
		 "truncated?",                                                 \
		 (int)expected_size, (int)read_size, __FILE__, __LINE__);      \
	return SCAP_FAILURE;                                                   \
    }

#define CHECK_READ_SIZE_WITH_FREE(alloc_buffer, read_size, expected_size)      \
    if(read_size != expected_size)                                             \
    {                                                                          \
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,                         \
		 "expecting %d bytes, read %d at %s, line %d. Is the file "    \
...
#define SCAP_DRIVER_PROCINFO_MAX_SIZE 128000
    ...
    int32_t udig_set_snaplen(struct scap_engine_handle engine,
			     uint32_t snaplen);
    ...
    static inline int scap_reader_read(scap_reader_t* r, void* buf,
				       uint32_t len)
    {
	ASSERT(r != NULL);
	switch(r->m_type)
    ...
    static inline int64_t scap_reader_seek(scap_reader_t* r, int64_t offset,
					   int whence)
    {
	ASSERT(r != NULL);
	switch(r->m_type)
