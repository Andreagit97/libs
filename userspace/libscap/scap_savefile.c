...
#ifdef USE_ZLIB
int32_t compr(uint8_t *dest, uint64_t *destlen, const uint8_t *source,
	      uint64_t sourcelen, int level)
{
...
int32_t scap_write_proc_fds(scap_t *handle, struct scap_threadinfo *tinfo,
			    scap_dumper_t *d)
{
    block_header bh;
    uint32_t bt;
    uint32_t totlen =
	    MEMBER_SIZE(scap_threadinfo, tid); // This includes the tid
    uint32_t idx = 0;
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_write_proc_fds memory allocation failure");
	return SCAP_FAILURE;
    }
    ...
    bh.block_total_length =
	    scap_normalize_block_len(sizeof(block_header) + totlen + 4);

    if(scap_dump_write(d, &bh, sizeof(bh)) != sizeof(bh))
    {
	free(lengths);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "error writing to file (fd1)");
	return SCAP_FAILURE;
    }
    ...
    // Write the tid
    //
    if(scap_dump_write(d, &tinfo->tid, sizeof(tinfo->tid)) !=
       sizeof(tinfo->tid))
    {
	free(lengths);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "error writing to file (fd2)");
	return SCAP_FAILURE;
    }
	...
	if(fdi->type != SCAP_FD_UNINITIALIZED && fdi->type != SCAP_FD_UNKNOWN)
	{
	    if(scap_fd_write_to_disk(handle, fdi, d, lengths[idx++]) !=
	       SCAP_SUCCESS)
	    {
		free(lengths);
		return SCAP_FAILURE;
...
//
int32_t scap_write_proclist_header(scap_t *handle, scap_dumper_t *d,
				   uint32_t totlen)
{
    block_header bh;
...
// Write the process list block
//
int32_t scap_write_proclist_trailer(scap_t *handle, scap_dumper_t *d,
				    uint32_t totlen)
{
    block_header bh;
    uint32_t bt;
...
// Write the process list block
//
int32_t scap_write_proclist_entry(scap_t *handle, scap_dumper_t *d,
				  struct scap_threadinfo *tinfo, uint32_t len)
{
    ...
    return scap_write_proclist_entry_bufs(
	    handle, d, tinfo, len, tinfo->comm, tinfo->exe, tinfo->exepath,
	    &args, 1, &env, 1, tinfo->cwd, &cgroups, 1, tinfo->root);
}
...
int32_t scap_write_proclist_entry_bufs(
	scap_t *handle, scap_dumper_t *d, struct scap_threadinfo *tinfo,
	uint32_t len, const char *comm, const char *exe, const char *exepath,
	const struct iovec *args, int argscnt, const struct iovec *envs,
	int envscnt, const char *cwd, const struct iovec *cgroups,
	int cgroupscnt, const char *root)
{
    uint16_t commlen;
    uint16_t exelen;
    uint16_t exepathlen;
    ...
    if(scap_dump_write(d, &len, sizeof(uint32_t)) != sizeof(uint32_t) ||
       scap_dump_write(d, &(tinfo->tid), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->pid), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->ptid), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->sid), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->vpgid), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       ...
       scap_dump_write(d, &(tinfo->fdlimit), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->flags), sizeof(uint32_t)) !=
	       sizeof(uint32_t) ||
       ...
       scap_dump_write(d, &(tinfo->vmswap_kb), sizeof(uint32_t)) !=
	       sizeof(uint32_t) ||
       scap_dump_write(d, &(tinfo->pfmajor), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->pfminor), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       ...
       scap_dump_write(d, &(cgroupslen), sizeof(uint16_t)) !=
	       sizeof(uint16_t) ||
       ...
       scap_dump_write(d, &(tinfo->loginuid), sizeof(uint32_t)) !=
	       sizeof(uint32_t) ||
       scap_dump_write(d, &(tinfo->exe_writable), sizeof(uint8_t)) !=
	       sizeof(uint8_t) ||
       scap_dump_write(d, &(tinfo->cap_inheritable), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->cap_permitted), sizeof(uint64_t)) !=
	       sizeof(uint64_t) ||
       scap_dump_write(d, &(tinfo->cap_effective), sizeof(uint64_t)) !=
	       sizeof(uint64_t))
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
...
static int32_t scap_write_proclist(scap_t *handle, scap_dumper_t *d)
{
    ...
    uint32_t *lengths =
	    calloc(HASH_COUNT(handle->m_proclist), sizeof(uint32_t));
    if(lengths == NULL)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_write_proclist memory allocation failure");
	return SCAP_FAILURE;
    }
	...
	if(!tinfo->filtered_out)
	{
	    ...
	    uint32_t il =
		    (uint32_t)(sizeof(uint32_t) + // len
			       ...
			       2 + strnlen(tinfo->exe, SCAP_MAX_PATH_SIZE) + 2 +
			       strnlen(tinfo->exepath, SCAP_MAX_PATH_SIZE) + 2 +
			       tinfo->args_len + 2 +
			       strnlen(tinfo->cwd, SCAP_MAX_PATH_SIZE) +
			       ...
			       2 + tinfo->env_len + sizeof(int64_t) + // vtid
			       sizeof(int64_t) +		      // vpid
			       2 + tinfo->cgroups_len + 2 +
			       strnlen(tinfo->root, SCAP_MAX_PATH_SIZE) +
	...
	if(scap_write_proclist_entry(handle, d, tinfo, lengths[idx++]) !=
	   SCAP_SUCCESS)
	{
	    free(lengths);
	    return SCAP_FAILURE;
...
static int32_t scap_write_machine_info(scap_t *handle, scap_dumper_t *d)
{
    ...
    bh.block_total_length = scap_normalize_block_len(
	    sizeof(block_header) + sizeof(scap_machine_info) + 4);

    bt = bh.block_total_length;

    if(scap_dump_write(d, &bh, sizeof(bh)) != sizeof(bh) ||
       scap_dump_write(d, &handle->m_machine_info,
		       sizeof(handle->m_machine_info)) !=
	       sizeof(handle->m_machine_info) ||
       scap_dump_write(d, &bt, sizeof(bt)) != sizeof(bt))
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "error writing to file (MI1)");
	return SCAP_FAILURE;
    }
...
static int32_t scap_write_iflist(scap_t *handle, scap_dumper_t *d)
{
    ...
    {
	//
	// This can happen when the event source is a capture that was generated
	// by a plugin, no big deal
	//
	return SCAP_SUCCESS;
    }
    ...
    bh.block_total_length = scap_normalize_block_len(
	    sizeof(block_header) +
	    (handle->m_addrlist->n_v4_addrs + handle->m_addrlist->n_v6_addrs) *
		    sizeof(uint32_t) +
	...
	entrylen = sizeof(scap_ifinfo_ipv4) + entry->ifnamelen -
		   SCAP_MAX_PATH_SIZE;

	if(scap_dump_write(d, &entrylen, sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(entry->type), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(entry->ifnamelen), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(entry->addr), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(entry->netmask), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(entry->bcast), sizeof(uint32_t)) !=
		   sizeof(uint32_t) ||
	   scap_dump_write(d, &(entry->linkspeed), sizeof(uint64_t)) !=
		   sizeof(uint64_t) ||
	   scap_dump_write(d, &(entry->ifname), entry->ifnamelen) !=
		   entry->ifnamelen)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (IF2)");
	    return SCAP_FAILURE;
	}
	...
	entrylen = sizeof(scap_ifinfo_ipv6) + entry->ifnamelen -
		   SCAP_MAX_PATH_SIZE;
	...
	if(scap_dump_write(d, &entrylen, sizeof(uint32_t)) !=
	   ...
	   scap_dump_write(d, &(entry->ifnamelen), sizeof(uint16_t)) !=
		   sizeof(uint16_t) ||
	   scap_dump_write(d, &(entry->addr), SCAP_IPV6_ADDR_LEN) !=
		   SCAP_IPV6_ADDR_LEN ||
	   scap_dump_write(d, &(entry->netmask), SCAP_IPV6_ADDR_LEN) !=
		   SCAP_IPV6_ADDR_LEN ||
	   scap_dump_write(d, &(entry->bcast), SCAP_IPV6_ADDR_LEN) !=
		   SCAP_IPV6_ADDR_LEN ||
	   scap_dump_write(d, &(entry->linkspeed), sizeof(uint64_t)) !=
		   sizeof(uint64_t) ||
	   scap_dump_write(d, &(entry->ifname), entry->ifnamelen) !=
...
static int32_t scap_write_userlist(scap_t *handle, scap_dumper_t *d)
{
    ...
    uint32_t *lengths =
	    calloc(handle->m_userlist->nusers + handle->m_userlist->ngroups,
		   sizeof(uint32_t));
    if(lengths == NULL)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
	...
	// NB: new fields must be appended
	size_t ul = sizeof(uint32_t) + sizeof(type) + sizeof(info->uid) +
		    sizeof(info->gid) + sizeof(uint16_t) + namelen +
		    sizeof(uint16_t) + homedirlen + sizeof(uint16_t) + shelllen;
	totlen += ul;
	lengths[j] = ul;
    }
	...
	// NB: new fields must be appended
	uint32_t gl = sizeof(uint32_t) + sizeof(type) + sizeof(info->gid) +
	...
	if(scap_dump_write(d, &(lengths[handle->m_userlist->nusers + j]),
...
int32_t scap_setup_dump(scap_t *handle, scap_dumper_t *d, const char *fname)
{
    ...
    bh.block_total_length =
	    sizeof(block_header) + sizeof(section_header_block) + 4;
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "error writing to file %s  (5)", fname);
	return SCAP_FAILURE;
    }
	...
	if(scap_proc_scan_proc_dir(handle, filename, handle->m_lasterr) !=
	   SCAP_SUCCESS)
	{
...
static scap_dumper_t *scap_dump_open_gzfile(scap_t *handle, gzFile gzfile,
					    const char *fname,
					    bool skip_proc_scan)
{
    ...
    bool tmp_refresh_proc_table_when_saving =
    ...
    {
	handle->refresh_proc_table_when_saving =
		tmp_refresh_proc_table_when_saving;
    }
...
scap_dumper_t *scap_dump_open(scap_t *handle, const char *fname,
			      compression_mode compress, bool skip_proc_scan)
{
    gzFile f = NULL;
    int fd = -1;
    const char *mode;
...
scap_dumper_t *scap_dump_open_fd(scap_t *handle, int fd,
				 compression_mode compress, bool skip_proc_scan)
{
    gzFile f = NULL;
...
//
scap_dumper_t *scap_memory_dump_open(scap_t *handle, uint8_t *targetbuf,
				     uint64_t targetbufsize)
{
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_dump_memory_open memory allocation failure (1)");
	return NULL;
    }
    ...
    // Before doing that, backup handle->refresh_proc_table_when_saving so we
    // can restore whatever the current setting is as soon as we're done.
    //
    bool tmp_refresh_proc_table_when_saving =
	    handle->refresh_proc_table_when_saving;
    handle->refresh_proc_table_when_saving = false;
...
//
int32_t scap_number_of_bytes_to_write(scap_evt *e, uint16_t cpuid,
				      int32_t *bytes)
{
    *bytes = scap_normalize_block_len(sizeof(block_header) + sizeof(cpuid) +
				      e->len + 4);
		  ...
		  uint32_t flags)
{
	...
	//
	bh.block_type =
		large_payload ? EV_BLOCK_TYPE_V2_LARGE : EV_BLOCK_TYPE_V2;
	bh.block_total_length = scap_normalize_block_len(
		sizeof(block_header) + sizeof(cpuid) + e->len + 4);
	bt = bh.block_total_length;
	...
	//
	bh.block_type =
		large_payload ? EVF_BLOCK_TYPE_V2_LARGE : EVF_BLOCK_TYPE_V2;
	bh.block_total_length =
		scap_normalize_block_len(sizeof(block_header) + sizeof(cpuid) +
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error writing to file (7)");
	    return SCAP_FAILURE;
	}
    }

    //
    // Enable this to make sure that everything is saved to disk during the
    // tests
    //
#if 0
	fflush(f);
...
//
static int32_t scap_read_machine_info(scap_t *handle, scap_reader_t *r,
				      uint32_t block_length)
{
    //
    // Read the section header block
    //
    if(scap_reader_read(r, &handle->m_machine_info,
			sizeof(handle->m_machine_info)) !=
       sizeof(handle->m_machine_info))
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
...
//
static int32_t scap_read_proclist(scap_t *handle, scap_reader_t *r,
				  uint32_t block_length, uint32_t block_type)
{
    size_t readsize;
	...
	default:
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "corrupted process block type (fd1)");
	    ASSERT(false);
	    return SCAP_FAILURE;
	}
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "invalid commlen %d",
		     stlen);
	    return SCAP_FAILURE;
	}
	...
	if(stlen > SCAP_MAX_PATH_SIZE)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "invalid exelen %d",
		     stlen);
	    return SCAP_FAILURE;
	}
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "invalid argslen %d",
		     stlen);
	    return SCAP_FAILURE;
	}
	...
	if(stlen > SCAP_MAX_PATH_SIZE)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "invalid cwdlen %d",
		     stlen);
	    return SCAP_FAILURE;
	}
	...
	case PL_BLOCK_TYPE_V9:
	    ...
	    //
	    readsize =
		    scap_reader_read(r, &(tinfo.vmsize_kb), sizeof(uint32_t));
	    CHECK_READ_SIZE(readsize, sizeof(uint32_t));
	    ...
	    //
	    readsize =
		    scap_reader_read(r, &(tinfo.vmswap_kb), sizeof(uint32_t));
	    CHECK_READ_SIZE(readsize, sizeof(uint32_t));
	...
	{
	    readsize =
		    scap_reader_read(r, &(tinfo.exe_writable), sizeof(uint8_t));
	...
	{
	    readsize = scap_reader_read(r, &(tinfo.cap_inheritable),
					sizeof(uint64_t));
	    CHECK_READ_SIZE(readsize, sizeof(uint64_t));
	    subreadsize += readsize;
	}
	...
	if(sub_len && (subreadsize + sizeof(uint64_t)) <= sub_len)
	{
	    readsize = scap_reader_read(r, &(tinfo.cap_permitted),
					sizeof(uint64_t));
	    CHECK_READ_SIZE(readsize, sizeof(uint64_t));
	    subreadsize += readsize;
	}
	...
	if(sub_len && (subreadsize + sizeof(uint64_t)) <= sub_len)
	{
	    readsize = scap_reader_read(r, &(tinfo.cap_effective),
					sizeof(uint64_t));
	    CHECK_READ_SIZE(readsize, sizeof(uint64_t));
	    subreadsize += readsize;
	}
	...
	//
	// All parsed. Add the entry to the table, or fire the notification
	// callback
	//
	if(handle->m_proc_callback == NULL)
	{
	    //
	    // All parsed. Allocate the new entry and copy the temp one into
	    // into it.
	    //
	    struct scap_threadinfo *ntinfo =
		    (scap_threadinfo *)malloc(sizeof(scap_threadinfo));
	    if(ntinfo == NULL)
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "process table allocation error (fd1)");
		return SCAP_FAILURE;
	    }
	    ...
	    *ntinfo = tinfo;
		...
		free(ntinfo);
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "process table allocation error (fd2)");
		return SCAP_FAILURE;
	    }
	}
	else
	{
	    handle->m_proc_callback(handle->m_proc_callback_context, handle,
	...
	if(sub_len && subreadsize != sub_len)
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "corrupted input file. Had read %lu bytes, but "
			 "proclist entry have length %u.",
			 subreadsize, sub_len);
		return SCAP_FAILURE;
	    }
...
//
static int32_t scap_read_iflist(scap_t *handle, scap_reader_t *r,
				uint32_t block_length, uint32_t block_type)
{
    ...
    //
    // If the list of interfaces was already allocated for this handle (for
    // example because this is not the first interface list block), free it
    //
    if(handle->m_addrlist != NULL)
    {
    ...
    // We assume that this block is always small enough that we can read it in a
    // single shot
    //
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "memory allocation error in scap_read_iflist");
	return SCAP_FAILURE;
    }
	...
	if(block_type != IL_BLOCK_TYPE_V2)
	{
	    ...
	    {
		entrysize = sizeof(scap_ifinfo_ipv4) + ifnamlen -
			    SCAP_MAX_PATH_SIZE;
	    }
	    else if(iftype == SCAP_II_IPV6)
	    {
		entrysize = sizeof(scap_ifinfo_ipv6) + ifnamlen -
			    SCAP_MAX_PATH_SIZE;
	    }
	    else if(iftype == SCAP_II_IPV4_NOLINKSPEED)
	    {
		entrysize = sizeof(scap_ifinfo_ipv4_nolinkspeed) + ifnamlen -
			    SCAP_MAX_PATH_SIZE;
	    }
	    else if(iftype == SCAP_II_IPV6_NOLINKSPEED)
	    {
		entrysize = sizeof(scap_ifinfo_ipv6_nolinkspeed) + ifnamlen -
			    SCAP_MAX_PATH_SIZE;
	    }
	    else
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "trace file has corrupted interface list(1)");
		ASSERT(false);
		res = SCAP_FAILURE;
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "trace file has corrupted interface list(2) toread=%u, "
		     "entrysize=%u",
		     toread, entrysize);
	    res = SCAP_FAILURE;
	    goto scap_read_iflist_error;
	...
	else
	{
	    ASSERT(false);
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "unknown interface type %d", (int)iftype);
	    res = SCAP_FAILURE;
	    goto scap_read_iflist_error;
	}
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_read_iflist allocation failed(1)");
	res = SCAP_FAILURE;
	goto scap_read_iflist_error;
    }
    ...
    handle->m_addrlist->totlen =
	    block_length - (ifcnt4 + ifcnt6) * sizeof(uint32_t);

    if(ifcnt4 != 0)
    {
	handle->m_addrlist->v4list =
		(scap_ifinfo_ipv4 *)malloc(ifcnt4 * sizeof(scap_ifinfo_ipv4));
	if(!handle->m_addrlist->v4list)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "scap_read_iflist allocation failed(2)");
	    res = SCAP_FAILURE;
	    goto scap_read_iflist_error;
	}
    }
    ...
    if(ifcnt6 != 0)
    {
	handle->m_addrlist->v6list =
		(scap_ifinfo_ipv6 *)malloc(ifcnt6 * sizeof(scap_ifinfo_ipv6));
	if(!handle->m_addrlist->v6list)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "getifaddrs allocation failed(3)");
	    res = SCAP_FAILURE;
	    goto scap_read_iflist_error;
	}
    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "trace file has corrupted interface list(0)");
	    res = SCAP_FAILURE;
	    goto scap_read_iflist_error;
	}
	...
	if(iftype == SCAP_II_IPV4)
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "trace file has corrupted interface list(3)");
		res = SCAP_FAILURE;
		goto scap_read_iflist_error;
	    }
	    ...
	    memcpy(handle->m_addrlist->v4list[ifcnt4].ifname,
		   pif + ifsize - ifnamlen, ifnamlen);
	...
	else if(iftype == SCAP_II_IPV4_NOLINKSPEED)
	{
	    ...
	    ifsize = sizeof(scap_ifinfo_ipv4_nolinkspeed) + ifnamlen -
		     SCAP_MAX_PATH_SIZE;

	    if(toread < ifsize)
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "trace file has corrupted interface list(4)");
		res = SCAP_FAILURE;
		goto scap_read_iflist_error;
	    }
	    ...
	    dst->bcast = src->bcast;
	    dst->linkspeed = 0;
	    memcpy(dst->ifname, src->ifname,
		   MIN(dst->ifnamelen, SCAP_MAX_PATH_SIZE - 1));

	    // Make sure the name string is NULL-terminated
	    *((char *)(dst->ifname +
		       MIN(dst->ifnamelen, SCAP_MAX_PATH_SIZE - 1))) = 0;
	...
	else if(iftype == SCAP_II_IPV6)
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "trace file has corrupted interface list(5)");
		res = SCAP_FAILURE;
		goto scap_read_iflist_error;
	    }
	    ...
	    memcpy(handle->m_addrlist->v6list[ifcnt6].ifname,
		   pif + ifsize - ifnamlen, ifnamlen);
	...
	else if(iftype == SCAP_II_IPV6_NOLINKSPEED)
	{
	    ...
	    ifsize = sizeof(scap_ifinfo_ipv6_nolinkspeed) + ifnamlen -
		     SCAP_MAX_PATH_SIZE;

	    if(toread < ifsize)
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "trace file has corrupted interface list(6)");
		res = SCAP_FAILURE;
		goto scap_read_iflist_error;
	    }
...
//
static int32_t scap_read_userlist(scap_t *handle, scap_reader_t *r,
				  uint32_t block_length, uint32_t block_type)
{
    size_t readsize;
    size_t totreadsize = 0;
    ...
    //
    // If the list of users was already allocated for this handle (for example
    // because this is not the first interface list block), free it
    //
    if(handle->m_userlist != NULL)
    {
	...
	if(type == USERBLOCK_TYPE_USER)
	{
	    ...
	    handle->m_userlist->users = (scap_userinfo *)realloc(
		    handle->m_userlist->users,
		    handle->m_userlist->nusers * sizeof(scap_userinfo));
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "invalid user homedir len %d", stlen);
		return SCAP_FAILURE;
	    }
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "invalid user shell len %d", stlen);
		return SCAP_FAILURE;
	    }
	...
	else
	{
	    ...
	    handle->m_userlist->groups = (scap_groupinfo *)realloc(
		    handle->m_userlist->groups,
		    handle->m_userlist->ngroups * sizeof(scap_groupinfo));
	    ...
	    pgroup = &handle->m_userlist
			      ->groups[handle->m_userlist->ngroups - 1];
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "invalid group name len %d", stlen);
		return SCAP_FAILURE;
	    }
...
// Parse a process list block
//
static int32_t scap_read_fdlist(scap_t *handle, scap_reader_t *r,
				uint32_t block_length, uint32_t block_type)
{
    size_t readsize;
    size_t totreadsize = 0;
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "corrupted trace file. FD block references TID %" PRIu64
		     ", which doesn't exist.",
		     tid);
	    return SCAP_FAILURE;
	}
    ...
    while(((int32_t)block_length - (int32_t)totreadsize) >= 4)
    {
	if(scap_fd_read_from_disk(handle, &fdi, &readsize, block_type, r) !=
	   SCAP_SUCCESS)
	{
	    return SCAP_FAILURE;
	}
	...
	{
	    //
	    // Parsed successfully. Allocate the new entry and copy the temp one
	    // into into it.
	    //
	...
	ASSERT(false);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_read_fdlist read more %lu than a block %u", totreadsize,
		 block_length);
	return SCAP_FAILURE;
    }
    padding_len = block_length - totreadsize;
...
int32_t scap_read_init(scap_t *handle, scap_reader_t *r)
{
	...
	case MI_BLOCK_TYPE_INT:
	    ...
	    if(scap_read_machine_info(handle, r,
				      bh.block_total_length -
					      sizeof(block_header) - 4) !=
	       SCAP_SUCCESS)
	    {
		return SCAP_FAILURE;
	    }
	...
	case PL_BLOCK_TYPE_V3_INT:
	    ...
	    if(scap_read_proclist(handle, r,
				  bh.block_total_length - sizeof(block_header) -
					  4,
				  bh.block_type) != SCAP_SUCCESS)
	    {
		return SCAP_FAILURE;
	    }
	    break;
	...
	case EVF_BLOCK_TYPE_V2_LARGE:
	    ...
	    //
	    // We're done with the metadata headers. Rewind the file position so
	    // we are aligned to start reading the events.
	    //
	...
	case UL_BLOCK_TYPE_V2:
	    ...
	    if(scap_read_userlist(handle, r,
				  bh.block_total_length - sizeof(block_header) -
					  4,
				  bh.block_type) != SCAP_SUCCESS)
	    {
	...
	default:
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "corrupted input file. Can't skip block of type %x "
			 "and size %u.",
			 (int)bh.block_type, (unsigned int)toread);
		return SCAP_FAILURE;
	    }
	    break;
	}
	...
	if(bt != bh.block_total_length)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     ...
		     bh.block_total_length, bt);
	    return SCAP_FAILURE;
	}
    }

    //
    // NOTE: can't require a user list block, interface list block, or machine
    // info block
    //       any longer--with the introduction of source plugins, it is
    //       legitimate to have trace files that don't contain those blocks
    //
...
//
int32_t scap_next_offline(scap_t *handle, OUT scap_evt **pevent,
			  OUT uint16_t *pcpuid)
{
    block_header bh;
    size_t readsize;
	...
	if(readsize != sizeof(bh))
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "error reading file: %s, ernum=%d", err_str, err_no);
		return SCAP_FAILURE;
	    }
	    ...
	    {
		//
		// We read exactly 0 bytes. This indicates a correct end of
		// file.
		//
		return SCAP_EOF;
	    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "unexpected block type %u", (uint32_t)bh.block_type);
	...
	if(bh.block_type != EV_BLOCK_TYPE_V2_LARGE &&
	   bh.block_type != EVF_BLOCK_TYPE_V2_LARGE)
	{
	    if(readlen > READER_BUF_SIZE)
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "event block length %u greater than NON-LARGE read "
			 "buffer size %u",
			 readlen, READER_BUF_SIZE);
		return SCAP_FAILURE;
	    }
	}
	else if(readlen > handle->m_reader_evt_buf_size)
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "event block length %u greater than read buffer size "
			 "%zu",
			 readlen, handle->m_reader_evt_buf_size);
		return SCAP_FAILURE;
	    }
	...
	*pcpuid = *(uint16_t *)handle->m_reader_evt_buf;
	...
	if(bh.block_type == EVF_BLOCK_TYPE ||
	   ...
	   bh.block_type == EVF_BLOCK_TYPE_V2_LARGE)
	{
	    handle->m_last_evt_dump_flags =
		    *(uint32_t *)(handle->m_reader_evt_buf + sizeof(uint16_t));
	    *pevent =
		    (struct ppm_evt_hdr *)(handle->m_reader_evt_buf +
					   sizeof(uint16_t) + sizeof(uint32_t));
	}
	else
	{
	    handle->m_last_evt_dump_flags = 0;
	    *pevent = (struct ppm_evt_hdr *)(handle->m_reader_evt_buf +
					     sizeof(uint16_t));
	}
	...
	{
	    //
	    // We're reading an old capture whose events don't have nparams in
	    // the header. Convert it to the current version.
	    //
	    if((readlen + sizeof(uint32_t)) > READER_BUF_SIZE)
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "cannot convert v1 event block to v2 (%lu greater "
			 "than read buffer size %u)",
			 readlen + sizeof(uint32_t), READER_BUF_SIZE);
		return SCAP_FAILURE;
	    }

	    memmove((char *)*pevent + sizeof(struct ppm_evt_hdr),
		    (char *)*pevent + sizeof(struct ppm_evt_hdr) -
			    sizeof(uint32_t),
		    readlen - ((char *)*pevent - handle->m_reader_evt_buf) -
			    (sizeof(struct ppm_evt_hdr) - sizeof(uint32_t)));
	    (*pevent)->len += sizeof(uint32_t);

	    // In old captures, the length of PPME_NOTIFICATION_E and
	    // PPME_INFRASTRUCTURE_EVENT_E is not correct. Adjust it, otherwise
	    // the following code will never find a match
	    ...
	    //
	    // The number of parameters needs to be calculated based on the
	    // block len. Use the current number of parameters as starting point
	    // and decrease it until size matches.
	    //
	    char *end = (char *)*pevent + (*pevent)->len;
	    uint16_t *lens =
		    (uint16_t *)((char *)*pevent + sizeof(struct ppm_evt_hdr));
	    uint32_t nparams;
	    bool done = false;
	    for(nparams = g_event_info[(*pevent)->type].nparams;
		(int)nparams >= 0; nparams--)
	    {
		...
		{
		    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			     "cannot convert v1 event block to v2 (corrupted "
			     "trace file - can't calculate nparams).");
		    return SCAP_FAILURE;
		}
		ASSERT(valptr >= end);
		if(valptr == end)
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "cannot convert v1 event block to v2 (corrupted trace "
			 "file - can't calculate nparams) (2).");
		return SCAP_FAILURE;
	    }
