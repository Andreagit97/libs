...
uint32_t scap_event_getlen(scap_evt *e) { return e->len; }

uint64_t scap_event_get_num(scap_t *handle) { return handle->m_evtcnt; }

void scap_event_reset_count(scap_t *handle) { handle->m_evtcnt = 0; }

uint64_t scap_event_get_ts(scap_evt *e) { return e->ts; }
				  ...
				  struct scap_sized_buffer *params)
{
    ...
    uint32_t n =
	    event_info->nparams < e->nparams ? event_info->nparams : e->nparams;
...
void scap_event_set_param_length_regular(scap_evt *event, uint32_t n,
					 uint16_t len)
{
...
void scap_event_set_param_length_large(scap_evt *event, uint32_t n,
				       uint32_t len)
{
...
static inline int32_t scap_buffer_can_fit(struct scap_sized_buffer buf,
					  size_t len)
{
    return (buf.size >= len);
}
				 ...
				 ...)
{
    va_list args;
    va_start(args, n);
    int32_t ret = scap_event_encode_params_v(event_buf, event_size, error,
					     event_type, n, args);
    va_end(args);
				   ...
				   va_list args)
{
    ...
    // every buffer write access needs to be guarded by a scap_buffer_can_fit
    // call to check if it's large enough
	...
	case PT_SOCKTUPLE: /* A sockaddr tuple,1byte family + 12byte data +
			      12byte data */
	case PT_FDLIST:	   /* A list of fds, 16bit count + count * (64bit fd +
			      16bit flags) */
	case PT_DYN: /* Type can vary depending on the context. Used for filter
			fields like evt.rawarg. */
	case PT_CHARBUFARRAY: /* Pointer to an array of strings, exported by the
				 user events decoder. 64bit. For internal use
				 only. */
	case PT_CHARBUF_PAIR_ARRAY: /* Pointer to an array of string pairs,
				       exported by the user events decoder.
	...
	case PT_IPADDR: /* Either an IPv4 or IPv6 address. The length indicates
			   which one it is. */
	case PT_IPNET:	/* Either an IPv4 or IPv6 network. The length indicates
			   which one it is. */
	case PT_SOCKADDR:
	...
	default:   // Unsupported event
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "event param %d (param type %d) is unsupported", i,
		     pi->type);
	    return SCAP_FAILURE;
	}
	...
	case sizeof(uint16_t):
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "could not fit event param %d size %zu for event with "
			 "type %d in %zu bytes",
	...
	default:
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "unexpected param %d length %zu for event with type %d", i,
		     len_size, event->type);
	    return SCAP_FAILURE;
	}
    ...
    *event_size = len;
    ...
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "Could not encode event of size %zu into supplied buffer "
		 "sized %zu.",
		 len, event_buf.size);
