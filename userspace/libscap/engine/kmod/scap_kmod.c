...
{
    uint32_t max;
    FILE *pfile = fopen("/sys/module/" SCAP_KERNEL_MODULE_NAME
			"/parameters/max_consumers",
			"r");
    if(pfile != NULL)
    {
...
int32_t scap_kmod_init(scap_t *handle, scap_open_args *oargs)
{
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "_SC_NPROCESSORS_CONF: %s", scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "_SC_NPROCESSORS_ONLN: %s", scap_strerror(handle, errno));
	return SCAP_FAILURE;
    }

    rc = devset_init(&handle->m_engine.m_handle->m_dev_set, ndevs,
		     handle->m_lasterr);
    if(rc != SCAP_SUCCESS)
    {
	return rc;
    }
    fill_syscalls_of_interest(&oargs->ppm_sc_of_interest,
			      &handle->syscalls_of_interest);
    ...
    for(j = 0, all_scanned_devs = 0;
	j < devset->m_ndevs && all_scanned_devs < handle->m_ncpus;
	++all_scanned_devs)
    {
	...
	// Open the device
	//
	snprintf(filename, sizeof(filename), "%s/dev/" DRIVER_DEVICE_NAME "%d",
		 scap_get_host_root(), all_scanned_devs);
	...
	if((dev->m_fd = open(filename, O_RDWR | O_SYNC)) < 0)
	{
		...
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "Too many consumers attached to device %s. Current "
			 "value for /sys/module/" SCAP_KERNEL_MODULE_NAME
			 "/parameters/max_consumers is '%" PRIu32 "'.",
			 filename, curr_max_consumers);
	    }
	    else
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "error opening device %s. Make sure you have root "
			 "credentials and that the " DRIVER_NAME
			 " module is loaded.",
			 filename);
	    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Can not set close-on-exec flag for fd for device %s (%s)",
	...
	{
	    snprintf(
		    handle->m_lasterr, SCAP_LASTERR_SIZE,
	...
	if(handle->m_api_version != 0 && handle->m_api_version != api_version)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     ...
		     filename, PPM_API_VERSION_MAJOR(api_version),
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Kernel module does not support "
		     "PPM_IOCTL_GET_SCHEMA_VERSION");
	    close(dev->m_fd);
	    return SCAP_FAILURE;
	}
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "Schema version mismatch: device %s reports schema "
		     "version %llu.%llu.%llu, expected %llu.%llu.%llu",
		     filename, PPM_API_VERSION_MAJOR(schema_version),
	...
	//
	dev->m_buffer =
		(char *)mmap(0, len, PROT_READ, MAP_SHARED, dev->m_fd, 0);

	if(dev->m_buffer == MAP_FAILED)
	{
	    // we cleanup this fd and then we let scap_close() take care of the
	    // other ones
	    close(dev->m_fd);

	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error mapping the ring buffer for device %s", filename);
	    return SCAP_FAILURE;
	}
	...
	//
	dev->m_bufinfo = (struct ppm_ring_buffer_info *)mmap(
		0, sizeof(struct ppm_ring_buffer_info), PROT_READ | PROT_WRITE,
		MAP_SHARED, dev->m_fd, 0);
	...
	if(dev->m_bufinfo == MAP_FAILED)
	{
	    ...
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "error mapping the ring buffer info for device %s",
		     filename);
	    return SCAP_FAILURE;
	}
	...
	{
	    // Kmod driver event_mask check uses event_types instead of syscall
	    // nr
	    ...
	    // Filter unmapped syscalls (that have a g_syscall_table entry with
	    // both enter_ev and exit_ev 0ed)
...
int32_t scap_kmod_next(struct scap_engine_handle engine, OUT scap_evt **pevent,
		       OUT uint16_t *pcpuid)
{
...
int32_t scap_kmod_stop_capture(struct scap_engine_handle engine)
{
	...
	struct scap_device *dev = &devset->m_devs[j];
	{
		...
		snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "scap_stop_capture failed for device %" PRIu32, j);
		ASSERT(false);
		return SCAP_FAILURE;
	    }
	}
...
int32_t scap_kmod_start_capture(struct scap_engine_handle engine)
{
	...
	{
	    snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "scap_start_capture failed for device %" PRIu32, j);
	    ASSERT(false);
	    return SCAP_FAILURE;
	}
    }
...
static int32_t scap_kmod_set_dropping_mode(struct scap_engine_handle engine,
					   int request, uint32_t sampling_ratio)
{
    struct scap_device_set *devset = &engine.m_handle->m_dev_set;
    if(devset->m_ndevs)
    {
	ASSERT((request == PPM_IOCTL_ENABLE_DROPPING_MODE &&
		((sampling_ratio == 1) || (sampling_ratio == 2) ||
		 (sampling_ratio == 4) || (sampling_ratio == 8) ||
		 (sampling_ratio == 16) || (sampling_ratio == 32) ||
	...
	if(ioctl(devset->m_devs[0].m_fd, request, sampling_ratio))
	{
	    ...
	    snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "%s, request %d for sampling ratio %u: %s", __FUNCTION__,
		     request, sampling_ratio, scap_strerror_r(buf, errno));
	    ASSERT(false);
	    return SCAP_FAILURE;
	}
    }
...
{
    return scap_kmod_set_dropping_mode(engine, PPM_IOCTL_DISABLE_DROPPING_MODE,
				       0);
}
...
{
    return scap_kmod_set_dropping_mode(engine, PPM_IOCTL_ENABLE_DROPPING_MODE,
				       sampling_ratio);
}

int32_t scap_kmod_set_snaplen(struct scap_engine_handle engine,
			      uint32_t snaplen)
{
    struct scap_device_set *devset = &engine.m_handle->m_dev_set;
    //
    ...
    //
    // Force a flush of the read buffers, so we don't capture events with the
    // old snaplen
    //
    for(j = 0; j < devset->m_ndevs; j++)
    {
...
int32_t scap_kmod_handle_event_mask(struct scap_engine_handle engine,
				    uint32_t op, uint32_t event_id)
{
    ...
    {
	snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "%s(%d) failed for event type %d", __FUNCTION__, op, event_id);
	ASSERT(false);
	return SCAP_FAILURE;
    }
...
int32_t scap_kmod_enable_dynamic_snaplen(struct scap_engine_handle engine)
{
    ...
    // Tell the driver to change the snaplen
    //
    if(ioctl(engine.m_handle->m_dev_set.m_devs[0].m_fd,
	     PPM_IOCTL_ENABLE_DYNAMIC_SNAPLEN))
    {
	snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
...
int32_t scap_kmod_disable_dynamic_snaplen(struct scap_engine_handle engine)
{
    ...
    {
	snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_disable_dynamic_snaplen failed");
	ASSERT(false);
	return SCAP_FAILURE;
    }
    return SCAP_SUCCESS;
...
{
    if(ioctl(engine.m_handle->m_dev_set.m_devs[0].m_fd,
	     PPM_IOCTL_SET_SIMPLE_MODE))
    {
	snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "scap_enable_simpledriver_mode failed");
	ASSERT(false);
	return SCAP_FAILURE;
    }
...
int32_t scap_kmod_get_n_tracepoint_hit(struct scap_engine_handle engine,
				       long *ret)
{
    int ioctl_ret = 0;

    ioctl_ret = ioctl(engine.m_handle->m_dev_set.m_devs[0].m_fd,
		      PPM_IOCTL_GET_N_TRACEPOINT_HIT, ret);
    if(ioctl_ret != 0)
    {
	if(errno == ENOTTY)
	{
	    snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "scap_get_n_tracepoint_hit failed, ioctl not supported");
	}
	else
	{
	    char buf[SCAP_LASTERR_SIZE];
	    snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "scap_get_n_tracepoint_hit failed (%s)",
					...
					char *lasterr)
{
    ...
    int ioctlres = ioctl(kmod_engine->m_dev_set.m_devs[0].m_fd,
			 PPM_IOCTL_GET_PROCLIST, procinfo_p);
    if(ioctlres)
    {
	if(errno == ENOSPC)
	{
	...
	{
	    snprintf(kmod_engine->m_lasterr, SCAP_LASTERR_SIZE,
...
static int32_t scap_kmod_get_vpid(struct scap_engine_handle engine,
				  uint64_t pid, int64_t *vpid)
{
    struct kmod_engine *kmod_engine = engine.m_handle;
    *vpid = ioctl(kmod_engine->m_dev_set.m_devs[0].m_fd, PPM_IOCTL_GET_VPID,
		  pid);
	...
	ASSERT(false);
	snprintf(kmod_engine->m_lasterr, SCAP_LASTERR_SIZE,
		 "ioctl to get vpid failed (%s)", scap_strerror_r(buf, errno));
	return SCAP_FAILURE;
    }
...
static int32_t scap_kmod_get_vtid(struct scap_engine_handle engine,
				  uint64_t tid, int64_t *vtid)
{
    struct kmod_engine *kmod_engine = engine.m_handle;
    *vtid = ioctl(kmod_engine->m_dev_set.m_devs[0].m_fd, PPM_IOCTL_GET_VTID,
		  tid);
	...
	ASSERT(false);
	snprintf(kmod_engine->m_lasterr, SCAP_LASTERR_SIZE,
		 "ioctl to get vtid failed (%s)", scap_strerror_r(buf, errno));
	return SCAP_FAILURE;
    }
...
int32_t scap_kmod_getpid_global(struct scap_engine_handle engine, int64_t *pid,
				char *error)
{
    struct kmod_engine *kmod_engine = engine.m_handle;
    *pid = ioctl(kmod_engine->m_dev_set.m_devs[0].m_fd,
		 PPM_IOCTL_GET_CURRENT_PID);
    if(*pid == -1)
    {
	char buf[SCAP_LASTERR_SIZE];
	ASSERT(false);
