			...
			char* error)
{
	...
	*ring_fd = ud_shm_open(UDIG_RING_SM_FNAME, O_CREAT | O_RDWR,
	...
	if(*ring_fd >= 0)
	{
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "udig_alloc_ring ftruncate error: %s\n",
			 strerror(errno));
		close(*ring_fd);
		return SCAP_FAILURE;
	    }
	...
	{
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "udig_alloc_ring shm_open error: %s\n", strerror(errno));
	    return SCAP_FAILURE;
	}
    }
    ...
    //
    uint8_t* buf1 = (uint8_t*)mmap(NULL, (*ringsize) * 2, PROT_WRITE,
    ...
    *ring = (uint8_t*)mmap(buf1, *ringsize, PROT_WRITE, MAP_SHARED | MAP_FIXED,
			   *ring_fd, 0);
    if(*ring != buf1)
    {
    ...
    uint8_t* ring2 = (uint8_t*)mmap(buf2, *ringsize, PROT_WRITE,
				    MAP_SHARED | MAP_FIXED, *ring_fd, 0);
    if(ring2 != buf2)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "can't map second copy of buffer, needed %p, obtained %p, "
		 "base=%p\n",
		 buf2, ring2, buf1);
	close(*ring_fd);
	...
	struct udig_ring_buffer_status** ring_status, char* error)
{
    ...
    uint32_t mem_size = sizeof(struct ppm_ring_buffer_info) +
			sizeof(struct udig_ring_buffer_status);
    ...
    *ring_descs_fd = ud_shm_open(UDIG_RING_DESCS_SM_FNAME, O_RDWR, 0);
	...
	//
	*ring_descs_fd = ud_shm_open(
		UDIG_RING_DESCS_SM_FNAME, O_CREAT | O_RDWR | O_EXCL,
		S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
	if(*ring_descs_fd >= 0)
	{
	    //
	    ...
	    {
		snprintf(error, SCAP_LASTERR_SIZE,
			 "udig_alloc_ring_descriptors ftruncate error: %s\n",
			 strerror(errno));
		close(*ring_descs_fd);
	...
	{
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "udig_alloc_ring_descriptors shm_open error: %s\n",
		     strerror(errno));
	    shm_unlink(UDIG_RING_DESCS_SM_FNAME);
	    return SCAP_FAILURE;
	}
    ...
    // Map the memory
    //
    uint8_t* descs = (uint8_t*)mmap(NULL, mem_size, PROT_READ | PROT_WRITE,
				    MAP_SHARED, *ring_descs_fd, 0);
    if(descs == MAP_FAILED)
    {
    ...
    *ring_status =
	    (struct
	     udig_ring_buffer_status*)((uint64_t)*ring_info +
				       sizeof(struct ppm_ring_buffer_info));

    //
    // If we are the original creators of the shared buffer, proceed to
    // initialize it.
    // Note that, according to the man page of ud_shm_open, we are guaranteed
    // that the content of the buffer will initiually be initialized to 0.
    //
...
bool acquire_and_init_ring_status_buffer(struct scap_device* dev)
{
    ...
    bool res =
	    __sync_bool_compare_and_swap(&(rbs->m_capturing_pid), 0, getpid());
...
int32_t udig_begin_capture(struct scap_engine_handle engine, char* error)
{
	...
	// be set by a previous crashed consumer. To understand that, we check
	// if there is an alive process with that pid. If not, we reset the
	// variable.
	//
	char fbuf[48];
	...
	else
	{
	    fclose(f);
	    snprintf(error, SCAP_LASTERR_SIZE,
		     "another udig capture is already active");
	    return SCAP_FAILURE;
	}
    }
...
// This is the buffer that will be consumed by scap.
///////////////////////////////////////////////////////////////////////////////
int32_t udig_alloc_ring(HANDLE* ring_handle, uint8_t** ring, uint32_t* ringsize,
			char* error)
{
    *ring_handle = NULL;
    ...
    // First, try to open an existing ring
    //
    HANDLE fh = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE,
	...
	//
	uint8_t* pdbuf =
		(uint8_t*)MapViewOfFile(fh, FILE_MAP_ALL_ACCESS, 0, 0, 0);

	MEMORY_BASIC_INFORMATION info;
	SIZE_T szBufferSize =
		VirtualQueryEx(GetCurrentProcess(), pdbuf, &info, sizeof(info));
	...
	*ringsize = UDIG_RING_SIZE;
	...
	fh = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
			       *ringsize, (TCHAR*)TEXT(UDIG_RING_SM_FNAME));

	if(fh == NULL)
	{
	    _snprintf(error, SCAP_LASTERR_SIZE,
		      "udig_alloc_ring CreateFileMapping error: %u\n",
		      GetLastError());
	    return SCAP_FAILURE;
	}
    }
    ...
    *ring = (uint8_t*)MapViewOfFileEx(fh, FILE_MAP_ALL_ACCESS, 0, 0, *ringsize,
				      NULL);
    if(*ring == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
		 "can't map first copy of buffer error: %u\n", GetLastError());
	CloseHandle(fh);
	return SCAP_FAILURE;
    }
    ...
    uint8_t* ring2 = (uint8_t*)MapViewOfFileEx(fh, FILE_MAP_ALL_ACCESS, 0, 0,
					       *ringsize, buf2);
    if(ring2 != buf2)
    {
	snprintf(error, SCAP_LASTERR_SIZE,
...
int32_t udig_alloc_ring_descriptors(
	HANDLE* ring_descs_fd, struct ppm_ring_buffer_info** ring_info,
	struct udig_ring_buffer_status** ring_status, char* error)
{
    *ring_descs_fd = NULL;
    ...
    //
    HANDLE fh = OpenFileMapping(FILE_MAP_ALL_ACCESS, TRUE,
	...
	{
	    snprintf(
		    error, SCAP_LASTERR_SIZE,
		    "udig_alloc_ring_descriptors CreateFileMapping error: %u\n",
		    GetLastError());
	    return SCAP_FAILURE;
	}
    }
    ...
    // Map the memory
    //
    uint8_t* descs = (uint8_t*)MapViewOfFile(fh, FILE_MAP_ALL_ACCESS, 0, 0, 0);
    if(descs == NULL)
    {
	snprintf(error, SCAP_LASTERR_SIZE, "can't map descriptors\n");
	CloseHandle(fh);
    ...
    *ring_status =
    ...
    // guaranteed that the content of the buffer will initiually be initialized
    // to 0.
    //
    if(InterlockedCompareExchange(
	       (volatile LONG*)&((*ring_status)->m_initialized), 1, 0) == 0)
    {
	(*ring_status)->m_buffer_lock = 0;
	(*ring_status)->m_capturing_pid = 0;
	(*ring_status)->m_stopped = 0;
...
void udig_free_ring_descriptors(uint8_t* addr) { UnmapViewOfFile(addr); }
...
bool acquire_and_init_ring_status_buffer(struct scap_device* dev)
{
    struct udig_ring_buffer_status* rbs = dev->m_bufstatus;
#ifdef _WIN32
    LONG dval = InterlockedCompareExchange(&(rbs->m_capturing_pid),
...
{
    struct udig_ring_buffer_status* rbs =
	    engine.m_handle->m_dev_set.m_devs[0].m_bufstatus;
...
{
    struct udig_consumer_t* consumer =
	    &(engine.m_handle->m_dev_set.m_devs[0].m_bufstatus->m_consumer);
    consumer->dropping_mode = 0;
...
int32_t udig_start_dropping_mode(struct scap_engine_handle engine,
				 uint32_t sampling_ratio)
{
    ...
    if(sampling_ratio != 1 && sampling_ratio != 2 && sampling_ratio != 4 &&
...
static bool match(scap_open_args* open_args) { return open_args->udig; }
...
static int32_t init(scap_t* main_handle, scap_open_args* open_args)
{
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Can not set close-on-exec flag for fd for udig device (%s)",
