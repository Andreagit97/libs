...
static inline void scap_bpf_advance_to_next_evt(scap_device *dev,
						scap_evt *event)
{
    scap_bpf_advance_to_evt(dev, true, dev->m_sn_next_event,
			    &dev->m_sn_next_event, &dev->m_sn_len);
}
...
static bool match(scap_open_args *open_args)
{
    ...
    return !open_args->udig &&
	   resolve_bpf_probe(open_args->bpf_probe, bpf_probe_buf);
}
...
static int bpf_map_create(enum bpf_map_type map_type, int key_size,
...
static int bpf_load_program(const struct bpf_insn *insns,
			    enum bpf_prog_type type, size_t insns_cnt,
...
static int32_t get_elf_section(Elf *elf, int i, GElf_Ehdr *ehdr, char **shname,
...
static int32_t load_elf_maps_section(struct bpf_engine *handle,
				     struct bpf_map_data *maps, int maps_shndx,
				     ...
				     int *nr_maps)
{
	...
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "Failed to get Elf_Data from maps section %d", maps_shndx);
	return SCAP_FAILURE;
    }
...
static int32_t load_maps(struct bpf_engine *handle, struct bpf_map_data *maps,
			 int nr_maps)
{
    int j;
    ...
    for(j = 0; j < nr_maps; ++j)
    {
	if(j == SCAP_PERF_MAP || j == SCAP_LOCAL_STATE_MAP ||
	   j == SCAP_FRAME_SCRATCH_MAP || j == SCAP_TMP_SCRATCH_MAP)
	{
	    maps[j].def.max_entries = handle->m_ncpus;
	}

	handle->m_bpf_map_fds[j] = bpf_map_create(
		maps[j].def.type, maps[j].def.key_size, maps[j].def.value_size,
		maps[j].def.max_entries, maps[j].def.map_flags);
	...
	if(handle->m_bpf_map_fds[j] < 0)
	{
	    ...
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "can't create map: %s", scap_strerror_r(buf, errno));
	    return SCAP_FAILURE;
	}
...
static int32_t parse_relocations(struct bpf_engine *handle, Elf_Data *data,
				 Elf_Data *symbols, GElf_Shdr *shdr,
				 ...
				 struct bpf_map_data *maps, int nr_maps)
{
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "invalid relocation for insn[%d].code 0x%x", insn_idx,
		     insn[insn_idx].code);
	    return SCAP_FAILURE;
	}
...
static int32_t load_tracepoint(struct bpf_engine *handle, const char *event,
			       struct bpf_insn *prog, int size)
{
	...
	fprintf(stderr,
		...
		error);
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "libscap: bpf_load_program() err=%d event=%s", errno, event);
	free(error);
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "libscap: too many programs recorded: %d (limit is %d)",
		 handle->m_bpf_prog_cnt + 1, BPF_PROGS_MAX);
	return SCAP_FAILURE;
    }
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "invalid filler name: %s", event);
	    return SCAP_FAILURE;
	}
	else if(prog_id >= BPF_PROGS_MAX)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "program ID exceeds BPF_PROG_MAX limit (%d/%d)", prog_id,
		     BPF_PROGS_MAX);
	    return SCAP_FAILURE;
	}
	...
	err = bpf_map_update_elem(
		handle->m_bpf_map_fds[handle->m_bpf_prog_array_map_idx],
		&prog_id, &fd, BPF_ANY);
	if(err < 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "failure populating program array");
	    return SCAP_FAILURE;
	}
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "BPF_RAW_TRACEPOINT_OPEN: event %s: %s", event,
	...
	if(ioctl(efd, PERF_EVENT_IOC_SET_BPF, fd))
	{
	    close(efd);
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "PERF_EVENT_IOC_SET_BPF: %s", scap_strerror_r(buf, errno));
	    return SCAP_FAILURE;
	}
    }
...
static int32_t load_bpf_file(struct bpf_engine *handle, const char *path,
			     ...
			     uint64_t *schema_version_p)
{
    ...
    {
	if(get_elf_section(elf, j, &ehdr, &shname, &shdr, &data) !=
	   SCAP_SUCCESS)
	{
	    continue;
	}
	...
	else if(strcmp(shname, "kernel_version") == 0)
	{
	    ...
	    {
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "BPF probe is compiled for %s, but running version is "
			 "%s",
	...
	else if(strcmp(shname, "license") == 0)
	{
	    license = data->d_buf;
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "BPF probe license is %s", license);
	}
    }
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "missing schema_version section");
	goto cleanup;
    }
    ...
    if(maps_shndx)
    {
	if(load_elf_maps_section(handle, maps, maps_shndx, elf, symbols,
				 strtabidx, &nr_maps) != SCAP_SUCCESS)
	{
	    goto cleanup;
	}
	...
	if(shdr.sh_type == SHT_REL)
	{
	    ...
	    if(get_elf_section(elf, shdr.sh_info, &ehdr, &shname_prog,
			       &shdr_prog, &data_prog) != SCAP_SUCCESS)
	    {
		continue;
	    }
	    ...
	    if(parse_relocations(handle, data, symbols, &shdr, insns, maps,
				 nr_maps))
	    {
		continue;
	    }
	...
	if(memcmp(shname, "tracepoint/", sizeof("tracepoint/") - 1) == 0 ||
	   memcmp(shname, "raw_tracepoint/", sizeof("raw_tracepoint/") - 1) ==
		   0)
	{
	    if(load_tracepoint(handle, shname, data->d_buf, data->d_size) !=
	       SCAP_SUCCESS)
	    {
		goto cleanup;
	    }
...
static void *perf_event_mmap(struct bpf_engine *handle, int fd)
{
    ...
    void *tmp = mmap(NULL, total_size, PROT_READ | PROT_WRITE,
		     MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if(tmp == MAP_FAILED)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "mmap (1): %s",
		 scap_strerror_r(buf, errno));
	return MAP_FAILED;
    }

    // Map the second copy to allow us to handle the wrap case normally
    void *p1 = mmap(tmp + ring_size, ring_size + header_size,
		    PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd, 0);
    if(p1 == MAP_FAILED)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "mmap (2): %s",
		 scap_strerror_r(buf, errno));
    ...
    // Map the main copy
    void *p2 = mmap(tmp, ring_size + header_size, PROT_READ | PROT_WRITE,
		    MAP_SHARED | MAP_FIXED, fd, 0);
    if(p2 == MAP_FAILED)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "mmap (3): %s",
		 scap_strerror_r(buf, errno));
	munmap(tmp, total_size);
	return MAP_FAILED;
    }
...
static int32_t populate_syscall_routing_table_map(struct bpf_engine *handle)
{
	...
	if(bpf_map_update_elem(
		   handle->m_bpf_map_fds[SCAP_SYSCALL_CODE_ROUTING_TABLE], &j,
		   &code, BPF_ANY) != 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "SCAP_SYSCALL_CODE_ROUTING_TABLE bpf_map_update_elem < 0");
	    return SCAP_FAILURE;
	}
    }

    return bpf_map_freeze(
	    handle->m_bpf_map_fds[SCAP_SYSCALL_CODE_ROUTING_TABLE]);
}

static int32_t populate_syscall_table_map(struct bpf_engine *handle)
{
    static const struct syscall_evt_pair uninterested_pair = {
	    .flags = UF_UNINTERESTING};
    int j;
	...
	if(bpf_map_update_elem(handle->m_bpf_map_fds[SCAP_SYSCALL_TABLE], &j, p,
			       BPF_ANY) != 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "SCAP_SYSCALL_TABLE bpf_map_update_elem < 0");
	    return SCAP_FAILURE;
	}
    }
...
static int32_t populate_event_table_map(struct bpf_engine *handle)
{
	...
	if(bpf_map_update_elem(handle->m_bpf_map_fds[SCAP_EVENT_INFO_TABLE], &j,
			       e, BPF_ANY) != 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "SCAP_EVENT_INFO_TABLE bpf_map_update_elem < 0");
	    return SCAP_FAILURE;
	}
    }
...
static int32_t populate_fillers_table_map(struct bpf_engine *handle)
{
	...
	if(bpf_map_update_elem(handle->m_bpf_map_fds[SCAP_FILLERS_TABLE], &j, e,
			       BPF_ANY) != 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "SCAP_FILLERS_TABLE bpf_map_update_elem < 0");
	    return SCAP_FAILURE;
	}
    }
...
int32_t scap_bpf_start_capture(struct scap_engine_handle engine)
{
    ...
    if(bpf_map_lookup_elem(handle->m_bpf_map_fds[SCAP_SETTINGS_MAP], &k,
			   &settings) != 0)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "SCAP_SETTINGS_MAP bpf_map_lookup_elem < 0");
	return SCAP_FAILURE;
    }

    settings.capture_enabled = true;
    if(bpf_map_update_elem(handle->m_bpf_map_fds[SCAP_SETTINGS_MAP], &k,
			   &settings, BPF_ANY) != 0)
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "SCAP_SETTINGS_MAP bpf_map_update_elem < 0");
	return SCAP_FAILURE;
    }
...
int32_t scap_bpf_close(struct scap_engine_handle engine)
{
    ...
    for(j = 0;
	j < sizeof(handle->m_bpf_event_fd) / sizeof(handle->m_bpf_event_fd[0]);
	++j)
    {
    ...
    for(j = 0;
	j < sizeof(handle->m_bpf_prog_fds) / sizeof(handle->m_bpf_prog_fds[0]);
	++j)
    {
    ...
    for(j = 0;
	j < sizeof(handle->m_bpf_map_fds) / sizeof(handle->m_bpf_map_fds[0]);
	++j)
    {
	if(handle->m_bpf_map_fds[j] > 0)
	{
...
int32_t scap_bpf_load(struct bpf_engine *handle, const char *bpf_probe,
		      uint64_t *api_version_p, uint64_t *schema_version_p)
{
    int online_cpu;
    int j;
    char buf[SCAP_LASTERR_SIZE];
    ...
    if(load_bpf_file(handle, bpf_probe, api_version_p, schema_version_p) !=
       SCAP_SUCCESS)
    {
	return SCAP_FAILURE;
    }
	...
	if(j > 0)
	{
	    ...
	    snprintf(filename, sizeof(filename),
		     "/sys/devices/system/cpu/cpu%d/online", j);
	...
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "processors online: %d, expected: %d", online_cpu,
		     handle->m_dev_set.m_ndevs);
	    return SCAP_FAILURE;
	}
	...
	if(pmu_fd < 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "pmu_fd < 0: %s",
		     scap_strerror_r(buf, errno));
	    return SCAP_FAILURE;
	}
	...
	if(bpf_map_update_elem(handle->m_bpf_map_fds[SCAP_PERF_MAP], &j,
			       &pmu_fd, BPF_ANY) != 0)
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		     "SCAP_PERF_MAP bpf_map_update_elem < 0: %s",
		     scap_strerror_r(buf, errno));
	    return SCAP_FAILURE;
	}
    ...
    {
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "processors online: %d, expected: %d", j,
		 handle->m_dev_set.m_ndevs);
	return SCAP_FAILURE;
    }
...
int32_t scap_bpf_get_stats(struct scap_engine_handle engine,
			   OUT scap_stats *stats)
{
	...
	if(bpf_map_lookup_elem(handle->m_bpf_map_fds[SCAP_LOCAL_STATE_MAP], &j,
			       &v))
	{
	    snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
...
int32_t scap_bpf_get_n_tracepoint_hit(struct scap_engine_handle engine,
				      long *ret)
{
    struct bpf_engine *handle = engine.m_handle;
    int j;
...
int32_t scap_bpf_handle_event_mask(struct scap_engine_handle engine,
				   uint32_t op, uint32_t event_id)
{
    int j;
    bool quit = false;
    struct bpf_engine *handle = engine.m_handle;
    ...
    {
	/*
	 * In case PPM_IOCTL_MASK_ZERO_EVENTS is called, event_id will be 0. Set
	 * every syscall to false in that case. Otherwise, check {enter,exit}
	 * event for each syscall to see if it matches the requested event_id.
	 */
	if(event_id == 0 || g_syscall_table[j].enter_event_type == event_id ||
	   g_syscall_table[j].exit_event_type == event_id)
	{
	    switch(op)
	    {
	    ...
	    default:
		snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE,
			 "%s(%d) internal error", __FUNCTION__, op);
		ASSERT(false);
		return SCAP_FAILURE;
		break;
	    }
...
static int32_t unsupported_config(struct scap_engine_handle engine,
				  const char *msg)
{
    struct bpf_engine *handle = engine.m_handle;
			 ...
			 unsigned long arg2)
{
	...
	if(arg1 == 0)
	{
	    return unsupported_config(
		    engine, "Tracers cannot be disabled once enabled");
	}
	...
	{
	    return unsupported_config(
		    engine, "Page faults cannot be disabled once enabled");
	}
	...
	if(arg1 == 0)
	{
	    return unsupported_config(
		    engine,
		    "Simpledriver mode cannot be disabled once enabled");
	}
    ...
    {
	char msg[256];
	snprintf(msg, sizeof(msg), "Unsupported setting %d (args %lu, %lu)",
		 setting, arg1, arg2);
...
static int32_t init(scap_t *handle, scap_open_args *open_args)
{
    ...
    //
    // Find out how many devices we have to open, which equals to the number of
    // CPUs
    //
	...
	snprintf(engine.m_handle->m_lasterr, SCAP_LASTERR_SIZE,
		 "_SC_NPROCESSORS_ONLN: %s", scap_strerror_r(buf, errno));
	return SCAP_FAILURE;
    }

    engine.m_handle->m_ncpus = num_cpus;

    fill_syscalls_of_interest(&open_args->ppm_sc_of_interest,
			      &engine.m_handle->m_syscalls_of_interest);

    rc = devset_init(&engine.m_handle->m_dev_set, num_cpus,
		     engine.m_handle->m_lasterr);
    if(rc != SCAP_SUCCESS)
    {
	return rc;
    }

    rc = scap_bpf_load(engine.m_handle, bpf_probe, &handle->m_api_version,
		       &handle->m_schema_version);
    if(rc != SCAP_SUCCESS)
    {
	return rc;
    }
...
static int32_t init(scap_t *handle, scap_open_args *open_args)
{
    strlcpy(handle->m_lasterr,
	    "The eBPF probe driver is not supported when using a minimal build",
	    SCAP_LASTERR_SIZE);
