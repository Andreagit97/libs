...
extern "C"
{
    ...
    static SCAP_HANDLE_T *gvisor_alloc_handle(scap_t *main_handle,
					      char *lasterr_ptr)
    {
	...
	return gv->init(open_args->gvisor_config_path,
			open_args->gvisor_root_path);
    }
    ...
    static int32_t gvisor_next(struct scap_engine_handle engine,
			       scap_evt **pevent, uint16_t *pcpuid)
    {
    ...
    static int32_t gvisor_configure(struct scap_engine_handle engine,
    ...
    static int32_t gvisor_get_stats(struct scap_engine_handle engine,
				    scap_stats *stats)
    {
	return engine.m_handle->get_stats(stats);
    }

    static int32_t gvisor_get_n_tracepoint_hit(struct scap_engine_handle engine,
					       long *ret)
    {
    ...
    static int32_t gvisor_get_threadlist(struct scap_engine_handle engine,
    ...
    static int32_t gvisor_get_threadinfos(struct scap_engine_handle engine,
					  uint64_t *n,
    ...
    static int32_t gvisor_get_fdinfos(struct scap_engine_handle engine,
    ...
    static int32_t gvisor_get_vxid(struct scap_engine_handle engine,
				   uint64_t xid, int64_t *vxid)
    {
	*vxid = engine.m_handle->get_vxid(xid);
	return SCAP_SUCCESS;
    }

    static int32_t gvisor_getpid_global(struct scap_engine_handle engine,
