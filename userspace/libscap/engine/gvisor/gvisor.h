...
/*!
    ...
    \param gvisor_buf the source buffer that contains the raw event coming from
   gVisor \param scap_buf the buffer that will be used to store the encoded scap
   events \return a parse_result struct. If the encoding is successful:
	    - the status field will be set as SCAP_SUCCESS
	    - the scap_events vector will contain pointers to each encoded
   event, all located within scap_buf's memory
	...
	If the buffer is too small to contain all encoded events:
	    ...
	    - the size field will be set to the total required size to fully
   translate the supplied gVisor event In case of any error:
	    - the status field will be set to SCAP_FAILURE for parsing erros,
   SCAP_NOT_SUPPORTED for unsupported events
...
result trace_create(const std::string &root_path,
		    ...
		    const std::string &sandbox_id, bool force);
result trace_delete(const std::string &root_path,
		    const std::string &session_name,
		    const std::string &sandbox_id);
result trace_procfs(const std::string &root_path,
		    const std::string &sandbox_id);

} // namespace runsc

// contains entries to store per-sandbox data and buffers to use to write events
// in
class sandbox_entry
{
    public:
...
class engine
{
    ...
    uint32_t get_fdinfos(const scap_threadinfo *tinfo, uint64_t *n,
    ...
    // stores per-sandbox data. All buffers used to contain parsed event data
    // are owned by this map
    std::unordered_map<int, sandbox_entry> m_sandbox_data;

    // the following two maps store and manage memory for thread information
    // requested when get_threadinfos() is called. They are only updated upon
    // get_threadinfos()
    std::vector<scap_threadinfo> m_threadinfos_threads;
    std::unordered_map<uint64_t, std::vector<scap_fdinfo>> m_threadinfos_fds;

    // the following two strings contains the path of the root dir used by the
    // runsc command and the path the trace session configuration file used to
    // set up traces, respectively
