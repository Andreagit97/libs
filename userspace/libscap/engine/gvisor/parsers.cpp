...
{
    std::string container_id_64 = container_id_hex.length() > 16
					  ? container_id_hex.substr(0, 15)
					  : container_id_hex;
...
uint64_t get_vxid(uint64_t xid) { return xid & 0xffffffff; }

template<class T> static void fill_context_data(scap_evt *evt, T &gvisor_evt)
{
    auto &context_data = gvisor_evt.context_data();
    evt->ts = context_data.time_ns();
    evt->tid = generate_tid_field(context_data.thread_id(),
				  context_data.container_id());
}

static parse_result parse_container_start(const char *proto, size_t proto_size,
					  scap_sized_buffer scap_buf)
{
    ...
    ret.status = scap_event_encode_params(
	    event_buf, &event_size, scap_err, PPME_SYSCALL_CLONE_20_X, 20,
    ...
    ret.status = scap_event_encode_params(
	    event_buf, &event_size, scap_err, PPME_SYSCALL_EXECVE_19_E, 1,
    ...
    ret.status = scap_event_encode_params(
	    event_buf, &event_size, scap_err, PPME_SYSCALL_EXECVE_19_X, 20,
	    0,				// res
	    gvisor_evt.args(0).c_str(), // actual exe missing
	    scap_const_sized_buffer{args.data(), args.size()},
	    tid_field,	// tid
	    tgid_field, // pid
	    -1, // ptid is only needed if we don't have the corresponding clone
		// event
	    cwd.c_str(),		// cwd
	    75000,			// fdlimit ?
	    0,				// pgft_maj
...
static parse_result parse_execve(const char *proto, size_t proto_size,
				 scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    ret.status = SCAP_SUCCESS;
	...
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_EXECVE_19_X, 20,
		...
		-1, // ptid is only needed if we don't have the corresponding
		    // clone event
		cwd.c_str(),  // cwd
		75000,	      // fdlimit
		0,	      // pgft_maj
    ...
    {
	ret.status = scap_event_encode_params(scap_buf, &ret.size, scap_err,
...
static parse_result parse_clone(const gvisor::syscall::Syscall &gvisor_evt,
				scap_sized_buffer scap_buf, bool is_fork)
{
    parse_result ret = {0};
    ret.status = SCAP_SUCCESS;
    ret.size = 0;
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_CLONE_20_X, 20,
		gvisor_evt.exit().result(),	// res
		"",				// exe
		...
		is_fork ? PPM_CL_CLONE_CHILD_CLEARTID |
				  PPM_CL_CLONE_CHILD_SETTID
			: clone_flags_to_scap(gvisor_evt.arg1()),
		0, 0, gvisor_evt.context_data().thread_id(),
...
static parse_result parse_sentry_clone(const char *proto, size_t proto_size,
				       scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    ret.status = SCAP_SUCCESS;
    ...
    uint64_t tid_field = generate_tid_field(gvisor_evt.created_thread_id(),
					    context_data.container_id());
    ...
    ret.status = scap_event_encode_params(
	    ...
	    16, 0, 0, 0, 0, 0,
	    context_data.process_name().c_str(), // comm
	    scap_const_sized_buffer{cgroups.c_str(), cgroups.size() + 1}, 0, 0,
	    0, gvisor_evt.created_thread_id(),
...
static parse_result parse_read(const char *proto, size_t proto_size,
			       scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_READ_E, 2,
		gvisor_evt.fd(), gvisor_evt.count());
    }
    else
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_READ_X, 2,
		gvisor_evt.exit().result(), scap_const_sized_buffer{NULL, 0});
    }
...
// Converts a single address into a socktuple with a zeroed out local part and a
// remote counterpart Providing a large enough buffer is responsibility of the
// caller (socktuple_buffer_size is set for this reason)
static size_t pack_sockaddr_to_remote_tuple(sockaddr *sa, char *targetbuf)
{
    ...
    {
	size += pack_sock_family(sa, buf);
	memset(targetbuf + 1, 0,
	       sizeof(uint64_t)); // TODO: understand how to fill this
...
static parse_result parse_connect(const char *proto, size_t proto_size,
				  scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
	...
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SOCKET_CONNECT_X, 2,
...
static parse_result parse_socket(const char *proto, size_t proto_size,
				 scap_sized_buffer event_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    if(gvisor_evt.has_exit())
    {
	ret.status = scap_event_encode_params(event_buf, &ret.size, scap_err,
    ...
    {
	ret.status = scap_event_encode_params(
		event_buf, &ret.size, scap_err, PPME_SOCKET_SOCKET_E, 3,
		socket_family_to_scap(gvisor_evt.domain()), gvisor_evt.type(),
		gvisor_evt.protocol());
    }
...
static parse_result parse_generic_syscall(const char *proto, size_t proto_size,
					  scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    ...
    default:
	ret.error = std::string("Unhandled syscall: ") +
		    std::to_string(gvisor_evt.sysno());
	ret.status = SCAP_FAILURE;
	return ret;
    }
...
static parse_result parse_accept(const char *proto, size_t proto_size,
				 scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
	...
	type = gvisor_evt.sysno() == SYS_accept4 ? PPME_SOCKET_ACCEPT4_5_X
						 : PPME_SOCKET_ACCEPT_5_X;

	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, type, 5, gvisor_evt.fd(),
		scap_const_sized_buffer{targetbuf, size}, 0, 0,
		0); // queue information missing
    }
    else
    {
	type = gvisor_evt.sysno() == SYS_accept4 ? PPME_SOCKET_ACCEPT4_5_E
...
static parse_result parse_fcntl(const char *proto, size_t proto_size,
				scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_FCNTL_E, 2,
		gvisor_evt.fd(), fcntl_cmd_to_scap(gvisor_evt.cmd()));
    }
...
static parse_result parse_bind(const char *proto, size_t proto_size,
			       scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    char targetbuf[socktuple_buffer_size]; // XXX maybe a smaller version for
					   // addr
	...
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SOCKET_BIND_X, 2,
		gvisor_evt.exit().result(),
		scap_const_sized_buffer{targetbuf, size});
    }
    else
...
static parse_result parse_pipe(const char *proto, size_t proto_size,
			       scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_PIPE_X, 4,
		gvisor_evt.exit().result(), gvisor_evt.reader(),
		gvisor_evt.writer(),
		0); // missing "ino"
    }
    else
...
static parse_result parse_open(const char *proto, size_t proto_size,
			       scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
	...
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_OPEN_X, 5,
		gvisor_evt.exit().result(), gvisor_evt.pathname().c_str(),
...
static parse_result parse_chdir(const char *proto, size_t proto_size,
				scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_CHDIR_X, 2,
		gvisor_evt.exit().result(), gvisor_evt.pathname().c_str());
    }
    else
    {
...
static parse_result parse_setresid(const char *proto, size_t proto_size,
				   scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    if(gvisor_evt.has_exit())
    {
	type = gvisor_evt.sysno() == SYS_setresuid ? PPME_SYSCALL_SETRESUID_X
						   : PPME_SYSCALL_SETRESGID_X;

	ret.status =
		scap_event_encode_params(scap_buf, &ret.size, scap_err, type, 1,
					 gvisor_evt.exit().result());
    }
    else
    {
	type = gvisor_evt.sysno() == SYS_setresuid ? PPME_SYSCALL_SETRESUID_E
						   : PPME_SYSCALL_SETRESGID_E;

	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, type, 3, gvisor_evt.rgid(),
		gvisor_evt.egid(), gvisor_evt.sgid());
    }
...
static parse_result parse_setid(const char *proto, size_t proto_size,
				scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    if(gvisor_evt.has_exit())
    {
	type = gvisor_evt.sysno() == SYS_setuid ? PPME_SYSCALL_SETUID_X
    ...
    {
	type = gvisor_evt.sysno() == SYS_setuid ? PPME_SYSCALL_SETUID_E
						: PPME_SYSCALL_SETGID_E;

	ret.status = scap_event_encode_params(scap_buf, &ret.size, scap_err,
					      type, 1, gvisor_evt.id());
    }
...
static parse_result parse_chroot(const char *proto, size_t proto_size,
				 scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_CHROOT_X, 2,
		gvisor_evt.exit().result(), gvisor_evt.pathname().c_str());
    }
    else
    {
...
static parse_result parse_dup(const char *proto, size_t proto_size,
			      scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
	...
	case SYS_dup:
	    ret.status = scap_event_encode_params(
		    scap_buf, &ret.size, scap_err, PPME_SYSCALL_DUP_1_X, 2,
		    gvisor_evt.exit().result(), gvisor_evt.old_fd());
	    break;
	case SYS_dup2:
	    ret.status = scap_event_encode_params(
		    scap_buf, &ret.size, scap_err, PPME_SYSCALL_DUP2_X, 3,
		    gvisor_evt.exit().result(), gvisor_evt.old_fd(),
		    gvisor_evt.new_fd());
	    break;
	case SYS_dup3:
	    ret.status = scap_event_encode_params(
		    scap_buf, &ret.size, scap_err, PPME_SYSCALL_DUP3_X, 4,
		    gvisor_evt.exit().result(), gvisor_evt.old_fd(),
		    gvisor_evt.new_fd(),
...
static parse_result parse_task_exit(const char *proto, size_t proto_size,
				    scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
...
static parse_result parse_prlimit64(const char *proto, size_t proto_size,
				    scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_PRLIMIT_X, 5,
		gvisor_evt.exit().result(), gvisor_evt.new_limit().cur(),
		gvisor_evt.new_limit().max(), gvisor_evt.old_limit().cur(),
		gvisor_evt.old_limit().max());
    }
    else
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_PRLIMIT_E, 2,
		gvisor_evt.pid(),
...
static parse_result parse_signalfd(const char *proto, size_t proto_size,
				   scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_SIGNALFD_E, 3,
		gvisor_evt.fd(), gvisor_evt.sigset(), gvisor_evt.flags());
    }
...
parse_result parse_eventfd(const char *proto, size_t proto_size,
			   scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    char scap_err[SCAP_LASTERR_SIZE];
    ...
    {
	ret.status = scap_event_encode_params(
		scap_buf, &ret.size, scap_err, PPME_SYSCALL_EVENTFD_E, 2,
		gvisor_evt.val(),
...
parse_result parse_gvisor_proto(scap_const_sized_buffer gvisor_buf,
				scap_sized_buffer scap_buf)
{
    parse_result ret = {0};
    ...
    {
	ret.error = std::string("Header size (") +
		    std::to_string(hdr->header_size) +
		    ") is larger than message " +
		    std::to_string(gvisor_buf.size);
	ret.status = SCAP_FAILURE;
	return ret;
    }
    ...
    {
	ret.error = std::string("Invalid message type " +
...
procfs_result parse_procfs_json(const std::string &input,
				const std::string &sandbox)
{
    ...
    bool json_parse = reader->parse(input.c_str(), input.c_str() + input.size(),
				    &root, &err);
    if(!json_parse)
    {
    ...
    size_t args_size =
	    args.size() > SCAP_MAX_ARGS_SIZE ? SCAP_MAX_ARGS_SIZE : args.size();
    ...
    size_t env_size =
	    env.size() > SCAP_MAX_ENV_SIZE ? SCAP_MAX_ENV_SIZE : env.size();
	...
	if(S_ISREG(mode))
	{
	    ...
	    strlcpy(fdinfo.info.regularinfo.fname, entry["path"].asCString(),
		    SCAP_MAX_PATH_SIZE);
	}
	else
	{
...
config_result parse_config(std::string config)
{
    ...
    bool json_parse = reader->parse(
	    config.c_str(), config.c_str() + config.size(), &root, &err);
    if(!json_parse)
    {
    ...
    {
	res.error =
		"Could not find trace_session -> sinks array in configuration";
	return res;
    }
    ...
    // We don't know how to distinguish between sinks in case there is more than
    // one we're taking the first for now but this can be tweaked if necessary.
