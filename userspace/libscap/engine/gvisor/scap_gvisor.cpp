...
int32_t engine::init(std::string config_path, std::string root_path)
{
    ...
    if(config_file.fail())
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE,
    ...
    parsers::config_result config_result =
	    parsers::parse_config(config_buf.str());
    ...
    if(sock == -1)
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE, "Cannot create unix socket: %s",
		 strerror(errno));
	return SCAP_FAILURE;
    }
    sockaddr_un address;
    ...
    if(ret == -1)
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE, "Cannot bind unix socket: %s",
		 strerror(errno));
	umask(old_umask);
	return SCAP_FAILURE;
    }
	...
	umask(old_umask);
	snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		 "Cannot listen on gvisor unix socket: %s", strerror(errno));
	return SCAP_FAILURE;
    }
    ...
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		 "Cannot create epollfd socket: %s", strerror(errno));
	return SCAP_FAILURE;
    }
...
void engine::free_sandbox_buffers() { m_sandbox_data.clear(); }
...
int32_t engine::start_capture()
{
    ...
    {
	strlcpy(m_lasterr, "Error listing running sandboxes",
		SCAP_LASTERR_SIZE);
	return SCAP_FAILURE;
    }
    std::vector<std::string> &existing_sandboxes =
	    exisiting_sandboxes_res.output;
	...
	runsc::result trace_create_res = runsc::trace_create(
		m_root_path, m_trace_session_path, sandbox, true);
	if(trace_create_res.error)
	{
	    snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		     "Cannot create session for sandbox %s", sandbox.c_str());
	    return SCAP_FAILURE;
	}
    }
    ...
    new_sandboxes.erase(
	    remove_if(new_sandboxes.begin(), new_sandboxes.end(),
		      [&existing_sandboxes](const std::string &s) -> bool
		      {
			  auto res = find(existing_sandboxes.begin(),
					  existing_sandboxes.end(), s);
    ...
    {
	runsc::result trace_create_res = runsc::trace_create(
		m_root_path, m_trace_session_path, sandbox, false);
	if(trace_create_res.error)
	{
	    snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		     "Cannot create session for sandbox %s", sandbox.c_str());
...
int32_t engine::stop_capture()
{
	...
	runsc::result trace_delete_res =
		runsc::trace_delete(m_root_path, "Default", sandbox);
	if(trace_delete_res.error)
	{
	    snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		     "Cannot delete session for sandbox %s", sandbox.c_str());
	    return SCAP_FAILURE;
	}
    }
...
uint32_t engine::get_threadinfos(uint64_t *n, const scap_threadinfo **tinfos)
{
	...
	for(const auto &line : procfs_res.output)
	{
	    ...
	    parsers::procfs_result res =
		    parsers::parse_procfs_json(line, sandbox);
...
uint32_t engine::get_fdinfos(const scap_threadinfo *tinfo, uint64_t *n,
...
uint32_t engine::get_vxid(uint64_t xid) { return parsers::get_vxid(xid); }
...
// Reads one gvisor message from the specified fd, stores the resulting events
// overwriting m_buffers and adds pointers to m_event_queue. Returns:
// * SCAP_SUCCESS in case of success
// * SCAP_FAILURE in case of a fatal error while reading from the fd or
// allocating memory (m_lasterr is filled)
// * SCAP_NOT_SUPPORTED if the message type is not currently supported
// * SCAP_ILLEGAL_INPUT in case of parsing errors (invalid message or parsing
// issue)
...
int32_t engine::process_message_from_fd(int fd)
{
    ...
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		 "Error reading from gvisor client: %s", strerror(errno));
	return SCAP_FAILURE;
    }
    else if(nbytes == 0)
    {
	...
	if(m_sandbox_data[fd].expand_buffer(initial_event_buffer_size) ==
	   SCAP_FAILURE)
	{
	    snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		     "could not initialize %zu bytes for gvisor sandbox on fd "
		     "%d",
		     initial_event_buffer_size, fd);
	    return SCAP_FAILURE;
	}
    }
    ...
    parsers::parse_result parse_result =
	    parsers::parse_gvisor_proto(gvisor_msg, m_sandbox_data[fd].m_buf);
    if(parse_result.status == SCAP_INPUT_TOO_SMALL)
    {
	if(m_sandbox_data[fd].expand_buffer(parse_result.size) == SCAP_FAILURE)
	{
	    snprintf(m_lasterr, SCAP_LASTERR_SIZE,
		     "Cannot realloc gvisor buffer to %zu", parse_result.size);
	    return SCAP_FAILURE;
	};
	parse_result = parsers::parse_gvisor_proto(gvisor_msg,
						   m_sandbox_data[fd].m_buf);
    }
    ...
    uint64_t delta = parse_result.dropped_count -
		     m_sandbox_data[fd].m_last_dropped_count;
...
int32_t engine::next(scap_evt **pevent, uint16_t *pcpuid)
{
	...
	*pevent = m_event_queue.front();
    ...
    if(nfds < 0)
    {
	snprintf(m_lasterr, SCAP_LASTERR_SIZE, "epoll_wait error: %s",
		 strerror(errno));
	if(errno == EINTR)
	{
	...
	if(evts[i].events & EPOLLERR)
	{
	    ...
	    {
		snprintf(m_lasterr, SCAP_LASTERR_SIZE, "epoll error: %s",
