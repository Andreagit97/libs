...
template<typename T> class socket_collector
{
    ...
    {
	for(typename socket_map_t::iterator it = m_sockets.begin();
	    it != m_sockets.end(); ++it)
	{
    ...
    int subscription_count() const { return m_sockets.size(); }
	...
	if(g_logger.get_severity() >= sinsp_logger::SEV_TRACE)
	{
		...
		if(it->second)
		{
		    ...
		    {
			g_logger.log("Socket collector: examining socket " +
			...
			{
			    g_logger.log(
				    "Socket collector: activity on socket " +
					    std::to_string(it->first) + " (" +
					    it->second->get_id() + ')',
				    sinsp_logger::SEV_TRACE);
			}
		    ...
		    {
			g_logger.log("Socket collector: socket " +
					     std::to_string(it->first) +
					     " handler (" +
					     it->second->get_id() +
					     ") is not enabled.",
	...
	try
	{
		...
		tv.tv_usec = m_loop ? (m_timeout_ms % 1000) * 1000 : 0;
		{
			...
			g_logger.log("Socket collector: total sockets=" +
					     std::to_string(m_sockets.size()) +
					     ", select-enabled sockets= " +
					     std::to_string(
						     signaled_sockets_count()),
				     sinsp_logger::SEV_TRACE);
			...
			if(res == 0) // all quiet
			{
			    g_logger.log(
				    "Socket collector: " +
					    std::to_string(m_sockets.size()) +
					    " sockets total, no activity.",
				    sinsp_logger::SEV_DEBUG);
			}
			else if(res < 0) // select error
			{
			    // socket sets are undefined after error, nothing to
			    // do here ...
			    throw sinsp_exception(
				    std::string(
					    "Socket collector: select error (")
			...
			{
			    trace_sockets();
			    for(typename socket_map_t::iterator it =
					m_sockets.begin();
				it != m_sockets.end();)
			    {
				...
				if(FD_ISSET(it->first, &m_infd))
				{
				    if(it->second && it->second->is_enabled() &&
				       (err = it->second->on_data()))
				    {
					if((err != EAGAIN) &&
					   (err != EINPROGRESS))
					{
					    if(err !=
					       it->second->CONNECTION_CLOSED)
					    {
						g_logger.log(
							"Socket collector: "
							"data handling error " +
				...
				{
				    if(it->second &&
				       (err = it->second->get_socket_error()))
				    {
					g_logger.log(
		    ...
		    {
			g_logger.log("Socket collector is empty.",
				     sinsp_logger::SEV_DEBUG);
			m_stopped = true;
			return;
		    }
	...
	{
	    g_logger.log(std::string("Socket collector error: ") + ex.what(),
			 sinsp_logger::SEV_ERROR);
	    remove_all();
	    m_stopped = true;
	}
    ...
    bool is_active() const { return subscription_count() > 0; }
    ...
    void set_steady_state(bool state = true) { m_steady_state = true; }

    bool get_steady_state() const { return m_steady_state; }
