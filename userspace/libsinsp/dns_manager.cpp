				 ...
				 std::future<void> f_exit)
{
	...
	if(!manager.m_cache.empty())
	{
		...
		{
		    // remove the entry if it's hasn't been used for a whole
		    // hour
		    to_delete.push_back(name);
		}
		else if(ts > (info.m_last_resolve_ts + info.m_timeout))
		{
		    sinsp_dns_manager::dns_info refreshed_info =
			    manager.resolve(name, ts);
		    refreshed_info.m_timeout = base_refresh_timeout;
		    refreshed_info.m_last_resolve_ts = info.m_last_resolve_ts =
			    ts;
...
#if defined(HAS_CAPTURE) && !defined(CYGWING_AGENT) && !defined(_WIN32)
inline sinsp_dns_manager::dns_info
sinsp_dns_manager::resolve(const std::string &name, uint64_t ts)
{
    dns_info dinfo;
	...
	for(rp = result; rp != NULL; rp = rp->ai_next)
	{
		...
		ipv6addr v6;
		memcpy(v6.m_b,
		       ((struct sockaddr_in6 *)rp->ai_addr)->sin6_addr.s6_addr,
		       sizeof(ipv6addr));
...
bool sinsp_dns_manager::match(const char *name, int af, void *addr, uint64_t ts)
{
    ...
    if(!m_resolver)
    {
	m_resolver = new thread(sinsp_dns_resolver::refresh, m_erase_timeout,
				m_base_refresh_timeout, m_max_refresh_timeout,
    ...
    {
	return dinfo.m_v4_addrs.find(*(uint32_t *)addr) !=
	       dinfo.m_v4_addrs.end();
    }
#endif
    return false;
}
...
string sinsp_dns_manager::name_of(int af, void *addr, uint64_t ts)
{
	...
	for(auto &it : m_cache)
	{
	    ...
	    else if(af == AF_INET && info.m_v4_addrs.find(*(uint32_t *)addr) !=
					     info.m_v4_addrs.end())
	    {
		info.m_last_used_ts = ts;
		ret = name;
