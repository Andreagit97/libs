...
std::vector<std::string>
	sinsp_container_info::container_health_probe::probe_type_names = {
...
sinsp_container_info::container_health_probe::container_health_probe(
	...
	m_probe_type(ptype),
	m_health_probe_exe(exe), m_health_probe_args(args)
{
}
	...
	std::list<container_health_probe> &probes)
{
	...
	if(!probe_obj.isNull() && probe_obj.isObject())
	{
	    ...
	    if(!probe_exe_obj.isNull() &&
	       probe_exe_obj.isConvertibleTo(Json::stringValue))
	    {
		...
		g_logger.format(sinsp_logger::SEV_DEBUG,
				...
				probe_type_names[i].c_str(), probe_exe.c_str(),
				probe_args.size());
...
const sinsp_container_info::container_mount_info *
sinsp_container_info::mount_by_idx(uint32_t idx) const
{
...
const sinsp_container_info::container_mount_info *
sinsp_container_info::mount_by_source(std::string &source) const
{
    // note: linear search
...
const sinsp_container_info::container_mount_info *
sinsp_container_info::mount_by_dest(std::string &dest) const
{
    // note: linear search
    for(auto &mntinfo : m_mounts)
    {
...
sinsp_container_info::get_tinfo(sinsp *inspector) const
{
...
sinsp_container_info::container_health_probe::probe_type
sinsp_container_info::match_health_probe(sinsp_threadinfo *tinfo) const
{
    g_logger.format(sinsp_logger::SEV_DEBUG,
	...
	g_logger.format(
		...
		m_id.c_str(), tinfo->m_exe.c_str(), tinfo->m_args.size(),
