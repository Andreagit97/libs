...
void sinsp_protodecoder::on_reset(sinsp_evt* evt) { ASSERT(false); }
...
sinsp_protodecoder*
sinsp_protodecoder_list::new_protodecoder_from_name(const string& name,
						    sinsp* inspector)
{
    uint32_t j;
...
///////////////////////////////////////////////////////////////////////////////
const char* syslog_severity_strings[] = {"emerg", "alert",  "crit", "err",
					 "warn",  "notice", "info", "debug"};

const char* syslog_facility_strings[] = {
	"kern",	  "user",     "mail",	  "daemon", "auth",	"syslog",
	"lpr",	  "news",     "uucp",	  "clock",  "authpriv", "ftp",
	"ntp",	  "logaudit", "logalert", "cron",   "local0",	"local1",
...
void sinsp_decoder_syslog::on_event(sinsp_evt* evt,
				    sinsp_pd_callback_type etype)
{
    if(etype == CT_OPEN || etype == CT_CONNECT)
    {
...
void sinsp_decoder_syslog::on_reset(sinsp_evt* evt) { m_priority = -1; }

bool sinsp_decoder_syslog::is_data_valid() { return (m_priority != -1); }

const char* sinsp_decoder_syslog::get_severity_str()
{
    if(m_severity >=
       sizeof(syslog_severity_strings) / sizeof(syslog_severity_strings[0]))
    {
	return "<NA>";
    }
    else
...
{
    if(m_facility >=
       sizeof(syslog_facility_strings) / sizeof(syslog_facility_strings[0]))
    {
	return "<NA>";
    }
    else
...
void sinsp_decoder_syslog::decode_message(char* data, uint32_t len,
					  char* pristr, uint32_t pristrlen)
{
    ...
    bool res =
	    sinsp_numparser::tryparsed32_fast(pristr, pristrlen, &m_priority);
