...
sinsp_ssl::sinsp_ssl(const std::string& cert, const std::string& key,
		     const std::string& key_passphrase,
		     const std::string& ca_cert, bool verify_peer,
		     const std::string& cert_type):
	m_cert_type(cert_type),
	m_cert(cert), m_key(key), m_key_passphrase(key_passphrase),
...
std::string sinsp_ssl::memorize_file(const std::string& disk_file)
{
	...
	else
	{
	    ...
	    os << "Error occurred while trying to determine the real path of "
		  "memory file ["
	       << fd_path << "]: " << strerror(errno) << " (disk file ["
	       << disk_file << "] will be used).";
    ...
    {
	std::ostringstream os;
	os << "Memory file creation error: " << strerror(errno)
	   << " (disk file [" << disk_file << "] will be used).";
	g_logger.log(os.str(), sinsp_logger::SEV_WARNING);
	return disk_file;
    }
    return mem_file;
...
sinsp_bearer_token::sinsp_bearer_token(const std::string& bearer_token_file,
				       bool curl_support):
	m_bearer_token(stringize_file(bearer_token_file)),
	m_bt_auth_header(nullptr)
{
    if(curl_support)
    {
	std::size_t len =
		m_bearer_token
			.length(); // curl does not tolerate newlines in headers
	while(len && (m_bearer_token[len - 1] == '\r' ||
		      m_bearer_token[len - 1] == '\n'))
	{
