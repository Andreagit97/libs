...
// Split path /var/log/messages into a list of components (var, log, messages).
...
template<class Value> class path_prefix_map
{
    public:
    path_prefix_map();
    ...
    void add_search_path_components(
    ...
    Value *
    match_components(const path_prefix_map_ut::filter_components_t &components);
    ...
    std::unordered_map<filter_value_t, std::pair<path_prefix_map *, Value *>,
		       g_hash_membuf, g_equal_to_membuf>
	    m_dirs;

    std::list<std::string> m_strvals;
};

template<class Value> path_prefix_map<Value>::path_prefix_map() {}
...
template<class Value>
void path_prefix_map<Value>::add_search_path(const filter_value_t &path,
					     Value &v)
{
...
template<class Value>
void path_prefix_map<Value>::add_search_path_components(
	const path_prefix_map_ut::filter_components_t &components, Value &v)
{
...
void path_prefix_map<Value>::add_search_path_components(
	const path_prefix_map_ut::filter_components_t &components,
	path_prefix_map_ut::filter_components_t::const_iterator comp, Value &v)
{
	...
	m_dirs[*cur] = std::pair<path_prefix_map *, Value *>(
		subtree, (comp == components.end() ? new Value(v) : NULL));
    }
    else
    {
	...
	if(comp == components.end())
	{
	    ...
	    m_dirs.erase(*cur);
	    m_dirs[*cur] =
		    std::pair<path_prefix_map *, Value *>(NULL, new Value(v));
	}
...
// NOTE: this does not copy, so it is only valid as long as path is valid.
template<class Value> Value *path_prefix_map<Value>::match(const char *path)
{
    filter_value_t mem((uint8_t *)path, (uint32_t)strlen(path));
    return match(mem);
}
...
template<class Value>
Value *path_prefix_map<Value>::match_components(
	const path_prefix_map_ut::filter_components_t &components)
{
