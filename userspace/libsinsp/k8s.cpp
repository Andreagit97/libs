...
k8s::k8s(const std::string& uri, bool is_captured,
#ifdef HAS_CAPTURE
	 ssl_ptr_t ssl, bt_ptr_t bt, bool block,
#endif // HAS_CAPTURE
	 filter_ptr_t event_filter, ext_list_ptr_t extensions, bool events_only
#ifdef HAS_CAPTURE
	 ,
	...
	m_net(uri.empty() ? nullptr
			  : new k8s_net(*this, m_state, uri, ssl, bt,
					event_filter, block, node_selector))
#endif
{
    g_logger.log(
	...
	m_components.insert({k8s_component::K8S_REPLICATIONCONTROLLERS,
...
void k8s::check_components()
{
	...
	for(auto it = m_components.cbegin(); it != m_components.cend();)
	{
	    ...
	    {
		k8s_net::handler_ptr_t handler =
			k8s_net::get_handler(m_net->handlers(), *it);
		if(handler)
		{
...
const k8s_state_t& k8s::get_state() { return m_state; }
...
void k8s::simulate_watch_event(const std::string& json, int version)
{
	...
	if(!kind.isNull() && kind.isString())
	{
	    ...
	    if(type == "Namespace")
	    {
		component_type = k8s_component::K8S_NAMESPACES;
	    }
	    else if(type == "Node")
	    {
		component_type = k8s_component::K8S_NODES;
	    }
	    else if(type == "Pod")
	    {
		component_type = k8s_component::K8S_PODS;
	    }
	    else if(type == "ReplicationController")
	    {
		component_type = k8s_component::K8S_REPLICATIONCONTROLLERS;
	    }
	    else if(type == "ReplicaSet")
	    {
		component_type = k8s_component::K8S_REPLICASETS;
	    }
	    else if(type == "Service")
	    {
		component_type = k8s_component::K8S_SERVICES;
	    }
	    else if(type == "DaemonSet")
	    {
		component_type = k8s_component::K8S_DAEMONSETS;
	    }
	    else if(type == "Deployment")
	    {
		component_type = k8s_component::K8S_DEPLOYMENTS;
	    }
	    else if(type == "EventList")
	    {
		component_type = k8s_component::K8S_EVENTS;
	    }
	    else
	    {
		g_logger.log("Unrecognized component type: " + type,
	...
	{
	    g_logger.log("Component type not found in JSON",
			 sinsp_logger::SEV_ERROR);
	    return;
	}
    }
    ...
    if(!version_logged)
    {
	g_logger.log("K8s capture version: " + std::to_string(version),
		     sinsp_logger::SEV_DEBUG);
	version_logged = true;
    }
    switch(version)
	...
	if(component_type < k8s_component::K8S_COMPONENT_COUNT)
	{
		...
		m_dispatch_map[component_type] =
			std::unique_ptr<k8s_dispatcher>(
				new k8s_dispatcher(component_type, m_state));
	    }
	...
	else
	{
	    throw sinsp_exception(
		    std::string("K8s capture: unknown component type (") +
	...
	if(component_type < k8s_component::K8S_COMPONENT_COUNT)
	{
		...
		m_handler_map[component_type] =
			k8s_net::make_handler(m_state, component_type, false);
	    }
	    ...
	    {
		throw sinsp_exception(
			std::string("K8s capture replay: error creating ") +
			k8s_component::get_name(component_type) + " handler");
	    }
	}
	else
	{
    ...
    default:
	throw sinsp_exception(
		std::string("K8s capture: invalid capture version (") +
