...
mesos_auth::mesos_auth(const uri::credentials_t& dcos_enterprise_credentials,
		       string auth_hostname, int token_refresh_interval):
	m_dcos_enterprise_credentials(dcos_enterprise_credentials),
	m_auth_uri(string("https://") + auth_hostname +
		   string("/acs/api/v1/auth/login")),
...
void mesos_auth::authenticate()
{
	...
	if(auth_request.get_response_code() == 200)
	{
		...
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"Mesos authenticated with token=%s",
				m_token.c_str());
	    }
	    else if(!parse_ok)
	    {
		...
		throw sinsp_exception(
			string("Cannot parse json (" + errstr + ")"));
	    }
	    else
	    {
		throw sinsp_exception(string("Cannot authenticate on Mesos "
					     "master, response=") +
				      response);
	    }
	}
	...
	{
	    throw sinsp_exception(string("Cannot authenticate on Mesos master, "
					 "response_code=") +
				  to_string(auth_request.get_response_code()));
	}
