...
class gen_event_filter_check
{
    ...
    virtual int32_t parse_field_name(const char *str, bool alloc_state,
				     bool needed_for_filtering) = 0;
    virtual void add_filter_value(const char *str, uint32_t len,
				  uint32_t i = 0) = 0;
    virtual bool compare(gen_event *evt) = 0;
    virtual bool extract(gen_event *evt, std::vector<extract_value_t> &values,
			 bool sanitize_strings = true) = 0;
};

///////////////////////////////////////////////////////////////////////////////
// Filter expression class
// A filter expression contains multiple filters connected by boolean
// expressions, e.g. "check or check", "check and check and check", "not check"
///////////////////////////////////////////////////////////////////////////////
...
class gen_event_filter_expression : public gen_event_filter_check
{
    ...
    //
    // The following methods are part of the filter check interface but are
    // irrelevant for this class, because they are used only for the leaves of
    // the filtering tree.
    //
    ...
    bool extract(gen_event *evt, std::vector<extract_value_t> &values,
		 bool sanitize_strings = true);

    //
    // An expression is consistent if all its checks are of the same type
    // (or/and).
    //
    // This method returns the expression operator (BO_AND/BO_OR/BO_NONE) if the
    // expression is consistent. It returns -1 if the expression is not
    // consistent.
    //
...
class gen_event_filter
{
    ...
    /*!
      ...
      \return true if the event is accepted by the filter, false if it's
      rejected.
    */
...
class gen_event_filter_factory
{
	...
	// FILTER ONLY: for fields that can only be used in filters, not
	// outputs. IDX_REQUIRED: for fields that can take an optional index
	// EPF_TABLE_ONLY: for fields with the EPF_TABLE_ONLY (e.g. hidden) flag
	// set etc
...
public:
	...
	// website.
	std::string as_markdown(const std::set<std::string> &event_sources =
					std::set<std::string>());
...
class gen_event_formatter
{
    ...
    virtual bool tostring_withformat(gen_event *evt, std::string &output,
				     output_format of) = 0;

    // The map should map from field name, without the '%'
    // (e.g. "proc.name"), to field value (e.g. "nginx")
    virtual bool
    get_field_values(gen_event *evt,
...
class gen_event_formatter_factory
{
    ...
    virtual std::shared_ptr<gen_event_formatter>
    create_formatter(const std::string &format) = 0;
};
