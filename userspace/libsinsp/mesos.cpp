...
mesos::mesos(const std::string& mesos_state_json,
	...
	m_discover_marathon_uris(false), m_testing(true)
{
    if(mesos_state_json.empty() ||
       (marathon_groups_json.empty() && !marathon_apps_json.empty()) ||
       (!marathon_groups_json.empty() && marathon_apps_json.empty()))
    {
	throw sinsp_exception("Mesos state AND (both OR none [marathon apps "
			      "and groups]) are needed");
    }
    mesos_http::json_ptr_t state_json =
	    mesos_http::try_parse(mesos_state_json, "fixed-mesos-state");
    if(state_json)
    {
	...
	if(!marathon_groups_json.empty())
	{
		...
		throw sinsp_exception(
			"Unexpected condition while detecting Mesos master: "
			"frameworks entry not found.");
	    }
	    g_logger.log("Found " + std::to_string(frameworks.size()) +
				 " Mesos frameworks",
		...
		if(!name.isNull() && name.isConvertibleTo(Json::stringValue) &&
		   mesos_framework::is_root_marathon(name.asString()))
		{
	    ...
	    set_marathon_groups_json(
		    mesos_http::try_parse(marathon_groups_json,
					  "fixed-marathon-state"),
		    framework_id);
	    set_marathon_apps_json(
		    dummy_group /*mesos_http::try_parse(marathon_apps_json,
				   "fixed-groups-state")*/
		    ,
		    framework_id);
	}
...
mesos::mesos(const std::string& state_uri, const uri_list_t& marathon_uris,
	     bool discover_mesos_leader, bool discover_marathon_leader,
	     const credentials_t& mesos_credentials,
	     const credentials_t& marathon_credentials, int timeout_ms,
	     bool is_captured, bool verbose):
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	m_collector(false),
	m_mesos_uri(state_uri), m_marathon_uris(marathon_uris),
#endif // HAS_CAPTURE
	m_state(is_captured, verbose),
	m_discover_mesos_leader(discover_mesos_leader),
	m_discover_marathon_uris(discover_marathon_leader ||
				 marathon_uris.empty()),
	m_timeout_ms(timeout_ms), m_verbose(verbose), m_testing(false)
{
#if defined(HAS_CAPTURE) && !defined(_WIN32)
    g_logger.log(
	    std::string("Creating Mesos object for [" +
			(m_mesos_uri.empty() ? std::string("capture replay")
					     : m_mesos_uri) +
	...
	g_logger.log(
		"Multiple root marathon URIs configured; only the first one (" +
			marathon_uri +
			") will have effect;"
			" others will be treated as generic frameworks (user "
			"Marathon frameworks will be discovered).",
...
mesos::mesos(const std::string& state_uri, const uri_list_t& marathon_uris,
	     bool discover_mesos_leader, bool discover_marathon_leader,
	     const credentials_t& dcos_enterprise_credentials, int timeout_ms,
	     bool is_captured, bool verbose):
	mesos_auth(dcos_enterprise_credentials),
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	m_collector(false), m_mesos_uri(state_uri),
...
void mesos::init()
{
	...
	{
	    throw sinsp_exception("Invalid access to Mesos initializer: mesos "
				  "state http client for [" +
				  m_mesos_uri + "] not unique.");
	}
	m_state_http = std::make_shared<mesos_http>(
		*this, m_mesos_uri + default_state_api, m_discover_mesos_leader,
		m_marathon_uris.empty(), m_timeout_ms, m_token);
...
void mesos::init_marathon()
{
	...
	const uri_list_t& marathons =
		m_discover_marathon_uris ? m_state_http->get_marathon_uris()
					 : m_marathon_uris;
	if(marathons.size())
	{
	    g_logger.log("Found " + std::to_string(marathons.size()) +
				 " Marathon URIs",
			 sinsp_logger::SEV_DEBUG);
	    for(const auto& muri : marathons)
	    {
		g_logger.log("Creating Marathon http objects: " +
				     uri(muri).to_string(false),
			     sinsp_logger::SEV_DEBUG);
		m_marathon_groups_http[muri] = std::make_shared<marathon_http>(
			*this, muri + default_groups_api,
			m_discover_marathon_uris, m_timeout_ms, m_token);
		m_marathon_apps_http[muri] = std::make_shared<marathon_http>(
			*this, muri + default_apps_api,
			m_discover_marathon_uris, m_timeout_ms, m_token);
	    }
...
{
    return (m_discover_marathon_uris ? m_state_http->get_marathon_uris()
				     : m_marathon_uris);
}
#endif
...
void mesos::rebuild_marathon_state(bool full)
{
	...
	if(full)
	{
		...
		{
		    throw sinsp_exception(
			    "Marathon groups HTTP client is null.");
		}
	    }
	...
	if(m_state_http)
	{
	    const mesos_http::marathon_uri_t& marathon_uris =
		    m_marathon_uris.empty() ? m_state_http->get_marathon_uris()
					    : m_marathon_uris;
	    ...
	    {
		throw sinsp_exception(
			"Marathon detected but Marathon URI not found.");
	    }
	}
	else
	{
...
void mesos::send_marathon_data_request()
{
	...
	for(auto& group_http : m_marathon_groups_http)
	{
		...
		g_logger.log("Marathon groups request sent.",
			     sinsp_logger::SEV_DEBUG);
	    }
	    else
	    {
	...
	for(auto& app_http : m_marathon_apps_http)
	{
		...
		g_logger.log("Marathon apps request sent.",
			     sinsp_logger::SEV_DEBUG);
	    }
	    else
	    {
...
void mesos::connect_marathon()
{
	...
	for(auto& group_http : m_marathon_groups_http)
	{
	    ...
	    {
		throw sinsp_exception(
			"Connection to Marathon group API failed.");
	    }
	}
	for(auto& app_http : m_marathon_apps_http)
	{
...
bool mesos::is_alive() const
{
	...
	{
	    g_logger.log("Marathon groups connection loss.",
			 sinsp_logger::SEV_WARNING);
	    return false;
	}
    }
	...
	{
	    g_logger.log("Marathon apps connection loss.",
			 sinsp_logger::SEV_WARNING);
	    return false;
	}
    }
...
void mesos::check_collector_status(int expected)
{
    ...
    {
	throw sinsp_exception(
		"Mesos collector not healthy (has " +
...
void mesos::send_data_request(bool collect)
{
	...
	{
	    if(app.second && !app.second->isNull())
	    {
		return;
	    }
	}
...
bool mesos::collect_data()
{
    ...
    //       mesos_collector socket drop detection when remote end closes
    //       connection
    time_t now;
    time(&now);
    ...
    {
	throw sinsp_exception(
		"Detected stalled Mesos connection (" +
		std::to_string(difftime(now, m_last_mesos_refresh)) +
		"s)."
		" Reconnect attempt in next cycle ...");
    }
    if(m_last_marathon_refresh &&
       difftime(now, m_last_marathon_refresh) > tout_s)
    {
	throw sinsp_exception(
		"Detected stalled Marathon connection(" +
		std::to_string(difftime(now, m_last_marathon_refresh)) +
		"s)."
		" Reconnect attempt in next cycle ...");
    }
    if(m_json_error)
    {
	throw sinsp_exception("Detected JSON parsing error. Reconnect attempt "
			      "in next cycle ...");
    }
	...
	if(has_marathon())
	{
	    g_logger.log("Marathon connection detected.",
			 sinsp_logger::SEV_DEBUG);
	    if(!m_marathon_apps_json.empty() && !m_marathon_groups_json.empty())
	    {
		g_logger.log("Marathon data detected.",
			     sinsp_logger::SEV_DEBUG);
		for(auto& group : m_marathon_groups_json)
		{
		    if(group.second && !group.second->isNull())
		    {
			json_map_type_t::iterator app_it =
				m_marathon_apps_json.find(group.first);
			if(app_it != m_marathon_apps_json.end())
			{
				...
				if(m_inactive_frameworks.size())
				{
				    g_logger.log(
					    "Collection detected " +
						    std::to_string(
							    m_inactive_frameworks
								    .size()) +
						    " inactive frameworks",
					    sinsp_logger::SEV_DEBUG);
				}
				if(m_inactive_frameworks.find(group.first) ==
				   m_inactive_frameworks.end())
				{
				    g_logger.log("Detected active Marathon "
						 "framework " +
							 group.first,
						 sinsp_logger::SEV_DEBUG);
				    // +++ order is important - apps belong to
				    // groups and must be processed after
				    parse_groups(std::move(*group.second),
						 group.first);
				    parse_apps(std::move(*app_it->second),
					       app_it->first);
				    m_last_marathon_refresh = now;
				    // ---
				}
				else // framework was shut down, clear
				     // groups/apps
				{
				    g_logger.log("Detected inactive Marathon "
						 "framework " +
							 group.first,
						 sinsp_logger::SEV_DEBUG);
				    m_state.erase_groups(
					    group.first); // apps will go away
							  // with groups
			    ...
			    else if((difftime(now, m_last_marathon_refresh) >
				     tout_s) ||
				    m_json_error)
			    {
				std::string errstr =
					"Detected null Marathon app (" +
					app_it->first +
					"), resetting current state.";
				g_logger.log(errstr, sinsp_logger::SEV_WARNING);
				g_json_error_log.log(
					app_it->first, errstr,
			...
			{
			    // must never happen
			    throw sinsp_exception("A discrepancy found between "
						  "groups and apps "
						  "(app json for framework [" +
						  group.first +
						  "] not found in json map).");
			}
		    }
		    else if((difftime(now, m_last_marathon_refresh) > tout_s) ||
			    m_json_error)
		    {
			g_logger.log("Detected null Marathon group (" +
					     group.first +
					     "), resetting current state.",
				     sinsp_logger::SEV_WARNING);
			m_mesos_state_json.reset();
...
void mesos::handle_frameworks(const Json::Value& root)
{
	...
	if(frameworks.isArray())
	{
		...
		for(const auto& framework : frameworks)
		{
			...
			{
			    framework_list_t::iterator it =
				    m_inactive_frameworks.find(uid.asString());
				...
				g_logger.log("Mesos framework deactivated: " +
						     name + '[' + fid + ']',
					     sinsp_logger::SEV_INFO);
				remove_framework(framework);
#if defined(HAS_CAPTURE) && !defined(_WIN32)
				remove_framework_http(m_marathon_groups_http,
						      fid);
				remove_framework_http(m_marathon_apps_http,
						      fid);
#endif // HAS_CAPTURE
			    }
			}
			else // active framework detected
			{
			    ...
			    if((m_inactive_frameworks.erase(uid.asString())) ||
			       (m_activated_frameworks.find(uid.asString()) ==
				m_activated_frameworks.end()))
			    {
				g_logger.log("New or activated Mesos framework "
					     "detected: " +
						     name + " [" +
						     uid.asString() + ']',
					     sinsp_logger::SEV_INFO);
				...
				if(mesos_framework::is_root_marathon(name) &&
				   find_if(m_marathon_groups_http.begin(),
					   m_marathon_groups_http.end(),
					   [uid](const decltype(m_marathon_groups_http)::
							 value_type& item) {
					       return item.second
							      ->get_framework_id() ==
						      uid.asString();
	    ...
	    {
		throw sinsp_exception(
			"No Mesos frameworks found (possibly Mesos master HA "
	...
	{
	    throw sinsp_exception(
		    "Mesos frameworks entry found but not a JSON array.");
	}
    }
    else
    {
...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
void mesos::remove_framework_http(marathon_http_map& http_map,
				  const std::string& framework_id)
{
    for(marathon_http_map::iterator it = http_map.begin(), end = http_map.end();
	it != end; ++it)
    {
	if(it->second && it->second->get_framework_id() == framework_id)
	{
	    http_map.erase(it);
	    g_logger.log("Removed http for deactivated Marathon framework [" +
				 framework_id + ']',
			 sinsp_logger::SEV_DEBUG);
	    return;
	}
...
void mesos::add_tasks_impl(mesos_framework& framework, const Json::Value& tasks)
{
	...
	for(const auto& task : tasks)
	{
		...
		if(t)
		{
		    os << "Adding Mesos task: [" << framework.get_name() << ':'
		       << t->get_name() << ',' << t->get_uid() << ']';
		    g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
		...
		else
		{
		    ...
		    os << "Failed to add Mesos task: [" << framework.get_name()
		       << ':' << name << ',' << uid << "], running on slave "
		       << sid;
		    g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
		    g_json_error_log.log(framework.get_name(), os.str(),
					 sinsp_utils::get_current_time_ns(),
					 "add_tasks_impl");
		}
	    }
	...
	g_json_error_log.log(framework.get_name(), errstr,
			     sinsp_utils::get_current_time_ns(),
			     "add_tasks_impl for framework");
    }
}
...
void mesos::check_frameworks(const json_ptr_t& json)
{
	...
	else
	{
		...
		if(!frameworks.size())
		{
		    throw sinsp_exception("No Mesos frameworks found (possibly "
					  "Mesos master HA migration).");
		}
	    }
	    else
	    {
		throw sinsp_exception("Unexpected Mesos frameworks entry found "
				      "(not an array).");
	    }
	}
    }
...
void mesos::parse_state(Json::Value&& root)
{
	...
	m_state.enqueue_capture_event(mesos_state_t::capture::MESOS_STATE,
...
void mesos::set_marathon_groups_json(json_ptr_t json,
				     const std::string& framework_id)
{
    bool json_error = !json || (json && json->isNull());
    ...
    if(!json_error)
    {
	g_logger.log("Received groups JSON for Marathon framework [" +
			     framework_id + ']',
		     sinsp_logger::SEV_DEBUG);
    }
    else
    {
	g_logger.log("Received invalid Marathon groups JSON",
		     sinsp_logger::SEV_WARNING);
    }
    m_json_error = m_json_error || json_error;
}

void mesos::set_marathon_apps_json(json_ptr_t json,
				   const std::string& framework_id)
{
    bool json_error = !json || (json && json->isNull());
    ...
    if(!json_error)
    {
	g_logger.log("Received apps JSON for Marathon framework [" +
			     framework_id + ']',
		     sinsp_logger::SEV_DEBUG);
    }
    else
	...
	g_logger.log(errstr, sinsp_logger::SEV_WARNING);
	g_json_error_log.log("(null)", errstr,
			     sinsp_utils::get_current_time_ns(),
			     "set-marathon-apps-json");
    }
    m_json_error = m_json_error || json_error;
}
...
void mesos::simulate_event(const std::string& json)
{
	...
	for(auto& member : members)
	{
		...
		{
		    m_state.parse_groups(std::move(root[member]),
					 frameworkId.asString());
		}
		else
		{
		    throw sinsp_exception("Framework ID for Marathon groups "
					  "not found during event simulation.");
		}
	    }
		...
		if(!frameworkId.isNull() && frameworkId.isString())
		{
		    m_state.parse_apps(std::move(root[member]),
				       frameworkId.asString());
		}
		else
		{
    ...
    {
	std::string errstr = "Could not parse json (" +
			     reader.getFormattedErrorMessages() + ")";
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(json, errstr, sinsp_utils::get_current_time_ns(),
			     "parse-mesos-evt");
    }
}
#endif // CYGWING_AGENT
