...
void sinsp_parser::parse_clone_exit_caller(sinsp_evt *evt, int64_t child_tid)
{
	...
	else 
	{
		...
		child_tinfo->traverse_main_parent_state(scap_file_visitor);
	}
...
void sinsp_parser::parse_thread_exit(sinsp_evt *evt)
{
	...
	if(evt->m_tinfo != nullptr)
	{
		...
		printf("-----------> REMOVE[%s](%ld) tid: %ld, pid: %ld, ptid: %ld, vtid: %ld, vpid: %ld, reaper: %d\n", evt->m_tinfo->m_comm.c_str(), evt->get_num(), evt->m_tinfo->m_tid, evt->m_tinfo->m_pid, evt->m_tinfo->m_ptid, evt->m_tinfo->m_vtid, evt->m_tinfo->m_vpid, reaper);
	...
	{
		printf("-----------> REMOVE EMPTY(%ld) tid: %ld\n", evt->get_num(), evt->get_tid());
	}
}
