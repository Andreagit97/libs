...
sinsp_parser::sinsp_parser(sinsp *inspector):
	m_inspector(inspector), m_tmp_evt(m_inspector), m_fd_listener(NULL)
{
    m_fake_userevt = (scap_evt *)m_fake_userevt_storage;

    //
    // Note: allocated here instead of in the sinsp constructor because
    // sinsp_partial_tracer
    //       is not defined in sinsp.cpp
    //
    m_inspector->m_partial_tracers_pool =
	    new simple_lifo_queue<sinsp_partial_tracer>(128);
...
void sinsp_parser::init_scapevt(metaevents_state &evt_state, uint16_t evt_type,
				uint16_t buf_size)
{
    evt_state.m_piscapevt =
	    (scap_evt *)realloc(evt_state.m_piscapevt, buf_size);
...
void sinsp_parser::process_event(sinsp_evt *evt)
{
	...
	if(evt->get_tid() == m_inspector->m_self_pid &&
	   etype != PPME_SCHEDSWITCH_1_E && etype != PPME_SCHEDSWITCH_6_E &&
	   etype != PPME_DROP_E && etype != PPME_DROP_X &&
	   etype != PPME_SCAPEVENT_E && etype != PPME_PROCINFO_E &&
	   etype != PPME_CPU_HOTPLUG_E && m_inspector->m_self_pid)
	{
	...
	if(etype == PPME_SYSCALL_WRITE_X)
	{
	    ...
	    if(fdinfo &&
	       (fdinfo->m_flags & (sinsp_fdinfo_t::FLAGS_IS_TRACER_FD |
				   sinsp_fdinfo_t::FLAGS_IS_TRACER_FILE)))
	    {
		eflags = (ppm_event_flags)(((uint64_t)eflags) |
					   EF_MODIFIES_STATE);
	    }
	    else
	    {
	...
	parse_container_evt(
		evt); // deprecated, only here for backwards compatibility
	break;
    ...
    if(evt->m_fdinfo)
    {
	evt->set_fdinfo_name_changed(evt->m_fdinfo->m_name !=
				     evt->m_fdinfo->m_oldname);
    }
}

void sinsp_parser::event_cleanup(sinsp_evt *evt)
{
    if(evt->get_direction() == SCAP_ED_OUT && evt->m_tinfo &&
...
bool sinsp_parser::reset(sinsp_evt *evt)
{
	...
	{
	    evt->m_tinfo =
		    m_inspector->get_thread_ref(evt->m_pevt->tid, false, false)
			    .get();
	}
	else
	{
    ...
    {
	evt->m_tinfo =
		m_inspector->get_thread_ref(evt->m_pevt->tid, query_os, false)
			.get();
    }
	...
	if(etype == PPME_SYSCALL_CLONE_11_X ||
	   ...
	   etype == PPME_SYSCALL_CLONE_20_X || etype == PPME_SYSCALL_FORK_X ||
	   etype == PPME_SYSCALL_FORK_17_X || etype == PPME_SYSCALL_FORK_20_X ||
	   etype == PPME_SYSCALL_VFORK_X || etype == PPME_SYSCALL_VFORK_17_X ||
	   etype == PPME_SYSCALL_VFORK_20_X || etype == PPME_SYSCALL_CLONE3_X)
	{
	...
	if(eflags & EF_USES_FD)
	{
	    ...
	    {
		//
		// A close gets canceled when the same fd is created
		// successfully between close enter and close exit. If that
		// happens
		//
...
bool sinsp_parser::retrieve_enter_event(sinsp_evt *enter_evt,
					sinsp_evt *exit_evt)
{
    //
    ...
    if(!(exit_evt->m_tinfo->is_lastevent_data_valid() &&
	 exit_evt->m_tinfo->m_lastevent_data))
    {
	//
    ...
    enter_evt->init(exit_evt->m_tinfo->m_lastevent_data,
		    exit_evt->m_tinfo->m_lastevent_cpuid);

    //
    // Make sure that we're using the right enter event, to prevent
    // inconsistencies when events are dropped
    //
...
sinsp_protodecoder *sinsp_parser::add_protodecoder(string decoder_name)
{
    ...
    sinsp_protodecoder *nd =
	    g_decoderlist.new_protodecoder_from_name(decoder_name, m_inspector);
...
void sinsp_parser::register_event_callback(sinsp_pd_callback_type etype,
					   sinsp_protodecoder *dec)
{
    switch(etype)
    {
    case CT_OPEN:
...
void sinsp_parser::parse_clone_exit(sinsp_evt *evt)
{
    ...
    int64_t childtid;
    bool is_inverted_clone = false; // true if clone() in the child returns
				    // before the one in the parent
    ...
    // the flag check should always suffice but leave the tid/vtid check for
	...
	// an entry in the thread table for this process. If there is one, make
	// sure it was created recently. Otherwise, assume it's an old thread
	// for which we lost the exit event and remove it from the table.
	//
    ...
    sinsp_threadinfo *ptinfo =
	    m_inspector->get_thread_ref(tid, true, true).get();
    if(NULL == ptinfo)
    {
	//
	// No clone() caller, we probably missed earlier events.
	// We simply return and ignore the event, which means this thread won't
    ...
    //
    sinsp_threadinfo *child =
	    m_inspector->get_thread_ref(childtid, false, true).get();
    if(NULL != child)
    {
	//
	// If this was an inverted clone, all is fine, we've already taken care
	// of adding the thread table entry in the child.
	// Otherwise, we assume that the entry is there because we missed the
	// exit event for a previous thread and we replace the info structure.
	//
	...
	ptinfo = m_inspector->get_thread_ref(tid, true, true).get();
    ...
    // thread was created. In that case, we set the pid to the one of the CHILD
    // thread that is going to be created.
    //
	...
	// What we do is: if the child is NOT a thread, we copy all the parent
	// fds. The right thing to do is looking at PPM_CL_CLONE_FILES, but
	// there are syscalls like open and pipe2 that can override
	// PPM_CL_CLONE_FILES with the O_CLOEXEC flag
	//
	...
	//
	// It's important to reset the cache of the child thread, to prevent it
	// from referring to an element in the parent's table.
	//
...
#endif
	DBG_SINSP_INFO("tid collision for %" PRIu64 "(%s)", tinfo->m_tid,
		       tinfo->m_comm.c_str());
    }
...
void sinsp_parser::parse_execve_exit(sinsp_evt *evt)
{
    ...
    //
    // We get here when execve or execveat return. The thread has already been
    // added by a previous fork or clone, and we just update the entry with the
    // new information.
    //
    if(!evt->m_tinfo)
    {
	...
	// 3. vpid=1 execve to the real process the user wants to run inside the
	// container
	//
	m_inspector->m_container_manager.resolve_container(
		evt->m_tinfo, m_inspector->is_live());
	break;
    default:
	ASSERT(false);
    }
	...
	if(retrieve_enter_event(enter_evt, evt))
	{
	    ...
	    {
		sinsp_utils::concatenate_paths(
			fullpath, SCAP_MAX_PATH_SIZE,
			evt->m_tinfo->m_cwd.c_str(),
			(uint32_t)evt->m_tinfo->m_cwd.size(), parinfo->m_val,
			(uint32_t)parinfo->m_len, m_inspector->m_is_windows);
	...
	if(retrieve_enter_event(enter_evt, evt))
	{
	    ...
	    {
		// In this way, the pathname becomes an absolute path and in the
		// 'concatenate_paths' function the dirfd value is not
		// considered.
	    ...
	    sinsp_utils::concatenate_paths(fullpath, SCAP_MAX_PATH_SIZE,
					   sdir.c_str(),
					   (uint32_t)sdir.length(), pathname,
					   namelen, m_inspector->m_is_windows);
	    evt->m_tinfo->m_exepath = fullpath;
	}
	break;
    default:
    ...
    //
    // execve starts with a clean fd list, so we get rid of the fd list that
    // clone copied from the parent
    // XXX validate this
    //
    ...
    auto spf = evt->m_tinfo->m_flags &
	       (PPM_CL_PIPE_SRC | PPM_CL_PIPE_DST | PPM_CL_IS_MAIN_THREAD);
    ...
    //
    // This process' name changed, so we need to include it in the protocol
    // again
    //
...
void sinsp_parser::parse_dirfd(sinsp_evt *evt, char *name, int64_t dirfd,
			       OUT string *sdir)
{
...
template<typename T>
void schedule_more_evts(sinsp *inspector, void *data, T *client,
			ppm_event_type evt_type)
{
    ...
    std::size_t tot_len =
	    sizeof(scap_evt) + sizeof(uint16_t) + payload.size() + 1;
    ...
    uint16_t *plen = (uint16_t *)((char *)state->m_piscapevt +
...
{
    schedule_more_evts(inspector, data, inspector->get_k8s_client(),
		       PPME_K8S_E);
}
...
void sinsp_parser::schedule_k8s_events()
{
	...
	if(event_count)
	{
	    ...
	    m_k8s_metaevents_state.m_piscapevt->ts =
	    ...
	    m_inspector->add_meta_event_callback(&schedule_more_k8s_evts,
						 &m_k8s_metaevents_state);
...
{
    schedule_more_evts(inspector, data, inspector->get_mesos_client(),
		       PPME_MESOS_E);
}
...
void sinsp_parser::schedule_mesos_events()
{
	...
	if(event_count)
	{
	    ...
	    m_mesos_metaevents_state.m_piscapevt->ts =
		    m_inspector->m_lastevent_ts;
	    m_mesos_metaevents_state.m_new_group = true;
	    m_mesos_metaevents_state.m_n_additional_events_to_add = event_count;
	    m_inspector->add_meta_event_callback(&schedule_more_mesos_evts,
						 &m_mesos_metaevents_state);
...
//
// Helper function to allocate a socket fd, initialize it by parsing its
// parameters and add it to the fd table of the given thread.
//
inline void sinsp_parser::add_socket(sinsp_evt *evt, int64_t fd,
				     uint32_t domain, uint32_t type,
				     uint32_t protocol)
{
    sinsp_fdinfo_t fdi;
    ...
    {
	fdi.m_type =
		(domain == PPM_AF_INET) ? SCAP_FD_IPV4_SOCK : SCAP_FD_IPV6_SOCK;
	...
	{
	    //
	    // XXX: we mask type because, starting from linux 2.6.27, type can
	    // be ORed with
	    //      SOCK_NONBLOCK and SOCK_CLOEXEC. We need to validate that
	    //      byte masking is acceptable
	    //
	...
	{
	    memset(&(fdi.m_sockinfo.m_ipv6info), 0,
		   sizeof(fdi.m_sockinfo.m_ipv6info));
    ...
    {
	SINSP_STR_DEBUG("Unknown fd fd=" + to_string(fd) + " domain=" +
			to_string(domain) + " type=" + to_string(type) +
...
inline void sinsp_parser::infer_sendto_fdinfo(sinsp_evt *const evt)
{
	...
	const uint32_t domain =
		(addr_family == AF_INET) ? PPM_AF_INET : PPM_AF_INET6;
	...
	SINSP_DEBUG("Call to sendto() with fd=%d; missing socket() "
		    ...
		    evt->m_tinfo->get_comm().c_str(), evt->m_tinfo->m_pid);
#endif
...
void sinsp_parser::parse_socket_exit(sinsp_evt *evt)
{
    ...
    //
    // NOTE: we don't check the return value of get_param() because we know the
    // arguments we need are there.
    // XXX this extraction would be much faster if we parsed the event manually
    // to extract the parameters in one scan. We don't care too much because we
    // assume that we get here seldom enough that saving few tens of CPU cycles
    // is not important.
    //
    parinfo = evt->get_param(0);
    ASSERT(parinfo->m_len == sizeof(int64_t));
    ...
    //
    // Allocate a new fd descriptor, populate it and add it to the thread fd
    // table
    //
...
void sinsp_parser::parse_bind_exit(sinsp_evt *evt)
{
	...
	// This happens for socket types that we don't support, so we have the
	// assertion to make sure that this is not a type of socket that we
	// support.
	//
	ASSERT(!(evt->m_fdinfo->is_unix_socket() ||
	...
	if(port > 0)
	{
		...
		memcpy(evt->m_fdinfo->m_sockinfo.m_ipv6serverinfo.m_ip.m_b, ip,
		       sizeof(ipv6addr));
    ...
    //
    evt->m_fdinfo->m_name =
	    evt->get_param_as_str(1, &parstr, sinsp_evt::PF_SIMPLE);
...
void sinsp_parser::parse_connect_enter(sinsp_evt *evt)
{
	...
	case SCAP_FD_IPV6_SOCK:
	    evt->m_fdinfo->m_sockinfo.m_ipv6info.m_fields.m_dip =
		    ipv6addr::empty_address;
	...
	sinsp_utils::sockinfo_to_str(
		&evt->m_fdinfo->m_sockinfo, evt->m_fdinfo->m_type,
		&evt->m_paramstr_storage[0],
	...
	memcpy(&evt->m_fdinfo->m_sockinfo.m_ipv4info.m_fields.m_dip,
	       packed_data + 1, sizeof(uint32_t));
	memcpy(&evt->m_fdinfo->m_sockinfo.m_ipv4info.m_fields.m_dport,
	       packed_data + 5, sizeof(uint16_t));
    }
    else if(family == PPM_AF_INET6)
    {
	uint16_t port;
	...
	if(sinsp_utils::is_ipv4_mapped_ipv6(ip))
	{
	    evt->m_fdinfo->m_type = SCAP_FD_IPV4_SOCK;
	    memcpy(&evt->m_fdinfo->m_sockinfo.m_ipv4info.m_fields.m_dip,
		   packed_data + 13, sizeof(uint32_t));
	...
	else
	{
	    ...
	    memcpy(evt->m_fdinfo->m_sockinfo.m_ipv6info.m_fields.m_dip.m_b, ip,
		   sizeof(ipv6addr));
	}
    }
    else
    ...
    //
    // If there's a listener callback and we're tracking connection status,
    // invoke it
    //
						  ...
						  uint8_t *packed_data)
{
	...
	if(family == PPM_AF_INET6)
	{
	    ...
	    if(!(sinsp_utils::is_ipv4_mapped_ipv6(sip) &&
		 sinsp_utils::is_ipv4_mapped_ipv6(dip)))
	    {
		evt->m_fdinfo->m_type = SCAP_FD_IPV6_SOCK;
		changed = m_inspector->m_parser->set_ipv6_addresses_and_ports(
			evt->m_fdinfo, packed_data, false);
	    }
	    else
	    {
		evt->m_fdinfo->m_type = SCAP_FD_IPV4_SOCK;
		changed = m_inspector->m_parser
	...
	else
	{
	    ...
	    //
	    changed = m_inspector->m_parser->set_ipv4_addresses_and_ports(
		    evt->m_fdinfo, packed_data, false);
	}

	if(changed && evt->m_fdinfo->is_role_server() &&
	...
	{
	    //
	    // This should happen only in case of a bug in our code, because I'm
	    // assuming that the OS causes a connect with the wrong socket type
	    // to fail. Assert in debug mode and just keep going in release
	    // mode.
	    //
	    ASSERT(false);
	}
...
//
// This function takes care of cleaning up the FD and removing it from all the
// tables (process FD table, connection table...). It's invoked when a close()
// or a thread exit happens.
//
void sinsp_parser::erase_fd(erase_fd_params *params)
{
    ...
    {
	//
	// This happens when more than one close has been canceled at the same
	// time for this thread. Since we currently handle just one canceling at
	// at time (we don't have a list of canceled closes, just a single
	// entry), the second one will generate a failed FD lookup. We do
	// nothing. NOTE: I do realize that this can cause a connection leak, I
	// just assume that it's
	//       rare enough that the delayed connection cleanup (when the
	//       timestamp expires) is acceptable.
	//
...
void sinsp_parser::parse_close_exit(sinsp_evt *evt)
{
	...
	//
	// a close gets canceled when the same fd is created successfully
	...
	//
	// It is normal when a close fails that the fd lookup failed, so we
	// revert the increment of m_n_failed_fd_lookups (for the enter event
	// too if there's one).
	//
#ifdef GATHER_INTERNAL_STATS
...
void sinsp_parser::add_pipe(sinsp_evt *evt, int64_t tid, int64_t fd,
			    uint64_t ino)
{
    sinsp_fdinfo_t fdi;
...
inline bool sinsp_parser::update_ipv4_addresses_and_ports(
	sinsp_fdinfo_t *fdinfo, uint32_t tsip, uint16_t tsport, uint32_t tdip,
	uint16_t tdport, bool overwrite_dest)
{
...
bool sinsp_parser::set_ipv4_addresses_and_ports(sinsp_fdinfo_t *fdinfo,
						uint8_t *packed_data,
						bool overwrite_dest)
{
    uint32_t tsip, tdip;
...
bool sinsp_parser::set_ipv4_mapped_ipv6_addresses_and_ports(
	sinsp_fdinfo_t *fdinfo, uint8_t *packed_data, bool overwrite_dest)
{
    uint32_t tsip, tdip;
    uint16_t tsport, tdport;
...
bool sinsp_parser::set_ipv6_addresses_and_ports(sinsp_fdinfo_t *fdinfo,
						uint8_t *packed_data,
						bool overwrite_dest)
{
    ipv6addr tsip, tdip;
    ...
    if(fdinfo->m_sockinfo.m_ipv6info.m_fields.m_dip ==
	       ipv6addr::empty_address ||
...
{
    fdinfo->m_sockinfo.m_unixinfo.m_fields.m_source =
	    *(uint64_t *)(packed_data + 1);
    fdinfo->m_sockinfo.m_unixinfo.m_fields.m_dest =
	    *(uint64_t *)(packed_data + 9);

    return true;
}

// Return false if the update didn't happen (for example because the tuple is
// NULL)
bool sinsp_parser::update_fd(sinsp_evt *evt, sinsp_evt_param *parinfo)
{
	   ...
	   sinsp_utils::is_ipv4_mapped_ipv6(dip))
	{
	    ...
	    if(set_ipv4_mapped_ipv6_addresses_and_ports(evt->m_fdinfo,
							packed_data) == false)
	    {
		return false;
	    }
	...
	{
	    // It's not an ipv4-mapped ipv6 address. Extract it as a normal
	    // address.
	    if(set_ipv6_addresses_and_ports(evt->m_fdinfo, packed_data) ==
	       false)
	    {
		return false;
	    }
	...
	//
	// Call the protocol decoder callbacks to notify the decoders that this
	// FD changed.
	//
	vector<sinsp_protodecoder *>::iterator it;
	for(it = m_connect_callbacks.begin(); it != m_connect_callbacks.end();
	    ++it)
	{
    ...
    {
	if(evt->m_fdinfo->m_sockinfo.m_ipv4info.m_fields.m_l4proto ==
	   SCAP_L4_UNKNOWN)
	{
	    evt->m_fdinfo->m_sockinfo.m_ipv4info.m_fields.m_l4proto =
		    SCAP_L4_UDP;
    ...
    {
	if(evt->m_fdinfo->m_sockinfo.m_ipv6info.m_fields.m_l4proto ==
	   SCAP_L4_UNKNOWN)
	{
	    evt->m_fdinfo->m_sockinfo.m_ipv6info.m_fields.m_l4proto =
		    SCAP_L4_UDP;
...
void sinsp_parser::swap_addresses(sinsp_fdinfo_t *fdinfo)
{
	...
	fdinfo->m_sockinfo.m_ipv6info.m_fields.m_sip =
		fdinfo->m_sockinfo.m_ipv6info.m_fields.m_dip;
	;
...
uint32_t sinsp_parser::parse_tracer(sinsp_evt *evt, int64_t retval)
{
    ...
    //
    // Populate the user event that we will send up the stack instead of the
    // write
    //
	...
	uint16_t *lens =
		(uint16_t *)(fakeevt_storage + sizeof(struct ppm_evt_hdr));
	lens[0] = 8;
	lens[1] = 8;
	lens[2] = 8;

	*(uint64_t *)(fakeevt_storage + sizeof(struct ppm_evt_hdr) + 6) =
		p->m_id;
	*(uint64_t *)(fakeevt_storage + sizeof(struct ppm_evt_hdr) + 14) =
		(uint64_t)&p->m_tags;
	*(uint64_t *)(fakeevt_storage + sizeof(struct ppm_evt_hdr) + 22) =
		(uint64_t)&p->m_args;
    }
    else
    {
	...
	p->m_tags.clear();
	m_tracer_error_string = "invalid tracer " + string(data, datalen) +
				", len" + to_string(datalen);
	...
	else
	{
		...
		if(eflags & EF_WRITES_TO_FD)
		{
		    ...
		    sinsp_tracerparser::parse_result pres =
			    (sinsp_tracerparser::parse_result)parse_tracer(
				    evt, retval);
			...
			// We do two things: mark it for future reference, and
			// tell the driver to enable tracers capture (if we
			// haven't done it yet).
			//
			orifdinfo->m_flags |=
		    ...
		    {
			orifdinfo->m_flags |=
				sinsp_fdinfo_t::FLAGS_IS_NOT_TRACER_FD;
		    }
		}
	    }
	}
...
void sinsp_parser::parse_rw_exit(sinsp_evt *evt)
{
	...
	if(eflags & EF_READS_FROM_FD)
	{
	    ...
	    if(tupleparam != -1 && (evt->m_fdinfo->m_name.length() == 0 ||
				    !evt->m_fdinfo->is_tcp_socket()))
	    {
		//
		// recvfrom contains tuple info.
		// If the fd still doesn't contain tuple info (because the
		// socket is a datagram one or because some event was lost), add
		// it here.
		//
		if(update_fd(evt, evt->get_param(tupleparam)))
		{
			...
			if(evt->m_fdinfo->is_role_none())
			{
			    evt->m_fdinfo->set_net_role_by_guessing(
				    m_inspector, evt->m_tinfo, evt->m_fdinfo,
				    true);
			}
			...
			sinsp_utils::sockinfo_to_str(
				&evt->m_fdinfo->m_sockinfo, fdtype,
				&evt->m_paramstr_storage[0],
				(uint32_t)evt->m_paramstr_storage.size(),
				m_inspector
		    ...
		    {
			evt->m_fdinfo->m_name = evt->get_param_as_str(
				tupleparam, &parstr, sinsp_evt::PF_SIMPLE);
		    }
		}
	    }
	    ...
	    // Extract the data buffer
	    //
	    if(etype == PPME_SYSCALL_READV_X ||
	       etype == PPME_SYSCALL_PREADV_X || etype == PPME_SOCKET_RECVMSG_X)
	    {
		parinfo = evt->get_param(2);
	    }
	    else
	    ...
	    if(m_fd_listener)
	    {
		m_fd_listener->on_read(evt, tid, evt->m_tinfo->m_lastevent_fd,
				       evt->m_fdinfo, data, (uint32_t)retval,
				       datalen);
	    }
		...
		vector<sinsp_protodecoder *> *cbacks =
			&(evt->m_fdinfo->m_callbacks->m_read_callbacks);
	...
	else
	{
		...
		if(update_fd(evt, enter_evt->get_param(tupleparam)))
		{
		    ...
		    {
			evt->m_fdinfo->m_name = enter_evt->get_param_as_str(
				tupleparam, &parstr, sinsp_evt::PF_SIMPLE);
		    }
		}
	    }
	    ...
	    if(m_fd_listener)
	    {
		m_fd_listener->on_write(evt, tid, evt->m_tinfo->m_lastevent_fd,
					evt->m_fdinfo, data, (uint32_t)retval,
					datalen);
	    }
	    ...
	    {
		vector<sinsp_protodecoder *> *cbacks =
			&(evt->m_fdinfo->m_callbacks->m_write_callbacks);
...
void sinsp_parser::parse_getcwd_exit(sinsp_evt *evt)
{
	...
	{
	    //
	    // No thread in the table. We won't store this event, which mean
	    // that we won't be able to parse the corresponding exit event and
	    // we'll have to drop the information it carries.
	    //
	    ASSERT(false);
	    return;
	}
	...
	if(chkstr != "/")
	{
	    ...
	    {
		//
		// This shouldn't happen, because we should be able to stay in
		// synch by following chdir(). If it does, it's almost sure
		// there was an event drop. In that case, we use this value to
		...
		target_res = readlink((chkstr + "/").c_str(), target_name,
		...
		if(target_res > 0)
		{
		    ...
		    {
			printf("%s != %s", target_name,
			       evt->m_tinfo->get_cwd().c_str());
			ASSERT(false);
		    }
		}
...
void sinsp_parser::parse_dup_exit(sinsp_evt *evt)
{
	...
	// The old FD is:
	// 	- dup(): fd number of a previously closed fd that has not been
	// removed from the fd_table
	//		     and has been reassigned to the newly created fd by
	// dup()(very rare condition);
	//  - dup2(): fd number of an existing fd that we pass to the dup2() as
	//  the "newfd". dup2()
	//			  will close the existing one. So we need to
	// clean it up / overwrite;
	//  - dup3(): same as dup2().
	//
	...
	//
	// If we are handling the dup3() event exit then we add the flags to the
	// new file descriptor.
	//
	if(evt->get_type() == PPME_SYSCALL_DUP3_X)
	{
	    ...
	    //
	    // We keep the previously flags that has been set on the original
	    // file descriptor and just set/reset O_CLOEXEC flag base on the
	    // value received by dup3() syscall.
	    //
	    if(flags)
	    {
		//
...
void sinsp_parser::parse_getrlimit_setrlimit_exit(sinsp_evt *evt)
{
	...
	if(resource == PPM_RLIMIT_NOFILE)
	{
		...
		{
		    //					ASSERT(curval ==
		    // evt->m_tinfo->get_main_thread()->m_fdlimit);
		}
	    }
#endif
...
void sinsp_parser::parse_fcntl_exit(sinsp_evt *evt)
{
	...
	//
	// Add the new fd to the table.
	// NOTE: dup2 and dup3 accept an existing FD and in that case they close
	// it.
...
std::string generate_error_message(const Json::Value &value, const char *field)
{
    ...
    std::string err_msg = "Unable to convert json value '" + val_as_string +
			  "' for the field: '" + field + "'";

    return err_msg;
}

bool check_int64_json_is_convertible(const Json::Value &value,
				     const char *field)
{
    if(!value.isNull())
    {
...
bool check_json_val_is_convertible(const Json::Value &value,
				   Json::ValueType other, const char *field,
...
void sinsp_parser::parse_container_json_evt(sinsp_evt *evt)
{
	...
	{
	    SINSP_DEBUG("Ignoring container event for already successful "
			"lookup of %s",
			container_id.c_str());
	    evt->m_filtered_out = true;
	    return;
	...
	if(check_json_val_is_convertible(type, Json::uintValue, "type"))
	{
	    container_info->m_type =
		    static_cast<sinsp_container_type>(type.asUInt());
	}
	...
	if(check_json_val_is_convertible(is_pod_sandbox, Json::booleanValue,
					 "is_pod_sandbox"))
	{
					 ...
					 "lookup_state"))
	{
	    ...
	    if(!evt->m_tinfo_ref &&
	       container_info->m_lookup_state ==
		       sinsp_container_lookup_state::STARTED)
	    {
		SINSP_DEBUG("Rewriting lookup_state = STARTED from scap file "
			    "to FAILED for container %s",
	...
	libsinsp::container_engine::docker_async_source::parse_json_mounts(
		container["Mounts"], container_info->m_mounts);
#endif

	sinsp_container_info::container_health_probe::parse_health_probes(
		container, container_info->m_health_probes);
	...
	if(check_json_val_is_convertible(contip, Json::stringValue, "ip"))
	{
		...
		throw sinsp_exception(
			"Invalid 'ip' field while parsing container info: " +
			json);
	    }
	...
	if(check_json_val_is_convertible(port_mappings, Json::arrayValue,
					 "port_mappings"))
	{
		...
		if(check_json_val_is_convertible(host_ip, Json::intValue,
						 "HostIp", true))
		{
		    map.m_host_ip = host_ip.asInt();
		}
		const Json::Value &host_port = port_mappings[i]["HostPort"];
		// We log message for HostPort conversion failure at Warning
		// level
		if(check_json_val_is_convertible(host_port, Json::intValue,
						 "HostPort", true))
		{
		    map.m_host_port = (uint16_t)host_port.asInt();
		}
		const Json::Value &container_port =
			port_mappings[i]["ContainerPort"];
		// We log message for ContainerPort conversion failure at
		// Warning level
		if(check_json_val_is_convertible(container_port, Json::intValue,
						 "ContainerPort", true))
		{
	...
	for(vector<string>::const_iterator it = labels.begin();
	    it != labels.end(); ++it)
	{
	...
	if(check_json_val_is_convertible(cpuset_cpu_count, Json::intValue,
					 "cpuset_cpu_count"))
	{
	...
	if(check_json_val_is_convertible(mesos_task_id, Json::stringValue,
					 "mesos_task_id"))
	{
	...
	if(!metadata_deadline.isNull())
	{
	    ...
	    {
		container_info->m_metadata_deadline =
			metadata_deadline.asUInt64();
	    }
	    else
	    {
		SINSP_DEBUG("Unable to convert json value for field: %s",
			    "metadata_deadline");
	    }
	}
	...
	{
	    SINSP_DEBUG("Filtering container event for failed lookup of %s "
			"(but calling callbacks anyway)",
			container_info->m_id.c_str());
	    evt->m_filtered_out = true;
	}
	evt->m_tinfo_ref = container_info->get_tinfo(m_inspector);
	evt->m_tinfo = evt->m_tinfo_ref.get();
	m_inspector->m_container_manager.add_container(
		container_info, evt->get_thread_info(true));
	/*
	...
	*/
	...
	throw sinsp_exception(
		"Invalid JSON encountered while parsing container info: " +
		json + "error=" + errstr);
    }
}
...
void sinsp_parser::parse_container_evt(sinsp_evt *evt)
{
    ...
    m_inspector->m_container_manager.add_container(container,
						   evt->get_thread_info(true));
}
...
void sinsp_parser::parse_user_evt(sinsp_evt *evt)
{
    ...
    {
	m_inspector->m_usergroup_manager.add_user(container_id, uid, gid, name,
						  home, shell);
    }
    else
    {
...
void sinsp_parser::parse_k8s_evt(sinsp_evt *evt)
{
    ...
    {
	throw sinsp_exception(
		"Inspector is null, K8s client can not be created.");
    }
    ...
    if(m_inspector->m_k8s_client)
    {
	m_inspector->m_k8s_client->simulate_watch_event(std::move(json),
							get_k8s_version(json));
    }
    else
    {
...
void sinsp_parser::parse_getsockopt_exit(sinsp_evt *evt)
{
	...
	err = *(int64_t *)(parinfo->m_val +
			   1); // add 1 byte to skip over PT_DYN param index
