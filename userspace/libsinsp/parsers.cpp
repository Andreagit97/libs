...
void sinsp_parser::store_event(sinsp_evt *evt)
{
	if(evt->m_tinfo == nullptr)
	{
		...
		if (m_inspector->m_inspector_stats)
		{
			m_inspector->m_inspector_stats->m_n_store_evts_drops++;
		}
		return;
	}
	...
	if (m_inspector->m_inspector_stats)
	{
		m_inspector->m_inspector_stats->m_n_stored_evts++;
	}
}
...
bool sinsp_parser::retrieve_enter_event(sinsp_evt *enter_evt, sinsp_evt *exit_evt)
{
	...
	if(!(exit_evt->m_tinfo->is_lastevent_data_valid() && exit_evt->m_tinfo->m_lastevent_data))
	{
		...
		if (m_inspector->m_inspector_stats)
		{
			m_inspector->m_inspector_stats->m_n_retrieve_evts_drops++;
		}
		return false;
	}
		...
		enter_evt->get_type() == PPME_SYSCALL_EXECVEAT_E)
	{
		if (m_inspector->m_inspector_stats)
		{
			m_inspector->m_inspector_stats->m_n_retrieved_evts++;
		}
		return true;
	}
...
void sinsp_parser::parse_close_exit(sinsp_evt *evt)
{
	...
	else
	{
		...
		if (m_inspector->m_inspector_stats)
		{
			m_inspector->m_inspector_stats->m_n_failed_fd_lookups--;
		}
