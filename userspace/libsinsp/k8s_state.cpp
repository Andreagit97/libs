...
k8s_state_t::k8s_state_t(bool is_captured, int capture_version):
	m_is_captured(is_captured), m_capture_version(capture_version)
{
}
...
{
    k8s_pod_t::container_id_list container_ids =
	    k8s_pod_handler::extract_pod_container_ids(item);
    k8s_container::list containers =
	    k8s_pod_handler::extract_pod_containers(item);
...
void k8s_state_t::cache_pod(container_pod_map& map, const std::string& id,
			    const k8s_pod_t* pod)
{
    ASSERT(pod);
    ...
    throw sinsp_exception("Invalid container ID (expected one of: '" +
			  m_docker_prefix + "{ID}', '" + m_rkt_prefix +
			  "{ID}', '" + m_containerd_prefix + "{ID}', '" +
			  m_crio_prefix + "{ID}'): " + id);
}
...
void k8s_state_t::replace_items(k8s_component::type t, const std::string& name,
				const std::vector<k8s_pair_t>&& items)
{
    switch(t)
    {
    ...
    os << "Unknown component type " << static_cast<int>(t) << " or object name "
       << name;
    throw sinsp_exception(os.str().c_str());
}

k8s_component& k8s_state_t::add_common_single_value(
	k8s_component::type component, const std::string& name,
	const std::string& uid, const std::string& ns)
{
    ...
    case k8s_component::K8S_NAMESPACES:
	return get_component<k8s_namespaces, k8s_ns_t>(m_namespaces, name, uid,
						       ns);
    ...
    case k8s_component::K8S_REPLICATIONCONTROLLERS:
	return get_component<k8s_controllers, k8s_rc_t>(m_controllers, name,
							uid, ns);

    case k8s_component::K8S_REPLICASETS:
	return get_component<k8s_replicasets, k8s_rs_t>(m_replicasets, name,
							uid, ns);

    case k8s_component::K8S_SERVICES:
	return get_component<k8s_services, k8s_service_t>(m_services, name, uid,
							  ns);

    case k8s_component::K8S_DAEMONSETS:
	return get_component<k8s_daemonsets, k8s_daemonset_t>(m_daemonsets,
							      name, uid, ns);

    case k8s_component::K8S_DEPLOYMENTS:
	return get_component<k8s_deployments, k8s_deployment_t>(m_deployments,
								name, uid, ns);
...
void k8s_state_t::update_cache(
	const k8s_component::type_map::key_type& component)
{
#ifndef HAS_ANALYZER
    switch(component)
    {
	...
	for(const auto& ns : nspaces)
	{
	    ...
	    {
		g_logger.log("Attempt to cache already cached NAMESPACE: " +
				     ns_name,
	...
	k8s_state_t::container_pod_map& container_pod_map =
		get_container_pod_map();
	...
	for(const auto& pod : pods)
	{
		...
		{
		    g_logger.log("Attempt to cache already cached POD: " + c_id,
				 sinsp_logger::SEV_ERROR);
		}
	    }
	}
	...
	{
	    std::vector<const k8s_pod_t*> pod_subset =
		    rc.get_selected_pods(pods);
		...
		{
		    g_logger.log("Attempt to cache already cached REPLICATION "
				 "CONTROLLER: " +
					 pod_uid,
				 sinsp_logger::SEV_ERROR);
		}
	...
	for(const auto& rs : rss)
	{
		...
		{
		    g_logger.log(
			    "Attempt to cache already cached REPLICA SET: " +
				    pod_uid,
			    sinsp_logger::SEV_ERROR);
		}
	    }
	...
	{
	    std::vector<const k8s_pod_t*> pod_subset =
		    service.get_selected_pods(pods);
	    for(auto& pod : pod_subset)
	    {
		const std::string& pod_uid = pod->get_uid();
		...
		{
		    g_logger.log("Attempt to cache already cached SERVICE: " +
					 pod_uid,
				 sinsp_logger::SEV_ERROR);
		}
	    }
	...
	{
		std::vector<const k8s_pod_t*> pod_subset =
	daemonset.get_selected_pods(pods); for(auto& pod : pod_subset)
		{
			const std::string& pod_uid = pod->get_uid();
			if(!is_component_cached(pod_svc_map, pod_uid,
	&daemonset))
			{
				cache_component(pod_svc_map, pod_uid,
	&daemonset);
			}
			else
			{
				g_logger.log("Attempt to cache already cached
	SERVICE: " + pod_uid, sinsp_logger::SEV_ERROR);
			}
		}
	}*/
    }
	...
	const k8s_pods& pods = get_pods();
	k8s_state_t::pod_deployment_map& pod_deployment_map =
		get_pod_deployment_map();
	pod_deployment_map.clear();
	for(const auto& deployment : deployments)
	{
	    std::vector<const k8s_pod_t*> pod_subset =
		    deployment.get_selected_pods(pods);
	    for(auto& pod : pod_subset)
	    {
		const std::string& pod_uid = pod->get_uid();
		if(!is_component_cached(pod_deployment_map, pod_uid,
					&deployment))
		{
		...
		{
		    g_logger.log(
			    "Attempt to cache already cached Deployment: " +
				    pod_uid,
			    sinsp_logger::SEV_ERROR);
		}
	    }
...
const k8s_component* k8s_state_t::get_component(const std::string& uid,
						std::string* t) const
{
	...
	case k8s_component::K8S_DEPLOYMENTS:
		...
		*t = "deployment";
	    ...
	    return get_component<k8s_deployments, k8s_deployment_t>(
		    m_deployments, uid);
	    break;
	case k8s_component::K8S_EVENTS:
	    if(t)
...
void k8s_state_t::enqueue_capture_event(const Json::Value& item)
{
	...
	else
	{
	    throw sinsp_exception(
		    std::string("K8s : invalid capture version (") +
...
Json::Value k8s_state_t::extract_capture_data(const Json::Value& item)
{
	...
	if(status.isMember("containerStatuses") &&
	   status["containerStatuses"].isArray())
	{
    ...
    os << "Capture: [" << cap_item["type"].asString() << ','
       << cap_item["kind"].asString() << ',' << cap_metadata["name"].asString()
       << ',' << cap_metadata["uid"].asString() << ',' << nspace << ']';
