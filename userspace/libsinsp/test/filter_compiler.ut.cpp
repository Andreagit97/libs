...
class mock_compiler_filter_check : public gen_event_filter_check
{
    ...
    inline void add_filter_value(const char* str, uint32_t l,
				 uint32_t i) override
    {
	m_value = string(str);
    }

    inline bool extract(gen_event* e, OUT vector<extract_value_t>& v,
			bool) override
    {
	return false;
    }
...
class mock_compiler_filter_factory : public gen_event_filter_factory
{
    ...
    inline list<gen_event_filter_factory::filter_fieldclass_info>
    get_fields() override
    {
	return m_list;
    }
...
void test_filter_compile(std::shared_ptr<gen_event_filter_factory> factory,
			 string filter_str, bool expect_fail = false)
{
    sinsp_filter_compiler compiler(factory, filter_str);
    try
    {
	...
	{
	    FAIL() << filter_str
		   << " -> expected failure but compilation was successful";
	}
    }
    catch(const sinsp_exception& e)
    {
...
TEST(sinsp_filter_compiler, boolean_evaluation)
{
    ...
    test_filter_run(false, "not ((c.false=1 or not (c.false=1 and not "
			   "c.true=1)) and c.true=1)");
}
...
{
    std::shared_ptr<gen_event_filter_factory> factory(
	    new mock_compiler_filter_factory());
...
TEST(sinsp_filter_compiler, complex_filter)
{
    ...
    string filter_str =
	    ...
	    "		.bash_logout, .inputrc, .profile, .cshrc, .login, "
	    ".logout,"
	    "		.history, .tcshrc, .cshdirs, .zshenv, .zprofile, "
	    ".zshrc,"
	    "		.zlogin, .zlogout"
	    "	)"
	    "	or fd.name in (/etc/profile, /etc/bashrc, /etc/csh.cshrc, "
	    "/etc/csh.login)"
	    ...
	    "and not ("
	    "	proc.name = exe"
	    "	and (proc.cmdline contains \"/var/lib/docker\" or proc.cmdline "
	    "contains '/var/run/docker')"
	    "	and proc.pname in (dockerd, docker, dockerd-current, "
	    "docker-current)"
	    ")";
