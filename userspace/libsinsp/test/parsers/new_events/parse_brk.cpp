...
TEST_F(sinsp_with_test_input, parse_brk_updated_prog_break) {
	...
	ASSERT_EQ(get_field_as_string(evt, "evt.arg[4]"), "5017AF4");
	ASSERT_EQ(get_field_as_string(evt, "evt.rawarg.addr"), "5017AF4");
	// todo!: this is known issue because due to the longest prefix match we think that `addr` is a
	// SOCKADDR
