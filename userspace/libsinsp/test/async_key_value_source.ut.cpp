...
class test_key_value_source
	: public libsinsp::async_key_value_source<std::string, uint64_t>
{
    public:
    test_key_value_source(uint64_t delay_sec, uint64_t wait_response_ms):
	    async_key_value_source<std::string, uint64_t>(wait_response_ms,
							  UINT64_MAX),
...
{
    std::unique_ptr<test_key_value_source> t(
	    new test_key_value_source(0, UINT64_MAX));
}
...
{
    std::unique_ptr<test_key_value_source> t(
	    new test_key_value_source(5, UINT64_MAX));
    uint64_t value;
