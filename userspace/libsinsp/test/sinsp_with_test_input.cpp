...
scap_evt* sinsp_with_test_input::create_event_v(uint64_t ts,
	...
	if(ret != SCAP_INPUT_TOO_SMALL) {
		va_end(args2);
		throw std::runtime_error(std::string("could not compute the size of the event: ") + error);
	}
	...
	if(ret != SCAP_SUCCESS) {
		...
		event_buf.size = 0;
		va_end(args2);
		throw std::runtime_error(std::string("could not encode the event: ") + error);
	}
...
scap_evt* sinsp_with_test_input::add_event_v(uint64_t ts,
	...
	if(event == nullptr) {
		std::stringstream ss;
		ss << "could not create event type: " << event_type << ", ts: " << ts << ", tid: " << tid
		   << ", n: " << n;
