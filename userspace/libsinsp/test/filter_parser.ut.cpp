...
TEST(parser, supported_operators)
{
    static vector<string> expected_all = {
	    "=",	 "==",	       "!=",	      "<=",	    ">=",
	    "<",	 ">",	       "exists",      "contains",   "icontains",
	    "bcontains", "glob",       "bstartswith", "startswith", "endswith",
...
TEST(parser, parse_str)
{
    ...
    test_accept("test.str = \"mixed '\"");
    test_accept(
	    "test.str = \"bad escape \\ \" "); // todo(jasondellaluce): reject
...
{
    test_equal_ast(
	    "evt.name exists and evt.type in (a, b) and not evt.dir=< or "
	    "proc.name=cat",
	    new or_expr({
		    new and_expr({
			    new unary_check_expr("evt.name", "", "exists"),
			    new binary_check_expr("evt.type", "", "in",
						  new list_expr({"a", "b"})),
			    new not_expr(new binary_check_expr(
				    "evt.dir", "", "=", new value_expr("<"))),
		    }),
		    new binary_check_expr("proc.name", "", "=",
					  new value_expr("cat")),
	    }));
}
...
TEST(parser, expr_multi_negation)
{
    ...
    test_equal_ast("not not not not not(not not(not not_macro))",
