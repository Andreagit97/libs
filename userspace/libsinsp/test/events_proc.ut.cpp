...
TEST_F(sinsp_with_test_input, pid_over_32bit)
{
	...
	/// todo(@Andreagit97)
	/* This test is not clear.. why is child_pid different from child_vpid? Before this changes, this test worked because the child parser
	 * overwrite `child_vpid = 0x0000000100000010` written by the parent with `child_vpid = 2` but not sure why this is a correct behavior (?)
	 * ask @LucaGuerra
	 */
