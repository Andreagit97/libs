...
sinsp_ipv4_ifinfo make_ipv4_interface(const char *addr, const char *netmask,
				      const char *broadcast, const char *name)
{
    return sinsp_ipv4_ifinfo(parse_ipv4_addr(addr), parse_ipv4_netmask(netmask),
			     parse_ipv4_broadcast(broadcast), name);
}
...
TEST(sinsp_network_interfaces, infer_finds_exact_match)
{
    ...
    interfaces.m_ipv4_interfaces.push_back(make_ipv4_interface(
	    "192.168.22.149", "255.255.255.0", "192.168.22.255", "eth0"));
    EXPECT_ADDR_EQ("127.0.0.1",
		   interfaces.infer_ipv4_address(parse_ipv4_addr("127.0.0.1")));
    EXPECT_ADDR_EQ(
	    "192.168.22.149",
...
TEST(sinsp_network_interfaces, infer_finds_same_subnet)
{
    ...
    interfaces.m_ipv4_interfaces.push_back(make_ipv4_interface(
	    "192.168.22.149", "255.255.255.0", "192.168.22.255", "eth0"));
    EXPECT_ADDR_EQ("192.168.22.149", interfaces.infer_ipv4_address(
					     parse_ipv4_addr("192.168.22.11")));
}
