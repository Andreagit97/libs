...
typedef struct table_row_cmp
{
	...
	if(src.m_values[m_colid].m_cnt > 1 || dst.m_values[m_colid].m_cnt > 1)
	{
	    return flt_compare_avg(
		    op, m_type, src.m_values[m_colid].m_val,
		    dst.m_values[m_colid].m_val, src.m_values[m_colid].m_len,
		    dst.m_values[m_colid].m_len, src.m_values[m_colid].m_cnt,
		    dst.m_values[m_colid].m_cnt);
	}
	else
	{
	    return flt_compare(op, m_type, src.m_values[m_colid].m_val,
...
void sinsp_table::configure(vector<sinsp_view_column_info>* entries,
			    const string& filter, bool use_defaults,
			    uint32_t view_depth)
{
    ...
    //
    // If this is a list table, increase the refresh time to improve
    // realtimyiness
    //
    ...
    {
	sinsp_filter_check* chk = g_filterlist.new_filter_check_from_fldname(
		vit.get_field(m_view_depth), m_inspector, false);

	if(chk == NULL)
	{
	    throw sinsp_exception("invalid field name " +
				  vit.get_field(m_view_depth));
	}
	...
	if((vit.m_flags & TEF_IS_KEY) != 0)
	{
		...
		throw sinsp_exception(
			"invalid table configuration: multiple keys specified");
	    }
    ...
    m_premerge_fld_pointers =
	    new sinsp_table_field[m_premerge_extractors.size()];
    ...
    for(auto it = m_premerge_extractors.begin();
	it != m_premerge_extractors.end(); ++it)
    {
    ...
    {
	throw sinsp_exception(
		"invalid table definition: multiple groupby keys");
    }
	...
	chk->m_merge_aggregation =
		(sinsp_field_aggregation)vit.m_groupby_aggregation;

	if((vit.m_flags & TEF_IS_GROUPBY_KEY) != 0)
	{
	    if(m_is_groupby_key_present)
	    {
		throw sinsp_exception("invalid table configuration: more than "
				      "one groupby key specified");
	    }
    ...
    m_postmerge_fld_pointers =
	    new sinsp_table_field[m_postmerge_extractors.size()];
    ...
    for(auto it = m_postmerge_extractors.begin();
	it != m_postmerge_extractors.end(); ++it)
    {
...
void sinsp_table::add_row(bool merging)
{
	...
	else
	{
		...
		if(merging)
		{
		    add_fields(j, &m_fld_pointers[j],
			       m_postmerge_extractors[j]->m_merge_aggregation);
		}
		else
		{
		    add_fields(j, &m_fld_pointers[j],
			       m_premerge_extractors[j]->m_aggregation);
		}
	    }
	}
    }
...
void sinsp_table::process_event(sinsp_evt* evt)
{
	...
	if(!m_premerge_extractors[j]->extract(
		   evt, m_premerge_extractors[j]->m_extracted_values))
	{
	    if(m_use_defaults)
	    {
	...
	{
	    // todo: Do something better here. For now, only support
	    // single-value extracted fields
	    pfld->m_val = m_premerge_extractors[j]->m_extracted_values[0].ptr;
	    pfld->m_len = get_field_len(j);
	    pfld->m_val = m_buffer->copy(
		    m_premerge_extractors[j]->m_extracted_values[0].ptr,
		    pfld->m_len);
	    pfld->m_cnt = 1;
	}
    }
...
void sinsp_table::process_proctable(sinsp_evt* evt)
{
    ...
    threadinfo_map_t* threadtable =
	    m_inspector->m_thread_manager->get_threads();
    ...
    //       numbers, so we guarantee that filter checks will not wrongly pick
    //       it up
    //
...
void sinsp_table::flush(sinsp_evt* evt)
{
	...
	if(m_next_flush_time_ns != 0)
	{
	    ...
	    //
	    // If there is a merging step, switch the types to point to the
	    // merging ones.
	    //
	    if(m_do_merging)
	    {
	    ...
	    {
		//
		// Switch the data storage so that the current one is still
		// usable by the consumers of the table.
		//
		switch_buffers();
    ...
    m_next_flush_time_ns =
	    ts - (ts % m_refresh_interval_ns) + m_refresh_interval_ns;

    return;
}

void sinsp_table::print_raw(vector<sinsp_sample_row>* sample_data,
			    uint64_t time_delta)
{
	...
	for(uint32_t j = 0; j < m_n_fields - 1; j++)
	{
	    ...
	    m_printer->set_val(m_types->at(j + 1), it->m_values[j].m_val,
			       it->m_values[j].m_len, it->m_values[j].m_cnt,
...
void sinsp_table::print_json(vector<sinsp_sample_row>* sample_data,
			     uint64_t time_delta)
{
	...
	for(uint32_t j = 0; j < m_n_fields - 1; j++)
	{
	    ...
	    m_printer->set_val(m_types->at(j + 1), row.m_values[j].m_val,
			       row.m_values[j].m_len, row.m_values[j].m_cnt,
			       legend->at(j + 1).m_print_format);
...
void sinsp_table::filter_sample()
{
	...
	for(uint32_t j = 0; j < it.m_values.size(); j++)
	{
	    ...
	    if(type == PT_CHARBUF || type == PT_BYTEBUF ||
	       type == PT_SYSCALLID || type == PT_PORT || type == PT_L4PROTO ||
	       type == PT_SOCKFAMILY || type == PT_IPV4ADDR ||
	       type == PT_IPV6ADDR || type == PT_UID || type == PT_GID)
	    {
		m_printer->set_val(type, it.m_values[j].m_val,
				   it.m_values[j].m_len, it.m_values[j].m_cnt,
				   legend->at(j + 1).m_print_format);
...
sinsp_table_field* sinsp_table::search_in_sample(string text)
{
    ...
    for(auto it = m_full_sample_data.begin(); it != m_full_sample_data.end();
	++it)
    {
	for(uint32_t j = 0; j < it->m_values.size(); j++)
	{
...
void sinsp_table::sort_sample()
{
	...
	sort(m_sample_data->begin(), m_sample_data->end(), cc);
    }
}
...
uint32_t sinsp_table::get_sorting_col() { return (uint32_t)m_sorting_col + 1; }
...
void sinsp_table::create_sample()
{
	...
	if(m_do_merging)
	{
	    ...
	    for(auto it = m_premerge_table.begin();
		it != m_premerge_table.end(); ++it)
	    {
    ...
    {
	//
	// If this is a list, there's nothing to be done, since
	// m_full_sample_data is already prepared and doesn't need to be
	// cleaned.
	//
	return;
    }
...
void sinsp_table::add_fields_sum(ppm_param_type type, sinsp_table_field* dst,
				 sinsp_table_field* src)
{
...
void sinsp_table::add_fields_sum_of_avg(ppm_param_type type,
					sinsp_table_field* dst,
					sinsp_table_field* src)
{
    uint8_t* operand1 = dst->m_val;
    uint8_t* operand2 = src->m_val;
...
void sinsp_table::add_fields_max(ppm_param_type type, sinsp_table_field* dst,
				 sinsp_table_field* src)
{
    uint8_t* operand1 = dst->m_val;
    uint8_t* operand2 = src->m_val;
...
void sinsp_table::add_fields_min(ppm_param_type type, sinsp_table_field* dst,
				 sinsp_table_field* src)
{
    uint8_t* operand1 = dst->m_val;
    uint8_t* operand2 = src->m_val;
...
pair<filtercheck_field_info*, string>
sinsp_table::get_row_key_name_and_val(uint32_t rownum, bool force)
{
	...
	if(force)
	{
	    res.first = (filtercheck_field_info*)((*extractors)[0])
				->get_field_info();
	...
	vector<filtercheck_field_info>* legend = get_legend();
	res.first =
		(filtercheck_field_info*)((*extractors)[0])->get_field_info();
	ASSERT(res.first != NULL);

	m_printer->set_val(types->at(0), m_sample_data->at(rownum).m_key.m_val,
...
void sinsp_table::set_paused(bool paused) { m_paused = paused; }
