...
class mesos_state_t
{
    ...
    const capture_list& get_capture_events() const { return m_capture; }
    ...
    bool is_captured() const { return m_is_captured; }

    void capture_groups(const Json::Value& root,
			const std::string& framework_id, Json::Value& capt,
			bool capture_fw = false);
    ...
    get_framework(const std::string& framework_uid) const;
    ...
    const mesos_framework*
    get_framework_for_task(const std::string& task_id) const;
    ...
    get_tasks(const std::string& framework_uid) const;
    ...
    void add_or_replace_task(mesos_framework& framework,
			     mesos_task::ptr_t task);
    ...
    void add_task_to_app(marathon_group::app_ptr_t app,
    ...
    add_or_replace_group(marathon_group::ptr_t group,
			 marathon_group::ptr_t to_group = 0);
    ...
    private:
    marathon_group::ptr_t add_group(const Json::Value& group,
				    marathon_group::ptr_t to_group,
				    const std::string& framework_id);
    bool handle_groups(const Json::Value& groups,
		       marathon_group::ptr_t p_groups,
		       const std::string& framework_id);
    ...
    typedef std::unordered_map<std::string, mesos_framework*>
	    task_framework_map_t;
...
inline const mesos_framework&
mesos_state_t::get_framework(const std::string& framework_uid) const
{
...
inline mesos_framework&
mesos_state_t::get_framework(const std::string& framework_uid)
{
...
{
    for(mesos_frameworks::iterator it = m_frameworks.begin();
	it != m_frameworks.end();)
    {
...
inline const mesos_framework*
mesos_state_t::get_framework_for_task(const std::string& task_id) const
{
    task_framework_map_t::const_iterator it =
	    m_task_framework_cache.find(task_id);
...
// tasks
//
inline void mesos_state_t::add_or_replace_task(mesos_framework& framework,
					       mesos_task::ptr_t task)
{
    if(task)
    {
...
inline void mesos_state_t::remove_task(mesos_framework& framework,
				       const std::string& uid)
{
	...
	if(!app_id.empty())
	{
		...
		{
		    std::string errstr = "Task [" + uid +
					 "] not found in Marathon app [" +
					 app_id + ']';
		    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
		    g_json_error_log.log(uid, errstr,
	    ...
	    {
		std::string errstr =
			"Group not found for Marathon app [" + app_id +
			"] while trying to remove task [" + uid + ']';
		g_logger.log(errstr, sinsp_logger::SEV_ERROR);
		g_json_error_log.log(app_id, errstr,
				     sinsp_utils::get_current_time_ns(),
				     "remove-task");
	    }
	}
	...
	{
	    g_logger.log("Task [" + uid + "] has no Marathon app ID.",
			 sinsp_logger::SEV_WARNING);
	}
    }
    else
    {
	g_logger.log("Task [" + uid + "] not found in framework [" +
			     framework.get_uid() + ']',
		     sinsp_logger::SEV_WARNING);
    }
...
inline mesos_slaves& mesos_state_t::get_slaves() { return m_slaves; }

inline const mesos_slave&
mesos_state_t::get_slave(const std::string& slave_uid) const
{
...
inline marathon_groups& mesos_state_t::get_groups() { return m_groups; }
...
inline void mesos_state_t::clear_marathon() { m_groups.clear(); }
