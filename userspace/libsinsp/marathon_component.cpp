	...
	m_type(t), m_id(id)
{
    ...
    if(it == list.end())
    {
	throw sinsp_exception("Invalid Marathon component type: " +
			      std::to_string(t));
    }
...
marathon_component::marathon_component(const marathon_component& other):
	m_type(other.m_type), m_id(other.m_id)
{
}

marathon_component::marathon_component(marathon_component&& other):
...
void marathon_component::cache_task_app(const std::string& task_id,
					app_ptr_t app)
{
...
marathon_component::app_ptr_t
marathon_component::get_cached_app(const std::string& task_id)
{
...
void marathon_app::add_task(mesos_framework::task_ptr_t ptask)
{
    ...
    {
	g_logger.log("Attempt to add null task to app [" + get_id() + ']',
...
bool marathon_app::remove_task(const std::string& task_id)
{
    ...
    g_logger.log("Task [" + task_id + "] not found in app [" + get_id() + ']',
		 sinsp_logger::SEV_WARNING);
    return false;
}
