...
class cgroup_list_counter
{
	...
	try
	{
		...
		if('-' == *position)
		{
			...
			throw std::runtime_error(
				"duplicate range indicator before start");
		    }
		...
		{
		    cpu_count += process_section(m_section_start, position,
						 m_range_indicator);
		    reset();
		}
	...
	catch(const std::exception &ex)
	{
	    g_logger.format(sinsp_logger::SEV_ERROR,
			    "Invalid List Format: %s. Detail: %s", buffer,
			    ex.what());
    ...
    private:
    static int process_number(const char *section_start,
	...
	{
	    throw std::runtime_error(
		    "invalid end of section before start of section");
	}
	...
	// We don't care what the value is, we just want to know that it is a
	// number
