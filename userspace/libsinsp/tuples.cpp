...
/*
 *  a few line below to enforce proper memory layout for ipv6addr class during
 * compile time
 */
template<bool T> class mem_layout_test;
template<> class mem_layout_test<true>
{
}; // empty struct take 1 byte of mem

static auto check_size = mem_layout_test<sizeof(ipv6addr) == 16>();
// is_pod check split into is_standard_layout && is_trivial to be C++20 future
// proof
static auto check_layout = mem_layout_test <
				   std::is_standard_layout<ipv6addr>::value &&
			   std::is_trivial<ipv6addr>::value > ();
// end layout checks

ipv6addr ipv6addr::empty_address(
...
bool ipv6addr::operator==(const ipv6addr &other) const
{
    return (m_b[0] == other.m_b[0] && m_b[1] == other.m_b[1] &&
	    m_b[2] == other.m_b[2] && m_b[3] == other.m_b[3]);
}
...
bool ipv6net::in_cidr(const ipv6addr &other) const
{
    ...
    return (this_bytes[i] >> m_mask_tail_bits) ==
	   (other_bytes[i] >> m_mask_tail_bits);
}
