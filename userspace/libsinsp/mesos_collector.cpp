...
mesos_collector::mesos_collector(bool do_loop, long timeout_ms):
	m_nfds(0), m_loop(do_loop), m_timeout_ms(timeout_ms), m_stopped(false)
{
    clear();
}
...
mesos_collector::socket_map_t::iterator&
mesos_collector::remove(socket_map_t::iterator& it)
{
    if(it != m_sockets.end())
    {
	m_sockets.erase(it++);
...
bool mesos_collector::is_active() const { return subscription_count() > 0; }
...
int mesos_collector::subscription_count() const { return m_sockets.size(); }
...
void mesos_collector::get_data()
{
	...
	while(!m_stopped)
	{
		...
		if(m_sockets.size())
		{
		    g_logger.log("Mesos collector number of sockets: " +
		    ...
		    {
			std::string err =
				std::string("Mesos collector select error, "
					    "removing all sockets (") +
				strerror(errno) + ")";
			g_logger.log(err, sinsp_logger::SEV_ERROR);
			g_json_error_log.log("", err,
					     sinsp_utils::get_current_time_ns(),
					     "mesos-collector-get-data");
			remove_all();
		    }
		    else // data or idle
		    {
			for(socket_map_t::iterator it = m_sockets.begin();
			    it != m_sockets.end();)
			{
				...
				if(it->second && !it->second->on_data())
				{
				    ...
				    {
					std::string fid =
						it->second->get_framework_id();
					if(!fid.empty())
					{
					    std::string errstr =
						    "Mesos collector data "
						    "handling error, removing "
						    "Marathon socket for "
						    "framework [" +
						    fid + ']';
					    g_logger.log(
				...
				if(errno != EAGAIN)
				{
				    ...
				    {
					std::string errstr =
						"Mesos collector socket error, "
						"removing Marathon socket for "
						"framework [" +
						fid + ']';
					g_logger.log(errstr,
						     sinsp_logger::SEV_ERROR);
