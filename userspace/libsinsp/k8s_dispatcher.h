...
class k8s_dispatcher
{
    ...
    k8s_dispatcher(k8s_component::type t, k8s_state_t& state,
    ...
    template<typename T>
    static void handle_labels(T& component, const Json::Value& metadata,
			      const std::string& name)
    {
	if(!metadata.isNull())
	{
	    k8s_pair_list entries =
		    k8s_component::extract_object(metadata, name);
	    component.set_labels(std::move(entries));
	}
	else
	{
	    g_logger.log("Null metadata object received",
			 sinsp_logger::SEV_ERROR);
	}
    }

    // clears the content of selectors and fills it with new values, if any;
    // the selector location depth in JSON tree is detected and handled
    // accordingly
    template<typename T>
	...
	if(!spec.isNull())
	{
		...
		k8s_pair_list selectors =
			match_labels.isNull()
				? k8s_component::extract_object(spec,
								"selector")
				: k8s_component::extract_object(selector,
								"matchLabels");
		component.set_selectors(std::move(selectors));
	    }
	    else
	    {
		g_logger.log("K8s: Null selector object.",
			     sinsp_logger::SEV_ERROR);
	    }
	}
	else
    ...
    template<typename T>
    void handle_rc(const Json::Value& root, const msg_data& data, T& cont,
	...
	if(data.m_reason == COMPONENT_ADDED)
	{
		...
		os << "ADDED message received for existing " << comp_name << '['
		   << data.m_uid << "], updating only.";
		g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	    }
	    comp_t& rc = m_state.get_component<T, comp_t>(
		    cont, data.m_name, data.m_uid, data.m_namespace);
	    ...
	    {
		g_logger.log("K8s: object is null for " + comp_name + ' ' +
	...
	else if(data.m_reason == COMPONENT_MODIFIED)
	{
	    ...
	    {
		std::ostringstream os;
		os << "MODIFIED message received for non-existing " << comp_name
	...
	else if(data.m_reason == COMPONENT_DELETED)
	{
	    ...
	    {
		g_logger.log("K8s: " + comp_name + " not found: " + data.m_name,
			     sinsp_logger::SEV_ERROR);
	    }
	}
	...
	{
	    g_logger.log(std::string("Unsupported K8S " + comp_name +
				     " event reason: ") +
...
inline void k8s_dispatcher::remove() { m_messages.pop_front(); }
