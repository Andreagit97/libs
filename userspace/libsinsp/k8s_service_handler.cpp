...
k8s_service_handler::k8s_service_handler(k8s_state_t& state
					 ...
					 bool connect, bool blocking_socket
#endif // HAS_CAPTURE
					 ):
	k8s_handler("k8s_service_handler", true,
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		    url, "/api/v1/services", STATE_FILTER, EVENT_FILTER,
		    NULL_FILTER, collector, http_version, 1000L, ssl, bt, true,
...
void k8s_service_handler::extract_services_data(const Json::Value& json,
						...
						const k8s_pods& pods)
{
	...
	if(!ports.isNull() && ports.isArray())
	{
		...
		if(!json_target_port.isNull())
		{
			...
			{
			    const k8s_container::list& containers =
				    pod->get_containers();
			    for(const auto& container : containers)
			    {
				const k8s_container::port* container_port =
					container.get_port(port_name);
				if(container_port)
				{
				    g_logger.log(
					    "K8s: found port for service [" +
						    service.get_name() +
						    "], "
						    "container [" +
						    container.get_name() + ']',
		    ...
		    {
			g_logger.log("Port of unknown or unsupported type.",
				     sinsp_logger::SEV_ERROR);
			p.m_target_port = 0;
		    }
		}
    ...
    {
	g_logger.log("Error while extracting data for service [" +
			     service.get_name() +
			     "]: "
			     " JSON is null.",
		     sinsp_logger::SEV_ERROR);
...
bool k8s_service_handler::handle_component(const Json::Value& json,
					   const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
		...
		k8s_service_t& service =
			m_state->get_component<k8s_services, k8s_service_t>(
				m_state->get_services(), data->m_name,
	    ...
	    {
		if(!m_state->delete_component(m_state->get_services(),
					      data->m_uid))
		{
		    log_not_found(*data);
		    return false;
