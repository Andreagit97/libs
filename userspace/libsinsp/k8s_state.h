...
class k8s_state_t
{
    ...
    typedef std::unordered_multimap<std::string, const k8s_service_t*>
	    pod_service_map;
    ...
    typedef std::unordered_multimap<std::string, const k8s_deployment_t*>
	    pod_deployment_map;
    ...
    k8s_state_t(bool is_captured = false,
		int capture_version = CAPTURE_VERSION_2);
    ...
    void replace_items(k8s_component::type t, const std::string& name,
		       const std::vector<k8s_pair_t>&& items);
    k8s_component& add_common_single_value(k8s_component::type component,
    ...
    template<typename C, typename T>
    T& add_component(C& container, const std::string& name,
    ...
    template<typename C, typename T>
    T& get_component(C& container, const std::string& name,
		     const std::string& uid, const std::string& ns = "")
    {
    ...
    const k8s_component* get_component(const std::string& uid,
				       std::string* t = 0) const;
    ...
    const container_pod_map& get_container_pod_map() const
    {
	return m_container_pods;
    }
    const pod_service_map& get_pod_service_map() const
    {
	return m_pod_services;
    }
    const pod_rc_map& get_pod_rc_map() const { return m_pod_rcs; }
    const pod_rs_map& get_pod_rs_map() const { return m_pod_rss; }
    const pod_deployment_map& get_pod_deployment_map() const
    {
	return m_pod_deployments;
    }
    ...
    template<typename C>
    const typename C::mapped_type* get_component(const C& map,
    ...
    bool is_component_cached(const C& map, const std::string& key,
			     const typename C::mapped_type value) const
    {
    ...
    void cache_pod(container_pod_map& map, const std::string& id,
		   const k8s_pod_t* pod);

    template<typename C>
    void cache_component(C& map, const std::string& key,
			 typename C::mapped_type component)
    {
	ASSERT(component);
    ...
    template<typename C> void uncache_component(C& map, const std::string& key)
    {
    ...
    typedef std::unordered_map<std::string, k8s_component::type>
	    component_map_t;
...
inline k8s_namespaces& k8s_state_t::get_namespaces() { return m_namespaces; }
...
// nodes
inline const k8s_nodes& k8s_state_t::get_nodes() const { return m_nodes; }

inline k8s_nodes& k8s_state_t::get_nodes() { return m_nodes; }
...
// pods
inline const k8s_pods& k8s_state_t::get_pods() const { return m_pods; }

inline k8s_pods& k8s_state_t::get_pods() { return m_pods; }
...
inline const k8s_pod_t::container_id_list&
k8s_state_t::get_pod_container_ids(k8s_pod_t& pod)
{
...
inline k8s_controllers& k8s_state_t::get_rcs() { return m_controllers; }
...
inline k8s_replicasets& k8s_state_t::get_rss() { return m_replicasets; }
...
inline k8s_services& k8s_state_t::get_services() { return m_services; }
...
inline k8s_daemonsets& k8s_state_t::get_daemonsets() { return m_daemonsets; }
...
inline k8s_deployments& k8s_state_t::get_deployments() { return m_deployments; }
...
// events
inline const k8s_events& k8s_state_t::get_events() const { return m_events; }

inline k8s_events& k8s_state_t::get_events() { return m_events; }
...
inline void
k8s_state_t::set_last_pod_internal_ip(const std::string& internal_ip)
{
    if(m_pods.size())
    {
...
{
    if(version != CAPTURE_VERSION_NONE && version != CAPTURE_VERSION_1 &&
