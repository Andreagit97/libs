...
/**
 * This api defines a relationship between libsinsp and an external event
 * processor. Such external processors should derive from event_processor and
 * register themselves with sinsp in order to start receiving appropriate
 * callbacks.
 */
...
class event_processor
{
     ...
     *
     * All event_processors should register with the sinsp before staring the
     * capture. If that is not done, this function may never be called on the
     * processor.
     */
     ...
     * If this is overridden by the event processor, the processor MUST be
     * registered before the sinsp object is init-ed
     */
