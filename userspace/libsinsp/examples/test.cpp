...
#define PROCESS_DEFAULTS                                                       \
    "*%evt.num %evt.time %evt.category %container.id %proc.ppid %proc.pid "    \
    "%evt.type %proc.exe %proc.cmdline %evt.args"
...
//   "evt.dir=< and (evt.category=net or (evt.type=execveat or evt.type=execve
//   or evt.type=clone or evt.type=fork or evt.type=vfork))"
//
int main(int argc, char** argv)
{
    ...
    while((op = getopt_long(argc, argv, "hr:s:f:ja", long_options,
			    &long_index)) != -1)
    {
	switch(op)
	{
	...
	case 'j':
	    ...
	    default_formatter =
		    new sinsp_evt_formatter(&inspector, DEFAULT_OUTPUT_STR);
	    process_formatter =
		    new sinsp_evt_formatter(&inspector, PROCESS_DEFAULTS);
	    net_formatter = new sinsp_evt_formatter(&inspector, PROCESS_DEFAULTS
						    " %fd.name");
...
void plaintext_dump(sinsp& inspector)
{
	...
	if(g_all_threads || thread->is_main_thread())
	{
		...
		if(nullptr != fd_info &&
		   (fd_info->get_l4proto() != SCAP_L4_UNKNOWN &&
		    fd_info->get_l4proto() != SCAP_L4_NA))
		{
		    cout << "[" << fd_info->tostring() << "]:";
		}
	    }
	    else if(ev->get_category() == EC_IO_READ ||
...
{
    // Initialize JSON formatters
    static sinsp_evt_formatter* default_formatter =
	    new sinsp_evt_formatter(&inspector, DEFAULT_OUTPUT_STR);
    static sinsp_evt_formatter* process_formatter =
	    new sinsp_evt_formatter(&inspector, PROCESS_DEFAULTS);
    static sinsp_evt_formatter* net_formatter =
	    new sinsp_evt_formatter(&inspector, PROCESS_DEFAULTS " %fd.name");

    sinsp_evt* ev = get_event(
	    inspector, [](const std::string& error_msg)
