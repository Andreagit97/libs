...
{
    public:
    using map_ptr_t = libsinsp::ConstMutexGuard<
	    std::unordered_map<std::string, sinsp_container_info::ptr_t>>;

    /**
     * Due to how the container manager is architected, it makes it difficult
     * to dynamically select which engines we want to be valid. As such, we
     * unfortunately need to indicate at construction time which we want, with
     * the only possible options right now being "static" or not. I'm sure we
     * will find time in the future to do this in a more general way. 2020/11/24
     */
    sinsp_container_manager(sinsp* inspector, bool static_container = false,
    ...
    /**
     * @brief Add/update a container in the manager map, executing
     * on_new_container callbacks
     *
     ...
     */
    void replace_container(
	    const sinsp_container_info::ptr_t& container_info) override;
    ...
    get_container(const std::string& id) const override;
     ...
     * 		false otherwise
     *
     * Note: a return value of false doesn't mean that container detection
     * failed, it may still be happening in the background asynchronously
     */
    bool resolve_container(sinsp_threadinfo* tinfo,
    ...
    typedef std::function<void(const sinsp_container_info&, sinsp_threadinfo*)>
	    new_container_cb;
    typedef std::function<void(const sinsp_container_info&)>
	    remove_container_cb;
     ...
     *
     * Container engines that do not do any lookups in external services need
     * not bother with this. Otherwise, the engine needs to maintain the current
    ...
    void set_lookup_status(const std::string& container_id,
			   ...
			   sinsp_container_lookup_state state) override
    {
    ...
    bool should_lookup(const std::string& container_id,
		       sinsp_container_type ctype) override
    {
    ...
    bool container_to_sinsp_event(const std::string& json, sinsp_evt* evt,
				  std::shared_ptr<sinsp_threadinfo> tinfo);
    std::string get_docker_env(const Json::Value& env_vars,
			       const std::string& mti);
    ...
    sinsp* m_inspector;
    libsinsp::Mutex<std::unordered_map<
	    std::string, std::shared_ptr<const sinsp_container_info>>>
	    m_containers;
    std::unordered_map<std::string,
		       std::unordered_map<sinsp_container_type,
					  sinsp_container_lookup_state>>
	    m_lookups;
    uint64_t m_last_flush_time_ns;
    std::list<new_container_cb> m_new_callbacks;
    std::list<remove_container_cb> m_remove_callbacks;

    // indicates whether we should use only the static container engine, or the
    // other engines. if true, we expect to have the subsequent bits of metadata
    // as well. If this bool is false, then the values of those metadata are
    // undefined
