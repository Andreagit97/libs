...
// k8s_component.h
//
// kubernetes components (nodes, namespaces, pods, replication controllers,
...
class k8s_component
{
	...
	bool is_valid() const { return m_reason != COMPONENT_UNKNOWN; }
    ...
    k8s_component(type comp_type, const std::string& name,
    ...
    static k8s_pair_list extract_object(const Json::Value& object,
    ...
    bool selector_in_labels(const k8s_pair_t& selector,
...
class k8s_ns_t : public k8s_component
{
    ...
    k8s_ns_t(const std::string& name, const std::string& uid,
	     const std::string& ns = "");
};
...
class k8s_node_t : public k8s_component
{
    ...
    k8s_node_t(const std::string& name, const std::string& uid,
	       const std::string& ns = "");
...
class k8s_pod_t : public k8s_component
{
    ...
    k8s_pod_t(const std::string& name, const std::string& uid,
	      const std::string& ns = "");
...
class k8s_replicas_t
{
    ...
    k8s_replicas_t(int spec_replicas = UNKNOWN_REPLICAS,
    ...
    static int get_count(const Json::Value& item,
			 const std::string& replica_name = "replicas");
...
{
    public:
    static const k8s_component::type COMPONENT_TYPE =
	    K8S_REPLICATIONCONTROLLERS;

    k8s_rc_t(const std::string& name, const std::string& uid,
	     const std::string& ns = "",
	     k8s_component::type type = K8S_REPLICATIONCONTROLLERS);

    std::vector<const k8s_pod_t*>
    get_selected_pods(const std::vector<k8s_pod_t>& pods) const;
...
class k8s_rs_t : public k8s_rc_t
{
    ...
    k8s_rs_t(const std::string& name, const std::string& uid,
	     const std::string& ns = "");
...
class k8s_service_t : public k8s_component
{
    ...
    k8s_service_t(const std::string& name, const std::string& uid,
		  const std::string& ns = "");
...
class k8s_daemonset_t : public k8s_component
{
    ...
    k8s_daemonset_t(const std::string& name, const std::string& uid,
		    const std::string& ns = "");
...
class k8s_deployment_t : public k8s_component
{
    ...
    k8s_deployment_t(const std::string& name, const std::string& uid,
		     const std::string& ns = "");
...
class k8s_event_t : public k8s_component
{
    ...
    k8s_event_t(const std::string& name, const std::string& uid,
		const std::string& ns);
    ...
    void make_scope_impl(const Json::Value& obj, std::string comp,
			 event_scope& scope, bool ns = true);
...
inline const std::string& k8s_container::get_name() const { return m_name; }

inline void k8s_container::set_name(const std::string& name) { m_name = name; }
...
inline void k8s_container::port::set_port(uint32_t port) { m_port = port; }

inline uint32_t k8s_container::port::get_port() const { return m_port; }
...
inline bool k8s_container::port::operator==(const port& other) const
{
    ...
    return other.m_name == m_name && other.m_port == m_port &&
...
inline const std::string& k8s_component::get_name() const { return m_name; }

inline void k8s_component::set_name(const std::string& name) { m_name = name; }

inline const std::string& k8s_component::get_uid() const { return m_uid; }

inline void k8s_component::set_uid(const std::string& uid) { m_uid = uid; }

inline const std::string& k8s_component::get_namespace() const { return m_ns; }

inline void k8s_component::set_namespace(const std::string& ns) { m_ns = ns; }
...
inline std::string k8s_component::get_node_name() const { return ""; }
...
inline std::string k8s_node_t::get_node_name() const { return get_name(); }
...
{
    m_container_ids.insert(m_container_ids.end(), container_ids.begin(),
			   container_ids.end());
}
...
inline void k8s_pod_t::set_restart_count(int rc)
{
    if(rc < 0)
    {
	g_logger.log("Unexpected K8S pod restart count received: " +
			     std::to_string(rc),
    ...
    {
	g_logger.log("Unexpected K8S pod restart count received (" +
			     std::to_string(rc) + ", last recorded value " +
			     std::to_string(m_restart_count_tot) +
...
inline bool k8s_pod_t::operator==(const k8s_pod_t& other) const
{
    ...
    return other.m_container_ids == m_container_ids &&
	   other.m_containers == m_containers && other.m_host_ip == m_host_ip &&
...
{
    m_containers.insert(m_containers.end(), containers.begin(),
			containers.end());
}
...
// getters/setters
inline std::string k8s_pod_t::get_node_name() const { return m_node_name; }
...
inline const std::string& k8s_pod_t::get_host_ip() const { return m_host_ip; }
...
inline int k8s_replicas_t::get_spec_replicas() const { return m_spec_replicas; }
...
inline int k8s_replicas_t::get_stat_replicas() const { return m_stat_replicas; }
...
{
    m_replicas.set_spec_replicas(k8s_replicas_t::get_count(
	    item["status"], "desiredNumberScheduled"));
    m_replicas.set_stat_replicas(k8s_replicas_t::get_count(
