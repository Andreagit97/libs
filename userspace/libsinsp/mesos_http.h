...
class mesos_http
{
    ...
    mesos_http(mesos& m, const uri& url,
	       ...
	       bool discover_marathon = false, int timeout_ms = 5000L,
	       const string& token = "");
    ...
    std::string make_request(uri url,
			     curl_version_info_data* m_curl_version = 0);
...
inline bool mesos_http::is_connected() const { return m_connected; }

inline const uri& mesos_http::get_url() const { return m_url; }

inline CURL* mesos_http::get_sync_curl() { return m_sync_curl; }

inline CURL* mesos_http::get_select_curl() { return m_select_curl; }

inline mesos& mesos_http::get_mesos() { return m_mesos; }

inline const std::string& mesos_http::get_request() const { return m_request; }
						    ...
						    const std::string& uri)
{
	...
	else
	{
	    ...
	    g_logger.log("mesos_http::try_parse could not parse json (" +
				 errstr + ")",
    ...
    {
	g_logger.log("Could not parse JSON document: " + string(e.what()),
		     sinsp_logger::SEV_WARNING);
	g_json_error_log.log(json, e.what(), sinsp_utils::get_current_time_ns(),
			     uri);
    }
    catch(...)
    {
