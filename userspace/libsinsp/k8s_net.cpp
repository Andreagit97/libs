...
k8s_net::k8s_net(k8s& kube, k8s_state_t& state, const std::string& uri,
		 ...
		 bool blocking_sockets, const std::string& node_selector):
	m_state(state),
	m_collector(std::make_shared<collector_t>()), m_uri(uri), m_ssl(ssl),
	m_bt(bt), m_stopped(true), m_blocking_sockets(blocking_sockets),
	m_event_filter(event_filter), m_node_selector(node_selector)
{
}

k8s_net::~k8s_net() { cleanup(); }
	...
	if(it->second)
	{
		...
		if(k8s_component::is_critical(comp_type))
		{
		    throw sinsp_exception(
			    "K8s: " + k8s_component::get_name(comp_type) +
			    " connection error.");
		}
		else
		{
		    g_logger.log(
			    "K8s: " + k8s_component::get_name(comp_type) +
				    " connection error, removing component.",
		    ...
		    m_handlers.erase(it++);
		    g_logger.log("K8s: " + k8s_component::get_name(comp_type) +
					 " removed from watched endpoints.",
...
k8s_net::handler_ptr_t
k8s_net::get_dependency_handler(const handler_map_t& handlers,
				const k8s_component::type& component)
{
    ...
    throw sinsp_exception(std::string("Invalid K8s component type:") +
			  std::to_string(component));
}

k8s_net::handler_ptr_t k8s_net::get_dependency_handler(
	const handler_map_t& handlers,
	const k8s_component::type_map::value_type& component)
{
    return get_dependency_handler(handlers, component.first);
}

bool k8s_net::has_dependency(
	const k8s_component::type_map::value_type& component)
{
...
k8s_net::handler_ptr_t k8s_net::make_handler(
	k8s_state_t& state, const k8s_component::type component, bool connect,
	...
	const std::string& node_selector)
{
    ...
    {
    case k8s_component::K8S_NODES:
	return std::make_shared<k8s_node_handler>(state, dep, collector, urlstr,
						  "1.1", ssl, bt, connect,
						  blocking);
    case k8s_component::K8S_NAMESPACES:
	return std::make_shared<k8s_namespace_handler>(state, dep, collector,
						       urlstr, "1.1", ssl, bt,
						       connect, blocking);
    case k8s_component::K8S_PODS:
	return std::make_shared<k8s_pod_handler>(state, dep, collector, urlstr,
						 "1.1", ssl, bt, connect,
						 blocking, node_selector);
    case k8s_component::K8S_REPLICATIONCONTROLLERS:
	return std::make_shared<k8s_replicationcontroller_handler>(
		state, dep, collector, urlstr, "1.1", ssl, bt, connect,
		blocking);
    case k8s_component::K8S_REPLICASETS:
	return std::make_shared<k8s_replicaset_handler>(state, dep, collector,
							urlstr, "1.1", ssl, bt,
							connect, blocking);
    case k8s_component::K8S_SERVICES:
	return std::make_shared<k8s_service_handler>(state, dep, collector,
						     urlstr, "1.1", ssl, bt,
						     connect, blocking);
    case k8s_component::K8S_DAEMONSETS:
	return std::make_shared<k8s_daemonset_handler>(state, dep, collector,
						       urlstr, "1.1", ssl, bt,
						       connect, blocking);
    case k8s_component::K8S_DEPLOYMENTS:
	return std::make_shared<k8s_deployment_handler>(state, dep, collector,
							urlstr, "1.1", ssl, bt,
							connect, blocking);
    case k8s_component::K8S_EVENTS:
	return std::make_shared<k8s_event_handler>(
		state, dep, collector, urlstr, "1.1", ssl, bt, connect,
		blocking, event_filter);
    case k8s_component::K8S_COMPONENT_COUNT:
    default:
	return nullptr;
    }
...
void k8s_net::add_handler(const k8s_component::type_map::value_type& component)
{
    ...
    {
	handler_ptr_t handler = make_handler(
		m_state, component.first, true,
		get_dependency_handler(m_handlers, component), m_collector,
		m_uri.to_string(), m_ssl, m_bt, m_blocking_sockets,
		m_event_filter, m_node_selector);
	if(handler)
	{
	    ...
	    {
		g_logger.log("K8s machine ID (MAC) is empty - scope may not be "
			     "available for " +
				     handler->name(),
			     sinsp_logger::SEV_WARNING);
	    }
	...
	else
	{
	    ...
	    os << "K8s: invalid component type encountered while creating "
		  "handler: "
	       << component.second << " (" << std::to_string(component.first)
	       << ')';
	...
	g_logger.log("K8s: created " + k8s_component::get_name(component) +
			     " handler.",
		     sinsp_logger::SEV_INFO);
    }
    else
    {
	g_logger.log("K8s: component " + k8s_component::get_name(component) +
			     " already exists.",
...
namespace k8s_net
{
k8s_handler::ptr_t make_handler(k8s_state_t& state,
				const k8s_component::type component,
				bool /*connect*/)
{
    switch(component)
