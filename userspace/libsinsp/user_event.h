...
class event_scope
{
    ...
    bool add(const std::string& key, const std::string& value,
	     const std::string& op = SCOPE_OP_AND);
...
#ifndef _WIN32
    static void regex_error(const std::string& call, size_t ret, regex_t* preg,
...
inline void event_scope::clear() { m_scope.clear(); }
...
inline const std::string& user_event_meta_t::kind() const { return m_kind; }
...
class user_event_filter_t
{
    ...
    // if the filter entry of the requested kind is found, returns const
    // iterator pointing to it; if "any event" entry is found, iterator pointing
    // to it is returned; otherwise, the iterator pointing to the end of the
    // filter list is returned (indicating the event kind was not found)
...
inline void user_event_filter_t::clear() { m_list.clear(); }

inline bool user_event_filter_t::ci_compare_str(const std::string& a,
						const std::string& b)
{
#ifndef _WIN32
...
class sinsp_user_event
{
    ...
    sinsp_user_event(uint64_t epoch_time_s, std::string&& name,
		     std::string&& desc, std::string&& scope, tag_map_t&& tags,
		     uint32_t sev);
...
inline const std::string& sinsp_user_event::name() const { return m_name; }
...
inline uint32_t sinsp_user_event::severity() const { return m_severity; }

inline const std::string& sinsp_user_event::scope() const { return m_scope; }
