...
k8s_deployment_handler::k8s_deployment_handler(
	...
	,
	ptr_t dependency_handler, collector_ptr_t collector, std::string url,
	const std::string& http_version, ssl_ptr_t ssl, bt_ptr_t bt,
	bool connect, bool blocking_socket
#endif // HAS_CAPTURE
	):
	k8s_handler("k8s_deployment_handler", true,
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		    url, "/apis/apps/v1/deployments", STATE_FILTER,
		    EVENT_FILTER, NULL_FILTER, collector, http_version, 1000L,
		    ssl, bt, true, connect, dependency_handler, blocking_socket,
#endif			 // HAS_CAPTURE
		    100, // max msgs
		    &state)
{
...
bool k8s_deployment_handler::handle_component(const Json::Value& json,
					      const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
		...
		k8s_deployment_t& deployment =
			m_state->get_component<k8s_deployments,
					       k8s_deployment_t>(
				m_state->get_deployments(), data->m_name,
	    ...
	    {
		if(!m_state->delete_component(m_state->get_deployments(),
					      data->m_uid))
		{
		    log_not_found(*data);
		    return false;
