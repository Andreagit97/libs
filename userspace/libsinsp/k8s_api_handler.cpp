...
k8s_api_handler::k8s_api_handler(collector_ptr_t collector,
				 ...
				 bool blocking_socket, uint32_t data_max_b,
	...
	k8s_handler("k8s_api_handler", false,
		    ...
		    ssl, bt, false, true, std::make_shared<k8s_dummy_handler>(),
		    blocking_socket,
#endif // HAS_CAPTURE
...
bool k8s_api_handler::handle_component(const Json::Value& json,
				       const msg_data* data)
{
    m_error = false;
    if(!json.isNull())
	...
	if(json.isArray())
	{
		...
		{
		    g_logger.log("K8s API handler error: could not extract API "
				 "versions or extensions from JSON.",
    ...
    {
	g_logger.log("K8s API handler error: json is null.",
		     sinsp_logger::SEV_ERROR);
	m_error = true;
	return false;
    }
