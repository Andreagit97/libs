...
std::shared_ptr<sinsp_plugin> sinsp_plugin::create(const std::string &filepath,
						   std::string &errstr)
{
#ifdef _WIN32
...
ss_plugin_caps sinsp_plugin::caps() const { return m_caps; }

bool sinsp_plugin::is_plugin_loaded(std::string &filepath)
{
#ifdef _WIN32
    /*
     * LoadLibrary maps the module into the address space of the calling
     * process, if necessary, and increments the modules reference count, if it
     * is already mapped. GetModuleHandle, however, returns the handle to a
     * mapped module without incrementing its reference count.
     *
     ...
     */
    sinsp_plugin_handle handle =
	    dlopen(filepath.c_str(), RTLD_LAZY | RTLD_NOLOAD);
#endif
    return handle != NULL;
}
...
const std::string &sinsp_plugin::name() const { return m_name; }

const std::string &sinsp_plugin::description() const { return m_description; }

const std::string &sinsp_plugin::contact() const { return m_contact; }
...
void sinsp_plugin::resolve_dylib_field_arg(Json::Value root,
					   filtercheck_field_info &tf)
{
	...
	{
	    throw sinsp_exception(string("error in plugin ") + m_name +
				  ": field " + tf.m_name +
    ...
    {
	throw sinsp_exception(string("error in plugin ") + m_name + ": field " +
			      tf.m_name +
			      " arg has isRequired true, but none of isKey nor "
			      "isIndex is true");
    }
    return;
}
...
bool sinsp_plugin::resolve_dylib_symbols(std::string &errstr)
{
    ...
    if((*(void **)(&(m_api.get_required_api_version)) =
		getsym("plugin_get_required_api_version", errstr)) == NULL)
    {
	errstr = string(
		"Could not resolve plugin_get_required_api_version function");
	return false;
    }

    std::string req_version_str =
	    str_from_alloc_charbuf(m_api.get_required_api_version());
    m_required_api_version = sinsp_version(req_version_str);
    if(!m_required_api_version.m_valid)
    {
	errstr = "Plugin provided an invalid required API version string: '" +
		 req_version_str + "'";
	return false;
    }
	...
	errstr = "Plugin required API version '" +
		 ...
		 "' is not supported by the plugin API version of the "
		 "framework '" +
    ...
    if((*(void **)(&(m_api.get_last_error)) =
		getsym("plugin_get_last_error", errstr)) == NULL ||
       (*(void **)(&(m_api.get_name)) = getsym("plugin_get_name", errstr)) ==
	       NULL ||
       (*(void **)(&(m_api.get_description)) =
		getsym("plugin_get_description", errstr)) == NULL ||
       (*(void **)(&(m_api.get_contact)) =
		getsym("plugin_get_contact", errstr)) == NULL ||
       (*(void **)(&(m_api.get_version)) =
		getsym("plugin_get_version", errstr)) == NULL)
    {
	return false;
    }
    ...
    (*(void **)(&m_api.get_init_schema)) =
	    getsym("plugin_get_init_schema", errstr);
    ...
    /** Sourcing API **/
    if((*(void **)(&(m_api.get_id)) = getsym("plugin_get_id", errstr)) !=
	       NULL &&
       (*(void **)(&(m_api.get_event_source)) =
		getsym("plugin_get_event_source", errstr)) != NULL &&
       (*(void **)(&(m_api.open)) = getsym("plugin_open", errstr)) != NULL &&
       (*(void **)(&(m_api.close)) = getsym("plugin_close", errstr)) != NULL &&
       (*(void **)(&(m_api.next_batch)) =
		getsym("plugin_next_batch", errstr)) != NULL)
    {
	m_caps = (ss_plugin_caps)((uint32_t)m_caps | (uint32_t)CAP_SOURCING);

	(*(void **)(&m_api.get_progress)) =
		getsym("plugin_get_progress", errstr);
	(*(void **)(&m_api.list_open_params)) =
		getsym("plugin_list_open_params", errstr);
	(*(void **)(&m_api.event_to_string)) =
		getsym("plugin_event_to_string", errstr);
    ...
    /** Extraction API **/
    if((*(void **)(&(m_api.get_fields)) =
		getsym("plugin_get_fields", errstr)) != NULL &&
       (*(void **)(&(m_api.extract_fields)) =
		getsym("plugin_extract_fields", errstr)) != NULL)
    {

	m_caps = (ss_plugin_caps)((uint32_t)m_caps | (uint32_t)CAP_EXTRACTION);

	(*(void **)(&m_api.get_extract_event_sources)) =
		getsym("plugin_get_extract_event_sources", errstr);

	//
	// If filter fields are exported by the plugin, get the json from
	// get_fields(), parse it, create our list of fields, and create a
	// filtercheck from the fields.
	//
	...
	for(Json::Value::ArrayIndex j = 0; j < root.size(); j++)
	{
	    ...
	    const Json::Value &jvIsList =
		    root[j].get("isList", Json::Value::null);
	    if(!jvIsList.isNull())
	    {
		if(!jvIsList.isBool())
		{
	    ...
	    const Json::Value &jvProperties =
		    root[j].get("properties", Json::Value::null);
		...
		for(const auto &prop : jvProperties)
		{
		    ...
		    // "hidden" is used inside and outside libs. "info" and
		    // "conversation" are used outside libs.
		    if(str == "hidden")
		    {
			tf.m_flags =
				(filtercheck_field_flags)((int)tf.m_flags |
	...
	{
	    std::string esources =
		    str_from_alloc_charbuf(m_api.get_extract_event_sources());
	    ...
	    Json::Value root;
	    if(!Json::Reader().parse(esources, root) ||
	       root.type() != Json::arrayValue)
	    {
		throw sinsp_exception(string("error in plugin ") + name() +
				      ": get_extract_event_sources did not "
				      "return a json array");
	    }
...
std::string
sinsp_plugin::get_init_schema(ss_plugin_schema_type &schema_type) const
{
...
void sinsp_plugin::validate_init_config_json_schema(std::string &config,
						    std::string &schema)
{
    Json::Value schemaJson;
    if(!Json::Reader().parse(schema, schemaJson) ||
       schemaJson.type() != Json::objectValue)
    {
	throw sinsp_exception(string("error in plugin ") + name() +
	...
	if(validationResults.popError(error))
	{
	    throw sinsp_exception(
		    ...
		    std::accumulate(error.context.begin(), error.context.end(),
				    std::string("")) +
		    ", " + error.description);
	}
...
scap_source_plugin &sinsp_plugin::as_scap_source()
{
    ...
    {
	throw sinsp_exception("Can't create scap_source_plugin from a plugin "
			      "without CAP_SOURCING capability.");
    }
...
uint32_t sinsp_plugin::id() const { return m_id; }

const std::string &sinsp_plugin::event_source() const { return m_event_source; }
...
std::string sinsp_plugin::get_progress(uint32_t &progress_pct) const
{
    ...
    ret = str_from_alloc_charbuf(
	    m_api.get_progress(m_state, m_scap_source_plugin.handle, &ppct));
...
std::vector<sinsp_plugin_cap_sourcing::open_param>
sinsp_plugin::list_open_params() const
{
	...
	ss_plugin_rc rc;
	string jsonString =
		str_from_alloc_charbuf(m_api.list_open_params(m_state, &rc));
	if(rc != SS_PLUGIN_SUCCESS)
	{
	    throw sinsp_exception(string("error in plugin ") + name() +
				  ": list_open_params has error " +
				  get_last_error());
	}
	...
	{
	    Json::Value root;
	    if(Json::Reader().parse(jsonString, root) == false ||
	       root.type() != Json::arrayValue)
	    {
		throw sinsp_exception(
			string("error in plugin ") + name() +
...
sinsp_filter_check *
sinsp_plugin::new_filtercheck(std::shared_ptr<sinsp_plugin> plugin)
{
    return new sinsp_filter_check_plugin(plugin);
}

bool sinsp_plugin::extract_fields(ss_plugin_event &evt, uint32_t num_fields,
				  ss_plugin_extract_field *fields) const
{
    if(!m_state)
    {
	return false;
    ...
    return m_api.extract_fields(m_state, &evt, num_fields, fields) ==
	   SS_PLUGIN_SUCCESS;
}
...
bool sinsp_plugin::is_source_compatible(const std::string &source) const
{
	...
	{
	    //
	    // If this is a plugin with event sourcing capabilities, reject
	    // events that have not been generated by a plugin with this id
	    // specifically.
	    //
    ...
    return m_extract_event_sources.find(source) !=
	   m_extract_event_sources.end();
}
