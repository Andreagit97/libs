...
class sinsp_utils
{
    ...
    //
    //
    static bool sockinfo_to_str(sinsp_sockinfo* sinfo, scap_fd_type stype,
    ...
    static bool endswith(const char* str, const char* ending, uint32_t lstr,
			 uint32_t lend);
    ...
    //
    static bool unhex(const std::vector<char>& hex_chars,
    ...
    // If path2 is absolute, the concatenation does not happen, target contains
    // path2 and the result is false. Assumes that path1 is well formed.
    // Supports both unix and windows paths. Use the windows_paths argument to
    // specify which one you want.
    //
    static bool concatenate_paths(char* target, uint32_t targetlen,
    ...
    //
    // Given a string, scan the event list and find the longest argument that
    // the input string contains
    //
    static const struct ppm_param_info*
    find_longest_matching_evt_param(std::string name);
    ...
    static bool find_first_env(std::string& out,
			       const std::vector<std::string>& env,
			       const std::vector<std::string>& keys);
    static bool find_env(std::string& out, const std::vector<std::string>& env,
    ...
    static void
    parse_suppressed_types(const std::vector<std::string>& supp_strs,
			   std::vector<uint16_t>* supp_ids);

    static const char* event_name_by_id(uint16_t id);

    static void ts_to_string(uint64_t ts, OUT std::string* res, bool date,
			     bool ns);
    ...
    static bool parse_iso_8601_utc_string(const std::string& time_str,
					  uint64_t& ns);

    //
    // Convert caps from their numeric representation to a space-separated
    // string list
    //
...
template<typename It> std::string sinsp_join(It begin, It end, char delim)
{
    if(begin == end)
    {
	return "";
...
std::string& replace_in_place(std::string& s, const std::string& search,
...
class sinsp_numparser
{
    ...
    static bool tryparseu32_fast(const char* str, uint32_t strlen,
				 uint32_t* res);
    static bool tryparsed32_fast(const char* str, uint32_t strlen,
				 int32_t* res);
};
...
template<typename OBJ> class simple_lifo_queue
{
    ...
    void push(OBJ* newentry) { m_avail_list.push_front(newentry); }
    ...
    bool empty() { return m_avail_list.empty(); }
...
///////////////////////////////////////////////////////////////////////////////
template<typename charT> struct ci_equal
{
