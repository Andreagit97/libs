...
class k8s
{
...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	ssl_ptr_t ssl = 0, bt_ptr_t bt = 0, bool block = false,
#endif // HAS_CAPTURE
	filter_ptr_t event_filter = nullptr,
	ext_list_ptr_t extensions = nullptr, bool events_only = false
#ifdef HAS_CAPTURE
	,
    ...
    const event_list_t& get_capture_events() const
    {
    ...
    typedef std::map<k8s_component::type, std::unique_ptr<k8s_dispatcher>>
	    dispatch_map_t;
    typedef std::map<k8s_component::type, std::shared_ptr<k8s_handler>>
	    handler_map_t;
    // dispatch map is deprecated and serves only for backward compatibility
    // with captures with old versions
    ...
    // a utility member containing pairs of enumerated values and component
    // names
...
inline void k8s::clear_events() { m_state.clear_events(); }
