...
class sinsp_parser
{
    ...
    //
    static void parse_dirfd(sinsp_evt* evt, char* name, int64_t dirfd,
			    OUT string* sdir);
    ...
    // Initializers
    //
    static void init_scapevt(metaevents_state& evt_state, uint16_t evt_type,
			     uint16_t buf_size);
    ...
    // Initializers
    //
    inline void init_metaevt(metaevents_state& evt_state, uint16_t evt_type,
			     uint16_t buf_size);
    ...
    inline bool detect_and_process_tracer_write(sinsp_evt* evt, int64_t retval,
    ...
    void parse_container_evt(
	    sinsp_evt* evt); // deprecated, only for backward-compatibility
    ...
    inline bool update_ipv4_addresses_and_ports(sinsp_fdinfo_t* fdinfo,
						uint32_t tsip, uint16_t tsport,
						uint32_t tdip, uint16_t tdport,
						bool overwrite_dest = true);
    inline void fill_client_socket_info(sinsp_evt* evt, uint8_t* packed_data);
    inline void add_socket(sinsp_evt* evt, int64_t fd, uint32_t domain,
    ...
    // Return false if the update didn't happen (for example because the tuple
    // is NULL)
    bool update_fd(sinsp_evt* evt, sinsp_evt_param* parinfo);

    // Next 4 return false if the update didn't happen because the tuple is
    // identical to the given address
    bool set_ipv4_addresses_and_ports(sinsp_fdinfo_t* fdinfo,
				      uint8_t* packed_data,
				      bool overwrite_dest = true);
    bool set_ipv4_mapped_ipv6_addresses_and_ports(sinsp_fdinfo_t* fdinfo,
						  uint8_t* packed_data,
						  bool overwrite_dest = true);
    bool set_ipv6_addresses_and_ports(sinsp_fdinfo_t* fdinfo,
				      uint8_t* packed_data,
				      bool overwrite_dest = true);
