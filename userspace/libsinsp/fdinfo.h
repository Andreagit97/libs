...
*/
template<class T> class SINSP_PUBLIC sinsp_fdinfo
{
    public:
    sinsp_fdinfo();
    sinsp_fdinfo(const sinsp_fdinfo& other) { copy(other, false); }
    ...
    */
    ...
    /*!
      ...
      Can be on of 'file', 'directory', ipv4', 'ipv6', 'unix', 'pipe', 'event',
      'signalfd', 'eventpoll', 'inotify', 'signalfd'.
    */
    char* get_typestring();

    /*!
      \brief Return the fd name, after removing unprintable or invalid
      characters from it.
    */
    ...
    */
    bool is_unix_socket() { return m_type == SCAP_FD_UNIX_SOCK; }

    /*!
      \brief Returns true if this is an IPv4 socket.
    */
    bool is_ipv4_socket() { return m_type == SCAP_FD_IPV4_SOCK; }

    /*!
      \brief Returns true if this is an IPv4 socket.
    */
    bool is_ipv6_socket() { return m_type == SCAP_FD_IPV6_SOCK; }
    ...
    */
    bool is_pipe() { return m_type == SCAP_FD_FIFO; }
    ...
    */
    bool is_directory() { return m_type == SCAP_FD_DIRECTORY; }
    ...
    uint32_t get_device() const { return m_dev; }

    // see new_encode_dev in include/linux/kdev_t.h
    uint32_t get_device_major() const { return (m_dev & 0xfff00) >> 8; }
    ...
    uint64_t get_ino() const { return m_ino; }

    /*!
      \brief If this is a socket, returns the IP protocol. Otherwise, return
      SCAP_FD_UNKNOWN.
    */
    ...
    */
    ...
    /*!
      \brief Used by protocol decoders to unregister callbacks related to this
      FD.
    */
    void unregister_event_callback(sinsp_pd_callback_type etype,
				   sinsp_protodecoder* dec);
    ...
    */
    ...
    uint32_t m_openflags; ///< If this FD is a file, the flags that were used
			  ///< when opening it. See the PPM_O_* definitions in
    ...
    */
    ...
    std::string m_name;	   ///< Human readable rendering of this FD. For files,
			   ///< this is the full file name. For sockets, this is
			   ///< the tuple. And so on.
    std::string m_oldname; // The name of this fd at the beginning of event
			   // parsing. Used to detect name changes that result
			   // from parsing an event.

    inline bool has_decoder_callbacks() { return (m_callbacks != NULL); }
...
// Doxygen doesn't understand VISIBILITY_PRIVATE
    ...
    */
    ...
    public:
    inline bool is_transaction() const { return (m_usrstate != NULL); }

    T* get_usrstate() { return m_usrstate; }
    ...
    bool set_net_role_by_guessing(sinsp* inspector, sinsp_threadinfo* ptinfo,
				  sinsp_fdinfo_t* pfdinfo, bool incoming);
    ...
    inline bool has_no_role() { return !is_role_client() && !is_role_server(); }
...
class sinsp_fdtable
{
    ...
    // If the key is already present, overwrite the existing value and return
    // false.
