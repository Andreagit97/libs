...
std::string thread_info_to_string(sinsp_threadinfo* tinfo)
{
	...
	std::string comm = out.str();

	char buffer[4096];
	/* it is called sid but tinfo is a vsid */
	sprintf(buffer, "%s t: %ld, p: %ld, rpt: %ld, vt: %ld, vp: %ld, vs: %ld, vpg: %ld, ct: %d, e: %s, t_e: %s",
		comm.c_str(),
		tinfo->m_tid,			       // t
		tinfo->m_pid,			       // p
		tinfo->m_ptid,			       // rpt (real parent tid)
		tinfo->m_vtid,			       // vt
		tinfo->m_vpid,			       // vp
		tinfo->m_sid,			       // vs (we call it sid but it is a vsid)
		tinfo->m_vpgid,			       // vpg
		tinfo->is_in_pid_namespace(),	       // ct (in container)
		tinfo->get_exepath().c_str(),	       // e
		tinfo->get_trusted_exepath().c_str()); // t_e
