...
void sinsp_tracerparser::set_storage_size(uint32_t newsize)
{
    ...
    if(m_storage == NULL)
    {
	throw sinsp_exception("memory allocation error in "
			      "sinsp_tracerparser::process_event_data.");
    }

    m_storage_size = newsize;
}

sinsp_tracerparser::parse_result
sinsp_tracerparser::process_event_data(char* data, uint32_t datalen,
				       uint64_t ts)
{
    ASSERT(data != NULL);
	...
	else
	{
	    ...
	    memcpy(m_storage, m_fullfragment_storage_str.c_str(), tlen);
	...
	{
	    //
	    // The list is completely used. This likely means that there have
	    // been drops and the entries will be stuck there forever. Better
	    // clean the list, miss the 128 events it contains, and start fresh.
	    //
	    list<sinsp_partial_tracer*>* partial_tracers_list =
		    &m_inspector->m_partial_tracers_list;
	    list<sinsp_partial_tracer*>::iterator it;

	    for(it = partial_tracers_list->begin();
		it != partial_tracers_list->end(); ++it)
	    {
	    ...
	    it != partial_tracers_list->end(); ++it)
	{
		...
		// despite removing the pae and returning it to the available
		// pool, we link to it so that the filters will use it. We do
		// that as an optimization (it avoids making a copy or
		// implementing logic for delayed list removal), and we base it
		// on the assumption that, since the processing is strictly
		// sequential and single thread, nobody will modify the pae
		// until the event is fully processed.
		//
		m_enter_pae = *it;
...
sinsp_partial_tracer* sinsp_tracerparser::find_parent_enter_pae()
{
	...
	if(*tse == 0)
	{
	    len = tse - m_enter_pae->m_tags_storage +
		  1; // 1 is for the trailing zero
	    break;
	}
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces(char* p, uint32_t* delta)
{
    char* start = p;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces_and_commas(char* p, uint32_t* delta,
					   uint32_t n_expected_commas)
{
    char* start = p;
    uint32_t nc = 0;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces_and_char(char* p, uint32_t* delta,
					 char char_to_skip)
{
    char* start = p;
    uint32_t nc = 0;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces_and_commas_and_sq_brackets(char* p,
							   uint32_t* delta)
{
    char* start = p;
    uint32_t nc = 0;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces_and_commas_and_cr_brackets(char* p,
							   uint32_t* delta)
{
    char* start = p;
    uint32_t nc = 0;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::skip_spaces_and_commas_and_all_brackets(char* p,
							    uint32_t* delta)
{
    char* start = p;
    uint32_t nc = 0;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::parsestr(char* p, char** res, uint32_t* delta)
{
    char* initial = p;
    *res = NULL;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::parsestr_not_enforce(char* p, char** res, uint32_t* delta)
{
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::parsenumber(char* p, int64_t* res, uint32_t* delta)
{
    char* start = p;
...
inline sinsp_tracerparser::parse_result
sinsp_tracerparser::parsenumber_colend(char* p, int64_t* res, uint32_t* delta)
{
    char* start = p;
    int64_t val = 0;
    bool negative = false;
...
inline void sinsp_tracerparser::init_partial_tracer(sinsp_partial_tracer* pae)
{
    ...
    {
	pae->m_tags_storage =
		(char*)realloc(pae->m_tags_storage, encoded_tags_len);
	pae->m_tags_storage_size = encoded_tags_len;
    }

    char* p = pae->m_tags_storage;
    for(it = m_tags.begin(), sit = m_taglens.begin(); it != m_tags.end();
	++it, ++sit)
    {
	memcpy(p, *it, (*sit) + 1);
    ...
    *p++ = 0;
    ...
    {
	pae->m_argnames_storage =
		(char*)realloc(pae->m_argnames_storage, encoded_argnames_len);
    ...
    *p++ = 0;
    ...
    {
	pae->m_argvals_storage =
		(char*)realloc(pae->m_argvals_storage, encoded_argvals_len);
...
{
    char doc1[] = "[\">\",     12345, [\"mysql\", \"query\", \"init\"], "
