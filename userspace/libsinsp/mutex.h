...
template<typename T> class MutexGuard
{
    ...
    MutexGuard(std::unique_lock<std::mutex> lock, T *inner):
	    m_lock(std::move(lock)), m_inner(inner)
    {
    }
    ...
    MutexGuard(MutexGuard &&rhs) noexcept:
	    m_lock(std::move(rhs.m_lock)), m_inner(rhs.m_inner)
    {
    }
...
/**
 * \brief A wrapper to allow synchronized const access to a value owned by a
 * Mutex<T>
 *
 * @tparam T type of the value protected by the mutex
 *
...
template<typename T> class ConstMutexGuard
{
    ...
    // a writable guard can be demoted to a read-only one, but *not* the other
    // way around
 ...
 *
 * The class owns a value of type T and a mutex. The only way to access the T
 * inside is via the lock() method, which returns a guard object that unlocks
 * the mutex once it falls out of scope
 *
 * To protect an object with a mutex, declare a variable of type `Mutex<T>`,
 * e.g.
 *
...
template<typename T> class Mutex
{
    ...
    {
	return ConstMutexGuard<T>(std::unique_lock<std::mutex>(m_lock),
				  &m_inner);
    }
