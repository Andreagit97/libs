			 ...
			 uint64_t ts_ns, const std::string &uri)
{
    ...
    {
	std::ofstream errs(m_json_parse_errors_file,
	...
	auto evt = sinsp_user_event(now, std::move(event_name), std::move(desc),
				    std::move(scope.get_ref()), std::move(tags),
				    user_event_logger::SEV_EVT_WARNING);

	g_logger.log("Logging user event: " + evt.to_string(),
...
token_bucket &json_error_log::get_bucket(const std::string &uri)
{
    ...
    if(it == m_buckets.end() || it->first != uri)
    {
	it = m_buckets.emplace_hint(it, std::make_pair(uri, token_bucket()));
