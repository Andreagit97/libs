...
static const vector<string> unary_ops = {"exists"};

static const vector<string> binary_num_ops = {"<=", "<", ">=", ">"};

static const vector<string> binary_str_ops = {
	"==",	      "=",	    "!=",	   "glob ",	   "contains ",
	"icontains ", "bcontains ", "startswith ", "bstartswith ", "endswith ",
};

static const vector<string> binary_list_ops = {"intersects", "in", "pmatch"};
...
void parser::set_max_depth(uint32_t max_depth) { m_max_depth = max_depth; }
...
ast::expr* parser::parse_check()
{
	...
	if(lex_helper_str("["))
	{
	    ...
	    {
		throw sinsp_exception("expected a valid field argument: a "
				      "quoted string or a bare string");
	    }
	...
	{
	    depth_pop();
	    return new ast::unary_check_expr(field, field_arg,
					     trim_str(m_last_token));
	}
	...
	depth_pop();
	return new ast::binary_check_expr(field, field_arg, trim_str(op),
					  value);
    }
    ...
    throw sinsp_exception(
	    "expected a '(' token, a field check, or an identifier");
}
...
inline bool parser::lex_identifier() { return lex_helper_rgx(RGX_IDENTIFIER); }

inline bool parser::lex_field_name() { return lex_helper_rgx(RGX_FIELDNAME); }
...
inline bool parser::lex_hex_num() { return lex_helper_rgx(RGX_HEXNUM); }

inline bool parser::lex_num() { return lex_helper_rgx(RGX_NUMBER); }
...
inline bool parser::lex_bare_str() { return lex_helper_rgx(RGX_BARESTR); }

inline bool parser::lex_unary_op() { return lex_helper_str_list(unary_ops); }

inline bool parser::lex_num_op() { return lex_helper_str_list(binary_num_ops); }

inline bool parser::lex_str_op() { return lex_helper_str_list(binary_str_ops); }
...
inline const char* parser::cursor() { return m_input.c_str() + m_pos.idx; }
...
string parser::escape_str(const string& str)
{
	...
	else
	{
		...
		break;
	    case ' ':
		// NOTE: we may need to initially support this to not create
		// breaking changes with some existing wrongly-escaped rules. So
		// far, I only found one, in Falco:
		// https://github.com/falcosecurity/falco/blob/204f9ff875be035e620ca1affdf374dd1c610a98/rules/falco_rules.yaml#L3046
		// todo(jasondellaluce): remove this once rules are rewritten
		...
		break;
	    case 'x':
		// todo(jasondellaluce): support hex num escaping (not needed
		// for now)
	    default:
...
inline void parser::depth_push()
{
    ...
    {
	throw sinsp_exception("exceeded max depth limit of " +
			      to_string(m_max_depth));
    }
}

inline void parser::depth_pop() { m_depth--; }
