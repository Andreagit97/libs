...
uri::uri(std::string str)
{
    ...
    m_scheme = str.substr(p_uri.scheme_start,
			  p_uri.scheme_end - p_uri.scheme_start);
    std::transform(m_scheme.begin(), m_scheme.end(), m_scheme.begin(),
		   ::tolower);
    if(!no_host)
    {
	m_host =
		str.substr(p_uri.host_start, p_uri.host_end - p_uri.host_start);
    ...
    m_query =
	    str.substr(p_uri.query_start, p_uri.query_end - p_uri.query_start);
    if(p_uri.user_info_end != p_uri.user_info_start)
    {
	std::string auth =
		str.substr(p_uri.user_info_start,
			   p_uri.user_info_end - p_uri.user_info_start);
...
void uri::set_path(const std::string& path)
{
	...
	throw sinsp_exception(
		std::string("Invalid URI Path: [").append(path).append(1, ']'));
    }
    m_path = path;
}
...
std::string uri::encode(const std::string& str, const std::string& reserved)
{
	...
	{
	    std::ostringstream ostr;
	    ostr << "%" << std::setfill('0') << std::setw(2) << std::uppercase
		 << std::hex << ((unsigned)(unsigned char)c);
...
std::string uri::decode(const std::string& str, bool plus_as_space)
{
	...
	else if(c == '%')
	{
	    if(it == end)
	    {
		throw sinsp_exception("URI encoding: no hex digit following "
				      "percent sign in " +
				      str);
	    }
	    char hi = *it++;
	    if(it == end)
	    {
		throw sinsp_exception("URI encoding: two hex digits must "
				      "follow percent sign in " +
				      str);
	    }
	    char lo = *it++;
	    if(hi >= '0' && hi <= '9')
	    ...
	    {
		throw sinsp_exception(
			"URI encoding: not a hex digit found in " + str);
	    }
	    c *= 16;
	    if(lo >= '0' && lo <= '9')
	    {
