...
bool sinsp_utils::sockinfo_to_str(sinsp_sockinfo* sinfo, scap_fd_type stype,
				  char* targetbuf, uint32_t targetbuf_size,
				  bool resolve)
{
	...
	{
	    snprintf(targetbuf, targetbuf_size, "%u.%u.%u.%u->%u.%u.%u.%u",
		     (unsigned int)(uint8_t)sb[0], (unsigned int)(uint8_t)sb[1],
		     (unsigned int)(uint8_t)sb[2], (unsigned int)(uint8_t)sb[3],
		     (unsigned int)(uint8_t)db[0], (unsigned int)(uint8_t)db[1],
	   ...
	   sinfo->m_ipv6info.m_fields.m_l4proto == SCAP_L4_UDP)
	{
		...
		{
		    snprintf(
			    targetbuf, targetbuf_size, "%s:%s->%s:%s", srcstr,
	...
	else if(sinfo->m_ipv6info.m_fields.m_l4proto == SCAP_L4_ICMP)
	{
		...
		if(inet_ntop(AF_INET6, sip6, srcstr, sizeof(srcstr)) &&
		   inet_ntop(AF_INET6, dip6, dststr, sizeof(dststr)))
		{
		    snprintf(targetbuf, targetbuf_size, "%s->%s", srcstr,
			     dststr);
...
//
void rewind_to_parent_path(char* targetbase, char** tc, const char** pc,
			   uint32_t delta)
{
    if(*tc <= targetbase + 1)
    {
			    ...
			    char separator)
{
	...
	else
	{
		...
		//
		// '..', with no separator.
		// This is valid if we are at the end of the string, and in that
		// case we rewind. Otherwise it shouldn't happen and we leave
		// the string intact
		//
		if(*(pc + 2) == 0)
		{
...
//
bool sinsp_utils::concatenate_paths(char* target, uint32_t targetlen,
				    const char* path1, uint32_t len1,
...
bool sinsp_utils::is_ipv4_mapped_ipv6(uint8_t* paddr)
{
	...
	(paddr[10] == 0 && paddr[11] == 0 && paddr[12] == 0 && paddr[13] == 0 &&
	 paddr[14] == 0 &&
...
const struct ppm_param_info*
sinsp_utils::find_longest_matching_evt_param(string name)
{
    uint32_t maxlen = 0;
...
void sinsp_utils::get_filtercheck_fields_info(
...
bool sinsp_utils::find_first_env(std::string& out,
				 ...
				 const vector<std::string>& keys)
{
	...
	{
	    if((env_var.size() > key.size()) &&
	       !env_var.compare(0, key.size(), key) &&
	       (env_var[key.size()] == '='))
	    {
...
bool sinsp_utils::find_env(std::string& out, const vector<std::string>& env,
...
void sinsp_utils::parse_suppressed_types(
	const std::vector<std::string>& supp_strs,
...
void sinsp_utils::ts_to_string(uint64_t ts, OUT string* res, bool date, bool ns)
{
	...
	{
	    bufsize = sprintf(buf, "%04d-%02d-%02d ", tm->tm_year + 1900,
    ...
    if(ns)
    {
	sprintf(buf + bufsize, "%02d:%02d:%02d.%09u", s / 3600, (s % 3600) / 60,
		s % 60, (unsigned)nsec);
    }
    else
    {
	sprintf(buf + bufsize, "%02d:%02d:%02d", s / 3600, (s % 3600) / 60,
		s % 60);
    }
...
bool sinsp_utils::parse_iso_8601_utc_string(const std::string& time_str,
					    uint64_t& ns)
{
#ifndef _WIN32
    char* rem;
    ...
    tm_time.tm_isdst =
	    -1; // strptime does not set this, signal timegm to determine DST
    ...
    throw sinsp_exception(
	    "parse_iso_8601_utc_string() not implemented on Windows");
#endif
}
			     ...
			     const std::string& fmt)
{
    ...
    {
	throw sinsp_exception(
		"get_epoch_utc_seconds(): empty time or format string.");
    }
    struct tm tm_time = {0};
...
string port_to_string(uint16_t port, uint8_t l4proto, bool resolve)
{
	...
	struct servent* res;
	res = getservbyport(ntohs(port), (proto != "") ? proto.c_str()
						       : NULL); // best effort!
	if(res)
	{
	    ret = res->s_name;
...
string ipv4serveraddr_to_string(ipv4serverinfo* addr, bool resolve)
{
    ...
    snprintf(buf, sizeof(buf), "%d.%d.%d.%d:%s", ip[0], ip[1], ip[2], ip[3],
...
string ipv6tuple_to_string(_ipv6tuple* tuple, bool resolve)
{
    ...
    if(NULL ==
       inet_ntop(AF_INET6, tuple->m_fields.m_sip.m_b, source_address, 100))
    {
	return string();
    }

    if(NULL ==
       inet_ntop(AF_INET6, tuple->m_fields.m_dip.m_b, destination_address, 100))
    {
	return string();
    }

    snprintf(buf, 200, "%s:%s->%s:%s", source_address,
	     port_to_string(tuple->m_fields.m_sport, tuple->m_fields.m_l4proto,
			    resolve)
		     .c_str(),
	     destination_address,
	     port_to_string(tuple->m_fields.m_dport, tuple->m_fields.m_l4proto,
			    resolve)
		     .c_str());
...
string& replace_in_place(string& str, const string& search,
...
string replace(const string& str, const string& search,
	       const string& replacement)
{
    string s(str);
...
bool sinsp_utils::endswith(const char* str, const char* ending, uint32_t lstr,
			   uint32_t lend)
{
    if(lstr >= lend)
    {
...
bool sinsp_utils::unhex(const std::vector<char>& hex_chars,
			std::vector<char>& hex_bytes)
{
    if(hex_chars.size() % 2 != 0 ||
       !std::all_of(hex_chars.begin(), hex_chars.end(),
...
bool sinsp_numparser::tryparseu32_fast(const char* str, uint32_t strlen,
				       uint32_t* res)
{
    const char* p = str;
...
bool sinsp_numparser::tryparsed32_fast(const char* str, uint32_t strlen,
				       int32_t* res)
{
    const char* p = str;
    const char* end = str + strlen;
