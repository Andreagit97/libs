...
{
    return t == CT_DOCKER || t == CT_CRI || t == CT_CONTAINERD ||
	   t == CT_CRIO || t == CT_PODMAN;
}
 ...
 * after failure, especially when multiple engines match against the same
 * process (e.g. Docker and containerd may use the same cgroup layout).
 *
 * If all engines fail to find metadata for a container, we need to remember
 * that for each engine individually and there's only one
 * sinsp_container_info->m_type
 */
...
public:
	container_port_mapping():
		m_host_ip(0), m_host_port(0), m_container_port(0)
	{
	}
	uint32_t m_host_ip;
...
public:
	...
	container_mount_info(const std::string &&source,
			     const std::string &&dest, const std::string &&mode,
			     const bool rw, const std::string &&propagation):
		m_source(source),
		m_dest(dest), m_mode(mode), m_rdwr(rw),
	...
	inline void get_string_value(const Json::Value &val,
				     std::string &result)
	{
...
public:
	...
	static void
	parse_health_probes(const Json::Value &config_obj,
			    std::list<container_health_probe> &probes);

	// Serialize the list of health probes, adding to the provided json
	// object
	static void
	add_health_probes(const std::list<container_health_probe> &probes,
    ...
    sinsp_container_info():
	    m_container_ip(0), m_privileged(false), m_memory_limit(0),
	    ...
	    m_metadata_deadline(0), m_size_rw_bytes(-1)
    {
    }
    ...
    match_health_probe(sinsp_threadinfo *tinfo) const;
