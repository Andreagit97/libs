...
void on_new_entry_from_proc(void* context, scap_t* handle, int64_t tid,
...
sinsp::sinsp(bool static_container, const std::string& static_id,
	     const std::string& static_name, const std::string& static_image):
	m_external_event_processor(),
	m_simpleconsumer(false), m_evt(this), m_lastevent_ts(0),
	m_container_manager(this, static_container, static_id, static_name,
			    static_image),
	m_usergroup_manager(this), m_ppm_sc_of_interest(), m_suppressed_comms(),
	m_inited(false)
{
    ++instance_count;
    ...
    m_inactive_container_scan_time_ns =
	    DEFAULT_INACTIVE_CONTAINER_SCAN_TIME_S * ONE_SECOND_IN_NS;
    m_deleted_users_groups_scan_time_ns =
	    DEFAULT_DELETED_USERS_GROUPS_SCAN_TIME_S * ONE_SECOND_IN_NS;
    m_cycle_writer = NULL;
    ...
    uint32_t evlen =
	    sizeof(scap_evt) + 2 * sizeof(uint16_t) + 2 * sizeof(uint64_t);
    ...
    uint16_t* lens = (uint16_t*)((char*)m_meinfo.m_piscapevt +
...
void sinsp::add_protodecoders() { m_parser->add_protodecoder("syslog"); }
...
void sinsp::consume_initialstate_events()
{
	...
	if(res == SCAP_SUCCESS)
	{
	    // Setting these to non-null will make sinsp::next use them as a
	    // scap event to avoid a call to scap_next. In this way, we can
	    // avoid the state parsing phase once we reach a container-unrelated
	    // event.
...
void sinsp::init()
{
    ...
    m_is_windows = (platform == SCAP_PFORM_WINDOWS_I386 ||
		    platform == SCAP_PFORM_WINDOWS_X64);
    ...
    //
    for(auto it = m_partial_tracers_list.begin();
	it != m_partial_tracers_list.end(); ++it)
    {
    ...
    //
    // If we're reading from file, we try to pre-parse the container events
    // before importing the thread table, so that thread table filtering will
    // work with container filters
    //
    if(is_capture())
    {
    ...
    // If we are in capture, this is already called by
    // consume_initialstate_events
...
void sinsp::fill_syscalls_of_interest(scap_open_args* oargs)
{
    ...
    for(int i = 0; i < PPM_SC_MAX; i++)
    {
	oargs->ppm_sc_of_interest.ppm_sc[i] =
		m_ppm_sc_of_interest.find(i) != m_ppm_sc_of_interest.end();
    }
}
...
void sinsp::open_live_common(uint32_t timeout_ms, scap_mode_t mode)
{
    ...
    scap_set_refresh_proc_table_when_saving(m_h,
...
void sinsp::set_simple_consumer() { m_simpleconsumer = true; }
...
unsigned sinsp::num_possible_cpus()
{
	...
	{
	    g_logger.log(
		    "Unable to read num_possible_cpus, falling back to 128",
...
{
#ifdef _WIN32
    DWORD err_no = GetLastError(); // first, so error is not wiped out by
				   // intermediate calls
...
#else
    char* msg_buf = strerror(
	    errno); // first, so error is not wiped out by intermediate calls
    std::string errstr = msg;
    if(msg_buf)
    {
	errstr.append(msg_buf, strlen(msg_buf));
...
void sinsp::on_new_entry_from_proc(void* context, scap_t* handle, int64_t tid,
				   scap_threadinfo* tinfo, scap_fdinfo* fdinfo)
{
    ASSERT(tinfo != NULL);
	...
	if(is_nodriver())
	{
	    ...
	    if(sinsp_tinfo == nullptr ||
	       newti->m_clone_ts > sinsp_tinfo->m_clone_ts)
	    {
...
void sinsp::import_user_list()
{
	...
	for(j = 0; j < ul->nusers; j++)
	{
	    m_usergroup_manager.add_user(
		    "", ul->users[j].uid, ul->users[j].gid, ul->users[j].name,
		    ul->users[j].homedir, ul->users[j].shell);
	}

	for(j = 0; j < ul->ngroups; j++)
	{
	    m_usergroup_manager.add_group("", ul->groups[j].gid,
					  ul->groups[j].name);
	}
    }
}
...
void sinsp::add_meta_event(sinsp_evt* metaevt) { m_metaevt = metaevt; }
...
void sinsp::remove_meta_event_callback() { m_meta_event_callback = NULL; }
...
void sinsp::get_procs_cpu_from_driver(uint64_t ts)
{
	...
	m_meinfo.m_piscapevt->ts =
		m_next_flush_time_ns - (ONE_SECOND_IN_NS + 1);
...
int32_t sinsp::next(OUT sinsp_evt** puevt)
{
	...
	if(m_decoders_reset_list.size() != 0)
	{
	    ...
	    for(it = m_decoders_reset_list.begin();
		it != m_decoders_reset_list.end(); ++it)
	    {
		(*it)->on_reset(evt);
	    }
	...
	if(res != SCAP_SUCCESS)
	{
		...
		if(m_external_event_processor)
		{
		    m_external_event_processor->process_event(
			    NULL, libsinsp::EVENT_RETURN_TIMEOUT);
		}
		*puevt = NULL;
		return res;
	    }
		...
		{
		    m_external_event_processor->process_event(
			    NULL, libsinsp::EVENT_RETURN_EOF);
		}
	    }
	    else if(res == SCAP_UNEXPECTED_BLOCK)
	    {
		// This mostly happens in concatenated scap files, where an
		// unexpected block represents the end of a file and the start
		// of the next appended one. In this case, we restart the
		// capture so that the internal states gets reset and the blocks
		// coming from the next appended file get consumed.
		restart_capture();
		return SCAP_TIMEOUT;
	    }
	    else
    ...
    if(m_firstevent_ts == 0 && evt->m_pevt->type != PPME_CONTAINER_JSON_E &&
       evt->m_pevt->type != PPME_CONTAINER_JSON_2_E &&
       evt->m_pevt->type != PPME_USER_ADDED_E &&
       evt->m_pevt->type != PPME_GROUP_ADDED_E)
    {
	m_firstevent_ts = ts;
    }
	...
	if(ts > m_next_stats_print_time_ns)
	{
		...
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"n_evts:%" PRIu64 " n_drops:%" PRIu64
				" n_drops_buffer:%" PRIu64
				" n_drops_scratch_map:%" PRIu64
				" n_drops_pf:%" PRIu64 " n_drops_bug:%" PRIu64,
				stats.n_evts, stats.n_drops,
				stats.n_drops_buffer, stats.n_drops_scratch_map,
				stats.n_drops_pf, stats.n_drops_bug);
	    }

	    m_next_stats_print_time_ns =
		    ts - (ts % ONE_SECOND_IN_NS) + ONE_SECOND_IN_NS;
	}
    }
	...
	sinsp_threadinfo* ptinfo =
		get_thread_ref(m_tid_of_fd_to_remove, true, true).get();
	if(!ptinfo)
	{
	    ASSERT(false);
	    return res;
	...
	*evt = NULL;
	...
	scap_evt* pdevt =
		(evt->m_poriginal_evt) ? evt->m_poriginal_evt : evt->m_pevt;
    ...
    if(m_external_event_processor)
    {
	m_external_event_processor->process_event(evt,
						  libsinsp::EVENT_RETURN_NONE);
    }
    ...
    //
    if(evt->m_tinfo && evt->get_type() != PPME_SCHEDSWITCH_1_E &&
...
threadinfo_map_t::ptr_t sinsp::get_thread_ref(int64_t tid,
					      bool query_os_if_not_found,
					      bool lookup_only,
					      bool main_thread)
{
    return m_thread_manager->get_thread_ref(tid, query_os_if_not_found,
					    lookup_only, main_thread);
}
...
void sinsp::set_fullcapture_port_range(uint16_t range_start, uint16_t range_end)
{
	...
	throw sinsp_exception(
		"set_fullcapture_port_range called on a trace file");
    }

    if(scap_set_fullcapture_port_range(m_h, range_start, range_end) !=
       SCAP_SUCCESS)
    {
	throw sinsp_exception(scap_getlasterr(m_h));
    }
...
sinsp::register_plugin(const std::string& filepath)
{
    string errstr;
    std::shared_ptr<sinsp_plugin> plugin =
	    sinsp_plugin::create(filepath, errstr);
    if(!plugin)
    {
	throw sinsp_exception("cannot load plugin " + filepath + ": " +
			      errstr.c_str());
    }
    ...
    {
	throw sinsp_exception("cannot add plugin " + filepath +
			      " to inspector: " + e.what());
    }
...
void sinsp::set_input_plugin(const string& name, const string& params)
{
	...
	if(it->name() == name)
	{
	    ...
	    {
		throw sinsp_exception("plugin " + name +
				      " has not event sourcing capabilities "
				      "and cannot be used as input.");
	    }
	    m_input_plugin = it;
...
void sinsp::start_dropping_mode(uint32_t sampling_ratio)
{
    if(m_mode == SCAP_MODE_LIVE)
    {
	g_logger.format(sinsp_logger::SEV_INFO, "setting drop mode to %" PRIu32,
			sampling_ratio);
...
const string sinsp::get_filter() { return m_filterstring; }
...
const scap_machine_info* sinsp::get_machine_info() { return m_machine_info; }

void sinsp::get_filtercheck_fields_info(
	OUT vector<const filter_check_info*>& list)
{
...
uint32_t sinsp::reserve_thread_memory(uint32_t size)
{
    ...
    {
	throw sinsp_exception(
		"reserve_thread_memory can't be called after capture starts");
    }
...
void sinsp::set_log_file(string filename) { g_logger.add_file_log(filename); }

void sinsp::set_log_stderr() { g_logger.add_stderr_log(); }
...
sinsp_evttables* sinsp::get_event_info_tables() { return &g_infotables; }
...
sinsp_evt::param_fmt sinsp::get_buffer_format() { return m_buffer_format; }

void sinsp::set_large_envs(bool enable) { m_large_envs_enabled = enable; }
...
void sinsp::set_max_evt_output_len(uint32_t len) { m_max_evt_output_len = len; }
...
sinsp_parser* sinsp::get_parser() { return m_parser; }

bool sinsp::setup_cycle_writer(string base_file_name, int rollover_mb,
			       int duration_seconds, int file_limit,
			       unsigned long event_limit, bool compress)
{
    m_compress = compress;

    if(rollover_mb != 0 || duration_seconds != 0 || file_limit != 0 ||
       event_limit != 0)
    {
	m_write_cycling = true;
    }

    return m_cycle_writer->setup(base_file_name, rollover_mb, duration_seconds,
				 file_limit, event_limit, &m_dumper);
}
...
void sinsp::init_mesos_client(string* api_server, bool verbose)
{
	...
	m_mesos_client = new mesos(
		m_mesos_api_server, m_marathon_api_server,
		true, // mesos leader auto-follow
		m_marathon_api_server
			.empty(),	// marathon leader auto-follow if no uri
		mesos::credentials_t(), // mesos creds, the only way to provide
					// creds is embedded in URI
		mesos::credentials_t(), // marathon creds
		mesos::default_timeout_ms, is_live, m_verbose_json);
    }
}
...
void sinsp::init_k8s_ssl(const string* ssl_cert)
{
	...
	if(pos == std::string::npos) // ca_cert-only is obsoleted, single entry
				     // is now bearer token
	{
	...
	else
	{
	    ...
	    if(cert.empty())
	    {
		throw sinsp_exception(string("Invalid K8S SSL entry: ") +
				      *ssl_cert);
	    }
	...
	g_logger.format(sinsp_logger::SEV_TRACE,
			"Creating sinsp_ssl with cert %s, key %s, key_pwd %s, "
			"ca_cert %s",
			cert.c_str(), key.c_str(), key_pwd.c_str(),
			ca_cert.c_str());
	m_k8s_ssl = std::make_shared<sinsp_ssl>(cert, key, key_pwd, ca_cert,
						ca_cert.empty() ? false : true,
						"PEM");
    }
#endif // HAS_CAPTURE
}
...
#ifdef HAS_CAPTURE
			   ,
			   m_k8s_node_name ? *m_k8s_node_name
...
void sinsp::collect_k8s()
{
	...
	if(m_k8s_api_server)
	{
		...
		init_k8s_client(m_k8s_api_server, m_k8s_api_cert,
				m_k8s_node_name, m_verbose_json);
		if(m_k8s_client)
		{
		    g_logger.log("K8s client created.",
				 sinsp_logger::SEV_DEBUG);
		}
		else
		{
		    g_logger.log("K8s client NOT created.",
				 sinsp_logger::SEV_DEBUG);
		}
	    }
	    if(m_k8s_client)
	    {
		if(m_lastevent_ts >
		   m_k8s_last_watch_time_ns +
			   (m_metadata_download_params.m_data_watch_freq_sec *
			    ONE_SECOND_IN_NS))
		{
		    m_k8s_last_watch_time_ns = m_lastevent_ts;
		    g_logger.log("K8s updating state ...",
				 sinsp_logger::SEV_DEBUG);
		    ...
		    g_logger.format(sinsp_logger::SEV_DEBUG,
				    "Updating Kubernetes state took %" PRIu64
				    " ms",
				    delta / 1000000LL);
		}
...
void sinsp::k8s_discover_ext()
{
	   ...
	   !m_k8s_ext_detect_done)
	{
		...
		m_k8s_ext_handler.reset(new k8s_api_handler(
			m_k8s_collector, *m_k8s_api_server, "/apis/apps/v1",
			"[.resources[].name]", "1.1", m_k8s_ssl, m_k8s_bt,
			true));
		...
		if(m_k8s_ext_handler->ready())
		{
		    ...
		    {
			g_logger.log("K8s API extensions handler: data error "
				     "occurred while detecting API extensions.",
				     sinsp_logger::SEV_WARNING);
			...
			g_logger.log("K8s API extensions handler extensions "
				     "found: " +
					     ostr.str(),
				     sinsp_logger::SEV_DEBUG);
    ...
    {
	g_logger.log(std::string("K8s API extensions handler error: ")
			     .append(ex.what()),
    ...
    g_logger.log("K8s API extensions handler: detection done.",
		 sinsp_logger::SEV_TRACE);
#endif // HAS_CAPTURE
}
...
void sinsp::update_k8s_state()
{
	...
	if(m_k8s_api_server && !m_k8s_api_server->empty())
	{
		...
		if(!m_k8s_api_handler)
		{
		    ...
		    m_k8s_api_handler.reset(new k8s_api_handler(
			    m_k8s_collector, *m_k8s_api_server, "/api",
			    ".versions", "1.1", m_k8s_ssl, m_k8s_bt, true,
		...
		else
		{
		    ...
		    {
			g_logger.log("K8s API handler data received.",
				     sinsp_logger::SEV_DEBUG);
			if(m_k8s_api_handler->error())
			{
			    g_logger.log("K8s API handler data error occurred "
			...
			else
			{
			    ...
			    if(m_k8s_api_detected)
			    {
				g_logger.log("K8s API server v1 detected.",
					     sinsp_logger::SEV_DEBUG);
			    }
			}
			m_k8s_collector.reset();
		    ...
		    {
			g_logger.log("K8s API handler not ready yet.",
				     sinsp_logger::SEV_DEBUG);
		    }
		}
	    }
    ...
    {
	g_logger.log(std::string("Error fetching K8s data: ").append(e.what()),
		     sinsp_logger::SEV_ERROR);
	throw;
    }
#endif // HAS_CAPTURE
...
bool sinsp::get_mesos_data()
{
    ...
    {
	g_logger.log(std::string("Mesos exception: ") + ex.what(),
		     sinsp_logger::SEV_ERROR);
	delete m_mesos_client;
	m_mesos_client = NULL;
...
void sinsp::update_mesos_state()
{
	...
	if(m_mesos_client->is_alive())
	{
		...
		delta = sinsp_utils::get_current_time_ns() - delta;
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"Updating Mesos state took %" PRIu64 " ms",
				delta / 1000000LL);
	    }
	}
	else
	{
	    g_logger.format(
		    sinsp_logger::SEV_ERROR,
		    "Mesos connection not active anymore, retrying ...");
	    delete m_mesos_client;
	    m_mesos_client = NULL;
	    init_mesos_client(0, m_verbose_json);
	}
...
bool sinsp_thread_manager::remove_inactive_threads()
{
    ...
    {
	//
	// Set the first table scan for 30 seconds in, so that we can spot bugs
	// in the logic without having to wait for tens of minutes
	//
	if(m_inspector->m_inactive_thread_scan_time_ns > 30 * ONE_SECOND_IN_NS)
	{
	    m_last_flush_time_ns =
		    (m_inspector->m_lastevent_ts -
		     m_inspector->m_inactive_thread_scan_time_ns +
		     30 * ONE_SECOND_IN_NS);
	}
	else
	{
	...
	m_threadtable.loop(
		[&](sinsp_threadinfo& tinfo)
		{
		    ...
		    if(closed || ((m_inspector->m_lastevent_ts >
				   tinfo.m_lastaccess_ts +
					   m_inspector->m_thread_timeout_ns) &&
				  !scap_is_thread_alive(
					  m_inspector->m_h, tinfo.m_pid,
	...
	//
	// Rebalance the thread table dependency tree, so we free up threads
	// that exited but that are stuck because of reference counting.
	//
...
std::shared_ptr<std::string> sinsp::lookup_cgroup_dir(const string& subsys)
{
	...
	struct mntent* entry = getmntent_r(fp, &mntent_buf, mntent_string_buf,
					   sizeof(mntent_string_buf));
	while(entry != nullptr)
	{
...
sinsp_threadinfo* libsinsp::event_processor::build_threadinfo(sinsp* inspector)
{
