...
class sinsp_plugin_manager
{
    ...
    */
    ...
    /*!
	    \brief Returns the source names from all the plugins. The index of
       each source name does not change, so it may be used by consumers for
    ...
    */
    ...
    /*!
	    \brief Returns a plugin given its ID. The plugin is guaranteed to
       have the CAP_EVENT_SOURCE capability.
    */
    ...
    /*!
	    \brief Returns a plugin given an event. The plugin is guaranteed to
       have the CAP_EVENT_SOURCE capability.
    */
    ...
    /*!
	    \brief Returns a the index of a source name as in the order of
       sources() given a plugin id
    */
    inline std::size_t source_idx_by_plugin_id(uint32_t plugin_id,
					       bool& found) const
    {
