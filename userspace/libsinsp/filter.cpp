...
// Essentially, after dealing with that stuff multiple times in the past, and
// fighting for a day to configure everything with crappy documentation and code
// that doesn't compile, I decided that I agree with this
// http://mortoray.com/2012/07/20/why-i-dont-use-a-parser-generator/ and that
// I'm going with a manually written parser. The grammar is simple enough that
// it's not going to take more time. On the other hand I will avoid a crappy
// dependency that breaks my code at every new release, and I will have a
// cleaner and easier to understand code base.
//
...
bool flt_compare_uint64(cmpop op, uint64_t operand1, uint64_t operand2)
{
	...
	throw sinsp_exception(
		"'bstartswith' not supported for numeric filters");
	return false;
    case CO_ENDSWITH:
...
bool flt_compare_string(cmpop op, char *operand1, char *operand2)
{
    ...
    default:
	ASSERT(false);
	throw sinsp_exception("invalid filter operator " +
			      std::to_string((long long)op));
	return false;
    }
}

bool flt_compare_buffer(cmpop op, char *operand1, char *operand2,
			uint32_t op1_len, uint32_t op2_len)
{
    switch(op)
    {
    case CO_EQ:
...
bool flt_compare_ipv4net(cmpop op, uint64_t operand1, const ipv4net *operand2)
{
    ...
    case CO_IN:
    {
	return ((operand1 & operand2->m_netmask) ==
		(operand2->m_ip & operand2->m_netmask));
    }
    case CO_NE:
	return ((operand1 & operand2->m_netmask) !=
		(operand2->m_ip & operand2->m_netmask));
    ...
    default:
	throw sinsp_exception(
		"comparison operator not supported for ipv4 networks");
    }
}
...
bool flt_compare_ipv6addr(cmpop op, ipv6addr *operand1, ipv6addr *operand2)
{
    ...
    default:
	throw sinsp_exception(
		"comparison operator not supported for ipv6 addresses");
    }
}

bool flt_compare_ipv6net(cmpop op, const ipv6addr *operand1,
			 const ipv6net *operand2)
{
    switch(op)
    {
    case CO_EQ:
    ...
    default:
	throw sinsp_exception(
		"comparison operator not supported for ipv6 networks");
    }
}

bool flt_compare(cmpop op, ppm_param_type type, void *operand1, void *operand2,
		 uint32_t op1_len, uint32_t op2_len)
{
    //
    ...
    {
    case PT_INT8:
	return flt_compare_int64(op, (int64_t) * (int8_t *)operand1,
				 (int64_t) * (int8_t *)operand2);
    case PT_INT16:
	return flt_compare_int64(op, (int64_t) * (int16_t *)operand1,
				 (int64_t) * (int16_t *)operand2);
    case PT_INT32:
	return flt_compare_int64(op, (int64_t) * (int32_t *)operand1,
				 (int64_t) * (int32_t *)operand2);
    case PT_INT64:
    case PT_FD:
    case PT_PID:
    case PT_ERRNO:
	return flt_compare_int64(op, *(int64_t *)operand1,
				 *(int64_t *)operand2);
    case PT_FLAGS8:
    case PT_ENUMFLAGS8:
    case PT_UINT8:
    case PT_SIGTYPE:
	return flt_compare_uint64(op, (uint64_t) * (uint8_t *)operand1,
				  (uint64_t) * (uint8_t *)operand2);
    case PT_FLAGS16:
    case PT_UINT16:
    case PT_ENUMFLAGS16:
    case PT_PORT:
    case PT_SYSCALLID:
	return flt_compare_uint64(op, (uint64_t) * (uint16_t *)operand1,
				  (uint64_t) * (uint16_t *)operand2);
    case PT_UINT32:
    case PT_FLAGS32:
    case PT_ENUMFLAGS32:
    case PT_MODE:
    case PT_BOOL:
    case PT_IPV4ADDR:
	return flt_compare_uint64(op, (uint64_t) * (uint32_t *)operand1,
				  (uint64_t) * (uint32_t *)operand2);
    case PT_IPV4NET:
	return flt_compare_ipv4net(op, (uint64_t) * (uint32_t *)operand1,
				   (ipv4net *)operand2);
    case PT_IPV6ADDR:
	return flt_compare_ipv6addr(op, (ipv6addr *)operand1,
				    (ipv6addr *)operand2);
    case PT_IPV6NET:
	return flt_compare_ipv6net(op, (ipv6addr *)operand1,
				   (ipv6net *)operand2);
    case PT_IPADDR:
	if(op1_len == sizeof(struct in_addr))
	{
	    return flt_compare(op, PT_IPV4ADDR, operand1, operand2, op1_len,
			       op2_len);
	}
	else if(op1_len == sizeof(struct in6_addr))
	{
	    return flt_compare(op, PT_IPV6ADDR, operand1, operand2, op1_len,
			       op2_len);
	}
	else
	{
	    throw sinsp_exception("rawval_to_string called with IP address of "
				  "incorrect size " +
				  to_string(op1_len));
	}
    case PT_IPNET:
	if(op1_len == sizeof(struct in_addr))
	{
	    return flt_compare(op, PT_IPV4NET, operand1, operand2, op1_len,
			       op2_len);
	}
	else if(op1_len == sizeof(struct in6_addr))
	{
	    return flt_compare(op, PT_IPV6NET, operand1, operand2, op1_len,
			       op2_len);
	}
	else
	{
	    throw sinsp_exception("rawval_to_string called with IP network of "
				  "incorrect size " +
				  to_string(op1_len));
	}
    case PT_UINT64:
    case PT_RELTIME:
    case PT_ABSTIME:
	return flt_compare_uint64(op, *(uint64_t *)operand1,
				  *(uint64_t *)operand2);
    case PT_CHARBUF:
    case PT_FSPATH:
    case PT_FSRELPATH:
    ...
    case PT_BYTEBUF:
	return flt_compare_buffer(op, (char *)operand1, (char *)operand2,
				  op1_len, op2_len);
    case PT_DOUBLE:
...
bool flt_compare_avg(cmpop op, ppm_param_type type, void *operand1,
		     void *operand2, uint32_t op1_len, uint32_t op2_len,
		     uint32_t cnt1, uint32_t cnt2)
{
    ...
    //
    // If count = 0 we assume that the value is zero too (there are assertions
    // to check that, and we just divide by 1
    //
    if(cnt1 == 0)
    {
	cnt1 = 1;
					       ...
					       uint32_t len)
{
    ...
    default:
	ASSERT(false);
	throw sinsp_exception("wrong param type " +
			      to_string((long long)ptype));
    }
}
					   ...
					   uint32_t len)
{
    ...
    case PT_IPV4ADDR:
	snprintf(m_getpropertystr_storage, sizeof(m_getpropertystr_storage),
		 "%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8, rawval[0],
		 rawval[1], rawval[2], rawval[3]);
	return m_getpropertystr_storage;
    case PT_IPV6ADDR:
    {
	...
	strlcpy(m_getpropertystr_storage, address,
...
char *sinsp_filter_check::tostring(sinsp_evt *evt)
{
	...
	for(auto &val : m_extracted_values)
	{
	    ...
	    res += rawval_to_string(val.ptr, m_field->m_type,
				    m_field->m_print_format, val.len);
	}
	res += ")";
	strncpy(m_getpropertystr_storage, res.c_str(),
		sizeof(m_getpropertystr_storage) - 1);
	return m_getpropertystr_storage;
    }
    return rawval_to_string(m_extracted_values[0].ptr, m_field->m_type,
			    m_field->m_print_format, m_extracted_values[0].len);
}
...
Json::Value sinsp_filter_check::tojson(sinsp_evt *evt)
{
	...
	if(m_field->m_flags & EPF_IS_LIST)
	{
	    for(auto &val : m_extracted_values)
	    {
		jsonval.append(rawval_to_json(val.ptr, m_field->m_type,
					      m_field->m_print_format,
					      val.len));
	    }
	    return jsonval;
	}
	return rawval_to_json(m_extracted_values[0].ptr, m_field->m_type,
			      m_field->m_print_format,
					  ...
					  uint32_t i)
{
    ...
    parsed_len = parse_filter_value(str, len, filter_value_p(i),
				    filter_value(i)->size());
...
size_t sinsp_filter_check::parse_filter_value(const char *str, uint32_t len,
					      uint8_t *storage,
					      uint32_t storage_len)
{
	...
	parsed_len = sinsp_filter_value_parser::string_to_rawval(
		str, len, storage, storage_len, m_field->m_type);
    }
...
bool sinsp_filter_check::flt_compare(cmpop op, ppm_param_type type,
				     ...
				     uint32_t op2_len)
{
	...
	// flt_compare uses some additional logic for certain data types (e.g.
	// ipv6). However, for now we only support lists of types PT_CHARBUF and
	...
	// refactor filter_value_t to actually use flt_compare instead of
	// memcmp.
	if(type != PT_CHARBUF && type != PT_UINT64)
	{
	    throw sinsp_exception("list filters are only supported for CHARBUF "
				  "and UINT64 types");
	}
	...
	case CO_IN:
		...
		if((*it).len < m_val_storages_min_size ||
		   ...
		   m_val_storages_members.find(item) ==
			   m_val_storages_members.end())
		{
		    return false;
		}
	    }
    ...
    return flt_compare(m_cmpop, m_info.m_fields[m_field_id].m_type,
		       values[0].ptr, values[0].len, op2_len);
}

bool sinsp_filter_check::flt_compare(cmpop op, ppm_param_type type,
				     void *operand1, uint32_t op1_len,
				     uint32_t op2_len)
{
	...
	case PT_FSRELPATH:
	    ...
	    {
		if(::flt_compare(CO_EQ, type, operand1, filter_value_p(i),
				 op1_len, filter_value(i)->size()))
		{
		    return true;
		}
	    }
	    return false;
	default:
	    // For raw strings, the length may not be set. So we do a strlen to
	    // find it.
	    ...
	    {
		// CO_INTERSECTS is really more interesting when a filtercheck
		// can extract multiple values, and you're comparing the set of
		// extracted values against the set of rhs values.
		// sinsp_filter_checks only extract a single value, so
		// CO_INTERSECTS is really the same as CO_IN.
    ...
    {
	return (::flt_compare(op, type, operand1, filter_value_p(), op1_len,
			      op2_len));
    }
}

bool sinsp_filter_check::extract(gen_event *evt,
				 OUT vector<extract_value_t> &values,
...
uint8_t *sinsp_filter_check::extract(sinsp_evt *evt, OUT uint32_t *len,
				     bool sanitize_strings)
{
    return NULL;
}

bool sinsp_filter_check::extract_cached(sinsp_evt *evt,
					OUT vector<extract_value_t> &values,
					bool sanitize_strings)
{
...
bool sinsp_filter_check::compare(sinsp_evt *evt)
{
    ...
    return flt_compare(m_cmpop, m_info.m_fields[m_field_id].m_type,
		       m_extracted_values, m_val_storage_len);
}

sinsp_filter::sinsp_filter(sinsp *inspector) { m_inspector = inspector; }
...
///////////////////////////////////////////////////////////////////////////////
sinsp_filter_compiler::sinsp_filter_compiler(sinsp *inspector,
					     const string &fltstr,
					     bool ttable_only)
{
...
sinsp_filter_compiler::sinsp_filter_compiler(
	std::shared_ptr<gen_event_filter_factory> factory, const string &fltstr,
	bool ttable_only)
{
    m_factory = factory;
    m_filter = NULL;
...
sinsp_filter_compiler::sinsp_filter_compiler(
	...
	libsinsp::filter::ast::expr *fltast, bool ttable_only)
{
    m_factory = factory;
    m_filter = NULL;
    m_flt_ast = fltast;
...
sinsp_filter *sinsp_filter_compiler::compile()
{
	...
	delete new_filter;
	throw sinsp_exception(
		"filter error: factory did not create a sinsp_filter");
    }
...
static void add_filtercheck_value(gen_event_filter_check *chk, size_t idx,
				  const std::string &value)
{
    ...
    case CO_BCONTAINS:
    case CO_BSTARTSWITH:
	if(!sinsp_utils::unhex(std::vector<char>(value.c_str(),
						 value.c_str() + value.size()),
			       hex_bytes))
	{
	    throw sinsp_exception("filter error: bcontains and bstartswith "
				  "operator support hex strings only");
	}
...
void sinsp_filter_compiler::visit(libsinsp::filter::ast::value_expr *e)
{
	...
	throw sinsp_exception("filter error: unexpected identifier '" +
			      e->value + "'");
    }
...
gen_event_filter_check *sinsp_filter_compiler::create_filtercheck(string &field)
{
    ...
    if(chk == NULL)
    {
	throw sinsp_exception("filter_check called with nonexistent field " +
			      field);
    }
    return chk;
}

void sinsp_filter_compiler::check_ttable_only(string &field,
					      gen_event_filter_check *check)
{
    if(m_ttable_only)
    {
	sinsp_filter_check *sinsp_check =
		dynamic_cast<sinsp_filter_check *>(check);
	if(sinsp_check != nullptr &&
	   !(sinsp_check->get_fields()->m_flags &
	     filter_check_info::FL_WORKS_ON_THREAD_TABLE))
	{
	    if(field != "evt.rawtime" && field != "evt.rawtime.s" &&
	       field != "evt.rawtime.ns" && field != "evt.time" &&
	       field != "evt.time.s" && field != "evt.datetime" &&
	       field != "evt.reltime")
	    {
		throw sinsp_exception(
			"filter error: '" + field +
			"' is not supported for thread table filtering");
	    }
	}
    }
}
...
cmpop sinsp_filter_compiler::str_to_cmpop(string &str)
{
    ...
    ASSERT(false);
    throw sinsp_exception("filter error: unrecognized comparison operator '" +
			  string(str) + "'");
}
...
sinsp_filter_factory::new_filtercheck(const char *fldname)
{
    return m_available_checks.new_filter_check_from_fldname(fldname,
							    m_inspector, true);
}

std::list<gen_event_filter_factory::filter_fieldclass_info>
sinsp_filter_factory::get_fields()
{
