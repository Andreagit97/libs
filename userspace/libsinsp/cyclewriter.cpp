...
cycle_writer::cycle_writer(bool is_live):
	m_base_file_name(""), m_rollover_mb(0L), m_duration_seconds(0),
	m_file_limit(0), m_event_limit(0L), m_last_time(0),
	m_file_count_total(0), m_file_index(0), m_first_consider(false),
	m_event_count(0L), m_dumper(NULL), m_past_names(NULL)
{
    //
...
bool cycle_writer::setup(string base_file_name, int rollover_mb,
			 int duration_seconds, int file_limit,
			 unsigned long event_limit, scap_dumper_t** dumper)
{
    if(m_first_consider)
    {
	return false;
...
cycle_writer::conclusion cycle_writer::consider(sinsp_evt* evt)
{
    ...
    {
	if(!live && m_duration_seconds > 0 &&
	   m_base_file_name.find("%") != string::npos) // Here's the fuckin' bug
	...
	if(m_last_time == 0)
	{
	    m_last_time = evt->get_ts() /
			  1000000000; // 10^(-9) because it's nanoseconds
	}

	if((int)difftime(evt->get_ts() / 1000000000, m_last_time) >=
	   m_duration_seconds)
	{
...
string cycle_writer::get_current_file_name() { return m_last_file_name; }
...
cycle_writer::conclusion cycle_writer::next_file()
{
	...
	if(m_base_file_name.find("%") != string::npos)
	{
	    ...
	    their_size = strftime(file_name, our_size, m_base_file_name.c_str(),
				  our_time);

	    if(their_size == 0)
	    { /* TODO: if fail but as string size has been increased to 4096
		 it's very unlikely we get here */
	    }
	...
	if(m_limit_format[0] == 0) // I have no idea if this part is executed
				   // and, if so, if it works correctly
	{
	    int digit_count = 0;
	    ...
	    snprintf(
		    ...
		    m_limit_format, sizeof(m_limit_format),
