...
class mesos : public mesos_auth
{
    ...
    mesos(const std::string& state_uri,
	  ...
	  int timeout_ms = default_timeout_ms, bool is_captured = false,
	  bool verbose = false);
    ...
    template<typename T>
    bool connect(T http, typename T::element_type::callback_func_t func,
    ...
    typedef std::unordered_map<std::string, marathon_http::ptr_t>
	    marathon_http_map;

    void remove_framework_http(marathon_http_map& http_map,
    ...
    void set_marathon_groups_json(json_ptr_t json,
				  const std::string& framework_id);
    ...
    void set_marathon_apps_json(json_ptr_t json,
				const std::string& framework_id);
...
inline const mesos_state_t& mesos::get_state() const { return m_state; }
...
inline void mesos::clear_mesos() { m_state.clear_mesos(); }

inline void mesos::clear_marathon() { m_state.clear_marathon(); }
...
inline void mesos::parse_groups(json_ptr_t json,
				const std::string& framework_id)
{
    m_state.parse_groups(std::move(json), framework_id);
}

inline void mesos::parse_apps(Json::Value&& json,
			      const std::string& framework_id)
{
    m_state.parse_apps(std::move(json), framework_id);
}

inline void mesos::parse_groups(Json::Value&& json,
				const std::string& framework_id)
{
    m_state.parse_groups(std::move(json), framework_id);
}
