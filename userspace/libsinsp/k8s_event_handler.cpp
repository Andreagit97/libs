...
// filters normalize state and event JSONs, so they can be processed
// generically: event is turned into a single-entry array, state is turned into
// an array of ADDED events
...
k8s_event_handler::k8s_event_handler(k8s_state_t& state
				     ...
				     collector_ptr_t collector, std::string url,
				     const std::string& http_version,
				     ssl_ptr_t ssl, bt_ptr_t bt, bool connect,
	...
	k8s_handler("k8s_event_handler", true,
		    ...
		    collector, http_version, 1000L, ssl, bt, true, connect,
		    dependency_handler, blocking_socket,
#endif // HAS_CAPTURE
		    ~0, &state),
...
bool k8s_event_handler::handle_component(const Json::Value& json,
					 const msg_data* data)
{
    if(m_event_filter)
    {
	if(m_state)
	{
		...
		{
		    g_logger.log("K8s EVENT: handling event.",
				 sinsp_logger::SEV_TRACE);
		    const Json::Value& involved_object = json["involvedObject"];
		    if(!involved_object.isNull())
		    {
			bool is_aggregate = (get_json_string(json, "message")
						     .find("events with common "
							   "reason combined") !=
					     std::string::npos);
			time_t last_ts = 0;
			...
			// If that is also empty, use current timestamp and log
			// it. This change is necessitated because in
			// v1beta1/events, "EventTime" is the main field that
			// holds timestamp and `lastTimestamp` is deprecated.
			// This change is addressed towards that.
			std::string evtTime =
				get_json_string(json, "eventTime");
			...
			{
			    // Ideally we should NEVER hit this case. But log it
			    // if we do, so we know.
			    g_logger.log("K8s EVENT: both eventTime and "
					 "lastTimestamp are null, using "
					 "current timestamp. Event Json : " +
						 Json::FastWriter().write(json),
					 sinsp_logger::SEV_INFO);
			    last_ts = now_ts;
			}
			g_logger.log(
				"K8s EVENT: lastTimestamp=" +
					std::to_string(last_ts) +
					", now_ts=" + std::to_string(now_ts),
				sinsp_logger::SEV_TRACE);
			if(((last_ts > 0) &&
			    (now_ts > 0)) && // we got good timestamps
			   !is_aggregate &&  // not an aggregated cached event
			   ((now_ts - last_ts) <
			    10)) // event not older than 10 seconds
			{
			    const Json::Value& kind = involved_object["kind"];
			    const Json::Value& event_reason = json["reason"];
			    g_logger.log("K8s EVENT: involved object and event "
					 "reason found:" +
						 kind.asString() + '/' +
				...
				if(!is_allowed && !type.empty())
				{
				    std::string reason =
					    event_reason.asString();
				    is_allowed =
					    m_event_filter->allows_all(type);
				    if(!is_allowed && !reason.empty())
				    {
					is_allowed = m_event_filter->has(
						type, reason);
				    }
				}
				if(is_allowed)
				{
				    ...
				    if(evts.size() <
				       sinsp_user_event::max_events_per_cycle())
				    {
					k8s_event_t& evt =
						m_state->add_component<
							k8s_events,
							k8s_event_t>(
							evts, data->m_name,
							data->m_uid,
					   ...
					   sinsp_logger::SEV_DEBUG)
					{
					    g_logger.log(
						    ...
						    sinsp_logger::SEV_DEBUG);
				    ...
				    else if(!m_event_limit_exceeded) // only get
								     // in here
								     // once per
								     // cycle,
								     // to send
								     ...
								     // warning
				    {
					sinsp_user_event::emit_event_overflow(
						"Kubernetes", get_machine_id());
					m_event_limit_exceeded = true;
					return false;
				    }
				    else // event limit exceeded and overflow
			    ...
			    {
				g_logger.log("K8s EVENT: event type or "
					     "involvedObject kind not found.",
					     sinsp_logger::SEV_ERROR);
				if(g_logger.get_severity() >=
				   sinsp_logger::SEV_TRACE)
				{
				    g_logger.log(Json::FastWriter().write(json),
						 sinsp_logger::SEV_TRACE);
				}
				return false;
			    }
	    ...
	    {
		g_logger.log("K8s EVENT: msg data is null.",
			     sinsp_logger::SEV_ERROR);
		g_logger.log(Json::FastWriter().write(json),
			     sinsp_logger::SEV_TRACE);
		return false;
    ...
    {
	g_logger.log("K8s EVENT: no filter, K8s events disabled.",
		     sinsp_logger::SEV_TRACE);
	return false;
    }
    return true;
...
void k8s_event_handler::handle_json(Json::Value&& root)
{
    ...
    if(!m_state)
    {
	throw sinsp_exception("k8s_handler (" + get_id() +
			      "), state is null for " + get_url() + ").");
    }
	...
	if(type.isConvertibleTo(Json::stringValue))
	{
		...
		if(kind.isConvertibleTo(Json::stringValue))
		{
			...
			{
			    g_logger.log(std::string("Unsupported K8S " +
						     name() +
						     " event reason: ") +
