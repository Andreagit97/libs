...
template<> string* sinsp_fdinfo_t::tostring() { return &m_name; }
...
template<>
bool sinsp_fdinfo_t::set_net_role_by_guessing(sinsp* inspector,
					      sinsp_threadinfo* ptinfo,
					      sinsp_fdinfo_t* pfdinfo,
					      bool incoming)
{
    //
    // If this process owns the port, mark it as server, otherwise mark it as
    // client
    //
    if(ptinfo->is_bound_to_port(
	       pfdinfo->m_sockinfo.m_ipv4info.m_fields.m_dport))
    {
	if(ptinfo->uses_client_port(
		   pfdinfo->m_sockinfo.m_ipv4info.m_fields.m_sport))
	{
	    goto wildass_guess;
	}
...
wildass_guess:
    if(!(pfdinfo->m_flags & (sinsp_fdinfo_t::FLAGS_ROLE_CLIENT |
			     sinsp_fdinfo_t::FLAGS_ROLE_SERVER)))
    {
	//
	// We just assume that a server usually starts with a read and a client
	// with a write
	//
	if(incoming)
	{
...
template<> scap_l4_proto sinsp_fdinfo_t::get_l4proto()
{
    ...
    {
	if((scap_l4_proto)m_sockinfo.m_ipv4info.m_fields.m_l4proto ==
	   SCAP_L4_RAW)
	{
	    return SCAP_L4_RAW;
	}
    ...
    {
	if((scap_l4_proto)m_sockinfo.m_ipv6info.m_fields.m_l4proto ==
	   SCAP_L4_RAW)
	{
	    return SCAP_L4_RAW;
	}
...
template<>
void sinsp_fdinfo_t::register_event_callback(sinsp_pd_callback_type etype,
...
template<>
void sinsp_fdinfo_t::unregister_event_callback(sinsp_pd_callback_type etype,
					       sinsp_protodecoder* dec)
{
    ...
    {
    case CT_READ:
	for(it = m_callbacks->m_read_callbacks.begin();
	    it != m_callbacks->m_read_callbacks.end(); ++it)
	{
	    if(*it == dec)
	    {
	...
	break;
    case CT_WRITE:
	for(it = m_callbacks->m_write_callbacks.begin();
	    it != m_callbacks->m_write_callbacks.end(); ++it)
	{
	    if(*it == dec)
	    {
...
sinsp_fdinfo_t* sinsp_fdtable::add(int64_t fd, sinsp_fdinfo_t* fdinfo)
{
...
#endif
	    pair<unordered_map<int64_t, sinsp_fdinfo_t>::iterator, bool>
	...
	{
	    //
	    // Sometimes an FD-creating syscall can be called on an FD that is
	    // being closed (i.e the close enter has arrived but the close exit
	    // has not arrived yet). If this is the case, mark the new entry so
	    // that the successive close exit won't destroy it.
	    //
	...
	else
	{
	    //
	    // This can happen if:
	    //  - the event is a dup2 or dup3 that overwrites an existing FD
	    //  (perfectly legal)
	    //  - a close() has been dropped when capturing
	    //  - an fd has been closed by clone() or execve() (it happens when
	    //  the fd is opened with the FD_CLOEXEC flag,
	    //    which we don't currently parse.
	    // In either case, removing the old fd, replacing it with the new
	    // one and keeping going is a reasonable choice. We include an
	    // assertion to catch the situation.
	    //
	    // XXX Can't have this enabled until the FD_CLOEXEC flag is
	    // supported
	    // ASSERT(false);
	}
...
void sinsp_fdtable::erase(int64_t fd)
{
	...
	// Either the fd creation event was dropped or (more likely) our logic
	// doesn't support the call that created this fd. The assertion will
	// detect it, while in release mode we just keep going.
	//
	ASSERT(false);
#ifdef GATHER_INTERNAL_STATS
...
void sinsp_fdtable::clear() { m_table.clear(); }

size_t sinsp_fdtable::size() { return m_table.size(); }

void sinsp_fdtable::reset_cache() { m_last_accessed_fd = -1; }
...
void sinsp_fdtable::lookup_device(sinsp_fdinfo_t* fdi, uint64_t fd)
{
	...
	snprintf(procdir, sizeof(procdir), "%s/proc/%ld/", scap_get_host_root(),
		 m_tid);
	fdi->m_dev = scap_get_device_by_mount_id(m_inspector->m_h, procdir,
						 fdi->m_mount_id);
