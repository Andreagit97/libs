...
k8s_container::k8s_container(const std::string& name, const port_list& ports):
	m_name(name), m_ports(ports)
{
}

k8s_container::k8s_container(const k8s_container& other):
	m_name(other.m_name), m_ports(other.m_ports)
{
}

k8s_container::k8s_container(k8s_container&& other):
	m_name(std::move(other.m_name)), m_ports(std::move(other.m_ports))
{
}
...
const k8s_container::port*
k8s_container::get_port(const std::string& port_name) const
{
    for(const auto& port : m_ports)
    {
	if(port.get_name() == port_name)
...
k8s_component::k8s_component(type comp_type, const std::string& name,
			     const std::string& uid, const std::string& ns):
	m_type(comp_type),
...
k8s_pair_list k8s_component::extract_object(const Json::Value& object,
...
std::string k8s_component::get_api(const component_pair& p,
				   ext_list_ptr_t extensions)
{
    return get_api(p.first, extensions);
}

std::string k8s_component::get_api(const std::string& name,
				   ext_list_ptr_t extensions)
{
...
// TODO: proper selection process is more complicated, see “Labels and
// Selectors” at http://kubernetes.io/v1.0/docs/user-guide/labels.html
bool k8s_component::selector_in_labels(const k8s_pair_t& selector,
				       const k8s_pair_list& labels) const
{
    if(!labels.size())
    {
	return false;
...
// namespace
//
k8s_ns_t::k8s_ns_t(const std::string& name, const std::string& uid,
		   const std::string& ns):
...
k8s_node_t::k8s_node_t(const std::string& name, const std::string& uid,
		       const std::string& ns):
	k8s_component(COMPONENT_TYPE, name, uid, ns)
{
}

k8s_node_t::host_ip_list
k8s_node_t::extract_addresses(const Json::Value& status)
{
	...
	if(!addresses.isNull() && addresses.isArray())
	{
		...
		if(address.isObject())
		{
		    Json::Value::Members addr_names_list =
			    address.getMemberNames();
...
k8s_pod_t::k8s_pod_t(const std::string& name, const std::string& uid,
		     const std::string& ns):
	k8s_component(COMPONENT_TYPE, name, uid, ns)
{
}
...
k8s_replicas_t::k8s_replicas_t(int spec_replicas, int stat_replicas):
	m_spec_replicas(spec_replicas), m_stat_replicas(stat_replicas)
{
}

int k8s_replicas_t::get_count(const Json::Value& item,
...
void k8s_replicas_t::set_replicas(k8s_replicas_t& replicas,
				  const Json::Value& item)
{
    ...
    {
	int unavailable_replicas = k8s_replicas_t::get_count(
		item["status"], "unavailableReplicas");
	int spec_replicas = replicas.get_spec_replicas();
	if(spec_replicas != k8s_replicas_t::UNKNOWN_REPLICAS &&
	   unavailable_replicas < spec_replicas)
	{
...
k8s_rc_t::k8s_rc_t(const std::string& name, const std::string& uid,
		   const std::string& ns, k8s_component::type type):
	k8s_component(type, name, uid, ns)
{
}

std::vector<const k8s_pod_t*>
k8s_rc_t::get_selected_pods(const std::vector<k8s_pod_t>& pods) const
{
    std::vector<const k8s_pod_t*> pod_vec;
    for(const auto& pod : pods)
    {
	if(selectors_in_labels(pod.get_labels()) &&
	   get_namespace() == pod.get_namespace())
	{
...
// replica set
//
k8s_rs_t::k8s_rs_t(const std::string& name, const std::string& uid,
		   const std::string& ns):
...
k8s_service_t::k8s_service_t(const std::string& name, const std::string& uid,
			     const std::string& ns):
	k8s_component(COMPONENT_TYPE, name, uid, ns)
{
}
...
std::vector<const k8s_pod_t*>
k8s_service_t::get_selected_pods(const std::vector<k8s_pod_t>& pods) const
{
    std::vector<const k8s_pod_t*> pod_vec;
    for(const auto& pod : pods)
    {
...
k8s_daemonset_t::k8s_daemonset_t(const std::string& name,
				 const std::string& uid, const std::string& ns):
	k8s_component(COMPONENT_TYPE, name, uid, ns)
{
}
...
k8s_deployment_t::k8s_deployment_t(const std::string& name,
...
std::vector<const k8s_pod_t*>
k8s_deployment_t::get_selected_pods(const std::vector<k8s_pod_t>& pods) const
{
    std::vector<const k8s_pod_t*> pod_vec;
    for(const auto& pod : pods)
    {
...
k8s_event_t::k8s_event_t(const std::string& name, const std::string& uid,
			 const std::string& ns):
	k8s_component(COMPONENT_TYPE, name, uid, ns),
	m_name_translation{
		//
...
void k8s_event_t::post_process(k8s_state_t& state)
{
    ...
    {
	g_logger.log("K8s event: " + std::to_string(m_postponed_events.size()) +
			     " postponed events. "
	...
	{
	    g_logger.log("K8s event: event [" + it->first + "] post-processed.",
			 sinsp_logger::SEV_TRACE);
...
bool k8s_event_t::update(const Json::Value& item, k8s_state_t& state)
{
	...
	if(sev == "Warning")
	{
	    severity = user_event_logger::SEV_EVT_WARNING;
	}
	if(g_logger.get_severity() >= sinsp_logger::SEV_TRACE)
	{
	    g_logger.log(
    ...
    {
	g_logger.log("K8s event: cannot get involved object (null)",
	...
	{
	    g_logger.log("K8s event: cannot convert [" + ts +
				 "] to epoch timestamp",
			 sinsp_logger::SEV_ERROR);
	}
	g_logger.log("K8s EVENT update: time:" +
			     std::to_string(epoch_time_evt_s),
		     sinsp_logger::SEV_DEBUG);
    }
    else
    {
	g_logger.log(
		"K8s event: cannot convert time (null, empty or not string)",
		sinsp_logger::SEV_ERROR);
    }
    ...
    // Although it's easier and more efficient to obtain the involved object
    // data from the event itself, there is a downside - event may not carry the
    // data in the same format as reported in metadata protobuf (generated from
    // k8s state); an example is IP address vs. DNS name for node, there may be
    // other cases. For that reason, we try to obtain info about involved object
    // from state; if object is not found in state (due to undefined arrival
    // order of event and metadata messages), we get scope data from the event
    // itself.
    ...
    {
	g_logger.log(
		"K8s event: seconds since event occurred:" +
			std::to_string(epoch_time_now_s - epoch_time_evt_s),
		sinsp_logger::SEV_TRACE);
	std::string t;
	...
	if(comp && !t.empty())
	{
	    ...
	    if(!comp_name.empty())
	    {
		scope.add(std::string("kubernetes.").append(t).append(".name"),
			  comp_name);
	    }
	    /* no labels for now
	    for(const auto& label : comp->get_labels())
	    {
		    ...
		    //g_logger.log("EVENT label: [" + label.first + ':' +
	    label.second + ']', sinsp_logger::SEV_DEBUG);
		    if(event_scope::check(label.second))
		    {
			    scope.append(" and
	    kubernetes.").append(t).append(".label.").append(label.first).append(1,
	    '=').append(label.second);
		    }
		    else
		    {
			    g_logger.log("K8s invalid scope entry: [" +
	    label.second + ']', sinsp_logger::SEV_WARNING);
		    }
	    }*/
	}
	else
	{
	    g_logger.log(
		    "K8s event: cannot obtain component (component with UID [" +
			    component_uid +
			    "] not found), trying to build scope directly from "
			    "event ...",
		    sinsp_logger::SEV_TRACE);
    ...
    {
	g_logger.log("K8s event: cannot obtain component UID, trying to build "
		     "scope directly from event ...",
		     sinsp_logger::SEV_TRACE);
	make_scope(obj, scope);
    }
    ...
    auto evt = sinsp_user_event(
	    epoch_time_evt_s, std::move(event_name), std::move(description),
	    std::move(scope.get_ref()), std::move(tags), severity);
...
void k8s_event_t::make_scope_impl(const Json::Value& obj, std::string comp,
				  event_scope& scope, bool ns)
{
    if(ns)
    {
    ...
    if(comp.length() &&
       ci_compare::is_equal(get_json_string(obj, "kind"), comp))
    {
	const std::string& comp_name = get_json_string(obj, "name");
	if(!comp_name.empty())
	{
	    comp[0] = tolower(comp[0]);
	    scope.add(std::string("kubernetes.").append(comp).append(".name"),
		      comp_name);
	}
	if(comp_name.empty())
	{
	    g_logger.log("K8s " + comp + " event detected but " + comp +
				 " name could not be determined. Scope will be "
				 "empty.",
    ...
    {
	g_logger.log("K8s event detected but component name was empty. Scope "
		     "will be empty.",
		     sinsp_logger::SEV_WARNING);
    }
}
...
void k8s_event_t::make_scope(const Json::Value& obj, event_scope& scope)
{
    ...
    else if(ci_compare::is_equal(get_json_string(obj, "kind"),
