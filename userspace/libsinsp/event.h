...
typedef enum filtercheck_field_flags
{
    ...
    EPF_ARG_REQUIRED = 1 << 2, ///< this field includes an argument, under the
			       ///< form 'property.argument'.
    EPF_TABLE_ONLY = 1 << 3,   ///< this field is designed to be used in a table
			       ///< and won't appear in the field listing.
    EPF_INFO =
	    1
	    << 4, ///< this field contains summary information about the event.
    EPF_CONVERSATION =
	    1 << 5,	  ///< this field can be used to identify conversations.
...
typedef struct filtercheck_field_info
{
    ...
    ppm_print_format m_print_format; ///< If this is a numeric field, this flag
				     ///< specifies if it should be rendered as
				     ///< octal, decimal or hex.
    char m_name[64];		     ///< Field name.
    char m_display[64]; ///< Field display name (short description). May be
			///< empty.
    char m_description[1024]; ///< Field description.
} filtercheck_field_info;

/** @defgroup event Event manipulation
 * Classes to manipulate events, extract their content and convert them into
 * strings.
 *  @{
 */
...
class SINSP_PUBLIC sinsp_evttables
{
    ...
    const struct ppm_event_info*
	    m_event_info; ///< List of events supported by the capture and
			  ///< analysis subsystems. Each entry fully documents
			  ///< an event and its parameters.
    const struct ppm_syscall_desc*
	    m_syscall_info_table; ///< List of system calls that the capture
				  ///< subsystem recognizes, including the ones
...
class SINSP_PUBLIC sinsp_evt : public gen_event
{
    ...
    */
	...
	PF_JSON =
		(1 << 1), ///< Json formatting with data in normal screen format
	PF_SIMPLE =
		(1 << 2),  ///< Reduced output, e.g. not type character for FDs
	PF_HEX = (1 << 3), ///< Hexadecimal output
	PF_HEXASCII = (1 << 4),	    ///< Hexadecimal + ASCII output
	PF_EOLS = (1 << 5),	    ///< Normal + end of lines
	PF_EOLS_COMPACT = (1 << 6), ///< Normal + end of lines but with no force
				    ///< EOL at the beginning
	PF_BASE64 = (1 << 7),	    ///< Base64 output
	PF_JSONEOLS =
		(1 << 8), ///< Json formatting with data in hexadecimal format
	PF_JSONHEX =
		(1 << 9), ///< Json formatting with data in hexadecimal format
	PF_JSONHEXASCII = (1 << 10), ///< Json formatting with data in
				     ///< hexadecimal + ASCII format
	PF_JSONBASE64 =
		(1 << 11), ///< Json formatting with data in base64 format
    };
    ...
    */
    void inspector(sinsp* value) { m_inspector = value; }
    ...
    */
    inline int16_t get_cpuid() const { return m_cpuid; }
    ...
    */
    inline uint16_t get_type() const override { return m_pevt->type; }
    ...
    */
    inline uint64_t get_ts() const override { return m_pevt->ts; }
    ...
    */
    inline ppm_event_category get_category() const { return m_info->category; }
    ...
    */
    inline sinsp_fdinfo_t* get_fd_info() { return m_fdinfo; }
    ...
    /*!
      ...
      \note For events that are not I/O related, get_fd_num() returns
      sinsp_evt::INVALID_FD_NUM.
    */
    int64_t get_fd_num();
    ...
    */
    std::string get_param_value_str(const std::string& name,
				    bool resolved = true);
    ...
    */
    ...
    /*!
      \brief Return whether or not a simple consumer that privileges low
      overhead to full event capture should consider this event. (Generally,
      these events are automatically filtered out, but some events related to
      internal tracking are returned by next() anyway).
    */

    bool simple_consumer_consider();

    inline uint16_t get_source() const override { return ESRC_SINSP; }
...
// Doxygen doesn't understand VISIBILITY_PRIVATE
    ...
    Json::Value get_param_as_json(uint32_t id, OUT const char** resolved_str,
				  param_fmt fmt = PF_NORMAL);

    const char* get_param_value_str(const char* name,
    ...
    inline void init(scap_evt* scap_event, ppm_event_info* ppm_event,
		     sinsp_threadinfo* threadinfo, sinsp_fdinfo_t* fdinfo)
    {
	m_pevt = scap_event;
	m_info = ppm_event;
	m_tinfo_ref.reset(); // we don't own the threadinfo so don't try to
			     // manage its lifetime
	m_tinfo = threadinfo;
	m_fdinfo = fdinfo;
    }
    ...
    char* render_fd(int64_t fd, const char** resolved_str,
		    sinsp_evt::param_fmt fmt);
    int render_fd_json(Json::Value* ret, int64_t fd, const char** resolved_str,
		       sinsp_evt::param_fmt fmt);
    ...
    sinsp* m_inspector;
    scap_evt* m_pevt;
    scap_evt* m_poriginal_evt; // This is used when the original event is
			       // replaced by a different one (e.g. in the case
			       // of user events)
    char* m_pevt_storage; // In some cases an alternate buffer is used to hold
			  // m_pevt. This points to that storage.
    uint16_t m_cpuid;
    uint64_t m_evtnum;
    uint32_t m_flags;
    ...
    // reference to keep threadinfo alive. currently only used for synthetic
    // container event thread info it should either be null, or point to the
    // same place as m_tinfo
