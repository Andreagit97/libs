...
mesos_http::mesos_http(mesos& m, const uri& url,
		       bool discover_mesos_lead_master, bool discover_marathon,
		       int timeout_ms, const string& token):
	m_sync_curl(curl_easy_init()),
	m_select_curl(curl_easy_init()), m_mesos(m), m_url(url),
	m_connected(true), m_watch_socket(-1), m_timeout_ms(timeout_ms),
	m_callback_func(0), m_curl_version(curl_version_info(CURLVERSION_NOW)),
	m_is_mesos_state(url.to_string().find(mesos::default_state_api) !=
			 std::string::npos),
	m_discover_lead_master(discover_mesos_lead_master),
	m_discover_marathon(discover_marathon), m_token(token)
{
	...
	check_error(curl_easy_setopt(m_sync_curl, CURLOPT_HTTPHEADER,
				     m_sync_curl_headers.ptr()));
    }
    if(m_url.is_secure())
    {
    ...
    check_error(curl_easy_setopt(m_sync_curl, CURLOPT_CONNECTTIMEOUT_MS,
				 m_timeout_ms));
    check_error(
	    curl_easy_setopt(m_sync_curl, CURLOPT_TIMEOUT_MS, m_timeout_ms));

    check_error(curl_easy_setopt(m_select_curl, CURLOPT_CONNECTTIMEOUT_MS,
				 m_timeout_ms));
    discover_mesos_leader();
}

mesos_http::~mesos_http() { cleanup(); }
...
Json::Value mesos_http::get_state_frameworks()
{
	...
	if(reader.parse(os.str(), root))
	{
		...
		throw sinsp_exception(
			"mesos_http: Unexpected condition while detecting "
			"Mesos master: frameworks entry not found.");
	    }
	}
	else
	{
	    ...
	    g_json_error_log.log(os.str(), errstr,
				 ...
				 m_url.to_string());
	    throw sinsp_exception(
		    "mesos_http: Mesos master leader detection failed in "
		    "get_state_frameworks(): Invalid JSON (" +
		    errstr + ")");
	}
    }
    else
    {
	throw sinsp_exception(std::string("mesos_http: Mesos master leader [") +
...
void mesos_http::discover_mesos_leader()
{
    if(m_is_mesos_state)
    {
	g_logger.log("mesos_http: Inspecting Mesos leader [" +
			     m_url.to_string(false) + ']',
	...
	if(res == CURLE_OK)
	{
	    ...
	    check_error(curl_easy_getinfo(m_sync_curl, CURLINFO_RESPONSE_CODE,
					  &http_response_code));
	    if(sinsp_curl::is_redirect(http_response_code))
	    {
		uri newurl(m_redirect);
		m_url.set_host(newurl.get_host());
		g_logger.log("mesos_http: Detected Mesos master leader HTTP "
			     "redirect: [" +
				     m_url.to_string(false) + ']',
		...
		if(frameworks.size()) // this is master leader
		{
		    ...
		    g_logger.log("mesos_http: Found Mesos master leader [" +
					 m_url.to_string(false) + ']',
				 sinsp_logger::SEV_INFO);
		    return;
		}
		else if(!m_discover_lead_master) // this is standby server and
						 // autodiscovery is disabled
		{
		    throw sinsp_exception("mesos_http: Detected standby Mesos "
					  "master: autodiscovery not enabled. "
					  "Giving up (will retry).");
		}
		else // autodiscovery is enabled, find where is the master
		{
			...
			if(pos != std::string::npos &&
			   (pos + 1) < leader_address.size())
			{
			    std::string address = m_url.get_scheme() + "://";
			    if(!m_mesos.m_mesos_credentials.first.empty())
			    {
				address.append(m_mesos.m_mesos_credentials
						       .first)
					.append(1, ':')
					.append(m_mesos.m_mesos_credentials
							.second)
					.append(1, '@');
			    }
			    address.append(leader_address.substr(pos + 1))
				    .append(mesos::default_state_api);
			    if(address != m_url.to_string(true))
			    {
				g_logger.log(
					"mesos_http: Detected Mesos master "
					"leader redirect: [" +
			    ...
			    {
				throw sinsp_exception(
					"mesos_http: Mesos master leader not "
					"discovered at [" +
					uri(address).to_string(false) +
					"] . "
			...
			{
			    throw sinsp_exception(
				    "mesos_http: Unexpected leader entry "
				    "format while detecting Mesos master: " +
				    leader_address);
			}
		    }
		    else
			...
			g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
			throw sinsp_exception(
				"mesos_http: Unexpected condition while "
				"detecting Mesos master leader [" +
				m_url.to_string(false) +
		...
		g_json_error_log.log(os.str(), errstr,
				     sinsp_utils::get_current_time_ns(),
				     m_url.to_string());
		throw sinsp_exception("mesos_http: Mesos master leader [" +
				      m_url.to_string(false) +
				      "] detection failed: Invalid JSON (" +
				      errstr + ")");
	    }
	}
	else
	{
	    throw sinsp_exception(
		    "mesos_http: Mesos master leader [" +
		    m_url.to_string(false) +
		    "] detection failed: " + curl_easy_strerror(res));
	}
    }
...
std::string mesos_http::get_framework_url(const Json::Value& framework)
{
	...
	if(!fw_url.isNull() && fw_url.isString() && !fw_url.asString().empty())
	{
	    uri url(std::string("http://")
			    .append(fw_url.asString())
			    .append(":8080"));
	    if(is_marathon && has_creds)
	    {
		url.set_credentials(m_mesos.m_marathon_credentials);
...
void mesos_http::discover_framework_uris(const Json::Value& frameworks)
{
    ...
    {
	throw sinsp_exception(
		"mesos_http: Unexpected condition while inspecting Marathon "
		"framework: frameworks entry not found.");
    }
    if(frameworks.isArray())
    {
	g_logger.log("Discovered " + std::to_string(frameworks.size()) +
			     " frameworks.",
		     sinsp_logger::SEV_DEBUG);
	for(const auto& framework : frameworks)
	{
		...
		throw sinsp_exception(
			"mesos_http: Unexpected condition while detecting "
			"Marathon framework: ID entry not found.");
	    }
	    else
	    {
		...
		g_logger.log("Examining " + name + " [" + id.asString() +
				     "] framework.",
			     sinsp_logger::SEV_DEBUG);
		if(!active.isNull() && active.isBool() && active.asBool())
		{
			...
			if(m_discover_marathon)
			{
			    ...
			    {
				g_logger.log(std::string("mesos_http: Found "
							 "Marathon framework ")
						     .append(name)
						     .append(" (")
						     .append(id.asString())
						     .append(") at [")
						     .append(framework_url)
						     .append(1, ']'),
					     sinsp_logger::SEV_INFO);
				if(!m_marathon_uris.size())
				{
				    m_marathon_uris.emplace_back(
					    get_framework_url(framework));
				}
				else
				{
				    g_logger.log(
					    "mesos_http: Multiple marathon "
					    "URIs discovered; only the first "
					    "one (" +
						    m_marathon_uris[0] +
						    ") will have effect;"
						    " others will be treated "
						    "as generic frameworks.",
			    ...
			    {
				g_logger.log(
					std::string("mesos_http: Skipping "
						    "non-Marathon framework "
						    "URL detection ")
						.append(name)
						.append(" (")
						.append(id.asString())
			...
			{
			    g_logger.log(std::string("mesos_http: Marathon "
		    ...
		    {
			if(m_discover_marathon &&
			   mesos_framework::is_root_marathon(name))
			{
			    std::string errstr = "mesos_http: Can not obtain "
						 "URL for Marathon framework.";
		...
		else // framework exists, but is not active - remove it if we
		     // were watching it so far
		{
		    g_logger.log(std::string("mesos_http: Mesos framework ")
					 .append(name)
					 .append(" (")
					 .append(id.asString())
					 .append(") deactivated."),
				 sinsp_logger::SEV_INFO);
		    std::string framework_url = get_framework_url(framework);
		    for(marathon_uri_t::iterator it = m_marathon_uris.begin();
			it != m_marathon_uris.end();)
		    {
			if(framework_url == *it)
			{
    ...
    {
	throw sinsp_exception(
		"mesos_http: Mesos master leader detection failed: " +
		m_url.to_string(false));
    }
}
...
{
    g_logger.log(std::string("mesos_http: Retrieving data from ") +
    ...
    check_error(curl_easy_setopt(m_sync_curl, CURLOPT_HEADERFUNCTION,
				 sinsp_curl::header_callback));

    check_error(curl_easy_setopt(
	    m_sync_curl, CURLOPT_NOSIGNAL,
	    1)); // Prevent "longjmp causes uninitialized stack frame" bug
    check_error(
	    curl_easy_setopt(m_sync_curl, CURLOPT_ACCEPT_ENCODING, "deflate"));
    check_error(
	    curl_easy_setopt(m_sync_curl, CURLOPT_TIMEOUT_MS, m_timeout_ms));
    check_error(curl_easy_setopt(m_sync_curl, CURLOPT_WRITEFUNCTION,
				 sinsp_curl::write_data));
...
bool mesos_http::get_all_data(callback_func_t parse)
{
    ...
    {
	std::string errstr =
		std::string("Could not fetch url:") + curl_easy_strerror(res);
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log("", errstr, sinsp_utils::get_current_time_ns(),
			     m_url.to_string());
	m_connected = false;
    }
    else
	...
	{
	    g_logger.log("mesos_http: HTTP redirect (" +
	...
	else
	{
	    std::string errstr;
	    errstr = reader.getFormattedErrorMessages();
	    g_logger.log("mesos_http: Mesos or Marathon Invalid JSON received "
			 "from [" +
				 m_url.to_string(false) + "]: " + errstr,
			 sinsp_logger::SEV_WARNING);
...
int mesos_http::get_socket(long timeout_ms)
{
    ...
    {
	throw sinsp_exception(
		"mesos_http: Cannot create watch socket (request empty).");
    }
	...
	check_error(
		curl_easy_setopt(m_select_curl, CURLOPT_TCP_KEEPIDLE, 300L));
	// interval time between keep-alive probes
	check_error(
		curl_easy_setopt(m_select_curl, CURLOPT_TCP_KEEPINTVL, 10L));
#endif // LIBCURL_VERSION_MAJOR >= 7 && LIBCURL_VERSION_MINOR >= 25

	check_error(curl_easy_perform(m_select_curl));

	check_error(curl_easy_getinfo(m_select_curl, CURLINFO_LASTSOCKET,
				      &sockextr));
	...
	{
	    throw sinsp_exception(
		    "mesos_http: Error obtaining socket: timeout.");
	}

	g_logger.log(
		std::string("mesos_http: Connected; collecting data from ") +
			uri(url).to_string(false),
		sinsp_logger::SEV_DEBUG);
    }
    ...
    if(m_watch_socket <= 0)
    {
	throw sinsp_exception("mesos_http: Error obtaining socket: " +
			      std::to_string(m_watch_socket));
    }
...
void mesos_http::send_request()
{
    ...
    // size_t iolen = send(m_watch_socket, m_request.c_str(), m_request.size(),
    // 0);
    size_t iolen;
    check_error(curl_easy_send(m_select_curl, m_request.c_str(),
			       m_request.size(), &iolen));
    if((iolen <= 0) || (m_request.size() != iolen))
    {
	throw sinsp_exception(
		"mesos_http: Mesos send socket connection error.");
    }
    else if(!wait(0))
    {
...
void mesos_http::handle_json(std::string::size_type end_pos, bool chunked)
{
	...
	if(m_data_buf.length() >= end_pos + 1)
	{
	    ...
	    {
		std::string errstr = "mesos_http: Invalid Mesos or Marathon "
				     "JSON data detected (chunked transfer).";
		g_logger.log(errstr, sinsp_logger::SEV_ERROR);
		g_json_error_log.log(m_data_buf, errstr,
				     sinsp_utils::get_current_time_ns(),
				     m_url.to_string());
	    ...
	    {
		(m_mesos.*
		 m_callback_func)(try_parse(m_data_buf, m_url.to_string()),
				  m_framework_id);
	    }
	    m_data_buf.clear();
...
bool mesos_http::detect_chunked_transfer(const std::string& data)
{
	...
	if(cl_pos != std::string::npos)
	{
		...
		{
		    std::string errstr =
			    "Invalid HTTP content length from [: " +
			    m_url.to_string(false) + ']' + std::to_string(len);
		    (m_mesos.*m_callback_func)(nullptr, m_framework_id);
		    m_data_buf.clear();
		    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
		    g_json_error_log.log(data, errstr,
					 sinsp_utils::get_current_time_ns(),
					 m_url.to_string());
		    return false;
		}
...
void mesos_http::extract_data(std::string& data)
{
    ...
    {
	string errstr = "mesos_http: An error occurred while detecting chunked "
			"transfer.";
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(data, errstr, sinsp_utils::get_current_time_ns(),
			     m_url.to_string());
	return;
    }
...
bool mesos_http::on_data()
{
    ...
    {
	throw sinsp_exception(
		"mesos_http: Cannot parse data (parse function null).");
    }
	...
	do
	{
	    check_error(ret = curl_easy_recv(m_select_curl, buf, sizeof(buf),
					     &iolen));
	    if(iolen > 0)
	    {
    ...
    {
	errstr = std::string("mesos_http: Data receive error [" +
			     m_url.to_string() + "]: ")
			 .append(ex.what());
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(buf, errstr, sinsp_utils::get_current_time_ns(),
			     m_url.to_string());
	return false;
    }
    return true;
...
connection_closed:
    errstr = "mesos_http: Mesos or Marathon API connection [" +
	     m_url.to_string() + "] closed.";
    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
    g_json_error_log.log(buf, errstr, sinsp_utils::get_current_time_ns(),
			 m_url.to_string());
    m_connected = false;
...
std::string mesos_http::make_uri(const std::string& path)
{
    ...
    if(!user.empty())
    {
	target_uri.append(user)
		.append(1, ':')
		.append(url.get_password())
		.append(1, '@');
    }
...
Json::Value mesos_http::get_task_labels(const std::string& task_id)
{
	...
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(task_id, errstr,
			     sinsp_utils::get_current_time_ns(), uri);
	return labels;
    }
	...
	{
	    std::string errstr;
	    errstr = "mesos_http: Error parsing tasks (" +
    ...
    {
	std::string errstr =
		std::string("mesos_http: Error parsing tasks:") + ex.what();
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(os.str(), errstr,
			     sinsp_utils::get_current_time_ns(), uri);
    }
