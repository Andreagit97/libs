...
sinsp_threadinfo::sinsp_threadinfo(sinsp* inspector):
	m_tracer_parser(NULL), m_inspector(inspector), m_fdtable(inspector)
{
    init();
}
...
void sinsp_threadinfo::fix_sockets_coming_from_proc()
{
	...
	if(it->second.m_type == SCAP_FD_IPV4_SOCK)
	{
	    if(m_inspector->m_thread_manager->m_server_ports.find(
		       it->second.m_sockinfo.m_ipv4info.m_fields.m_sport) !=
		...
		it->second.m_sockinfo.m_ipv4info.m_fields.m_sip =
			it->second.m_sockinfo.m_ipv4info.m_fields.m_dip;
		it->second.m_sockinfo.m_ipv4info.m_fields.m_dip = tip;
		it->second.m_sockinfo.m_ipv4info.m_fields.m_sport =
			it->second.m_sockinfo.m_ipv4info.m_fields.m_dport;
		it->second.m_sockinfo.m_ipv4info.m_fields.m_dport = tport;

		it->second.m_name = ipv4tuple_to_string(
			&it->second.m_sockinfo.m_ipv4info,
					...
					OUT sinsp_fdinfo_t* res)
{
	...
	newfdi->m_sockinfo.m_ipv4info.m_fields.m_sport =
		fdi->info.ipv4info.sport;
	newfdi->m_sockinfo.m_ipv4info.m_fields.m_dport =
		fdi->info.ipv4info.dport;
	newfdi->m_sockinfo.m_ipv4info.m_fields.m_l4proto =
		fdi->info.ipv4info.l4proto;
	...
	newfdi->m_name = ipv4tuple_to_string(
		&newfdi->m_sockinfo.m_ipv4info,
		m_inspector->m_hostname_and_port_resolution_enabled);
	break;
    case SCAP_FD_IPV4_SERVSOCK:
	newfdi->m_sockinfo.m_ipv4serverinfo.m_ip = fdi->info.ipv4serverinfo.ip;
	newfdi->m_sockinfo.m_ipv4serverinfo.m_port =
		fdi->info.ipv4serverinfo.port;
	newfdi->m_sockinfo.m_ipv4serverinfo.m_l4proto =
		fdi->info.ipv4serverinfo.l4proto;
	newfdi->m_name = ipv4serveraddr_to_string(
		&newfdi->m_sockinfo.m_ipv4serverinfo,
		m_inspector->m_hostname_and_port_resolution_enabled);
	...
	if(sinsp_utils::is_ipv4_mapped_ipv6(
		   (uint8_t*)&fdi->info.ipv6info.sip) &&
	   sinsp_utils::is_ipv4_mapped_ipv6((uint8_t*)&fdi->info.ipv6info.dip))
	{
	    ...
	    newfdi->m_sockinfo.m_ipv4info.m_fields.m_sip =
		    fdi->info.ipv6info.sip[3];
	    newfdi->m_sockinfo.m_ipv4info.m_fields.m_dip =
		    fdi->info.ipv6info.dip[3];
	    newfdi->m_sockinfo.m_ipv4info.m_fields.m_sport =
		    fdi->info.ipv6info.sport;
	    newfdi->m_sockinfo.m_ipv4info.m_fields.m_dport =
		    fdi->info.ipv6info.dport;
	    newfdi->m_sockinfo.m_ipv4info.m_fields.m_l4proto =
		    fdi->info.ipv6info.l4proto;
	...
	else
	{
	    ...
	    newfdi->m_sockinfo.m_ipv6info.m_fields.m_sport =
		    fdi->info.ipv6info.sport;
	    newfdi->m_sockinfo.m_ipv6info.m_fields.m_dport =
		    fdi->info.ipv6info.dport;
	    newfdi->m_sockinfo.m_ipv6info.m_fields.m_l4proto =
		    fdi->info.ipv6info.l4proto;
	    if(fdi->info.ipv6info.l4proto == SCAP_L4_TCP)
	    {
		newfdi->m_flags |= sinsp_fdinfo_t::FLAGS_SOCKET_CONNECTED;
	    }
	    newfdi->m_name = ipv6tuple_to_string(
		    &newfdi->m_sockinfo.m_ipv6info,
		    m_inspector->m_hostname_and_port_resolution_enabled);
	}
	break;
	...
	newfdi->m_sockinfo.m_ipv6serverinfo.m_port =
		fdi->info.ipv6serverinfo.port;
	newfdi->m_sockinfo.m_ipv6serverinfo.m_l4proto =
		fdi->info.ipv6serverinfo.l4proto;
	newfdi->m_name = ipv6serveraddr_to_string(
		&newfdi->m_sockinfo.m_ipv6serverinfo,
		m_inspector->m_hostname_and_port_resolution_enabled);
	...
	newfdi->m_sockinfo.m_unixinfo.m_fields.m_source =
		fdi->info.unix_socket_info.source;
	newfdi->m_sockinfo.m_unixinfo.m_fields.m_dest =
    ...
    //
    // Call the protocol decoder callbacks associated to notify them about this
    // FD
    //
...
void sinsp_threadinfo::init(scap_threadinfo* pi)
{
    ...
    m_flags |= pi->flags;
    m_flags |= PPM_CL_ACTIVE; // Assume that all the threads coming from /proc
			      // are real, active threads
    m_fdtable.clear();
	...
	if(m_inspector->m_filter != NULL &&
	   m_inspector->m_filter_proc_table_when_saving)
	{
	    tevt.m_tinfo = this;
...
{
    scap_userinfo* user =
	    m_inspector->m_usergroup_manager.get_user(m_container_id, uid);
    if(!user)
    {
	// this can fail if import_user is disabled
	user = m_inspector->m_usergroup_manager.add_user(
		m_container_id, uid, m_group.gid, NULL, NULL, NULL,
		m_inspector->is_live());
    }
...
void sinsp_threadinfo::set_group(uint32_t gid)
{
    scap_groupinfo* group =
	    m_inspector->m_usergroup_manager.get_group(m_container_id, gid);
    if(!group)
    {
	// this can fail if import_user is disabled
	group = m_inspector->m_usergroup_manager.add_group(
		m_container_id, gid, NULL, m_inspector->is_live());
    }
...
{
    scap_userinfo* login_user =
	    m_inspector->m_usergroup_manager.get_user(m_container_id, loginuid);
    if(!login_user)
    {
	// this can fail if import_user is disabled
	login_user = m_inspector->m_usergroup_manager.add_user(
		m_container_id, loginuid, m_group.gid, NULL, NULL, NULL,
		m_inspector->is_live());
    }
...
std::string sinsp_threadinfo::get_comm() const { return m_comm; }

std::string sinsp_threadinfo::get_exe() const { return m_exe; }

std::string sinsp_threadinfo::get_exepath() const { return m_exepath; }
...
void sinsp_threadinfo::set_env(const char* env, size_t len)
{
	...
	else
	{
	    g_logger.format(sinsp_logger::SEV_INFO,
			    "Failed to load environment for process %lu [%s] "
			    "from /proc, using first %d bytes",
			    m_pid, m_comm.c_str(), SCAP_MAX_ENV_SIZE);
	}
    }
...
{
    string environ_path = string(scap_get_host_root()) + "/proc/" +
			  to_string(m_pid) + "/environ";
...
void sinsp_threadinfo::remove_fd(int64_t fd) { get_fd_table()->erase(fd); }
...
void sinsp_threadinfo::set_cwd(const char* cwd, uint32_t cwdlen)
{
    ...
    if(tinfo)
    {
	sinsp_utils::concatenate_paths(tpath, SCAP_MAX_PATH_SIZE,
				       (char*)tinfo->m_cwd.c_str(),
				       (uint32_t)tinfo->m_cwd.size(), cwd,
				       cwdlen, m_inspector->m_is_windows);
...
void sinsp_threadinfo::allocate_private_state()
{
	...
	vector<uint32_t>* sizes =
		&m_inspector->m_thread_privatestate_manager.m_memory_sizes;
...
void* sinsp_threadinfo::get_private_state(uint32_t id)
{
    ...
    {
	ASSERT(false);
	throw sinsp_exception("invalid thread state ID" +
...
void sinsp_threadinfo::traverse_parent_state(visitor_func_t& visitor)
{
	...
	for(uint32_t i = 0; i < 2; i++)
	{
	    ...
	    {
		// Note we only log a loop once for a given main thread, to
		// avoid flooding logs.
		if(!m_parent_loop_detected)
		{
		    g_logger.log(
			    string("Loop in parent thread state detected for "
				   "pid ") +
				    std::to_string(m_pid) +
				    ". stopped at tid= " +
				    std::to_string(slow->m_tid) +
...
void sinsp_threadinfo::populate_cmdline(string& cmdline,
...
string sinsp_threadinfo::get_path_for_dir_fd(int64_t dir_fd)
{
	...
	int ret;
	snprintf(proc_path, sizeof(proc_path), "%s/proc/%lld/fd/%lld",
		 scap_get_host_root(), (long long)m_pid, (long long)dir_fd);
...
size_t sinsp_threadinfo::args_len() const { return strvec_len(m_args); }

size_t sinsp_threadinfo::env_len() const { return strvec_len(m_env); }
...
void sinsp_threadinfo::add_to_iovec(const string& str,
				    ...
				    struct iovec& iov, uint32_t& alen,
				    std::string& rem) const
{
...
void sinsp_threadinfo::fd_to_scap(scap_fdinfo* dst, sinsp_fdinfo_t* src)
{
	...
	dst->info.ipv4info.l4proto =
		src->m_sockinfo.m_ipv4info.m_fields.m_l4proto;
	break;
    case SCAP_FD_IPV4_SERVSOCK:
	...
	dst->info.ipv4serverinfo.l4proto =
		src->m_sockinfo.m_ipv4serverinfo.m_l4proto;
	break;
    case SCAP_FD_IPV6_SOCK:
	copy_ipv6_address(dst->info.ipv6info.sip,
			  src->m_sockinfo.m_ipv6info.m_fields.m_sip.m_b);
	copy_ipv6_address(dst->info.ipv6info.dip,
			  src->m_sockinfo.m_ipv6info.m_fields.m_dip.m_b);
	...
	dst->info.ipv6info.l4proto =
		src->m_sockinfo.m_ipv6info.m_fields.m_l4proto;
	break;
    case SCAP_FD_IPV6_SERVSOCK:
	copy_ipv6_address(dst->info.ipv6serverinfo.ip,
			  src->m_sockinfo.m_ipv6serverinfo.m_ip.m_b);
	dst->info.ipv6serverinfo.port = src->m_sockinfo.m_ipv6serverinfo.m_port;
	dst->info.ipv6serverinfo.l4proto =
		src->m_sockinfo.m_ipv6serverinfo.m_l4proto;
	break;
    case SCAP_FD_UNIX_SOCK:
	dst->info.unix_socket_info.source =
		src->m_sockinfo.m_unixinfo.m_fields.m_source;
	dst->info.unix_socket_info.destination =
		src->m_sockinfo.m_unixinfo.m_fields.m_dest;
	strlcpy(dst->info.unix_socket_info.fname, src->m_name.c_str(),
		sizeof(dst->info.unix_socket_info.fname));
	break;
    case SCAP_FD_FILE_V2:
	dst->info.regularinfo.open_flags = src->m_openflags;
	strlcpy(dst->info.regularinfo.fname, src->m_name.c_str(),
...
void sinsp_thread_manager::clear()
{
    ...
    m_failed_lookups =
	    &m_inspector->m_stats.get_metrics_registry().register_counter(
		    internal_metrics::metric_name("thread_failed_lookups",
						  "Failed thread lookups"));
    m_cached_lookups =
	    &m_inspector->m_stats.get_metrics_registry().register_counter(
		    internal_metrics::metric_name("thread_cached_lookups",
						  "Cached thread lookups"));
    m_non_cached_lookups =
	    &m_inspector->m_stats.get_metrics_registry().register_counter(
		    internal_metrics::metric_name("thread_non_cached_lookups",
						  "Non cached thread lookups"));
    m_added_threads =
	    &m_inspector->m_stats.get_metrics_registry().register_counter(
		    internal_metrics::metric_name("thread_added",
						  "Number of added threads"));
    m_removed_threads =
	    &m_inspector->m_stats.get_metrics_registry().register_counter(
		    internal_metrics::metric_name("thread_removed",
						  "Removed threads"));
#endif
}

void sinsp_thread_manager::increment_mainthread_childcount(
	sinsp_threadinfo* threadinfo)
{
	...
	sinsp_threadinfo* main_thread =
		m_inspector->get_thread_ref(threadinfo->m_pid, true, true)
			.get();
	if(main_thread)
	{
...
bool sinsp_thread_manager::add_thread(sinsp_threadinfo* threadinfo,
				      bool from_scap_proctable)
{
	...
	{
	    g_logger.format(sinsp_logger::SEV_INFO,
			    "Thread table full, dropping tid %lu (pid %lu, "
			    "comm \"%s\")",
			    threadinfo->m_tid, threadinfo->m_pid,
...
void sinsp_thread_manager::remove_thread(int64_t tid, bool force)
{
	...
	// Either the thread creation event was dropped or our logic doesn't
	// support the call that created this thread. The assertion will detect
	// it, while in release mode we just keep going.
	//
#ifdef GATHER_INTERNAL_STATS
	...
	if(tinfo->m_flags & PPM_CL_CLONE_THREAD)
	{
	    ...
	    sinsp_threadinfo* main_thread =
		    m_inspector->get_thread_ref(tinfo->m_pid, false, true)
			    .get();
	    if(main_thread)
	    {
	...
	//
	// If this is the main thread of a process, erase all the FDs that the
	// process owns
	//
	if((tinfo->m_pid == tinfo->m_tid) ||
	   tinfo->m_flags & PPM_CL_IS_MAIN_THREAD)
	{
	    unordered_map<int64_t, sinsp_fdinfo_t>* fdtable =
		    &(tinfo->get_fd_table()->m_table);
		...
		//
		// The canceled fd should always be deleted immediately, so if
		// it appears here it means we have a problem.
		//
	...
	//
	// If the thread has a nonzero refcount, it means that we are forcing
	// the removal of a main process or program that some child refer to. We
	// need to recalculate the child relationships, or the table will become
	// corrupted.
	//
	if(nchilds != 0)
	{
...
void sinsp_thread_manager::update_statistics()
{
    ...
    for(threadinfo_map_iterator_t it = m_threadtable.begin();
...
void sinsp_thread_manager::free_dump_fdinfos(
	vector<scap_fdinfo*>* fdinfos_to_free)
{
...
void sinsp_thread_manager::thread_to_scap(sinsp_threadinfo& tinfo,
...
void sinsp_thread_manager::dump_threads_to_file(scap_dumper_t* dumper)
{
    ...
    m_threadtable.loop(
		...
		uint32_t il =
			(uint32_t)(sizeof(uint32_t) + // len
				   ...
				   2 +
				   MIN(tinfo.m_comm.size(),
				       SCAP_MAX_PATH_SIZE) +
				   2 +
				   MIN(tinfo.m_exe.size(), SCAP_MAX_PATH_SIZE) +
				   2 +
				   MIN(tinfo.m_exepath.size(),
				       SCAP_MAX_PATH_SIZE) +
				   2 +
				   MIN(tinfo.args_len(), SCAP_MAX_ARGS_SIZE) +
				   // 1 is sizeof("/")
				   2 +
				   MIN((tinfo.m_cwd == "") ? 1
							   : tinfo.m_cwd.size(),
				   ...
				   sizeof(int64_t) + // vpid
				   2 +
				   MIN(tinfo.cgroups_len(),
				       SCAP_MAX_CGROUPS_SIZE) +
				   2 +
				   MIN(tinfo.m_root.size(),
				       SCAP_MAX_PATH_SIZE)) +
    ...
    //
    if(scap_write_proclist_header(m_inspector->m_h, dumper, totlen) !=
       SCAP_SUCCESS)
    {
    ...
    m_threadtable.loop(
		...
		if(scap_write_proclist_entry_bufs(
			   m_inspector->m_h, dumper, sctinfo, lengths[idx++],
			   tinfo.m_comm.c_str(), tinfo.m_exe.c_str(),
			   tinfo.m_exepath.c_str(), args_iov, argscnt, envs_iov,
			   envscnt,
    ...
    if(scap_write_proclist_trailer(m_inspector->m_h, dumper, totlen) !=
       SCAP_SUCCESS)
    {
	throw sinsp_exception(scap_getlasterr(m_inspector->m_h));
    }
    ...
    m_threadtable.loop(
		...
		if(tinfo.is_main_thread())
		{
			...
			//
			scap_fdinfo* scfdinfo =
				(scap_fdinfo*)malloc(sizeof(scap_fdinfo));
			if(scfdinfo == NULL)
			{
			    scap_proc_free(m_inspector->m_h, sctinfo);
			    throw sinsp_exception(
				    "thread memory allocation error in "
				    "sinsp_thread_manager::to_scap");
			}
			...
			//
			if(scap_fd_add(m_inspector->m_h, sctinfo, it->first,
				       scfdinfo) != SCAP_SUCCESS)
			{
			    scap_proc_free(m_inspector->m_h, sctinfo);
			    throw sinsp_exception(
				    "error calling scap_fd_add in "
				    "sinsp_thread_manager::to_scap (" +
		...
		//
		if(scap_write_proc_fds(m_inspector->m_h, sctinfo, dumper) !=
		   SCAP_SUCCESS)
		{
		    scap_proc_free(m_inspector->m_h, sctinfo);
		    throw sinsp_exception(
			    "error calling scap_proc_add in "
			    "sinsp_thread_manager::to_scap (" +
			    string(scap_getlasterr(m_inspector->m_h)) + ")");
		}
...
threadinfo_map_t::ptr_t
sinsp_thread_manager::get_thread_ref(int64_t tid, bool query_os_if_not_found,
				     bool lookup_only, bool main_thread)
{
    ...
    {
	// Certain code paths can lead to this point from scap_open()
	// (incomplete example: scap_proc_scan_proc_dir() -> resolve_container()
	// -> get_env()). Adding a defensive check here to protect both, callers
	// of get_env and get_thread.
	if(!m_inspector->m_h)
	{
	    g_logger.format(sinsp_logger::SEV_INFO,
...
threadinfo_map_t::ptr_t sinsp_thread_manager::find_thread(int64_t tid,
							  bool lookup_only)
{
