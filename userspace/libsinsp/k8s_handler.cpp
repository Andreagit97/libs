...
k8s_handler::k8s_handler(const std::string& id, bool is_captured,
			 ...
			 const std::string& http_version, int timeout_ms,
			 ssl_ptr_t ssl, bt_ptr_t bt, bool watch, bool connect,
			 ptr_t dependency_handler, bool blocking_socket,
#endif // HAS_CAPTURE
			 unsigned max_messages, k8s_state_t* state,
			 uint32_t data_max_b, uint32_t data_chunk_wait_us):
	m_state(state),
	m_id(id + "_state"),
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	m_collector(collector),
	m_path(path + ((path.find('?') == std::string::npos)
			       ? "?pretty=false"
			       : "&pretty=false")),
	m_state_filter(state_filter), m_event_filter(event_filter),
	m_null_filter(null_filter), m_filter(&m_state_filter),
	m_timeout_ms(timeout_ms), m_url(url), m_http_version(http_version),
	m_ssl(ssl), m_bt(bt), m_watch(watch), m_connect(connect),
	m_dependency_handler(dependency_handler),
	m_blocking_socket(blocking_socket),
#endif // HAS_CAPTURE
	m_max_messages(max_messages), m_is_captured(is_captured)
{
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	...
	g_logger.log(std::string("K8s (" + m_id + ") creating handler for " +
				 uri(m_url).to_string(false) + m_path),
		     sinsp_logger::SEV_DEBUG);
	m_handler = std::make_shared<handler_t>(
		*this, m_id, m_url, m_path, m_http_version, m_timeout_ms, m_ssl,
		m_bt, !m_blocking_socket, m_blocking_socket,
		SOCKET_HANDLER_DATA_LIMIT, true, data_max_b,
		data_chunk_wait_us);
	...
	// 2. null filter  (filters init state JSONs when there are no K8s
	// entities)
...
void k8s_handler::make_http()
{
	...
	if(!m_handler)
	{
	    ...
	    m_handler = std::make_shared<handler_t>(
		    *this, m_id, m_url, m_path, m_http_version, m_timeout_ms,
		    m_ssl, m_bt, true, m_blocking_socket);
	    m_handler->set_json_callback(&k8s_handler::set_event_json);
	}
	...
	{
	    g_logger.log("K8s (" + m_id +
				 ") error filter was not present in state "
				 "handler, adding it for events.",
...
bool k8s_handler::connect()
{
	...
	if(!m_collector->has(m_handler))
	{
	    g_logger.log(std::string("k8s_handler (" + m_id +
				     ") k8s_handler::connect() adding handler "
				     "to collector"),
			 sinsp_logger::SEV_TRACE);
	...
	if(m_handler->is_connecting())
	{
	    g_logger.log(
		    std::string("k8s_handler (" + m_id +
				"), k8s_handler::connect() connecting to " +
    ...
    {
	g_logger.log(std::string("k8s_handler (" + m_id +
				 ") k8s_handler::connect(), http is null, "
				 "(re)creating ... "),
		     sinsp_logger::SEV_WARNING);
	make_http();
    }
...
void k8s_handler::send_data_request()
{
    ...
    {
	throw sinsp_exception("k8s_handler (" + m_id + ") HTTP client (" +
			      uri(m_url).to_string(false) + ") is null.");
    }
#endif // HAS_CAPTURE
}
	...
	if(m_req_sent)
	{
		...
		else
		{
		    throw sinsp_exception(
			    "K8s k8s_handler::receive_response(): no data "
			    "received.");
		}
	    }
	    else
	    {
		throw sinsp_exception("K8s k8s_handler::receive_response(): "
				      "invalid call (in watch mode).");
	    }
	}
	else
	{
	    throw sinsp_exception("K8s k8s_handler::receive_response(): "
				  "invalid call (request not sent).");
	}
    }
    else
    {
	throw sinsp_exception(
		"K8s k8s_handler::receive_response(): handler is null.");
    }
#endif // HAS_CAPTURE
}
...
bool k8s_handler::is_alive() const
{
    ...
    {
	g_logger.log("k8s_handler (" + m_id + ") connection (" +
			     m_handler->get_url().to_string(false) + ") loss.",
		     sinsp_logger::SEV_WARNING);
	return false;
    }
#endif // HAS_CAPTURE
...
void k8s_handler::check_state()
{
	...
	{
	    g_logger.log("k8s_handler (" + m_id +
				 ") switching to watch connection for " +
				 uri(m_url).to_string(false) + m_path,
			 sinsp_logger::SEV_DEBUG);
		...
		{
		    throw sinsp_exception("k8s_handler (" + m_id +
					  "), invalid URL path: " + m_path);
		}
	    }
	    m_handler->set_socket_option(SOCK_NONBLOCK);
	    make_http();
	}
	if(m_watching && m_id.find("_state") == std::string::npos &&
	   m_handler->wants_send())
	{
	    m_req_sent = false;
	    m_resp_recvd = false;
	}
...
void k8s_handler::collect_data()
{
    ...
    if(m_collector && m_handler)
    {
	process_events(); // there may be leftovers from state connection closed
			  // by collector
	check_state(); // switch to events, if needed
	g_logger.log("k8s_handler (" + m_id +
			     ")::collect_data(), checking connection to " +
			     uri(m_url).to_string(false),
		     sinsp_logger::SEV_DEBUG);
	if(m_handler->is_connecting())
	{
	    g_logger.log("k8s_handler (" + m_id +
				 ")::collect_data(), connecting to " +
				 uri(m_url).to_string(false),
			 sinsp_logger::SEV_DEBUG);
	    return;
	}
	else if(m_handler->is_connected())
	{
	    ...
	    {
		g_logger.log(
			"k8s_handler (" + m_id + ")::collect_data() [" +
				uri(m_url).to_string(false) +
				"], requesting data "
				"from " +
				m_path + "... m_blocking_socket=" +
				std::to_string(m_blocking_socket) +
				", m_watching=" + std::to_string(m_watching),
			sinsp_logger::SEV_DEBUG);
		send_data_request();
	    ...
	    {
		g_logger.log("k8s_handler (" + m_id + ")::collect_data() [" +
				     uri(m_url).to_string(false) +
				     "], getting data "
				     "from " +
				     m_path + "...",
	    ...
	    {
		g_logger.log("k8s_handler (" + m_id +
				     ") collect_data(), no subscriptions to " +
				     uri(m_url).to_string(false) + m_path,
			     sinsp_logger::SEV_DEBUG);
	    }
	    return;
    ...
    {
	g_logger.log("k8s_handler (" + m_id +
			     "), http interface not (yet?) created for " +
			     uri(m_url).to_string(false) + ").",
		     sinsp_logger::SEV_TRACE);
    }
#endif // HAS_CAPTURE
...
k8s_handler::msg_data k8s_handler::get_msg_data(const std::string& type,
						...
						const Json::Value& json)
{
	...
	if(type[0] == 'A')
	{
	    data.m_reason = k8s_component::COMPONENT_ADDED;
	}
	else if(type[0] == 'M')
	{
...
void k8s_handler::handle_json(Json::Value&& root)
{
    ...
    {
#if defined(HAS_CAPTURE) && !defined(_WIN32)
	throw sinsp_exception("k8s_handler (" + m_id + "), state is null for " +
			      uri(m_url).to_string(false) + ").");
#else
	...
	if(type.isConvertibleTo(Json::stringValue))
	{
		...
		if(kind.isConvertibleTo(Json::stringValue))
		{
			...
			//if(name() == "replicasets") // vary name to verify
			(non)critical component error handling if(name() ==
			"pods")
			{
				std::string j = "{"
								" \"metadata\":
			...
			*/
			...
			if(data.m_reason == k8s_component::COMPONENT_ADDED)
			{
				...
				os << "K8s " + reason_type
				   << " message received by " << m_id <<
#if defined(HAS_CAPTURE) && !defined(_WIN32)
					" [" << uri(m_url).to_string(false)
				   << "]"
#endif // HAS_CAPTURE
				      "for existing "
				   << data.m_kind << " [" << data.m_uid
				   << "], updating only.";
				g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
			    }
			}
			else if(data.m_reason ==
				k8s_component::COMPONENT_MODIFIED)
			{
			    if(!m_state->has(data.m_uid))
			    {
				std::ostringstream os;
				...
				k8s_component::COMPONENT_DELETED)
			{
			    if(!m_state->has(data.m_uid))
			    {
				std::ostringstream os;
			...
			{
			    if(data.m_reason ==
			       k8s_component::COMPONENT_NONEXISTENT)
			    {
				g_logger.log(
					std::string(
						"Non-existent K8S component (" +
						name() + "), reason: ") +
			...
			if(handle_component(item, &data))
			{
			    ...
			    m_state->update_cache(
				    k8s_component::get_type(name()));
			}
			else
			{
			    g_logger.log("K8s: error occurred while handling " +
						 reason_type + " event for " +
						 data.m_kind + ' ' +
						 data.m_name + " [" +
						 data.m_uid + ']',
					 sinsp_logger::SEV_ERROR);
	...
	{
	    g_logger.log(std::string("K8S event type is not string."),
			 sinsp_logger::SEV_ERROR);
	}
    }
    else
    {
	g_logger.log(std::string("K8S event type is null."),
		     sinsp_logger::SEV_ERROR);
    }
}
...
k8s_handler::ip_addr_list_t
k8s_handler::hostname_to_ip(const std::string& hostname)
{
    ...
    {
	g_logger.log("Can't determine IP address for hostname: " + hostname,
		     sinsp_logger::SEV_WARNING);
	return ip_addrs;
    }
...
void k8s_handler::process_events()
{
	...
	for(auto evt = m_events.begin(); evt != m_events.end();)
	{
	    ...
	    {
		g_logger.log(
			"k8s_handler (" + m_id + ") error " +
#if defined(HAS_CAPTURE) && !defined(_WIN32)
				"(" + uri(m_url).to_string(false) + ") " +
#endif // HAS_CAPTURE
				(!(*evt) ? "data is null."
					 : ((*evt)->isNull() ? "JSON is null."
							     : "Unknown")),
			sinsp_logger::SEV_ERROR);
	    }
	    evt = m_events.erase(evt);
	}
	if(!m_state_built && m_state_processing_started && !m_events.size())
	{
	    m_state_built = true;
	}
    }
...
{
    g_logger.log("k8s_handler adding event, (" + m_id + ") has " +
			 std::to_string(m_events.size())
#if defined(HAS_CAPTURE) && !defined(_WIN32)
			 ...
			 ,
		 sinsp_logger::SEV_TRACE);
    // empty JSON is fine here; if there are no entities, state and first watch
    // will pass nothing in here null is checked when processing
    m_events.emplace_back(json);
    g_logger.log("k8s_handler added event, (" + m_id + ") has " +
			 std::to_string(m_events.size())
#if defined(HAS_CAPTURE) && !defined(_WIN32)
			 + " events from " + uri(m_url).to_string(false)
#endif // HAS_CAPTURE
...
std::string k8s_handler::name() const
{
    ...
    std::string::size_type length = (qm_pos == std::string::npos)
					    ? std::string::npos
					    : qm_pos - slash_pos - 1;
...
void k8s_handler::handle_error(const msg_data& data, const Json::Value& root,
			       bool log)
{
    if(log)
    {
...
void k8s_handler::log_error(const msg_data& data, const Json::Value& json)
{
    ...
    std::ostringstream os;
    ;
    os << "K8S server reported " << name()
       << " error for [" + uri(m_url).to_string(false) + m_path + "]: ";
    if(!json.isNull())
    {
