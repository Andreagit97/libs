...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		    url, "/apis/apps/v1/replicasets", STATE_FILTER,
...
bool k8s_replicaset_handler::handle_component(const Json::Value& json,
					      const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
	    ...
	    {
		k8s_rs_t& rs =
			m_state->get_component<k8s_replicasets, k8s_rs_t>(
				m_state->get_rss(), data->m_name, data->m_uid,
				data->m_namespace);
