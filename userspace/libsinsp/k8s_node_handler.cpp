...
bool k8s_node_handler::handle_component(const Json::Value& json,
					const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
	    ...
	    {
		k8s_node_t& node =
			m_state->get_component<k8s_nodes, k8s_node_t>(
				m_state->get_nodes(), data->m_name,
				data->m_uid);
		k8s_node_t::host_ip_list addresses;
		k8s_component::extract_string_array(json["addresses"],
						    addresses);
		if(addresses.size() > 0)
		{
		...
		{
		    g_logger.log("K8s Node handler: Can not obtain IP "
				 "address(es) for node" +
	    ...
	    {
		if(!m_state->delete_component(m_state->get_nodes(),
					      data->m_uid))
		{
		    log_not_found(*data);
		    return false;
