...
sinsp_logger::sinsp_logger():
	m_file(nullptr), m_callback(nullptr), m_flags(OT_NONE), m_sev(SEV_INFO)
{
}
...
uint32_t sinsp_logger::get_log_output_type() const { return m_flags; }

void sinsp_logger::add_stdout_log() { m_flags |= sinsp_logger::OT_STDOUT; }

void sinsp_logger::add_stderr_log() { m_flags |= sinsp_logger::OT_STDERR; }
...
void sinsp_logger::add_file_log(const std::string& filename)
{
    ...
    if(!m_file)
    {
	throw sinsp_exception("Unable to open file " + filename +
			      " for writing");
    }
...
void sinsp_logger::disable_timestamps() { m_flags |= sinsp_logger::OT_NOTS; }
...
sinsp_logger::severity sinsp_logger::get_severity() const { return m_sev; }
...
void sinsp_logger::log(std::string msg, const severity sev)
{
	...
	{
	    const std::string::size_type ts_length =
		    sizeof("31-12 23:59:59.999999 ");
...
const char* sinsp_logger::format_and_return(const severity sev,
...
const char* SEV_LEVELS[] = {"SEV_DEF ", "SEV_FAT ", "SEV_CRI ",
			    "SEV_ERR ", "SEV_WAR ", "SEV_NOT ",
			    "SEV_INF ", "SEV_DEB ", "SEV_TRA "};

static_assert(sizeof(SEV_LEVELS) ==
		      sizeof(*SEV_LEVELS) *
			      ((size_t)(sinsp_logger::SEV_MAX) + 1),
