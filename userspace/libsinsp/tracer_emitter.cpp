...
void tracer_writer::write(const std::string &trc)
{
	...
	g_logger.format(sinsp_logger::SEV_ERROR,
			"Unable to write tracer (%s) to %s: %s", trc.c_str(),
	...
	ASSERT(false);
	g_logger.format(sinsp_logger::SEV_ERROR,
			"Incomplete write of tracer (%s) to %s", trc.c_str(),
			m_file);
	close_fd();
    }
    return;
...
void tracer_writer::close_fd()
{
    ...
    {
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"Closing %s (fd %d) for writing tracers", m_file, m_fd);
	::close(m_fd);
	m_fd = -1;
    }
}
...
tracer_emitter::tracer_emitter(std::string tag, uint64_t timeout_ns):
	m_tag(std::move(tag)), m_start_ns(sinsp_utils::get_current_time_ns()),
...
void tracer_emitter::start() { write_tracer(true); }
...
uint64_t tracer_emitter::elapsed_time() const
{
	...
	g_logger.format(sinsp_logger::SEV_INFO,
			"Tracer %s elapsed time %llu ns", m_tag.c_str(),
			elapsed);
    }
    return elapsed;
}
