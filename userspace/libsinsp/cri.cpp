...
namespace
{
bool pod_uses_host_netns(
	const runtime::v1alpha2::PodSandboxStatusResponse &resp)
{
...
cri_interface::cri_interface(const std::string &cri_path)
{
    ...
    auto deadline = std::chrono::system_clock::now() +
		    std::chrono::milliseconds(s_cri_timeout);
	...
	g_logger.format(sinsp_logger::SEV_NOTICE,
			"cri: CRI runtime returned an error after version "
			"check at %s: %s",
    ...
    g_logger.format(sinsp_logger::SEV_INFO, "cri: CRI runtime: %s %s",
...
grpc::Status cri_interface::get_container_status(
...
grpc::Status cri_interface::get_container_stats(
	const std::string &container_id,
	runtime::v1alpha2::ContainerStatsResponse &resp)
{
    ...
    grpc::ClientContext context;
    auto deadline = std::chrono::system_clock::now() +
		    std::chrono::milliseconds(s_cri_size_timeout);
    context.set_deadline(deadline);
    return m_cri->ContainerStats(&context, req, &resp);
}

bool cri_interface::parse_cri_image(
	const runtime::v1alpha2::ContainerStatus &status,
	sinsp_container_info &container)
{
    ...
    sinsp_utils::split_container_image(status.image().image(), hostname, port,
				       container.m_imagerepo,
				       container.m_imagetag, digest, false);
...
bool cri_interface::parse_cri_mounts(
	const runtime::v1alpha2::ContainerStatus &status,
	sinsp_container_info &container)
{
	...
	container.m_mounts.emplace_back(mount.host_path(),
					...
					!mount.readonly(), propagation);
    }
    return true;
}

bool walk_down_json(const Json::Value &root, const Json::Value **out,
...
bool set_numeric_32(const Json::Value &dict, const std::string &key,
		    int32_t &val)
{
...
bool set_numeric_64(const Json::Value &dict, const std::string &key,
		    int64_t &val)
{
    if(!dict.isMember(key))
    {
...
bool cri_interface::parse_cri_env(const Json::Value &info,
				  sinsp_container_info &container)
{
...
bool cri_interface::parse_cri_json_image(const Json::Value &info,
					 sinsp_container_info &container)
{
    const Json::Value *image;
    if(!walk_down_json(info, &image, "config", "image", "image") ||
       !image->isString())
    {
	return false;
    }
...
bool cri_interface::parse_cri_ext_container_info(
	const Json::Value &info, sinsp_container_info &container)
{
    const Json::Value *linux = nullptr;
    if(!walk_down_json(info, &linux, "runtimeSpec", "linux") ||
       !linux->isObject())
    {
	return false;
    }
    ...
    // containerd
    if(!priv_found &&
       walk_down_json(info, &privileged, "config", "linux", "security_context",
		      "privileged") &&
       privileged->isBool())
    {
	container.m_privileged = privileged->asBool();
    ...
    // cri-o
    if(!priv_found && walk_down_json(info, &privileged, "privileged") &&
       privileged->isBool())
    {
	container.m_privileged = privileged->asBool();
	priv_found = true;
...
uint32_t cri_interface::get_container_ip(const std::string &container_id)
{
	...
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Container id %s not in list from CRI",
			container_id.c_str());
	ASSERT(false);
	break;
    case 1:
	...
	g_logger.format(
		...
		"Container id %s matches more than once in list from CRI",
		container_id.c_str());
	ASSERT(false);
	break;
    }
...
std::string cri_interface::get_container_image_id(const std::string &image_ref)
{
    ...
    case 0:
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Image ref %s not in list from CRI", image_ref.c_str());
	ASSERT(false);
	break;
    case 1:
    {
