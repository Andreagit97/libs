...
template<typename T> class socket_data_handler
{
    public:
    ...
    socket_data_handler(T& obj, const std::string& id, const std::string& url,
			const std::string& path = "",
			const std::string& http_version = HTTP_VERSION_11,
			int timeout_ms = 1000, ssl_ptr_t ssl = nullptr,
			bt_ptr_t bt = nullptr, bool keep_alive = true,
			bool blocking = false,
	    ...
	    m_blocking(blocking), m_ssl(ssl), m_bt(bt),
	    m_timeout_ms(timeout_ms),
	    m_request(make_request(url, http_version)),
	    m_http_version(http_version), m_data_limit(data_limit),
	    m_fetching_state(fetching_state), m_data_max_b(data_max_b),
    ...
    virtual ~socket_data_handler() { cleanup(); }
    ...
    virtual bool is_connected() const { return m_connected; }
    ...
    const uri& get_url() const { return m_url; }
    ...
    void set_json_callback(json_callback_func_t f) { m_json_callback = f; }

    SSL* ssl_connection() { return m_ssl_connection; }

    bool wants_send() const { return m_wants_send; }
	...
	if(m_request.size())
	{
	    g_logger.log("Socket handler (" + m_id +
				 ...
				 ", m_ssl_connection=" +
				 std::to_string((int64_t)m_ssl_connection),
		...
		if(m_url.is_secure())
		{
			...
			throw sinsp_exception(
				"Socket handler (" + m_id +
				") send: SSL connection is null.");
		    }
		    iolen = SSL_write(m_ssl_connection, m_request.c_str(),
				      m_request.size());
		}
		else
		{
		    iolen = send(m_socket, m_request.c_str(), m_request.size(),
				 0);
		}
		if(iolen == static_cast<int>(req.size()))
		{
		    break;
		}
	...
	std::ostringstream os;
	os << "Socket handler (" << m_id << ") send_request(), connection ["
	   << m_url.to_string(false) << "] error: " << err;
	if(m_url.is_secure())
	{
	...
	case SSL_ERROR_ZERO_RETURN:
	    throw sinsp_exception("SSL Socket handler (" + m_id +
				  "): Connection closed.");
	    break;
	...
	case SSL_ERROR_WANT_WRITE:
	    break;
	default:
	    g_logger.log("SSL Socket handler (" + m_id + ") received=" +
				 std::to_string(err) + " SSL error.",
			 sinsp_logger::SEV_TRACE);
	    break;
	}
	return processed;
	...
	g_logger.log("Socket handler (" + m_id +
			     ") Retrieving all data in blocking mode ...",
		     sinsp_logger::SEV_TRACE);
	init_http_parser();
	do
	{
	    ...
	    // environments), give up after reading a certain size (by default,
	    // 100MB, but configurable).
	    ++counter;
		...
		throw sinsp_exception(
			...
			"): "
			"read more than " +
			to_string(m_data_max_b / 1024 / 1024) +
			" MB of data from " + m_url.to_string(false) + m_path +
			" (" + std::to_string(processed) + " bytes, " +
			std::to_string(counter) + " reads). Giving up");
	    }
	    else
	    {
	...
	std::ostringstream os;
	os << "Socket handler ("
	   << m_id + ") an error occurred during http parsing. "
		     "processed="
	   << nparsed << ", expected=" << data.size()
	   << ", status_code=" << std::to_string(m_http_parser->status_code)
	   << ", http_errno=" << std::to_string(m_http_parser->http_errno)
	   << "data:" << std::endl
	   << data;
	throw sinsp_exception(os.str());
    }
	...
	size_t nparsed = http_parser_execute(
		m_http_parser, &m_http_parser_settings, data, len);
	if(nparsed != len)
	{
	    data_handling_error(std::string(data, len), nparsed);
	}
    }
	...
	{
	    handled = false;
	    for(auto it = m_json_filters.cbegin(); it != m_json_filters.cend();
		++it)
	    {
		json_ptr_t pjson =
			try_parse(m_jq, *js, *it, m_id, m_url.to_string(false));
		if(pjson)
		{
	...
	if(len)
	{
		...
		{
		    g_logger.log(
			    "Socket handler (" + m_id + ") http parser error " +
				    std::to_string(parser_errno) + " ([" +
				    http_errno_name((http_errno)parser_errno) +
				    "]: " +
				    http_errno_description(
					    (http_errno)parser_errno) +
				    ')',
			    sinsp_logger::SEV_ERROR);
		}
		else
	    ...
	    {
		g_logger.log("Socket handler (" + m_id + ") response " +
				     std::to_string(m_http_response) + " (" +
				     get_http_reason(m_http_response) +
		...
		if(m_data_buf.size()) // should never happen
		{
		    g_logger.log(
			    "Socket handler (" + m_id +
				    ") response ended with unprocessed data, "
				    "clearing and sending new request ... ",
			    sinsp_logger::SEV_WARNING);
		    ASSERT(!m_data_buf.size());
		    m_data_buf.clear();
		}
		// In HTTP 1.1 connections with chunked transfer, this socket
		// may never be closed by server, (K8s API server is an example
		// of such behavior), in which case the chunked data will just
		// stop flowing. We can keep the good socket and resend the
		// request instead of severing the connection. The m_wants_send
		// flag has to be checked by the caller and request re-sent,
		// otherwise this pipeline will remain idle. To force
		// client-initiated socket close on chunked transfer end, set
		// the m_close_on_chunked_end flag to true (default).
		if(m_close_on_chunked_end)
		{
		    g_logger.log("Socket handler (" + m_id +
					 ") chunked response ended",
				 sinsp_logger::SEV_DEBUG);
		    return CONNECTION_CLOSED;
		}
		m_wants_send = true;
	...
	{
	    throw sinsp_exception("Socket handler (" + m_id +
				  "): cannot parse data (callback is null).");
	}
	...
	try
	{
		...
		if(m_url.is_secure())
		{
		    iolen = static_cast<ssize_t>(
			    SSL_read(m_ssl_connection, &m_buf[0], len_to_read));
		}
		else
		{
		...
		g_logger.log("Socket handler (" + m_id + ") " +
				     ...
				     ", iolen=" + std::to_string(iolen) +
				     ", data=" + std::to_string(len_read) +
				     " bytes, "
				     "errno=" +
		...
		/* uncomment to see raw HTTP stream data in trace logs
			if((iolen > 0) && g_logger.get_severity() >=
		   sinsp_logger::SEV_TRACE)
			{
				g_logger.log("Socket handler (" + m_id + "),
		   data --->" + std::string(&m_buf[0], iolen) + "<--- data",
							 sinsp_logger::SEV_TRACE);
			}
		*/
		if(iolen > 0)
		{
		    size_t len = (iolen <= static_cast<ssize_t>(m_buf.size()))
					 ? static_cast<size_t>(iolen)
					 : m_buf.size();
		...
		else if(iolen == 0 || m_sock_err == ENOTCONN ||
			m_sock_err == EPIPE)
		{
		    if(m_url.is_secure())
		    {
			if(m_ssl_connection)
			{
				...
				g_logger.log(
					"Socket handler (" + m_id +
						"): SSL zero bytes received, "
						"but no shutdown state set for "
						"[" +
						m_url.to_string(false) + "]: ",
					sinsp_logger::SEV_WARNING);
			    }
		...
		else if(iolen < 0)
		{
		    ...
		    (len_read < m_data_limit));
	    g_logger.log("Socket handler (" + m_id + ") " +
				 std::to_string(len_read) +
				 " bytes of data received",
			 sinsp_logger::SEV_TRACE);
	}
	catch(const sinsp_exception& ex)
	{
	    g_logger.log(std::string("Socket handler (" + m_id +
				     ") data receive error [" +
				     m_url.to_string(false) + "]: ")
				 .append(ex.what()),
			 sinsp_logger::SEV_ERROR);
    ...
    void add_json_filter(const std::string& filter,
			 const std::string& before_filter = "")
    {
	if(filter.empty())
	{
	...
	else
	{
		...
		g_logger.log("Socket handler (" + m_id + "), [" +
				     m_url.to_string(false) +
				     "] "
				     "attempt to insert filter before a "
				     "non-existing filter. "
				     "Filter will be added to the end of "
				     "filter list.",
			     sinsp_logger::SEV_WARNING);
	    }
	...
	g_logger.log("Socket handler (" + m_id + "), [" +
			     m_url.to_string(false) + "]" + filters.str(),
		     sev);
    }

    static json_ptr_t try_parse(json_query& jq, const std::string& json,
	...
	int ret = getsockopt(m_socket, SOL_SOCKET, SO_ERROR, &m_sock_err,
			     &optlen);
	g_logger.log("Socket handler (" + m_id +
			     ") getsockopt() ret=" + std::to_string(ret) +
			     ", m_sock_err=" + std::to_string(m_sock_err) +
			     " (" + strerror(m_sock_err) + ')',
		     sinsp_logger::SEV_TRACE);
	if(!ret)
	{
	...
	if(ssl)
	{
	    ...
	    {
		g_logger.log("Socket handler SSL CA verified: " +
				     std::string(buf),
			     sinsp_logger::SEV_DEBUG);
		return 1;
	    }
		...
		g_logger.log("Socket handler SSL CA verify error: num=" +
				     std::to_string(err) + ':' +
    ...
    {
	g_logger.log("Socket handler SSL CA verification disabled, certificate "
		     "accepted.",
		     sinsp_logger::SEV_DEBUG);
	return 1;
    }
	...
	if(m_url.is_secure())
	{
		...
		if(os.str().empty())
		{
		    os << "Socket handler (" << m_id
		       << ", "
			  "socket="
		       << std::to_string(m_socket) << ") SSL errors:\n";
		}
	...
	if(!m_ssl_context)
	{
		...
		if(m_ssl->verify_peer())
		{
		    ...
		    if(!ca_cert.empty() &&
		       !SSL_CTX_load_verify_locations(m_ssl_context,
						      ca_cert.c_str(), 0))
		    {
			throw sinsp_exception(
				"Socket handler (" + m_id +
				"): "
				"Can't load SSL CA certificate (" +
				ca_cert + ").\n" + ssl_errors());
		    }
			...
			throw sinsp_exception(
				...
				"(Verify Peer enabled but no CA certificate "
				"specified).");
		    }
		    SSL_CTX_set_verify(m_ssl_context, SSL_VERIFY_PEER,
				       ssl_verify_callback);
		    g_logger.log("Socket handler (" + m_id +
					 "): CA verify set to PEER",
				 sinsp_logger::SEV_TRACE);
		}
		else
		{
		    SSL_CTX_set_verify(m_ssl_context, SSL_VERIFY_NONE,
				       ssl_no_verify_callback);
		    g_logger.log("Socket handler (" + m_id +
					 "): CA verify set to NONE",
				 sinsp_logger::SEV_TRACE);
		}
		...
		if(!cert.empty())
		{
		    if(SSL_CTX_use_certificate_file(m_ssl_context, cert.c_str(),
						    SSL_FILETYPE_PEM) <= 0)
		    {
			throw sinsp_exception(
				"Socket handler (" + m_id +
				"): "
				"Can't load SSL certificate  from " +
				cert + ".\n" + ssl_errors());
		    }
		    else
		    {
			g_logger.log("Socket handler (" + m_id +
					     "): using SSL certificate from " +
					     cert,
				     sinsp_logger::SEV_TRACE);
		    }
			...
			if(!pass.empty())
			{
			    ...
			    SSL_CTX_set_default_passwd_cb_userdata(
				    m_ssl_context, (void*)&m_ssl_key_pass[0]);
			    SSL_CTX_set_default_passwd_cb(m_ssl_context,
							  ssl_key_password_cb);
			}

			if(SSL_CTX_use_PrivateKey_file(m_ssl_context,
						       key.c_str(),
						       SSL_FILETYPE_PEM) <= 0)
			{
			    throw sinsp_exception(
				    "Socket handler (" + m_id +
				    "): "
				    "Can't load SSL private key from " +
				    key + ".\n" + ssl_errors());
			}
			else
			{
			...
			if(!SSL_CTX_check_private_key(m_ssl_context))
			{
			    throw sinsp_exception(
				    ...
				    "SSL private key (" +
				    key +
				    ") does not match public certificate (" +
				    cert + ").\n" + ssl_errors());
			}
			else
			{
			...
			throw sinsp_exception(
				"Socket handler (" + m_id +
				"): "
				"Invalid SSL configuration: public certificate "
				"specified without private key.");
		    }
		}
		else
		{
		    g_logger.log(
			    "Socket handler (" + m_id +
				    "): SSL public certificate not provided.",
			    sinsp_logger::SEV_TRACE);
		}
	    }
	}
    ...
    {
	g_logger.log("Socket handler (" + m_id +
			     ") try_connect() entry, m_connecting=" +
			     std::to_string(m_connecting) +
	...
	g_logger.log("Socket handler (" + m_id +
			     ") try_connect() middle, m_connecting=" +
			     std::to_string(m_connecting) +
			     ", m_connected=" + std::to_string(m_connected),
		     sinsp_logger::SEV_TRACE);
	if(!m_connected)
	{
	    g_logger.log("Socket handler (" + m_id + ") connecting to " +
				 m_url.to_string(false) +
				 " (socket=" + std::to_string(m_socket) + ')',
			 sinsp_logger::SEV_DEBUG);
	    if(!m_sa || !m_sa_len)
		...
		os << m_sa;
		throw sinsp_exception("Socket handler (" + m_id +
				      ") invalid state connecting to " +
				      m_url.to_string(false) +
				      " (socket=" + std::to_string(m_socket) +
				      "), "
				      "sa=" +
				      os.str() +
				      ", sa_len=" + std::to_string(m_sa_len));
	    }
	    if(!m_connect_called)
	    {
		...
		{
		    throw sinsp_exception(
			    "Error during connection attempt to " +
			    m_url.to_string(false) +
			    " (socket=" + std::to_string(m_socket) +
		...
		if(m_ssl_connection)
		{
			...
			case SSL_ERROR_SYSCALL: // 5
			    throw sinsp_exception(
				    "Socket handler (" + m_id + "), error " +
				    std::to_string(err) + " (" +
				    strerror(errno) + ") while connecting to " +
				    m_url.get_host() + ':' +
				    std::to_string(m_url.get_port()));
			case SSL_ERROR_ZERO_RETURN: // 6
			    cleanup();
			    throw sinsp_exception(
				    "Socket handler (" + m_id +
				    "), "
				    "error (connection closed) while "
				    "connecting to " +
				    m_url.get_host() + ':' +
				    std::to_string(m_url.get_port()));
			case SSL_ERROR_WANT_CONNECT: // 7
			    throw sinsp_exception(
				    "Socket handler (" + m_id +
				    "), "
				    "error (the operation failed while "
			...
			case SSL_ERROR_WANT_ACCEPT: // 8
			    throw sinsp_exception(
				    ...
				    "), "
				    "error (the operation failed while "
				    "attempting to accept a"
				    " connection from the transport) while "
				    "connecting to " +
				    m_url.get_host() + ':' +
				    std::to_string(m_url.get_port()));
			}
	    ...
	    g_logger.log("Socket handler (" + m_id + "): Connected: socket=" +
				 std::to_string(m_socket) +
	...
	{
	    if(inet_aton(m_url.get_host().c_str(),
			 &m_serv_addr.sin_addr)) // IP address provided
	    {
		...
		{
		    g_logger.log("Socket handler (" + m_id + ") resolving " +
					 m_url.get_host(),
				 sinsp_logger::SEV_TRACE);

		    ares_init_options(&m_ares_channel, &m_ares_opts, 0);
		    ares_gethostbyname(m_ares_channel, m_url.get_host().c_str(),
				       AF_INET, ares_cb, &m_ares_cb_res);
	...
	if(!m_sa || !m_sa_len)
	{
		...
		{
		    throw sinsp_exception("Invalid address (too long): [" +
					  m_url.get_path() + ']');
		}
		m_file_addr.sun_family = AF_UNIX;
		strlcpy(m_file_addr.sun_path, m_url.get_path().c_str(),
			sizeof(m_file_addr.sun_path));
	...
	getsockname(m_socket, (struct sockaddr*)&local_address,
		    &address_length);
	...
	if(m_socket != -1)
	{
	    ...
	    {
		g_logger.log(
			"Socket handler (" + m_id + ") connection [" +
				m_url.to_string(false) + m_path +
	...
	if(parser)
	{
		...
		if(data && len)
		{
		    http_parser_data* parser_data =
			    (http_parser_data*)parser->data;
		    if(parser_data->m_data_buf && parser_data->m_json)
		    {
			parser_data->m_data_buf->append(data, len);
			// only try to parse this JSON if we are certain it is
			// not pretty-printed since this logic relies on JSONs
			// in the stream being delimited by newlines (and having
			// no newlines themselves), pretty-printed JSONs can not
			// be handled here, but must be handled in the
			// http_msg_completed_callback()
			if(parser_data->m_fetching_state)
			{
			    if(!*(parser_data->m_fetching_state))
			    {
				std::string::size_type pos =
					parser_data->m_data_buf->find('\n');
				while(pos != std::string::npos)
				{
				    parser_data->m_json->push_back(
					    parser_data->m_data_buf->substr(
						    0, pos));
			    ...
			    /*else
			    {
				    if(g_logger.get_severity() >=
			    sinsp_logger::SEV_TRACE)
				    {
					    g_logger.log("Socket handler
			    (http_body_callback) data received, will be parsed
			    on response end:" + *parser_data->m_data_buf,
			    sinsp_logger::SEV_TRACE);
				    }
			    }*/
			}
		    ...
		    {
			throw sinsp_exception(
				"Socket handler (http_body_callback): http or "
				"json buffer is null.");
		    }
		}
	    }
	    ...
	    {
		throw sinsp_exception("Socket handler (http_body_callback) "
				      "parser data is null.");
	    }
	}
	else
	{
	    throw sinsp_exception(
		    "Socket handler (http_body_callback): parser is null.");
	}
	return 0;
    }
	...
	if(parser && parser->data)
	{
		...
		if(*(parser_data->m_fetching_state))
		{
			...
			{
			    g_logger.log("Initial state fetch completed, but "
					 "no newline found!",
					 sinsp_logger::SEV_ERROR);
			}
			*(parser_data->m_fetching_state) = false;
		    ...
		    {
			throw sinsp_exception(
				"Socket handler (http_msg_completed_callback): "
				"parser data m_data_buf is null.");
		    }
		}
	    }
	    else
	    {
		throw sinsp_exception(
			"Socket handler (http_msg_completed_callback): parser "
			"data m_data_buf is null.");
	    }
		...
		*(parser_data->m_http_response) = parser->status_code;
		...
		throw sinsp_exception(
			"Socket handler (http_msg_completed_callback): parser "
			"data m_http_response is null.");
	    }
	}
	else
	{
	    throw sinsp_exception(
		    "Socket handler (http_msg_completed_callback): parser or "
		    "data null.");
	}
	return 0;
    }
    ...
    // prevent this for entities (eg. nodes, pods) URIs by specifying
    // '?pretty=false', some cluster-level URIs (eg. /api) do not honor this
    // parameter;
    //
    // this flag is true by default and it remains true until the first state
    // http request for this handler is completed, at which point all newlines
    // are purged from the string and the purged buffer is posted for further
    // processing
