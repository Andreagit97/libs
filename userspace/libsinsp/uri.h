...
class uri
{
    ...
    // URI-encodes the given string by escaping reserved, ambiguous and
    // non-ASCII characters. Returns the encoded string with uppercase hex
    // letters (eg. %5B, not %5b).
    ...
    //
    // When plus_as_space is true, non-encoded plus signs in the query are
    // decoded as spaces.
...
inline const std::string& uri::get_scheme() const { return m_scheme; }

inline void uri::set_scheme(std::string scheme) { m_scheme = move(scheme); }

inline const std::string& uri::get_user() const { return m_user; }

inline const std::string& uri::get_password() const { return m_password; }

inline const std::string& uri::get_host() const { return m_host; }

inline void uri::set_host(std::string host) { m_host = move(host); }

inline const std::string& uri::get_path() const { return m_path; }

inline const std::string& uri::get_query() const { return m_query; }

inline int uri::get_port() const { return m_port; }

inline bool uri::is_file() const { return m_scheme == "file"; }

inline bool uri::is_secure() const { return m_scheme == "https"; }
