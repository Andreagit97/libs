...
class k8s_handler
{
    ...
    k8s_handler(const std::string& id, bool is_captured,
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		std::string url, const std::string& path,
		const std::string& state_filter,
		const std::string& event_filter, const std::string& null_filter,
		collector_ptr_t collector = nullptr,
		const std::string& http_version = "1.1",
		int timeout_ms = default_timeout_ms, ssl_ptr_t ssl = nullptr,
		bt_ptr_t bt = nullptr, bool watch = true, bool connect = true,
		ptr_t dependency_handler = nullptr,
		bool blocking_socket = false,
#endif // HAS_CAPTURE
		unsigned max_messages = ~0, k8s_state_t* state = nullptr,
    ...
    virtual bool handle_component(const Json::Value& json,
				  const msg_data* data = 0) = 0;
    msg_data get_msg_data(const std::string& evt, const std::string& type,
	...
	{
	    g_logger.log("K8s Replication Controller: Null selector object.",
			 sinsp_logger::SEV_ERROR);
	}
    }

    void log_event(const msg_data& data);
    void handle_error(const msg_data& data, const Json::Value& root,
		      bool log = true);
    ...
    private:
    typedef void (k8s_handler::*callback_func_t)(json_ptr_t,
						 const std::string&);
    ...
    // some handlers only fetch state and die by design (eg. api or extensions
    // handlers have no need to continuously watch for updates) this flag
    // indicates whether handler should continue to watch after receiving the
    // initial state
    bool m_watch;
    bool m_watching = false; // indication of being in watch mode

    // flag indicating whether to connect to K8s API server (no connection
    // needed when replaying capture)
    bool m_connect;

    // k8s_handler on which this handler depends; the dependency handler must
    // not be null and it must have its state fully built before this handler
    // can begin building its own state
    ...
    // error indicating something went wrong with the K8s component handled by
    // this handler this error is later examined by k8s::check_components() and
    // if it is HTTP status > 400, one of the following actions is taken:
    //  - if component is critical for consistent k8s state (eg. namespace,
    //  node, pod),
    //    exception is thrown and, consequently, the whole k8s framework will be
    //    destroyed
    //  - if component is not critical (eg. extensions like daemonset or
    //  deployment),
    //    error is logged and handler is destroyed, but the k8s framework
    //    continues to exist without it, only receiving data for existing
    //    components
    api_error_ptr m_error;

    // this capture flag does not indicate whether we are in global capture
    // mode, it is only an indication of whether this handler data should be
    // captured at all (eg. there is no need to capture api or extensions
    // detection data)
    //
...
inline unsigned k8s_handler::get_max_messages() const { return m_max_messages; }
...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
inline k8s_handler::handler_ptr_t k8s_handler::handler() { return m_handler; }
#endif // HAS_CAPTURE
...
inline const std::string& k8s_handler::get_id() const { return m_id; }
...
inline bool k8s_handler::ready() const { return m_data_received; }

inline bool k8s_handler::is_state_built() const { return m_state_built; }

inline void k8s_handler::log_event(const msg_data& data)
{
    g_logger.log("K8s " + data.get_reason_desc() + ' ' + data.m_kind + ' ' +
...
{
    g_logger.log("K8s " + name() + " not found [" + data.m_uid +
			 "]: " + data.m_name,
		 sinsp_logger::SEV_ERROR);
}

inline k8s_handler::api_error_ptr k8s_handler::error() const { return m_error; }
...
class k8s_dummy_handler : public k8s_handler
{
    public:
    k8s_dummy_handler():
	    k8s_handler("k8s_dummy_handler", false,
#if defined(HAS_CAPTURE) && !defined(_WIN32)
