...
///////////////////////////////////////////////////////////////////////////////
gen_event_filter_expression::gen_event_filter_expression() { m_parent = NULL; }
...
bool gen_event_filter_expression::extract(gen_event *evt,
...
void gen_event_filter::pop_expression()
{
    ...
    {
	throw sinsp_exception("expression mixes 'and' and 'or' in an ambiguous "
			      "way. Please use brackets.");
    }

    m_curexpr = m_curexpr->m_parent;
}

bool gen_event_filter::run(gen_event *evt) { return m_filter->compare(evt); }
...
uint32_t gen_event_filter_factory::filter_fieldclass_info::s_rightblock_start =
	30;
uint32_t gen_event_filter_factory::filter_fieldclass_info::s_width = 120;

void gen_event_filter_factory::filter_fieldclass_info::wrapstring(
	const std::string &in, std::ostringstream &os)
{
...
std::string gen_event_filter_factory::filter_fieldclass_info::as_markdown(
	const std::set<std::string> &event_sources)
{
	...
	os << "`" << fld_info.name << "` | " << fld_info.data_type << " | "
	   << fld_info.desc << std::endl;
    }

    return os.str();
}

std::string gen_event_filter_factory::filter_fieldclass_info::as_string(
	bool verbose, const std::set<std::string> &event_sources)
{
    std::ostringstream os;
