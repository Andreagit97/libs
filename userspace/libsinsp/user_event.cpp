...
bool event_scope::add(const std::string& key, const std::string& value,
		      const std::string& op)
{
	...
	if(!v.empty())
	{
	    ...
	    m_scope.append(trim(k)).append("='").append(trim(v)).append(1,
									0x27);
	    return true;
	}
    }
    ...
    {
	g_logger.log("Scope key is invalid: [" + key +
			     "], entry will not be added to scope.",
...
#ifndef _WIN32
void event_scope::regex_error(const std::string& call, size_t ret,
			      regex_t* preg, const std::string& str)
{
    if(!preg)
    {
	return;
    ...
    {
	g_logger.log("Can't obtain " + call + "() [" + str + "] error.",
		     sinsp_logger::SEV_WARNING);
    }
}
...
bool event_scope::check_key_format(const std::string& key)
{
	...
	{
	    regex_error("regexec", ret, &reg, key);
	}
    }
    else
    {
...
user_event_meta_t::user_event_meta_t(const std::string& kind,
...
user_event_filter_t::list_t::const_iterator
user_event_filter_t::get(const std::string& evt_kind) const
{
    list_t::const_iterator it = m_list.begin(), end = m_list.end();
    for(; it != end; ++it)
    {
	if(ci_compare_str(it->kind(), evt_kind))
	{
	    return it;
	}
    }
...
bool user_event_filter_t::has(const std::string& evt_kind,
			      const std::string& evt_type) const
{
...
user_event_filter_t::list_t::const_iterator
user_event_filter_t::get_meta(const std::string& evt_kind) const
{
    list_t::const_iterator it = m_list.begin(), end = m_list.end();
    for(; it != end; ++it)
    {
	if(ci_compare_str(it->kind(), user_event_meta_t::PERMIT_ALL) ||
	   ci_compare_str(it->kind(), evt_kind))
	{
	    return it;
	}
    }
...
sinsp_user_event::sinsp_user_event(uint64_t epoch_time_s, string&& name,
				   string&& desc, string&& scope,
				   tag_map_t&& tags, uint32_t sev):
	m_epoch_time_s(epoch_time_s),
...
sinsp_user_event::sinsp_user_event(sinsp_user_event&& other):
	m_epoch_time_s(other.m_epoch_time_s), m_name(std::move(other.m_name)),
	m_description(std::move(other.m_description)),
	m_severity(other.m_severity), m_scope(std::move(other.m_scope)),
					   ...
					   const std::string& source)
{
    ...
    auto evt = sinsp_user_event(get_epoch_utc_seconds_now(),
				std::move(event_name), description.str(),
