	...
	m_inspector(inspector)
{
    ...
    {
	throw sinsp_exception("Could not convert ipv6 loopback address ::1 to "
			      "ipv6addr struct");
    }
}

sinsp_ipv4_ifinfo::sinsp_ipv4_ifinfo(uint32_t addr, uint32_t netmask,
				     uint32_t bcast, const char* name)
{
    m_addr = addr;
    m_netmask = netmask;
    m_bcast = bcast;
...
{
    sprintf(dest, "%d.%d.%d.%d", (addr & 0xFF), ((addr & 0xFF00) >> 8),
...
string sinsp_ipv4_ifinfo::to_string() const
{
    ...
    snprintf(s, sizeof(s), "%s inet %s netmask %s broadcast %s", m_name.c_str(),
	     str_addr, s_netmask, s_bcast);
    return string(s);
}

uint32_t
sinsp_network_interfaces::infer_ipv4_address(uint32_t destination_address)
{
    ...
    for(it = m_ipv4_interfaces.begin(); it != m_ipv4_interfaces.end(); it++)
    {
	if((it->m_addr & it->m_netmask) ==
	   (destination_address & it->m_netmask))
	{
	    return it->m_addr;
	}
    }
...
bool sinsp_network_interfaces::is_ipv4addr_in_subnet(uint32_t addr)
{
    ...
    //
    if((addr & 0x000000ff) == 0x0000000a || (addr & 0x0000ffff) == 0x0000a8c0 ||
...
bool sinsp_network_interfaces::is_ipv4addr_in_local_machine(
	uint32_t addr, sinsp_threadinfo* tinfo)
{
	...
	// To at least achieve consistency across client and server, we just
	// match the host interface addresses.
	//
	if(container_info)
	{
	    ...
	    {
		//
		// We have a container info with a valid container IP. Let's use
		// it.
		//
		...
		if(!container_info->is_successful())
		{
		    g_logger.format(sinsp_logger::SEV_DEBUG,
				    "Checking IP address of container %s with "
				    "incomplete metadata (state=%d)",
		...
		for(const auto& it : *clist)
		{
		    ...
		    {
			g_logger.format(sinsp_logger::SEV_DEBUG,
					"Checking IP address of container %s "
					"with incomplete metadata (in context "
					"of %s; state=%d)",
...
void sinsp_network_interfaces::import_ipv4_ifaddr_list(uint32_t count,
						       scap_ifinfo_ipv4* plist)
{
    if(count == 0)
    {
	return;
...
ipv6addr
sinsp_network_interfaces::infer_ipv6_address(ipv6addr& destination_address)
{
...
bool sinsp_network_interfaces::is_ipv6addr_in_local_machine(
	ipv6addr& addr, sinsp_threadinfo* tinfo)
{
    if(!tinfo->m_container_id.empty())
    {
	// For now, not supporting ipv6 networking for containers. So always
	// return false;
	return false;
    }
...
void sinsp_network_interfaces::import_ipv6_ifaddr_list(uint32_t count,
						       scap_ifinfo_ipv6* plist)
{
    if(count == 0)
    {
	return;
...
void sinsp_network_interfaces::import_ipv4_interface(
	const sinsp_ipv4_ifinfo& ifinfo)
{
    m_ipv4_interfaces.push_back(ifinfo);
}

void sinsp_network_interfaces::import_ipv6_interface(
	const sinsp_ipv6_ifinfo& ifinfo)
{
