...
static const filtercheck_field_info sinsp_filter_check_fd_fields[] =
{
	...
	{PT_IPADDR, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER | EPF_NO_EXTR_MODIFIER, PF_NA, "fd.ip", "FD IP Address", "matches the ip address (client or server) of the fd."},
	...
	{PT_PORT, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER | EPF_NO_EXTR_MODIFIER, PF_DEC, "fd.port", "FD Port", "matches the port (either client or server) of the fd."},
	...
	{PT_PORT, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER | EPF_NO_EXTR_MODIFIER, PF_NA, "fd.proto", "FD Protocol", "matches the protocol (either client or server) of the fd."},
	...
	{PT_IPNET, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER | EPF_NO_EXTR_MODIFIER, PF_NA, "fd.net", "FD IP Network", "matches the IP network (client or server) of the fd."},
	{PT_IPNET, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER, PF_NA, "fd.cnet", "FD Client Network", "matches the client IP network of the fd."},
	{PT_IPNET, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER, PF_NA, "fd.snet", "FD Server Network", "matches the server IP network of the fd."},
	{PT_IPNET, EPF_FILTER_ONLY | EPF_NO_RHS_FILTER, PF_NA, "fd.lnet", "FD Local Network", "matches the local IP network of the fd."},
	...
	{PT_CHARBUF, EPF_IS_LIST | EPF_ARG_ALLOWED | EPF_NO_EXTR_MODIFIER, PF_DEC, "fd.types", "FD Type", "List of FD types in used. Can be passed an fd number e.g. fd.types[0] to get the type of stdout as a single item list."},
};
...
bool sinsp_filter_check_fd::extract(sinsp_evt *evt, OUT std::vector<extract_value_t>& values, bool sanitize_strings)
{
	...
	if(m_field_id == TYPE_FDTYPES && m_argid == -1)
	{
		...
		// Here we don't apply the modifiers but we will call the `compare_nocache` with the "modified_type"
