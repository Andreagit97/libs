...
// è un vettore di costanti con tutti i field, con cui confronto i miei filter che passo da tastiera 
...
// gestisce solo alcuni casi particolari per filter_check se no chiama quello della funzione padre
...
{
	// si va a prendere un parametro direttamente per nome !!! nella zona di memoria corrispondente al buffer ring
	const sinsp_evt_param* pi = evt->get_param_value_raw(argname);

	if(pi != NULL)
	{
		// ottiene valore e lunghezza del campo !!
		*len = pi->m_len;
...
uint8_t* sinsp_filter_check_event::extract(sinsp_evt *evt, OUT uint32_t* len, bool sanitize_strings)
{
	*len = 0;
	...
	case TYPE_ARGRAW:
	    // la len=0 non so dove si sia preso il name 
		// questo name sembra il nome del camp che sto cercando tipo "category". stabilisco solo se l'evento che sto consideranod lo ha tra i suoi campi
...
// la funzione di compare che viene chiamata quando controllo se l'evento matcha con il filter check
bool sinsp_filter_check_event::compare(sinsp_evt *evt)
{
	bool res;

	// all'inizio è true poi passa a false
	m_is_compare = true;
	...
	if(m_field_id == TYPE_ARGRAW)
	{
		uint32_t len;
		bool sanitize_strings = false;
		// l'altro metodo extract che mi nominava jason
		uint8_t* extracted_val = extract(evt, &len, sanitize_strings);

		// può essere che non si abbia quel tipo di valore e quindi finisce li
		if(extracted_val == NULL)
		{
			return false;
		}
		...
		// ho estratto il valore dall'evento che desideravo e faccio il compare con quello che deve essere 
		res = flt_compare(m_cmpop,
			m_arginfo->type,
			extracted_val);
	}
