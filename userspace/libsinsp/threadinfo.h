...
class SINSP_PUBLIC sinsp_threadinfo
{
    ...
    */
    ...
    /*!
      \brief Return the name of the process containing this thread from argv[0],
      e.g. "/bin/top".
    */
    std::string get_exe() const;

    /*!
      \brief Return the full executable path of the process containing this
      thread, e.g. "/bin/top".
    */
    ...
    */
    ...
    /*!
      \brief Return the value of the specified environment variable for the
      process containing this thread. Returns empty string if variable is not
      found.
    */
    ...
    */
	...
	if(!main_thread)
	{
	    ...
	    {
		//
		// No, this is either a single thread process or the root thread
		// of a multithread process. Note: we don't set m_main_thread
		// because there are cases in which this is
		//       invoked for a threadinfo that is in the stack. Caching
		//       the this pointer would cause future mess.
		//
    ...
    */
    ...
    /*!
      \brief Return true if this thread has a client socket open on the given
      port.
    */
    ...
    /*!
      \brief Return the ratio between open FDs and maximum available FDs for
      this thread.
    */
    ...
    */
    ...
    static void populate_cmdline(std::string& cmdline,
    ...
    int64_t m_pid;  ///< The id of the process containing this thread. In single
		    ///< thread threads, this is equal to tid.
    ...
    std::vector<std::pair<std::string, std::string>>
	    m_cgroups;		///< subsystem-cgroup pairs
    std::string m_container_id; ///< heuristic-based container id
    uint32_t m_flags;  ///< The thread flags. See the PPM_CL_* declarations in
		       ///< ppm_events_public.h.
    ...
    int64_t m_vpid;  ///< The virtual id of the process containing this thread.
		     ///< In single thread threads, this is equal to vtid.
    int64_t m_vpgid; // The virtual process group id, as seen from its pid
		     // namespace
    std::string m_root;
    size_t m_program_hash;	   ///< Unique hash of the current program
    size_t m_program_hash_scripts; ///< Unique hash of the current program,
				   ///< including arguments for scripting
				   ///< programs (like python or ruby)
    int32_t m_tty;
    ...
    uint64_t m_prevevent_ts;  ///< timestamp of the event before the last for
			      ///< this thread.
    uint64_t m_lastaccess_ts; ///< The last time this thread was looked up. Used
	...
	{
	    return lhs->get_main_thread()->m_program_hash ==
		   rhs->get_main_thread()->m_program_hash;
	}
    };
    ...
    VISIBILITY_PRIVATE
    void init();
    // return true if, based on the current inspector filter, this thread should
    // be kept
    ...
    void add_to_iovec(const std::string& str, const bool include_trailing_null,
    ...
    uint8_t* m_lastevent_data; // Used by some event parsers to store the last
			       // enter event
...
///////////////////////////////////////////////////////////////////////////////
// Little class that manages the allocation of private state in the thread info
// class
///////////////////////////////////////////////////////////////////////////////
class sinsp_thread_privatestate_manager
{
    ...
    uint32_t get_size() { return (uint32_t)m_memory_sizes.size(); }
