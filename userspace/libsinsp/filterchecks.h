...
bool flt_compare(cmpop op, ppm_param_type type, void* operand1, void* operand2,
		 uint32_t op1_len = 0, uint32_t op2_len = 0);
...
class sinsp_filter_check : public gen_event_filter_check
{
    ...
    //
    virtual filter_check_info* get_fields() { return &m_info; }
    ...
    // Doesn't return the field length because the filtering engine can
    // calculate it.
    //
    void add_filter_value(const char* str, uint32_t len, uint32_t i = 0);
    virtual size_t parse_filter_value(const char* str, uint32_t len,
				      uint8_t* storage, uint32_t storage_len);
    ...
    //
    bool extract(gen_event* evt, OUT vector<extract_value_t>& values,
		 bool sanitize_strings = true);

    // Alias of extract that uses the sinsp_evt type.
    // By default, this fills the vector with only one value, retireved by
    // calling the single-result extract method. If a NULL value is returned by
    // extract, the vector is emptied. Subclasses are meant to either override
    // this, or the single-valued extract method.
    virtual bool extract(sinsp_evt* evt, OUT vector<extract_value_t>& values,
			 bool sanitize_strings = true);

    //
    // Wrapper for extract() that implements caching to speed up multiple
    // extractions of the same value, which are common in Falco.
    //
    bool extract_cached(sinsp_evt* evt, OUT vector<extract_value_t>& values,
			bool sanitize_strings = true);
    ...
    //
    // Compare the field with the constant value obtained from
    // parse_filter_value()
    //
    ...
    protected:
    // This is a single-value version of extract for subclasses non supporting
    // extracting multiple values. By default, this returns NULL. Subclasses are
    // meant to either override this, or the multi-valued extract method.
    virtual uint8_t* extract(sinsp_evt* evt, OUT uint32_t* len,
			     bool sanitize_strings = true);

    bool flt_compare(cmpop op, ppm_param_type type, void* operand1,
		     uint32_t op1_len = 0, uint32_t op2_len = 0);
    bool flt_compare(cmpop op, ppm_param_type type,
		     vector<extract_value_t>& values, uint32_t op2_len = 0);

    char* rawval_to_string(uint8_t* rawval, ppm_param_type ptype,
			   ppm_print_format print_format, uint32_t len);
    Json::Value rawval_to_json(uint8_t* rawval, ppm_param_type ptype,
			       ppm_print_format print_format, uint32_t len);
    ...
    {
	return &m_val_storages[i][0];
    }
    inline vector<uint8_t>* filter_value(uint16_t i = 0)
    {
	return &m_val_storages[i];
    }

    unordered_set<filter_value_t, g_hash_membuf, g_equal_to_membuf>
...
class sinsp_filter_check_thread : public sinsp_filter_check
{
    ...
    uint8_t* extract_thread_cpu(sinsp_evt* evt, OUT uint32_t* len,
				sinsp_threadinfo* tinfo, bool extract_user,
				bool extract_system);
...
class sinsp_filter_check_event : public sinsp_filter_check
{
    ...
    int32_t parse_field_name(const char* str, bool alloc_state,
			     bool needed_for_filtering);
    size_t parse_filter_value(const char* str, uint32_t len, uint8_t* storage,
    ...
    int32_t extract_arg(string fldname, string val,
			OUT const struct ppm_param_info** parinfo);
    int32_t extract_type(string fldname, string val,
			 OUT const struct ppm_param_info** parinfo);
...
class sinsp_filter_check_tracer : public sinsp_filter_check
{
    ...
    int32_t extract_arg(string fldname, string val,
			OUT const struct ppm_param_info** parinfo);
    inline uint8_t* extract_duration(uint16_t etype,
...
class sinsp_filter_check_k8s : public sinsp_filter_check
{
    ...
    void
    concatenate_container_labels(const map<std::string, std::string>& labels,
				 string* s);
    bool find_label(const k8s_pair_list& labels, const string& key,
		    string* value);
...
class sinsp_filter_check_mesos : public sinsp_filter_check
{
    ...
    bool find_label(const mesos_pair_list& labels, const string& key,
		    string* value);
