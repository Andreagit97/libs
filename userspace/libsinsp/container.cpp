...
sinsp_container_manager::sinsp_container_manager(
	sinsp* inspector, bool static_container, const std::string static_id,
	const std::string static_name, const std::string static_image):
	m_inspector(inspector),
	m_last_flush_time_ns(0), m_static_container(static_container),
	m_static_id(static_id), m_static_name(static_name),
...
bool sinsp_container_manager::remove_inactive_containers()
{
	...
	m_last_flush_time_ns = m_inspector->m_lastevent_ts -
			       m_inspector->m_inactive_container_scan_time_ns +
			       30 * ONE_SECOND_IN_NS;
    }
...
sinsp_container_info::ptr_t
sinsp_container_manager::get_container(const string& container_id) const
{
    auto containers = m_containers.lock();
						...
						bool query_os_for_missing_info)
{
    ...
    {
	matches = m_inspector->m_parser->m_fd_listener->on_resolve_container(
		this, tinfo, query_os_for_missing_info);
    }
...
string sinsp_container_manager::container_to_json(
	const sinsp_container_info& container_info)
{
    ...
    container["created_time"] =
	    static_cast<Json::Value::Int64>(container_info.m_created_time);
    ...
    sinsp_container_info::container_health_probe::add_health_probes(
	    container_info.m_health_probes, container);
    ...
    container["memory_limit"] =
	    (Json::Value::Int64)container_info.m_memory_limit;
    ...
    container["cpuset_cpu_count"] =
	    (Json::Value::Int)container_info.m_cpuset_cpu_count;
    ...
    container["metadata_deadline"] =
	    (Json::Value::UInt64)container_info.m_metadata_deadline;
    return Json::FastWriter().write(obj);
}

bool sinsp_container_manager::container_to_sinsp_event(
	const string& json, sinsp_evt* evt, shared_ptr<sinsp_threadinfo> tinfo)
{
...
sinsp_container_manager::map_ptr_t
sinsp_container_manager::get_containers() const
{
    return m_containers.lock();
}

void sinsp_container_manager::add_container(
	const sinsp_container_info::ptr_t& container_info,
	sinsp_threadinfo* thread)
{
    set_lookup_status(container_info->m_id, container_info->m_type,
...
void sinsp_container_manager::replace_container(
	const sinsp_container_info::ptr_t& container_info)
{
    auto containers = m_containers.lock();
...
void sinsp_container_manager::notify_new_container(
	const sinsp_container_info& container_info, sinsp_threadinfo* tinfo)
{
	...
	// * being called in capture mode (no need to send any event as we will
	// read it)
	//
	// Fallback at just storing the new container.
	// NOTE: this must be kept in sync with what happens on container event
	// parsing, in parsers.cpp.
	const auto container = m_inspector->m_container_manager.get_container(
		container_info.m_id);
	if(container != nullptr && container->is_successful())
	{
	    SINSP_DEBUG("Ignoring new container notification for already "
			"successful lookup of %s",
			container_info.m_id.c_str());
	}
	else
	{
	    add_container(
		    std::make_shared<sinsp_container_info>(container_info),
		    tinfo);
	}
	return;
    }
    ...
    if(container_to_sinsp_event(container_to_json(container_info), evt,
				container_info.get_tinfo(m_inspector)))
    {
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"notify_new_container (%s): created CONTAINER_JSON "
			"event, queuing to inspector",
			container_info.m_id.c_str());

	std::shared_ptr<sinsp_evt> cevt(evt);

	// Enqueue it onto the queue of pending container events for the
	// inspector
#ifndef _WIN32
	...
	g_logger.format(sinsp_logger::SEV_ERROR,
			"notify_new_container (%s): could not create "
			"CONTAINER_JSON event, dropping",
			container_info.m_id.c_str());
	delete evt;
    }
}
...
void sinsp_container_manager::dump_containers(scap_dumper_t* dumper)
{
    ...
    {
	sinsp_evt evt;
	if(container_to_sinsp_event(container_to_json(*it.second), &evt,
				    it.second->get_tinfo(m_inspector)))
	{
	    int32_t res = scap_dump(m_inspector->m_h, dumper, evt.m_pevt,
				    evt.m_cpuid, SCAP_DF_LARGE);
	    if(res != SCAP_SUCCESS)
	    {
...
string
sinsp_container_manager::get_container_name(sinsp_threadinfo* tinfo) const
{
    string res;
...
void sinsp_container_manager::identify_category(sinsp_threadinfo* tinfo)
{
	...
	{
	    g_logger.format(sinsp_logger::SEV_DEBUG,
			    "identify_category (%ld) (%s): container metadata "
			    "incomplete",
			    tinfo->m_tid, tinfo->m_comm.c_str());
	}
    ...
    {
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"identify_category (%ld) (%s): not under container "
			"init, assigning category %s",
			tinfo->m_tid, tinfo->m_comm.c_str(),
...
void sinsp_container_manager::subscribe_on_new_container(
	new_container_cb callback)
{
    m_new_callbacks.emplace_back(callback);
}

void sinsp_container_manager::subscribe_on_remove_container(
	remove_container_cb callback)
{
...
void sinsp_container_manager::create_engines()
{
    ...
    {
	auto engine = std::make_shared<container_engine::static_container>(
		*this, m_static_id, m_static_name, m_static_image);
...
#ifdef CYGWING_AGENT
    {
	auto docker_engine = std::make_shared<container_engine::docker_win>(
		*this, m_inspector /*wmi source*/);
    ...
    {
	auto docker_engine =
		std::make_shared<container_engine::docker_linux>(*this);
	m_container_engines.push_back(docker_engine);
	m_container_engine_by_type[CT_DOCKER] = docker_engine;
    }
    ...
    {
	auto libvirt_lxc_engine =
		std::make_shared<container_engine::libvirt_lxc>(*this);
...
void sinsp_container_manager::update_container_with_size(
	sinsp_container_type type, const std::string& container_id)
{
	...
	g_logger.format(
		...
		"Container type %d not found when requesting size for %s", type,
		container_id.c_str());
    }

    g_logger.format(sinsp_logger::SEV_DEBUG, "Request size for %s",
		    container_id.c_str());
...
void sinsp_container_manager::set_docker_socket_path(std::string socket_path)
{
    ...
    libsinsp::container_engine::docker_linux::set_docker_sock(
...
void sinsp_container_manager::set_query_docker_image_info(bool query_image_info)
{
    ...
    libsinsp::container_engine::docker_async_source::set_query_image_info(
	    query_image_info);
#endif
}
...
void sinsp_container_manager::set_container_labels_max_len(
