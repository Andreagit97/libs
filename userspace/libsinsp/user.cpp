...
sinsp_usergroup_manager::sinsp_usergroup_manager(sinsp *inspector):
	m_import_users(true), m_last_flush_time_ns(0), m_inspector(inspector)
{
}
...
void sinsp_usergroup_manager::init()
{
    if(m_import_users)
    {
	// Emplace container manager listener to delete container users upon
	// container deletion
...
void sinsp_usergroup_manager::dump_users_groups(scap_dumper_t *dumper)
{
	...
	{
	    sinsp_evt evt;
	    if(user_to_sinsp_event(&user.second, &evt, container_id,
				   PPME_USER_ADDED_E))
	    {
	...
	{
	    sinsp_evt evt;
	    if(group_to_sinsp_event(&group.second, &evt, container_id,
				    PPME_GROUP_ADDED_E))
	    {
		int32_t res = scap_dump(m_inspector->m_h, dumper, evt.m_pevt,
					evt.m_cpuid, 0);
...
void sinsp_usergroup_manager::delete_container_users_groups(
...
bool sinsp_usergroup_manager::clear_host_users_groups()
{
	...
	m_last_flush_time_ns =
		...
		m_inspector->m_deleted_users_groups_scan_time_ns +
		60 * ONE_SECOND_IN_NS;
    }
...
scap_userinfo *sinsp_usergroup_manager::add_user(const string &container_id,
						 uint32_t uid, uint32_t gid,
						 const char *name,
						 const char *home,
						 const char *shell, bool notify)
{
    if(!m_import_users)
    {
	return nullptr;
...
const unordered_map<uint32_t, scap_userinfo> *
sinsp_usergroup_manager::get_userlist(const string &container_id)
{
...
scap_userinfo *sinsp_usergroup_manager::get_user(const string &container_id,
						 uint32_t uid)
{
    if(uid == 0xffffffff)
    {
...
const unordered_map<uint32_t, scap_groupinfo> *
sinsp_usergroup_manager::get_grouplist(const string &container_id)
{
...
scap_groupinfo *
sinsp_usergroup_manager::get_group(const std::string &container_id,
				   uint32_t gid)
{
    if(gid == 0xffffffff)
    {
						  ...
						  uint16_t ev_type)
{
    ...
    size_t totlen = sizeof(scap_evt) + 6 * sizeof(uint16_t) + sizeof(uint32_t) +
		    sizeof(uint32_t) + strlen(user->name) + 1 +
		    strlen(user->homedir) + 1 + strlen(user->shell) + 1 +
