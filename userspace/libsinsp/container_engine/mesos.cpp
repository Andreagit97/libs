...
bool libsinsp::container_engine::mesos::match(
	sinsp_threadinfo* tinfo, sinsp_container_info& container_info)
{
	...
	if(pos != string::npos)
	{
	    ...
	    if(id.size() == 36 &&
	       id.find_first_not_of("0123456789abcdefABCDEF-") == string::npos)
	    {
		container_info.m_type = CT_MESOS;
		container_info.m_id = move(id);
		// Consider a mesos container valid only if we find the
		// mesos_task_id this will exclude from the container itself the
		// mesos-executor but makes sure that we have task_id parsed
		// properly. Otherwise what happens is that we'll create a mesos
		// container struct without a mesos_task_id and for all other
		// processes we'll use it
...
string libsinsp::container_engine::mesos::get_env_mesos_task_id(
...
bool libsinsp::container_engine::mesos::set_mesos_task_id(
	sinsp_container_info& container, sinsp_threadinfo* tinfo)
{
    ASSERT(tinfo);

    // there are applications that do not share their environment in
    // /proc/[PID]/environ since we need MESOS_TASK_ID environment variable to
    // discover Mesos containers, there is a workaround for such cases:
    // - for docker containers, we discover it directly from container, through
    // Remote API
    //   (see sinsp_container_manager::parse_docker() for details)
    // - for mesos native containers, parent process has the MESOS_TASK_ID (or
    // equivalent, see
    //   get_env_mesos_task_id(sinsp_threadinfo*) implementation) environment
    //   variable, so we peek into the parent process environment to discover it
	...
	if(mtid.empty())
	{
	    ...
	    {
		g_logger.log("Mesos native container: [" + container.m_id +
				     "], Mesos task ID: " + mtid,
		...
		g_logger.log("Mesos container [" + container.m_id +
				     ...
				     "], has likely malformed mesos task id [" +
				     mtid + "], ignoring",
			     sinsp_logger::SEV_DEBUG);
	    }
	}
