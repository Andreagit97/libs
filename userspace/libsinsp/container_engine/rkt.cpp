...
bool rkt::match(container_cache_interface* cache, sinsp_threadinfo* tinfo,
		sinsp_container_info& container_info, string& rkt_podid,
		string& rkt_appname, bool query_os_for_missing_info)
{
	...
	if(cgroup.rfind(SERVICE_SUFFIX) ==
	   cgroup.size() - SERVICE_SUFFIX.size())
	{
		...
		rkt_appname = cgroup.substr(last_slash + 1,
					    cgroup.size() - last_slash -
						    SERVICE_SUFFIX.size() - 1);

		char image_manifest_path[SCAP_MAX_PATH_SIZE];
		snprintf(image_manifest_path, sizeof(image_manifest_path),
			 "%s/var/lib/rkt/pods/run/%s/appsinfo/%s/manifest",
			 scap_get_host_root(), rkt_podid.c_str(),
			 rkt_appname.c_str());

		// First lookup if the container exists in our table, otherwise
		// only if we are live check if it has an entry in /var/lib/rkt.
		// In capture mode only the former will be used. In live mode
		// former will be used only if we already hit that container
		bool is_rkt_pod_id_valid = cache->container_exists(
			rkt_podid + ":" +
			rkt_appname); // if it's already on our table
#ifdef HAS_CAPTURE
		if(!is_rkt_pod_id_valid && query_os_for_missing_info)
		{
		    is_rkt_pod_id_valid =
			    (access(image_manifest_path, F_OK) == 0);
		}
#endif
	...
	if(suffix != string::npos)
	{
	    ...
	    // Try the current thread first. visitor returns true if no coreos
	    // pid info was found. In this case we traverse the parents.
	    if(visitor(tinfo))
	    {
		tinfo->traverse_parent_state(visitor);
	    }
	...
	if(prefix == 0)
	{
	    auto podid_suffix = tinfo->m_root.find(FLY_PODID_SUFFIX,
						   prefix + FLY_PREFIX.size());
	    if(podid_suffix != string::npos)
	    {
		rkt_podid = tinfo->m_root.substr(prefix + FLY_PREFIX.size(),
						 podid_suffix - prefix -
							 FLY_PREFIX.size());
		auto appname_suffix = tinfo->m_root.find(
			FLY_APP_SUFFIX, podid_suffix + FLY_PODID_SUFFIX.size());
		if(appname_suffix != string::npos)
		{
		    rkt_appname = tinfo->m_root.substr(
			    podid_suffix + FLY_PODID_SUFFIX.size(),
			    appname_suffix - podid_suffix -
				    FLY_PODID_SUFFIX.size());
		    container_info.m_type = CT_RKT;
		    container_info.m_id = rkt_podid + ":" + rkt_appname;
		    container_info.m_name = rkt_appname;
		    return true;
...
bool rkt::rkt::resolve(sinsp_threadinfo* tinfo, bool query_os_for_missing_info)
{
    ...
    if(!match(cache, tinfo, container, rkt_podid, rkt_appname,
    ...
    if(!query_os_for_missing_info ||
       !cache->should_lookup(container.m_id, CT_RKT))
    {
	return true;
    }
    ...
    if(have_rkt)
    {
	cache->add_container(std::make_shared<sinsp_container_info>(container),
			     tinfo);
...
bool rkt::rkt::parse_rkt(sinsp_container_info& container, const string& podid,
			 const string& appname)
{
    ...
    snprintf(image_manifest_path, sizeof(image_manifest_path),
	     "%s/var/lib/rkt/pods/run/%s/appsinfo/%s/manifest",
	     scap_get_host_root(), podid.c_str(), appname.c_str());
    ...
    snprintf(net_info_path, sizeof(net_info_path),
	     "%s/var/lib/rkt/pods/run/%s/net-info.json", scap_get_host_root(),
	     podid.c_str());
    ifstream net_info(net_info_path);
    if(reader.parse(net_info, jroot) && jroot.size() > 0)
    {
	const auto& first_net = jroot[0];
	if(inet_pton(AF_INET, first_net["ip"].asCString(),
    ...
    snprintf(pod_manifest_path, sizeof(pod_manifest_path),
	     "%s/var/lib/rkt/pods/run/%s/pod", scap_get_host_root(),
	     podid.c_str());
	...
	for(const auto& japp : jroot["apps"])
	{
		...
		{
		    image_ports[image_port["name"].asString()] =
			    image_port["port"].asUInt();
		}
		break;
	    }
	}
