...
size_t docker_curl_write_callback(const char* ptr, size_t size, size_t nmemb,
				  std::string* json)
{
...
docker_connection::docker_connection():
	m_api_version("/v1.24"), m_curlm(nullptr)
{
    m_curlm = curl_multi_init();

    if(m_curlm)
    {
	curl_multi_setopt(m_curlm, CURLMOPT_PIPELINING,
			      ...
			      const std::string& req_url, std::string& json)
{
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG, "docker_async (%s): Fetching url",
		    url.c_str());
    ...
    curl_easy_cleanup(curl);
    g_logger.format(sinsp_logger::SEV_DEBUG, "docker_async (%s): http_code=%ld",
		    url.c_str(), http_code);
	...
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker_async (%s): returning RESP_ERROR", url.c_str());
	return docker_response::RESP_ERROR;
    case 200:
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker_async (%s): returning RESP_OK", url.c_str());
