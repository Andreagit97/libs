...
{
    // the kernel driver does not return cgroups without subsystems (e.g.
    // name=systemd) in the cgroups field, so we have to do a check here, and
    // load /proc/pid/cgroups ourselves if needed
    ...
    cgroups_file << scap_get_host_root() << "/proc/" << tinfo->m_tid
		 << "/cgroup";
...
int get_userns_root_uid(const sinsp_threadinfo *tinfo)
{
    ...
    uid_map_file << scap_get_host_root() << "/proc/" << tinfo->m_tid
		 << "/uid_map";
...
int detect_podman(const sinsp_threadinfo *tinfo, std::string &container_id)
{
    ...
    char c;	    // ^ same
    if(sscanf(systemd_cgroup.c_str() + pos, "podman-%d.scope/%c", &podman_pid,
	      &c) != 2)
    {
...
bool podman::resolve(sinsp_threadinfo *tinfo, bool query_os_for_missing_info)
{
    ...
    docker_lookup_request request(container_id, api_sock, CT_PODMAN, uid,
				  false);
