...
{
    this->stop();
    g_logger.format(sinsp_logger::SEV_DEBUG, "docker_async: Source destructor");
}

bool docker_async_source::lookup_sync(const docker_lookup_request& request,
				      sinsp_container_info& value)
{
	...
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker (%s): Failed to get Docker metadata, returning "
			"successful=false",
					   ...
					   Json::Value& spec)
{
	...
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Could not parse pod config '%s'", cfg_str.c_str());
	return false;
    }
					    ...
					    sinsp_container_info& container)
{
    ...
    {
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Could not parse health check from %s (No Healthcheck "
			"property)",
			Json::FastWriter().write(healthcheck_obj).c_str());
    ...
    {
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Could not parse health check from %s (Healthcheck "
			"does not have Test property)",
			Json::FastWriter().write(healthcheck_obj).c_str());
	...
	{
	    g_logger.format(sinsp_logger::SEV_WARNING,
			    "Could not parse health check from %s (Expected "
			    "NONE for single-element Test array)",
			    Json::FastWriter().write(healthcheck_obj).c_str());
	}
	return;
    }
	...
	g_logger.format(sinsp_logger::SEV_WARNING,
			"Could not parse health check from %s (Expected "
			"CMD/CMD-SHELL for multi-element Test array)",
			Json::FastWriter().write(healthcheck_obj).c_str());
	return;
    }
}
	...
	sinsp_container_info& container)
{
	...
	g_logger.format(sinsp_logger::SEV_DEBUG,
			...
			sinsp_container_info::container_health_probe::
				probe_type_names[ptype]
					.c_str(),
			exe.c_str(), args.size());

	container.m_health_probes.emplace_back(ptype, std::move(exe),
					       std::move(args));
    }
...
bool docker_async_source::get_sandbox_liveness_readiness_probes(
	const Json::Value& config_obj, sinsp_container_info& container)
{
    ...
    {
	SINSP_DEBUG("docker (%s): No sandbox label found, not copying "
    ...
    sinsp_container_info::ptr_t sandbox_container =
	    m_cache->get_container(sandbox_container_id);

    if(!sandbox_container)
    {
	SINSP_DEBUG("docker (%s): Sandbox container %s doesn't exist, not "
		    "copying liveness/readiness probes",
    ...
    {
	SINSP_DEBUG("docker (%s): Sandbox container %s has no "
		    "liveness/readiness probes, not copying",
		    container.m_id.c_str(), sandbox_container_id.c_str());
	return false;
    }

    SINSP_DEBUG("docker (%s): Copying liveness/readiness probes from sandbox "
		"container %s",
		container.m_id.c_str(), sandbox_container_id.c_str());
					      ...
					      sinsp_container_info& container)
{
    ...
    // When parsing the full container json for live containers, a label
    // contains stringified json that contains the probes.
    ...
    // Otherwise, try to copy the liveness/readiness probe from the sandbox
					   ...
					   sinsp_container_info& container)
{
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG,
		    ...
		    request.container_id.c_str(), container.m_imageid.c_str());

    std::string img_json;
    std::string url = "/images/" + container.m_imageid + "/json?digests=1";

    g_logger.format(sinsp_logger::SEV_DEBUG, "docker_async url: %s",
		    url.c_str());

    if(m_connection.get_docker(request, url, img_json) !=
       docker_connection::RESP_OK)
    {
	g_logger.format(
	...
	g_logger.format(sinsp_logger::SEV_ERROR,
			"docker_async (%s) image (%s): Could not parse json "
			"image info \"%s\"",
			request.container_id.c_str(),
			container.m_imageid.c_str(), img_json.c_str());
	return;
    }
...
void docker_async_source::fetch_image_info_from_list(
	const docker_lookup_request& request, sinsp_container_info& container)
{
    Json::Reader reader;
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG,
		    ...
		    request.container_id.c_str(), img_json.c_str());
    ...
    const std::string match_name =
	    container.m_imagerepo + ':' + container.m_imagetag;
					   ...
					   const Json::Value& img)
{
	...
	// e.g.
	// "sha256:b6a9fc3535388a6fc04f3bdb83fb4d9d0b4ffd85e7609a6ff2f0f731427823e3"
	// so just use it directly
	container.m_imagedigest = podman_digest.asString();
    }
    else
    {
	// img_root["RepoDigests"] contains only digests for images pulled from
	// registries. If an image gets retagged and is never pushed to any
	// registry, we will not find that entry in container.m_imagerepo. Also,
	// for locally built images we have the same issue. This leads to
	// container.m_imagedigest being empty as well.
	//
					 ...
					 const Json::Value& root)
{
	...
	// `sha256:ddcca4b8a6f0367b5de2764dfe76b0a4bfa6d75237932185923705da47004347`
	// can be used to run a container as:
	// - docker run
	// sha256:ddcca4b8a6f0367b5de2764dfe76b0a4bfa6d75237932185923705da47004347
	// - docker run
	// ddcca4b8a6f0367b5de2764dfe76b0a4bfa6d75237932185923705da47004347
	...
	bool no_name = sinsp_utils::startswith(container.m_imageid,
					       container.m_image) ||
	...
	if(!no_name || !m_query_image_info)
	{
	    ...
	    sinsp_utils::split_container_image(
		    container.m_image, hostname, port, container.m_imagerepo,
		    container.m_imagetag, container.m_imagedigest, false);
	}

	if(m_query_image_info && !container.m_imageid.empty() &&
	   (no_name || container.m_imagedigest.empty() ||
	    container.m_imagetag.empty()))
	{
	...
	std::string hostname, port;
	sinsp_utils::split_container_image(
		imgstr, hostname, port, container.m_imagerepo,
		container.m_imagetag, container.m_imagedigest, false);
...
void docker_async_source::parse_json_mounts(
	...
	vector<sinsp_container_info::container_mount_info>& mounts)
{
...
bool docker_async_source::parse_docker(const docker_lookup_request& request,
				       sinsp_container_info& container)
{
    string json;
    ...
    docker_connection::docker_response resp =
	    m_connection.get_docker(request, api_request, json);

    switch(resp)
    {
    case docker_connection::docker_response::RESP_BAD_REQUEST:
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker_async (%s): Initial url fetch failed, trying "
			"w/o api version",
			request.container_id.c_str());

	m_connection.set_api_version("");
	json = "";
	resp = m_connection.get_docker(
		request, "/containers/" + request.container_id + "/json", json);
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG,
		    ...
		    request.container_id.c_str(), json.c_str());
    ...
    {
	g_logger.format(sinsp_logger::SEV_ERROR,
			"docker_async (%s): Could not parse json \"%s\", "
			"returning false",
			request.container_id.c_str(), json.c_str());
    ...
    // Get the created time - this will be string format i.e.
    // "%Y-%m-%dT%H:%M:%SZ" Convert it to seconds. This can be done with
    // get_epoc_utc_seconds()
    container.m_created_time = static_cast<int64_t>(
	    get_epoch_utc_seconds(root["Created"].asString()));
	...
	{
	    std::string secondary_container_id =
		    net_mode.substr(net_mode.find(":") + 1);
	    ...
	    g_logger.format(sinsp_logger::SEV_DEBUG,
			    "docker_async (%s), secondary (%s): Doing blocking "
			    "fetch of secondary container",
			    request.container_id.c_str(),
			    secondary_container_id.c_str());

	    if(parse_docker(docker_lookup_request(
				    secondary_container_id,
				    request.docker_socket,
				    request.container_type, request.uid, false /*don't request size since we just need the IP*/),
			    pcnt))
	    {
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"docker_async (%s), secondary (%s): Secondary "
				"fetch successful",
				request.container_id.c_str(),
				secondary_container_id.c_str());
    ...
    for(vector<string>::const_iterator it = ports.begin(); it != ports.end();
	++it)
    {
	...
	if(v.isArray())
	{
		...
		if(inet_pton(AF_INET, ip.c_str(), &port_mapping.m_host_ip) ==
		   -1)
		{
		    ASSERT(false);
		    continue;
    ...
    for(vector<string>::const_iterator it = labels.begin(); it != labels.end();
	++it)
    {
    ...
    /**
     * 2 separate docker APIs use CFS CPU scheduler to constrain container CPU
     * usage Reference: https://docs.docker.com/engine/reference/run/ 1) docker
     * run --cpus=<Number of CPUS> <Number of CPUs> is converted into a
     * cfs_cpu_quota value for the default cfs_cpu_period=100000 cfs_cpu_period
     * cannot be changed with this API For example, if <Number of CPUs>=0.5,
     * cfs_cpu_quota=50000 and cfs_cpu_period=100000 2) docker run
     * --cpu-quota=<quota> --cpu-period=<period> CFS quota and/or period can be
     * set directly. The default period is 100000 and default quota is 0 (which
     * translates to unconstrained) For example, if <quota>=12345 and
     * <period>=67890, then cfs_cpu_quota=12345 and cfs_cpu_period=67890 These 2
     * APIs are mutually exclusive: docker throws an error if an attempt is made
     * to use --cpus in combination with either --cpu-quota or --cpu-period
     *
     * docker_response json output:
     * 1) When --cpus is used, the value is returned as NanoCpus; both CpuQuota
     * and CpuPeriod are 0 Since cfs_cpu_period=100000=10^5 and 10^9 NanoCpus is
     * 1 CPU, which translates to cfs_cpu_quota=100000=10^5, we need to divide
     * NanoCpus by 10^4=10000 to convert NanoCpus into cfs_cpu_quota
     *
     * 2) When --cpu-quota and/or --cpu-period are used, the corresponding
     * values are returned; NanoCpus is 0
     */
    container.m_cpu_quota = max(host_config_obj["CpuQuota"].asInt64(),
...
#ifdef HAS_ANALYZER
    sinsp_utils::find_env(container.m_sysdig_agent_conf, container.get_env(),
			  "SYSDIG_AGENT_CONF");
    // container.m_sysdig_agent_conf = get_docker_env(env_vars,
    // "SYSDIG_AGENT_CONF");
#endif
