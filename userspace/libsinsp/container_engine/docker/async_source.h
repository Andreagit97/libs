...
class docker_async_source
	: public libsinsp::async_key_value_source<docker_lookup_request,
						  sinsp_container_info>
{
    ...
    bool lookup_sync(const docker_lookup_request &request,
    ...
    private:
    bool parse_docker(const docker_lookup_request &request,
    ...
    static void parse_image_info(sinsp_container_info &container,
				 const Json::Value &img);

    // Fetch the image info for the current container's m_imageid
    void fetch_image_info(const docker_lookup_request &request,
			  sinsp_container_info &container);
