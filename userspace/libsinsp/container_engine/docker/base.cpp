...
void docker_base::cleanup() { m_docker_info_source.reset(NULL); }
			       ...
			       bool query_os_for_missing_info)
{
	...
	auto src = new docker_async_source(docker_async_source::NO_WAIT_LOOKUP,
					   max_wait_ms, cache);
    ...
    sinsp_container_info::ptr_t container_info =
	    cache->get_container(request.container_id);
...
{
    auto cb = [cache](const docker_lookup_request& request,
		      const sinsp_container_info& res)
    {
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker (%s): Source callback result=%d",
			request.container_id.c_str(), res.m_lookup_state);
	...
	if(cache->async_allowed())
	{
	    ...
	    g_logger.format(sinsp_logger::SEV_ERROR,
			    "docker_async (%s): Unexpected immediate return "
			    "from docker_info_source.lookup()",
			    request.container_id.c_str());
	}
    }
}
