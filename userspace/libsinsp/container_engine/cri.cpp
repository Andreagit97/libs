...
bool cri_async_source::parse_containerd(
	const runtime::v1alpha2::ContainerStatusResponse& status,
	sinsp_container_info& container)
{
	...
	container.m_container_ip =
		ntohl(m_cri->get_pod_sandbox_ip(pod_sandbox_id));
    }

    return ret;
}

bool cri_async_source::parse_cri(
	sinsp_container_info& container,
	const libsinsp::cgroup_limits::cgroup_limits_key& key)
{
	...
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"cri (%s): id is neither a container nor a pod sandbox",
    ...
    container.m_created_time = static_cast<int64_t>(
	    resp_container.created_at() / ONE_SECOND_IN_NS);

    for(const auto& pair : resp_container.labels())
    {
	if(pair.second.length() <=
	   sinsp_container_info::m_container_label_max_length)
	{
	...
	{
	    container.m_imageid =
		    m_cri->get_container_image_id(resp_container.image_ref());
	}
    }
...
bool cri_async_source::lookup_sync(
	const libsinsp::cgroup_limits::cgroup_limits_key& key,
	sinsp_container_info& value)
{
    ...
    {
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"cri (%s): Failed to get CRI metadata, returning "
			"successful=false",
			key.m_container_id.c_str());
...
cri::cri(container_cache_interface& cache): container_engine_base(cache)
{
    ...
    // NOTE: having multiple container runtimes on the same host is a sporadic
    // case, so we wouldn't make things complex to support that. On the other
    // hand, specifying multiple unix socket paths (and using only the first
    // match) will solve the "same config, multiple hosts" use case.
	...
	m_cri = std::unique_ptr<libsinsp::cri::cri_interface>(
		new libsinsp::cri::cri_interface(cri_path));
	if(!m_cri->is_ok())
	{
	    m_cri.reset(nullptr);
	}
...
void cri::set_async(bool async) { s_async = async; }

void cri::set_cri_delay(uint64_t delay_ms) { s_cri_lookup_delay_ms = delay_ms; }
...
bool cri::resolve(sinsp_threadinfo* tinfo, bool query_os_for_missing_info)
{
    ...
    {
	g_logger.format(sinsp_logger::SEV_DEBUG, "cri (%s): Performing lookup",
			container_id.c_str());

	container.m_lookup_state = sinsp_container_lookup_state::SUCCESSFUL;
	libsinsp::cgroup_limits::cgroup_limits_key key(
		container.m_id, tinfo->get_cgroup("cpu"),
		tinfo->get_cgroup("memory"), tinfo->get_cgroup("cpuset"));

	if(!m_async_source)
	{
	    auto async_source =
		    new cri_async_source(cache, m_cri.get(), s_cri_timeout);
	    m_async_source = std::unique_ptr<cri_async_source>(async_source);
	}

	cache->set_lookup_status(container_id, m_cri->get_cri_runtime_type(),
				 sinsp_container_lookup_state::STARTED);
	auto cb = [cache](const libsinsp::cgroup_limits::cgroup_limits_key& key,
			  const sinsp_container_info& res)
	{
	    g_logger.format(sinsp_logger::SEV_DEBUG,
			    "cri_async (%s): Source callback result=%d",
			    key.m_container_id.c_str(), res.m_lookup_state);
	...
	if(async)
	{
		    ...
		    key, result, chrono::milliseconds(s_cri_lookup_delay_ms),
		    cb);
	}
	else
	{
	...
	if(done)
	{
	    // if a previous lookup call already found the metadata, process it
	    // now
	    cb(key, result);

	    if(async)
	    {
		// This should *never* happen, in async mode as ttl is 0 (never
		// wait)
		g_logger.format(sinsp_logger::SEV_ERROR,
				"cri_async (%s): Unexpected immediate return "
				"from cri_async lookup",
				container_id.c_str());
	    }
	}
    }
...
{
    sinsp_container_info::ptr_t existing =
	    container_cache().get_container(container_id);
    if(!existing)
    {
	g_logger.format(sinsp_logger::SEV_ERROR,
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG,
		    ...
		    container_id.c_str(), existing->m_full_id.c_str(),
    ...
    shared_ptr<sinsp_container_info> updated(
	    std::make_shared<sinsp_container_info>(*existing));
