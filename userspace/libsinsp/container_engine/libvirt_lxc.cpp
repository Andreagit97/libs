...
bool libvirt_lxc::match(sinsp_threadinfo* tinfo,
			sinsp_container_info& container_info)
{
	...
	if(pos != std::string::npos)
	{
		...
		container_info.m_type = CT_LIBVIRT_LXC;
		container_info.m_id =
			cgroup.substr(pos + sizeof("-lxc\\x2"),
	...
	if(pos != std::string::npos)
	{
	    container_info.m_type = CT_LIBVIRT_LXC;
	    container_info.m_id =
		    cgroup.substr(pos + sizeof("/libvirt/lxc/") - 1);
	    return true;
	}
    }
    return false;
...
bool libvirt_lxc::resolve(sinsp_threadinfo* tinfo,
