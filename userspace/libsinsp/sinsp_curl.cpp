...
sinsp_curl_http_headers::sinsp_curl_http_headers(): m_curl_header_list(NULL) {}
...
sinsp_curl::sinsp_curl(const uri& url, const std::string& bearer_token_file,
		       long timeout_ms, bool debug):
	m_curl(curl_easy_init()),
	m_uri(url), m_timeout_ms(timeout_ms),
	m_bt(new bearer_token(bearer_token_file)), m_debug(debug)
{
    init();
}

sinsp_curl::sinsp_curl(const uri& url, const std::string& cert,
		       const std::string& key,
		       const std::string& key_passphrase,
		       const std::string& ca_cert, bool verify_peer,
		       const std::string& cert_type,
		       const std::string& bearer_token_file, long timeout_ms,
		       bool debug):
	m_curl(curl_easy_init()),
	m_uri(url), m_timeout_ms(timeout_ms),
	m_ssl(new ssl(cert, key, key_passphrase, ca_cert, verify_peer,
		      cert_type)),
	m_bt(new bearer_token(bearer_token_file)), m_debug(debug)
{
    init();
...
sinsp_curl::sinsp_curl(const uri& url, ssl::ptr_t p_ssl,
		       bearer_token::ptr_t p_bt, long timeout_ms, bool debug):
	m_curl(curl_easy_init()),
	m_uri(url), m_timeout_ms(timeout_ms), m_ssl(p_ssl), m_bt(p_bt),
	m_debug(debug)
{
    init();
}
...
sinsp_curl::~sinsp_curl() { curl_easy_cleanup(m_curl); }

void sinsp_curl::init_bt(CURL* curl, bearer_token::ptr_t bt)
{
    if(bt && bt->bt_auth_header())
    {
	check_error(curl_easy_setopt(curl, CURLOPT_HTTPHEADER,
				     bt->bt_auth_header()));
    }
}
...
void sinsp_curl::init_ssl(CURL* curl, ssl::ptr_t ssl_data)
{
	...
	if(!ssl_data->cert().empty())
	{
	    ...
	    {
		check_error(curl_easy_setopt(curl, CURLOPT_SSLCERTTYPE,
					     ssl_data->cert_type().c_str()));
	    }
	    check_error(curl_easy_setopt(curl, CURLOPT_SSLCERT,
					 ssl_data->cert().c_str()));
	    g_logger.log("CURL SSL certificate: " + ssl_data->cert(),
			 sinsp_logger::SEV_DEBUG);
	}

	if(!ssl_data->key_passphrase().empty())
	{
	    check_error(curl_easy_setopt(curl, CURLOPT_KEYPASSWD,
					 ssl_data->key_passphrase().c_str()));
	    g_logger.log("CURL SSL key password SET. ",
			 sinsp_logger::SEV_DEBUG);
	}
	...
	if(!ssl_data->key().empty())
	{
	    if(!ssl_data->cert_type().empty())
	    {
		check_error(curl_easy_setopt(curl, CURLOPT_SSLKEYTYPE,
					     ssl_data->cert_type().c_str()));
	    }
	    check_error(curl_easy_setopt(curl, CURLOPT_SSLKEY,
					 ssl_data->key().c_str()));
	    g_logger.log("CURL SSL key: " + ssl_data->key(),
			 sinsp_logger::SEV_DEBUG);
	}
	...
	if(ssl_data->verify_peer())
	{
	    ...
	    g_logger.log("CURL SSL peer and host verification ENABLED.",
			 sinsp_logger::SEV_DEBUG);
	}
	else
	{
	    ...
	    g_logger.log("CURL SSL peer and host verification DISABLED.",
			 sinsp_logger::SEV_DEBUG);
	}

	if(!ssl_data->ca_cert().empty())
	{
	    check_error(curl_easy_setopt(curl, CURLOPT_CAINFO,
					 ssl_data->ca_cert().c_str()));
	    g_logger.log("CURL SSL CA cert set to: " + ssl_data->ca_cert(),
			 sinsp_logger::SEV_DEBUG);
	}
    }
}
...
string sinsp_curl::get_data(bool do_log)
{
    ...
    if(do_log)
    {
	g_logger.log("CURL error while connecting to " +
			     m_uri.to_string(false) +
			     ", "
			     "response: [" +
			     os.str() + ']',
...
size_t sinsp_curl::header_callback(char* buffer, size_t size, size_t nitems,
				   void* userdata)
{
	...
	{
	    g_logger.log("HTTP redirect Location: (" + buf + ')',
...
bool sinsp_curl::get_data(std::ostream& os)
{
    ...
    check_error(curl_easy_setopt(m_curl, CURLOPT_CONNECTTIMEOUT,
				 static_cast<int>(m_timeout_ms / 1000)));
    check_error(curl_easy_setopt(m_curl, CURLOPT_TIMEOUT_MS, m_timeout_ms));
    check_error(curl_easy_setopt(
	    m_curl, CURLOPT_NOSIGNAL,
    ...
    check_error(curl_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION,
	...
	check_error(curl_easy_getinfo(m_curl, CURLINFO_RESPONSE_CODE,
				      &m_response_code));
	if(m_response_code >= 400)
	{
	    g_logger.log("CURL HTTP error while accessing [" +
				 m_uri.to_string(false) +
				 "]: " + std::to_string(m_response_code) +
				 " (" + http_reason::get(m_response_code) + ')',
			 sinsp_logger::SEV_ERROR);
	    return false;
	}
	else if(is_redirect(m_response_code))
	{
	    g_logger.log("HTTP redirect (" + std::to_string(m_response_code) +
				 ')',
			 sinsp_logger::SEV_DEBUG);
	    if(handle_redirect(m_uri, std::string(m_redirect), os))
	    {
		std::ostringstream* pos =
			dynamic_cast<std::ostringstream*>(&os);
		if(pos)
		{
		    pos->str("");
		    return get_data(*pos);
		...
		{
		    g_logger.log("HTTP redirect received from [" +
					 m_uri.to_string(false) +
					 "] but "
					 "output stream can not be obtained "
					 "(dynamic cast failed).",
				 sinsp_logger::SEV_ERROR);
		    return false;
		}
...
bool sinsp_curl::check_error(unsigned ret, bool exc)
{
    ...
    {
	throw sinsp_exception("Invalid CURL return value:" +
			      std::to_string(ret));
    }
    else
    {
...
void sinsp_curl::dump(const char* text, unsigned char* ptr, size_t size,
		      char nohex)
{
...
int sinsp_curl::trace(CURL* handle, curl_infotype type, char* data, size_t size,
		      void* userp)
{
