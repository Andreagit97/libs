	...
	std::shared_ptr<sinsp_plugin> plugin)
{
    ...
    {
	throw sinsp_exception("Creating a sinsp_filter_check_plugin with a non "
			      "extraction-capable plugin.");
    }

    m_eplugin = plugin;
    m_info.m_fields =
	    &m_eplugin->fields()[0]; // we use a vector so this should be safe
...
sinsp_filter_check_plugin::sinsp_filter_check_plugin(
	const sinsp_filter_check_plugin &p)
{
...
int32_t sinsp_filter_check_plugin::parse_field_name(const char *str,
						    bool alloc_state,
						    bool needed_for_filtering)
{
	...
	if(pos1 != string::npos)
	{
		...
		if(pos2 != string::npos)
		{
		    ...
		    if(!(m_info.m_fields[m_field_id].m_flags &
				 filtercheck_field_flags::EPF_ARG_ALLOWED ||
			 m_info.m_fields[m_field_id].m_flags &
				 filtercheck_field_flags::EPF_ARG_REQUIRED))
		    {
			throw sinsp_exception(
				string("filter ") + string(str) + string(" ") +
				m_field->m_name +
				string(" does not allow nor require an "
				       "argument but one is provided: " +
				       m_argstr));
		    }
		    ...
		    if(m_info.m_fields[m_field_id].m_flags &
		       filtercheck_field_flags::EPF_ARG_INDEX)
		    {
	    ...
	    throw sinsp_exception(string("filter ") + string(str) +
				  string(" ") + m_field->m_name +
				  string(" has a badly-formatted argument"));
	}
	if(m_info.m_fields[m_field_id].m_flags &
	   filtercheck_field_flags::EPF_ARG_REQUIRED)
	{
...
bool sinsp_filter_check_plugin::extract(sinsp_evt *evt,
					...
					bool sanitize_strings)
{
    ...
    //
    // todo(jasondellaluce): remove this once we support extractor plugins for
    // syscalls
    ...
    // Check that the current plugin is source compatible with the event source
    // plugin
    parinfo = evt->get_param(0);
    ASSERT(parinfo->m_len == sizeof(int32_t));
    uint32_t pgid = *(int32_t *)parinfo->m_val;

    // We know that plugin has source capabilities because it has an id and is
    // sending events todo(jasondellaluce): remove this check once we do it
    // properly at initialization time on the consumer side
    bool pfound = false;
    auto psource = m_inspector->get_plugin_manager()->source_idx_by_plugin_id(
	    pgid, pfound);
    if(!pfound)
    {
	return false;
	...
	for(auto &src : m_eplugin->extract_event_sources())
	{
	    ...
	    if(it != sources.end())
	    {
		m_compatible_sources->insert(
			std::distance(sources.begin(), it));
	    }
	}
    }
    ...
    if(!m_eplugin->extract_fields(pevt, num_fields, &efield) ||
       efield.res_len == 0)
    {
	return false;
    }
	...
	default:
	    ASSERT(false);
	    throw sinsp_exception(
		    "plugin extract error: unsupported field type " +
		    to_string(type));
	    break;
	}
...
void sinsp_filter_check_plugin::extract_arg_index(const char *full_field_name)
{
    ...
    // Please note that numbers starting with `0` (`01`, `02`, `0003`, ...) are
    // not indexes.
	...
	{
	    is_valid = false;
	    message = " has an invalid index argument not composed only by "
		      "digits: ";
	    break;
	}
    }
    ...
    // Please note that `stoul` alone is not enough, since it also consider as
    // valid strings like "0123 i'm a number", converting them into '0123'. This
    // is why in the previous step we check that every character is a digit.
    if(is_valid)
    {
	try
	{
	...
	{
	    message = " has an invalid index argument not representable on 64 "
		      "bit: ";
	}
    }
    throw sinsp_exception(string("filter ") + string(full_field_name) +
			  string(" ") + m_field->m_name + message + m_argstr);
}
