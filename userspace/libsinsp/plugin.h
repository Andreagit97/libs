...
class sinsp_plugin_cap_common
{
    ...
    virtual std::string
    get_init_schema(ss_plugin_schema_type &schema_type) const = 0;
};
...
class sinsp_plugin_cap_extraction : public sinsp_plugin_cap_common
{
    ...
    virtual bool extract_fields(ss_plugin_event &evt, uint32_t num_fields,
				ss_plugin_extract_field *fields) const = 0;
...
class sinsp_plugin : public sinsp_plugin_cap_sourcing,
		     public sinsp_plugin_cap_extraction
{
    ...
    // set with an error.
    static std::shared_ptr<sinsp_plugin> create(const std::string &filepath,
						std::string &errstr);
    ...
    static sinsp_filter_check *
    new_filtercheck(std::shared_ptr<sinsp_plugin> plugin);
    ...
    virtual std::string
    get_init_schema(ss_plugin_schema_type &schema_type) const override;
    ...
    virtual std::vector<sinsp_plugin_cap_sourcing::open_param>
    list_open_params() const override;

    /** Field Extraction **/
    virtual const std::set<std::string> &extract_event_sources() const override;
    virtual bool extract_fields(ss_plugin_event &evt, uint32_t num_fields,
				ss_plugin_extract_field *fields) const override;
    ...
    void validate_init_config_json_schema(std::string &config,
					  std::string &schema);
