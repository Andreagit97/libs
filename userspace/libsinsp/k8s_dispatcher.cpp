...
k8s_dispatcher::k8s_dispatcher(k8s_component::type t, k8s_state_t& state,
	...
	m_type(t),
	m_state(state), m_event_filter(event_filter)
{
}
...
bool k8s_dispatcher::is_valid(const std::string& msg)
{
    ...
    // or those that are longer than one character and contain multiple
    // newlines.
...
k8s_dispatcher::msg_data k8s_dispatcher::get_msg_data(Json::Value& root)
{
	...
	if(!et.empty())
	{
	    if(et[0] == 'A')
	    {
		data.m_reason = COMPONENT_ADDED;
	    }
	    else if(et[0] == 'M')
	    {
		data.m_reason = COMPONENT_MODIFIED;
	    }
	    else if(et[0] == 'D')
	    {
		data.m_reason = COMPONENT_DELETED;
	    }
	    else if(et[0] == 'E')
	    {
		data.m_reason = COMPONENT_ERROR;
	    }
	}
	else
	{
...
void k8s_dispatcher::handle_node(const Json::Value& root, const msg_data& data)
{
	...
	{
	    std::ostringstream os;
	    os << "ADDED message received for existing node [" << data.m_uid
	       << "], updating only.";
	    g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	}
	k8s_node_t& node = m_state.get_component<k8s_nodes, k8s_node_t>(
		m_state.get_nodes(), data.m_name, data.m_uid);
	...
	if(!object.isNull())
	{
		...
		k8s_node_t::host_ip_list addresses =
			k8s_node_t::extract_addresses(status);
		if(addresses.size() > 0)
		{
	    ...
	    if(!metadata.isNull())
	    {
		k8s_pair_list entries =
			k8s_component::extract_object(metadata, "labels");
		if(entries.size() > 0)
		{
	...
	{
	    std::ostringstream os;
	    os << "MODIFIED message received for non-existing node ["
	...
	{
	    g_logger.log(std::string("NODE not found: ") + data.m_name,
    ...
    {
	g_logger.log(std::string("Unsupported K8S NODE event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
    }
}

void k8s_dispatcher::handle_namespace(const Json::Value& root,
				      const msg_data& data)
{
    if(data.m_reason == COMPONENT_ADDED)
    {
	...
	{
	    std::ostringstream os;
	    os << "ADDED message received for existing namespace ["
	       << data.m_uid << "], updating only.";
	    g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	}
	k8s_ns_t& ns = m_state.get_component<k8s_namespaces, k8s_ns_t>(
		m_state.get_namespaces(), data.m_name, data.m_uid);
	const Json::Value& object = root["object"];
	if(!object.isNull())
	{
	    const Json::Value& metadata = object["metadata"];
	...
	{
	    std::ostringstream os;
	    os << "MODIFIED message received for non-existing namespace ["
	       << data.m_uid << "], giving up.";
	    g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
	    return;
	}
	...
	{
	    g_logger.log(std::string("NAMESPACE not found: ") + data.m_name,
			 sinsp_logger::SEV_ERROR);
	}
    }
    else if(data.m_reason == COMPONENT_ERROR)
    ...
    {
	g_logger.log(std::string("Unsupported K8S NAMESPACE event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
    }
}
...
bool k8s_dispatcher::handle_pod(const Json::Value& root, const msg_data& data)
{
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "ADDED message received for existing pod [" << data.m_uid
		   << "], updating only.";
		g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	    }
	    k8s_pod_t& pod = m_state.get_component<k8s_pods, k8s_pod_t>(
		    m_state.get_pods(), data.m_name, data.m_uid,
		    data.m_namespace);
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "MODIFIED message received for non-existing pod ["
		   << data.m_uid << "], giving up.";
		g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
		return false;
	    }
    ...
    {
	k8s_pod_t* pod = m_state.get_component<k8s_pods, k8s_pod_t>(
		m_state.get_pods(), data.m_uid);
	if(pod)
	{
	    if(!m_state.delete_component(m_state.get_pods(), data.m_uid))
	    {
		g_logger.log(std::string("Error deleting POD: ") + data.m_name,
			     sinsp_logger::SEV_ERROR);
		return false;
	    }
	}
	...
	{
	    g_logger.log(std::string("POD not found: ") + data.m_name,
    ...
    {
	g_logger.log(std::string("Unsupported K8S POD event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
	return false;
    }
...
void k8s_dispatcher::handle_service(const Json::Value& root,
				    const msg_data& data)
{
    if(data.m_reason == COMPONENT_ADDED)
    {
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "ADDED message received for existing service ["
		   << data.m_uid << "], updating only.";
		g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	    }
	    k8s_service_t& service =
		    m_state.get_component<k8s_services, k8s_service_t>(
			    m_state.get_services(), data.m_name, data.m_uid,
			    data.m_namespace);
	    handle_labels(service, object["metadata"], "labels");
	    handle_selectors(service, object["spec"]);
	    k8s_service_handler::extract_services_data(object, service,
						       m_state.get_pods());
	}
	else
	{
	    g_logger.log("K8s: object is null for service " + data.m_name +
				 '[' + data.m_uid + ']',
			 sinsp_logger::SEV_ERROR);
	}
    }
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "MODIFIED message received for non-existing service ["
		   << data.m_uid << "], giving up.";
		g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
		return;
	    }
	...
	{
	    g_logger.log(std::string("SERVICE not found: ") + data.m_name,
			 sinsp_logger::SEV_ERROR);
	}
    }
    else if(data.m_reason == COMPONENT_ERROR)
    ...
    {
	g_logger.log(std::string("Unsupported K8S SERVICE event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
    }
}

void k8s_dispatcher::handle_deployment(const Json::Value& root,
				       const msg_data& data)
{
    if(data.m_reason == COMPONENT_ADDED)
    {
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "ADDED message received for existing deployment ["
		   << data.m_uid << "], updating only.";
		g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	    }
	    k8s_deployment_t& deployment =
		    m_state.get_component<k8s_deployments, k8s_deployment_t>(
			    m_state.get_deployments(), data.m_name, data.m_uid,
			    data.m_namespace);
	...
	{
	    g_logger.log("K8s: object is null for deployment " + data.m_name +
				 '[' + data.m_uid + ']',
			 sinsp_logger::SEV_ERROR);
	}
    }
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "MODIFIED message received for non-existing deployment ["
		   << data.m_uid << "], giving up.";
		g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
		return;
	    }
	...
	{
	    g_logger.log(std::string("DEPLOYMENT not found: ") + data.m_name,
			 sinsp_logger::SEV_ERROR);
	}
    }
    else if(data.m_reason == COMPONENT_ERROR)
    ...
    {
	g_logger.log(std::string("Unsupported K8S DEPLOYMENT event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
    }
}

void k8s_dispatcher::handle_daemonset(const Json::Value& root,
				      const msg_data& data)
{
    if(data.m_reason == COMPONENT_ADDED)
    {
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "ADDED message received for existing daemonset ["
		   << data.m_uid << "], updating only.";
		g_logger.log(os.str(), sinsp_logger::SEV_DEBUG);
	    }
	    k8s_daemonset_t& daemonset =
		    m_state.get_component<k8s_daemonsets, k8s_daemonset_t>(
			    m_state.get_daemonsets(), data.m_name, data.m_uid,
			    data.m_namespace);
	...
	if(!object.isNull())
	{
	    ...
	    {
		std::ostringstream os;
		os << "MODIFIED message received for non-existing daemonset ["
		   << data.m_uid << "], giving up.";
		g_logger.log(os.str(), sinsp_logger::SEV_ERROR);
		return;
	    }
	...
	{
	    g_logger.log(std::string("DAEMONSET not found: ") + data.m_name,
			 sinsp_logger::SEV_ERROR);
	}
    }
    else if(data.m_reason == COMPONENT_ERROR)
    ...
    {
	g_logger.log(std::string("Unsupported K8S DAEMONSET event reason: ") +
			     std::to_string(data.m_reason),
		     sinsp_logger::SEV_ERROR);
    }
}
...
void k8s_dispatcher::handle_event(const Json::Value& root, const msg_data& data)
{
	...
	if(!object.isNull())
	{
	    ...
	    {
		bool is_aggregate =
			(get_json_string(object, "message")
				 .find("events with common reason combined") !=
			 std::string::npos);
		time_t last_ts = get_epoch_utc_seconds(
			get_json_string(object, "lastTimestamp"));
		time_t now_ts = get_epoch_utc_seconds_now();
		g_logger.log(
			"K8s EVENT: lastTimestamp=" + std::to_string(last_ts) +
				", now_ts=" + std::to_string(now_ts),
		   ...
		   ((now_ts - last_ts) < 10)) // event not older than 10 seconds
		{
		    ...
		    g_logger.log("K8s EVENT: involved object and event reason "
				 "found:" +
					 kind.asString() + '/' +
					 event_reason.asString(),
				 sinsp_logger::SEV_TRACE);
		    if(!kind.isNull() &&
		       kind.isConvertibleTo(Json::stringValue) &&
		       !event_reason.isNull() &&
		       event_reason.isConvertibleTo(Json::stringValue))
		    {
			...
			if(is_allowed)
			{
			    g_logger.log("K8s EVENT: adding event.",
					 sinsp_logger::SEV_TRACE);
			    k8s_event_t& evt =
				    m_state.add_component<k8s_events,
							  k8s_event_t>(
					    m_state.get_events(), data.m_name,
					    data.m_uid, data.m_namespace);
			    m_state.update_event(evt, object);
			}
			else
			{
			    g_logger.log(
				    "K8s EVENT: filter does not allow {\"" +
					    type + "\", \"{" +
					    event_reason.asString() + "\"} }",
				    sinsp_logger::SEV_TRACE);
			    g_logger.log(m_event_filter->to_string(),
					 sinsp_logger::SEV_TRACE);
			}
		    }
		    else
		    {
			g_logger.log("K8s EVENT: event type or involvedObject "
				     "kind not found.",
				     sinsp_logger::SEV_ERROR);
			g_logger.log(Json::FastWriter().write(root),
				     sinsp_logger::SEV_TRACE);
		    }
		}
		else
	    ...
	    {
		g_logger.log("K8s EVENT: involvedObject not found.",
			     sinsp_logger::SEV_ERROR);
		g_logger.log(Json::FastWriter().write(root),
			     sinsp_logger::SEV_TRACE);
	    }
	...
	{
	    g_logger.log("K8s EVENT: object not found.",
			 sinsp_logger::SEV_ERROR);
	    g_logger.log(Json::FastWriter().write(root),
			 sinsp_logger::SEV_TRACE);
	}
...
void k8s_dispatcher::extract_data(const std::string& json, bool enqueue)
{
    ...
    {
	g_logger.log("Bad JSON message received :[" + json + ']',
		     sinsp_logger::SEV_ERROR);
    }
}
