...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		    url, "/apis/apps/v1/daemonsets", STATE_FILTER, EVENT_FILTER,
...
bool k8s_daemonset_handler::handle_component(const Json::Value& json,
					     const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
		...
		k8s_daemonset_t& ds =
			m_state->get_component<k8s_daemonsets, k8s_daemonset_t>(
				m_state->get_daemonsets(), data->m_name,
		...
		if(!desired.isNull() &&
		   desired.isConvertibleTo(Json::intValue) &&
	    ...
	    {
		if(!m_state->delete_component(m_state->get_daemonsets(),
					      data->m_uid))
		{
		    log_not_found(*data);
		    return false;
