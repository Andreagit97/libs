...
char* sinsp_filter_check_reference::print_double(uint8_t* rawval, uint32_t str_len)
{
	...
	switch(get_original_field_type()) // todo!: not sure we need to check we want the original or the modified, Same in all this file
	{
	case PT_INT8:
...
char* sinsp_filter_check_reference::print_int(uint8_t* rawval, uint32_t str_len)
{
	...
	switch(get_original_field_type()) // todo!: not sure we need to check we want the original or the modified
	{
	case PT_INT8:
	...
	uint64_t time_delta)
{
	...
	auto type = get_original_field_type(); // todo!: not sure we need to check we want the original or the modified
