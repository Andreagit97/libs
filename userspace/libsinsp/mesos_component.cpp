...
mesos_component::mesos_component(type t, const std::string& name,
	...
	m_name(name), m_uid(uid)
{
    ...
    if(it == list.end())
    {
	throw sinsp_exception("Invalid Mesos component type: " +
			      std::to_string(t));
    }
...
mesos_component::mesos_component(const mesos_component& other):
	m_type(other.m_type), m_name(other.m_name), m_uid(other.m_uid),
	m_labels(other.m_labels)
{
}

mesos_component::mesos_component(mesos_component&& other):
	m_type(other.m_type), m_name(std::move(other.m_name)),
	m_uid(std::move(other.m_uid)), m_labels(std::move(other.m_labels))
{
}
...
mesos_framework::mesos_framework(const std::string& name,
				 const std::string& uid):
...
void mesos_framework::remove_task(const std::string& uid)
{
    ...
    g_logger.log(
	    "Removal of non-existing task (possible deployment failure): " +
		    uid,
...
mesos_framework::task_map& mesos_framework::get_tasks() { return m_tasks; }
...
mesos_task::mesos_task(const mesos_task& other):
	mesos_component(other), m_marathon_app_id(other.m_marathon_app_id),
	m_slave_id(other.m_slave_id)
{
}

mesos_task::mesos_task(mesos_task&& other):
	mesos_component(std::move(other)),
...
void mesos_task::add_labels(mesos_task::ptr_t task, const Json::Value& t_val)
{
	...
	g_json_error_log.log("", os.str(), sinsp_utils::get_current_time_ns(),
			     "mesos-task-add-labels");
    }
}
