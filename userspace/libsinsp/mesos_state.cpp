...
mesos_framework::task_ptr_t
mesos_state_t::get_task(const std::string& uid) const
{
...
const mesos_framework::task_map&
mesos_state_t::get_tasks(const std::string& framework_uid) const
{
    for(const auto& framework : m_frameworks)
    {
...
mesos_framework::task_map&
mesos_state_t::get_tasks(const std::string& framework_uid)
{
...
marathon_app::ptr_t mesos_state_t::get_app(const std::string& app_id)
{
    ...
    if(group)
    {
	g_logger.log("Found group for app [" + app_id + "]: " + group->get_id(),
...
marathon_group::app_ptr_t
mesos_state_t::add_or_replace_app(const std::string& app_id,
				  ...
				  const std::string& task_id)
{
	...
	g_json_error_log.log(app_id, errstr, sinsp_utils::get_current_time_ns(),
			     "add-replace-app");
	return 0;
    }
    ...
    if(!task_id.empty())
    {
	g_logger.log("Adding task [" + task_id + "] to app [" + app_id + ']',
		     sinsp_logger::SEV_DEBUG);
    ...
    if(group)
    {
	g_logger.log("Adding app [" + app_id + "] to group [" + group_id + ']',
		     sinsp_logger::SEV_DEBUG);
...
void mesos_state_t::add_task_to_app(marathon_group::app_ptr_t app,
				    const std::string& task_id)
{
    if(app)
    {
	...
	else
	{
	    std::string errstr =
		    "Task [" + task_id +
		    "] can not be obtained (null). Task not added to app [" +
		    app->get_id() + ']';
	    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
    ...
    {
	std::string errstr = "Attempt to add task [" + task_id +
			     "] to non-existing (null) app.";
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(task_id, errstr,
			     sinsp_utils::get_current_time_ns(),
			     "add-task-to-app");
...
marathon_group::ptr_t mesos_state_t::get_group(const std::string& group_id)
{
	...
	{
	    if(marathon_group::ptr_t p_group =
		       group.second->get_group(group_id))
	    {
		return p_group;
	    }
	}
...
mesos_state_t::add_or_replace_group(marathon_group::ptr_t group,
				    marathon_group::ptr_t to_group)
{
...
bool mesos_state_t::handle_groups(const Json::Value& root,
				  marathon_group::ptr_t to_group,
...
#ifdef HAS_CAPTURE
void mesos_state_t::capture_groups(const Json::Value& root,
				   const std::string& framework_id,
				   Json::Value& capt, bool capture_fw)
{
    if(!m_is_captured)
    {
	return;
...
void mesos_state_t::capture_apps(const Json::Value& root,
				 const std::string& framework_id)
{
    if(!m_is_captured)
    {
	...
	for(const auto& app : apps)
	{
		...
		{
		    Json::Value& c_task =
			    c_app["tasks"].append(Json::objectValue);
		    c_task["id"] = task["id"];
    ...
    enqueue_capture_event(capture::MARATHON_APPS,
			  Json::FastWriter().write(capt));
}
#endif // HAS_CAPTURE

bool mesos_state_t::parse_groups(Json::Value&& root,
				 const std::string& framework_id)
{
	...
	enqueue_capture_event(capture::MARATHON_GROUPS,
			      Json::FastWriter().write(capt));
    }
#endif // HAS_CAPTURE
    if(m_verbose)
...
bool mesos_state_t::parse_groups(json_ptr_t json,
				 const std::string& framework_id)
{
...
marathon_group::ptr_t mesos_state_t::add_group(const Json::Value& group,
					       marathon_group::ptr_t to_group,
					       const std::string& framework_id)
{
	...
	if(!apps.isNull())
	{
		...
		if(!app_id.isNull())
		{
			...
			if(p_app)
			{
				...
				{
				    if(task.second->get_marathon_app_id() ==
				       app_id.asString())
				    {
			...
			{
			    std::string errstr =
				    "An error occurred adding app [" +
				    app_id.asString() + "] to group [" + id +
				    ']';
			    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
			    g_json_error_log.log(
				    app_id.asString(), errstr,
				    sinsp_utils::get_current_time_ns(),
				    "add-group");
			}
		    }
...
void mesos_state_t::parse_apps(Json::Value&& root,
			       const std::string& framework_id)
{
    const Json::Value& apps = root["apps"];
    if(!apps.isNull())
					   ...
					   const std::string& /*framework_id*/)
{
	...
	if(!group_id.empty())
	{
		...
		g_logger.log("Added app [" + id + "] to Marathon group: [" +
				     group_id + ']',
			     sinsp_logger::SEV_DEBUG);
		const Json::Value& tasks = app["tasks"];
		if(tasks.size())
		{
		    g_logger.log("App [" + id + "] has " +
			...
			if(!task_id.isNull())
			{
			    ...
			    g_logger.log("Adding Mesos task ID to app [" + id +
						 "]: " + tid,
					 sinsp_logger::SEV_DEBUG);
			    mesos_framework::task_ptr_t pt =
				    get_task(task_id.asString());
			    if(pt)
			    {
			    ...
			    {
				std::string errstr = "Marathon task not found "
						     "in mesos state: " +
						     tid;
	    ...
	    {
		std::string errstr = "NOT added app [" + id +
				     "] to Marathon group: [" + group_id + ']';
		g_logger.log(errstr, sinsp_logger::SEV_ERROR);
		g_json_error_log.log(id, errstr,
				     sinsp_utils::get_current_time_ns(),
				     "add-app");
	    }
	}
	else
	{
	    ...
	    g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	    g_json_error_log.log(id, errstr, sinsp_utils::get_current_time_ns(),
				 "add-app");
	}
    }
    return p_app;
