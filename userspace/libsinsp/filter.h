	...
	: private libsinsp::filter::ast::expr_visitor
{
    ...
    /*!
	    ...
	    \param inspector Pointer to the inspector instance that will
       generate the events to be filtered \param fltstr The filter string to
       compile \param ttable_only For internal use only

	    \note This is not the primary constructor, and is only maintained
    ...
    */
    sinsp_filter_compiler(std::shared_ptr<gen_event_filter_factory> factory,
			  const std::string& fltstr, bool ttable_only = false);
    ...
    */
    ...
    /*!
	    ...
	    \return The resulting pointer is owned by the caller and must be
