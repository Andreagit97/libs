...
sinsp_dumper::sinsp_dumper(sinsp* inspector, uint8_t* target_memory_buffer,
...
void sinsp_dumper::open(const string& filename, bool compress,
			bool threads_from_sinsp)
{
    if(m_inspector->m_h == NULL)
    {
	throw sinsp_exception(
		"can't start event dump, inspector not opened yet");
    }

    if(m_target_memory_buffer)
    {
	m_dumper =
		scap_memory_dump_open(m_inspector->m_h, m_target_memory_buffer,
	...
	{
	    m_dumper =
		    scap_dump_open(m_inspector->m_h, filename.c_str(),
				   SCAP_COMPRESSION_GZIP, threads_from_sinsp);
	}
	else
	{
	    m_dumper =
		    scap_dump_open(m_inspector->m_h, filename.c_str(),
				   SCAP_COMPRESSION_NONE, threads_from_sinsp);
	}
    }
...
void sinsp_dumper::fdopen(int fd, bool compress, bool threads_from_sinsp)
{
    ...
    if(compress)
    {
	m_dumper = scap_dump_open_fd(m_inspector->m_h, fd,
				     SCAP_COMPRESSION_GZIP, threads_from_sinsp);
    }
    else
    {
...
bool sinsp_dumper::is_open() { return (m_dumper != NULL); }

bool sinsp_dumper::written_events() { return m_nevts; }
...
void sinsp_dumper::dump(sinsp_evt* evt)
{
    ...
    int32_t res = scap_dump(m_inspector->m_h, m_dumper, pdevt, evt->m_cpuid, 0);
