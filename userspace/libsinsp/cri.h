...
class cri_interface
{
     ...
     * @param resp reference to the response (if the RPC is successful, it will
     * be filled out)
     * @return status of the gRPC call
     */
    grpc::Status
    get_container_status(const std::string &container_id,
     ...
     */
    grpc::Status
    get_container_stats(const std::string &container_id,
     ...
     */
    bool parse_cri_image(const runtime::v1alpha2::ContainerStatus &status,
     ...
     * @return true if successful
     */
    bool parse_cri_mounts(const runtime::v1alpha2::ContainerStatus &status,
			  sinsp_container_info &container);

    /**
     * @brief fill out container environment variables based on CRI response
     * @param info the `info` key of the `info` field of the
     ...
     */
    bool parse_cri_env(const Json::Value &info,
		       sinsp_container_info &container);
     ...
     * Note: only containerd exposes this data
     */
    bool parse_cri_json_image(const Json::Value &info,
			      sinsp_container_info &container);

    /**
     * @brief fill out extra container info (e.g. resource limits) based on CRI
     * response
     * @param info the `info` key of the `info` field of the
     * ContainerStatusResponse
     * @param container the container info to fill out
     * @return true if successful
     */
    bool parse_cri_ext_container_info(const Json::Value &info,
				      sinsp_container_info &container);

    /**
     * @brief check if the passed container ID is a pod sandbox (pause
     * container)
     ...
     * @return the IP address if possible, 0 otherwise (e.g. when the pod uses
     * host netns)
     */
