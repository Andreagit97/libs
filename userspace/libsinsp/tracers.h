...
class sinsp_partial_tracer
{
	...
	if(memcmp(m_tags_storage, other->m_tags_storage, m_tags_len) == 0)
	{
	    return true;
	}
	...
	if(memcmp(m_tags_storage, other->m_tags_storage, len) == 0)
	{
	    return true;
	}
...
class sinsp_tracerparser
{
    ...
    ~sinsp_tracerparser();
    uint32_t get_storage_size() { return m_storage_size; }
    ...
    inline parse_result skip_spaces_and_commas(char* p, uint32_t* delta,
					       uint32_t n_expected_commas);
    inline parse_result skip_spaces_and_char(char* p, uint32_t* delta,
					     char char_to_skip);
    inline parse_result skip_spaces_and_commas_and_sq_brackets(char* p,
							       uint32_t* delta);
    inline parse_result skip_spaces_and_commas_and_cr_brackets(char* p,
							       uint32_t* delta);
    inline parse_result
    skip_spaces_and_commas_and_all_brackets(char* p, uint32_t* delta);
    inline parse_result parsestr(char* p, char** res, uint32_t* delta);
    inline parse_result parsestr_not_enforce(char* p, char** res,
					     uint32_t* delta);
    inline parse_result parsenumber(char* p, int64_t* res, uint32_t* delta);
    inline parse_result parsenumber_colend(char* p, int64_t* res,
					   uint32_t* delta);
