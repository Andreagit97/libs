...
marathon_http::marathon_http(mesos& m, const uri& url, bool discover_marathon,
			     int timeout_ms, const string& token):
	mesos_http(m, url, false, discover_marathon, timeout_ms, token)
{
    g_logger.log("Creating Marathon HTTP object for [" + url.to_string(false) +
			 "] ...",
		 sinsp_logger::SEV_DEBUG);
    if(refresh_data())
    {
	g_logger.log("Created Marathon HTTP connection (" +
			     url.to_string(false) + ") for framework [" +
			     get_framework_name() + "] (" + get_framework_id() +
	...
	throw sinsp_exception(
		"Could not obtain Mesos Marathon framework information.");
    }

    g_logger.log("Marathon request [" + get_request() + ']',
		 sinsp_logger::SEV_DEBUG);
}
...
bool marathon_http::refresh_data()
{
    ...
    if(res != CURLE_OK)
    {
	std::string errstr = std::string("Problem accessing /v2/info: ") +
	...
	if(reader.parse(os.str(), root, false))
	{
	    ...
	    g_logger.log("Found Marathon framework: " + get_framework_name() +
				 " (" + get_framework_id() +
				 "), version: " + get_framework_version(),
			 sinsp_logger::SEV_DEBUG);
	}
    ...
    {
	std::string errstr =
		std::string("Error parsing framework info:") + ex.what();
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(os.str(), errstr,
			     sinsp_utils::get_current_time_ns(), uri);
	return false;
...
std::string marathon_http::get_groups(const std::string& group_id)
{
    ...
    if(res != CURLE_OK)
    {
	std::string errstr = std::string("Problem accessing /v2/groups: ") +
			     curl_easy_strerror(res);
	g_logger.log(errstr, sinsp_logger::SEV_ERROR);
	g_json_error_log.log(os.str(), errstr,
			     sinsp_utils::get_current_time_ns(), uri);
