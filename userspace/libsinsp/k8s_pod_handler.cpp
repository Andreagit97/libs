...
k8s_pod_handler::k8s_pod_handler(k8s_state_t& state
				 ...
				 const std::string& http_version, ssl_ptr_t ssl,
				 bt_ptr_t bt, bool connect,
				 bool blocking_socket, std::string node_selector
#endif // HAS_CAPTURE
				 ):
	k8s_handler("k8s_pod_handler", true,
		    ...
		    url,
		    "/api/v1/"
		    "pods?fieldSelector=status.phase!=Failed,status.phase!="
		    "Unknown,status.phase!=Succeeded" +
			    (node_selector.size() == 0
				     ? ""
				     : ",spec.nodeName=" + node_selector),
		    STATE_FILTER, EVENT_FILTER, "", collector, http_version,
		    1000L, ssl, bt, true, connect, dependency_handler,
		    blocking_socket,
#endif // HAS_CAPTURE
		    ~0, &state)
{
...
k8s_pod_handler::extract_pod_container_ids(const Json::Value& item)
{
...
k8s_container::list
k8s_pod_handler::extract_pod_containers(const Json::Value& item)
{
	...
	for(auto& container : containers)
	{
		...
		{
		    g_logger.log("Port not found, setting value to 0",
		...
		else
		{
		    std::string port_name =
			    name.isNull() ? "[NO NAME]" : name.asString();
		    g_logger.log("Protocol not found for port: " + port_name,
				 sinsp_logger::SEV_WARNING);
		}
...
bool k8s_pod_handler::handle_component(const Json::Value& json,
				       const msg_data* data)
{
    if(data)
    {
	...
	{
	    g_logger.log(std::string("Unsupported K8S " + name() +
				     " event reason: ") +
