...
void sinsp_evt_formatter::set_format(gen_event_formatter::output_format of,
				     const string &fmt)
{
    uint32_t j;
	...
	if(cfmt[j] == '%')
	{
	    ...
	    {
		rawstring_check *newtkn = new rawstring_check(lfmt.substr(
			last_nontoken_str_start, j - last_nontoken_str_start));
	    ...
	    if(j == lfmtlen - 1)
	    {
		throw sinsp_exception("invalid formatting syntax: formatting "
				      "cannot end with a %");
	    }

	    //
	    // If the field specifier starts with a number, it means that we
	    // have a length modifier
	    //
	    if(isdigit(cfmt[j + 1]))
	    {
		//
		...
		while(true)
		{
			...
			throw sinsp_exception(
				"invalid formatting syntax: formatting cannot "
				"end with a number");
		    }
	    ...
	    sinsp_filter_check *chk =
		    m_available_checks.new_filter_check_from_fldname(
			    string(cfmt + j + 1), m_inspector, false);

	    if(chk == NULL)
	    {
		throw sinsp_exception("invalid formatting token " +
				      string(cfmt + j + 1));
	    }
    ...
    if(last_nontoken_str_start != j)
    {
	sinsp_filter_check *chk = new rawstring_check(lfmt.substr(
		last_nontoken_str_start, j - last_nontoken_str_start));
...
bool sinsp_evt_formatter::get_field_values(
	gen_event *gevt, std::map<std::string, std::string> &fields)
{
...
bool sinsp_evt_formatter::tostring_withformat(
	gen_event *gevt, std::string &output,
	gen_event_formatter::output_format of)
{
    bool retval = true;
    const filtercheck_field_info *fi;
...
std::shared_ptr<sinsp_evt_formatter> &
sinsp_evt_formatter_cache::get_cached_formatter(string &format)
{
    auto it = m_formatter_cache.lower_bound(format);

    if(it == m_formatter_cache.end() || it->first != format)
    {
	it = m_formatter_cache.emplace_hint(
		it, std::make_pair(format, make_shared<sinsp_evt_formatter>(
						   m_inspector, format)));
    }
...
bool sinsp_evt_formatter_cache::resolve_tokens(sinsp_evt *evt, string &format,
					       map<string, string> &values)
{
    return get_cached_formatter(format)->resolve_tokens(evt, values);
}

bool sinsp_evt_formatter_cache::tostring(sinsp_evt *evt, string &format,
					 OUT string *res)
{
...
sinsp_evt_formatter_factory::sinsp_evt_formatter_factory(
	sinsp *inspector, filter_check_list &available_checks):
	m_inspector(inspector),
	m_available_checks(available_checks),
...
void sinsp_evt_formatter_factory::set_output_format(
	gen_event_formatter::output_format of)
{
    m_formatters.clear();
...
std::shared_ptr<gen_event_formatter>
sinsp_evt_formatter_factory::create_formatter(const std::string &format)
{
