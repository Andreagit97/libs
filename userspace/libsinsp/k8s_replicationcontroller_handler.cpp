...
#if defined(HAS_CAPTURE) && !defined(_WIN32)
		    url, "/api/v1/replicationcontrollers", STATE_FILTER,
...
bool k8s_replicationcontroller_handler::handle_component(
	const Json::Value& json, const msg_data* data)
{
    if(data)
    {
	if(m_state)
	{
	    ...
	    {
		k8s_rc_t& rc =
			m_state->get_component<k8s_controllers, k8s_rc_t>(
				m_state->get_rcs(), data->m_name, data->m_uid,
				data->m_namespace);
