...
/* This function should be idempotent every time is called it should enforce again */
int pman_enforce_sc_set(bool *sc_set)
{
	/* When we want to disable the capture we receive a NULL pointer here */
	bool empty_sc_set[PPM_SC_MAX];
	if(!sc_set)
	{
		memset(&empty_sc_set, 0, sizeof(empty_sc_set));
		sc_set = &empty_sc_set;
	}
	...
	{
		/* We are not interested in this sc */
		if(!sc_set[sc])
		{
			continue;
		}
	...
	/* todo: Not sure we need the double tracepoint ppm_sc here! */
